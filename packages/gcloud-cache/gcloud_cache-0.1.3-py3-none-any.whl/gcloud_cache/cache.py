...
def save_to_cache(hash_key, result):
    """Stores result in Google Cloud Storage as JSON or a binary file."""
    bucket = storage_client.bucket(BUCKET_NAME)

    if isinstance(result, str) and os.path.exists(result):  # Jeśli wynik to plik
        blob = bucket.blob(f"cache/{hash_key}.bin")  # Zmieniamy rozszerzenie na .bin, aby obsługiwać różne typy
        blob.upload_from_filename(result)
        print(f"📤 Plik {result} zapisany w Cloud Storage jako {blob.name}")
    else:
        blob = bucket.blob(f"cache/{hash_key}.json")
