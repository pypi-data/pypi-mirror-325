...
class Packet:
    ...
    def calc_checksum(cls, code, param_bytes=b"", pkt_type=1):
        ...
        return (
            sum(header + struct.pack("<BB", code, len(param_bytes)) + param_bytes) % 256
        )
    ...
    def verify(cls, packet_bytes, pkt_type=1):
        ...
        code, size = struct.unpack(
            "<BB",
            packet_bytes[cls.HEADER_LEN : cls.HEADER_LEN + cls.CODE_LEN + cls.SIZE_LEN],
        )

        # 长度校验
        param_bytes = packet_bytes[
            cls.HEADER_LEN + cls.CODE_LEN + cls.SIZE_LEN : -cls.CHECKSUM_LEN
        ]
    ...
    def pack(cls, code, param_bytes=b""):
        ...
        frame_bytes = (
            cls.HEADERS[cls.PKT_TYPE_REQUEST]
            + struct.pack("<BB", code, size)
            + param_bytes
    ...
    def unpack(cls, packet_bytes):
        ...
        if not cls.verify(packet_bytes, pkt_type=cls.PKT_TYPE_RESPONSE):
            ...
            return None
        code = struct.unpack(
            "<B", packet_bytes[cls.HEADER_LEN : cls.HEADER_LEN + cls.CODE_LEN]
        )[0]
        param_bytes = packet_bytes[
            cls.HEADER_LEN + cls.CODE_LEN + cls.SIZE_LEN : -cls.CHECKSUM_LEN
        ]
    ...
    def update(self, next_byte):
        ...
        if not self.header_flag:
            ...
            if len(self.header) < Packet.HEADER_LEN:
                ...
                if (
                    len(self.header) == Packet.HEADER_LEN
                    and self.header == Packet.HEADERS[Packet.PKT_TYPE_RESPONSE]
                ):
            ...
            # 构建一个完整的Packet
            tmp_packet_bytes = (
                self.header + self.code + self.size + self.param_bytes + next_byte
            )
...
class UartServoInfo:
    ...
    def __str__(self):
        return "目标角度:{:.1f} 实际角度:{:.1f} 角度误差:{:.2f}".format(
            self.target_angle, self.angle, self.target_angle - self.angle
        )
...
class UartServoManager:
    ...
    CODE_STOP_ON_CONTROL_MODE = 24  # 控制模式停止指令
    ...
    def __init__(
        self, uart, is_scan_servo=True, srv_num=254, mean_dps=100, is_debug=False
    ):
        self.is_debug = is_debug
        self.uart = uart
    ...
    def query_servo_angle(self, servo_id):
        ...
        if self.servos[servo_id].is_mturn:
            # 多圈模式
            self.send_request(
                self.CODE_QUERY_SERVO_ANGLE_MTURN, struct.pack("<B", servo_id)
            )
        else:
            # 单圈模式
    ...
    def response_query_servo_angle(self, param_bytes):
        ...
        if servo_id not in self.servos:
            ...
            if self.is_debug:
                logging.info(
                    "[INFO] 更新舵机角度 id={}  角度: {:.2f} deg".format(
                        servo_id, angle
                    )
                )
        ...
        if interval is not None and interval != 0:
            ...
            if is_mturn:
                ...
                param_bytes = struct.pack(
                    "<BiIHHH", servo_id, angle, interval, t_acc, t_dec, power
                )
                self.send_request(
                    self.CODE_SET_SERVO_ANGLE_MTURN_BY_INTERVAL, param_bytes
                )
            else:
                param_bytes = struct.pack(
                    "<BhHHHH", servo_id, angle, interval, t_acc, t_dec, power
                )
            ...
            if is_mturn:
                param_bytes = struct.pack(
                    "<BiHHHH", servo_id, angle, velocity, t_acc, t_dec, power
                )
                self.send_request(
                    self.CODE_SET_SERVO_ANGLE_MTURN_BY_VELOCITY, param_bytes
                )
            else:
                param_bytes = struct.pack(
                    "<BhHHHH", servo_id, angle, velocity, t_acc, t_dec, power
                )
    ...
    def set_wheel(self, servo_id, mode, value=0, is_cw=True, mean_dps=None):
        ...
        # 发送请求
        self.send_request(
            self.CODE_SET_SPIN, struct.pack("<BBHH", servo_id, method, mean_dps, value)
        )
    ...
    def set_wheel_norm(self, servo_id, is_cw=True, mean_dps=None):
        """设置轮式为普通模式, 转速单位: °/s"""
        self.set_wheel(
            servo_id, self.WHEEL_MODE_NORMAL, value=0, is_cw=is_cw, mean_dps=mean_dps
        )
    ...
    def set_wheel_turn(self, servo_id, turn=1, is_cw=True, mean_dps=None, is_wait=True):
        ...
        self.set_wheel(
            servo_id, self.WHEEL_MODE_TURN, value=turn, is_cw=is_cw, mean_dps=mean_dps
        )
        # 根据圈数与转速, 估计延时的时间
        if is_wait:
    ...
    def set_wheel_time(
        self, servo_id, interval=1000, is_cw=True, mean_dps=None, is_wait=True
    ):
        """设置轮子为定时模式，时间单位: ms"""
        self.set_wheel(
            servo_id,
            self.WHEEL_MODE_TIME,
            value=interval,
            is_cw=is_cw,
            mean_dps=mean_dps,
        )
    ...
    def read_data(self, servo_id, address):
        ...
        if self.is_debug:
            ...
            logging.info(
                "DATA : {}".format(self.servos[servo_id].data_table_raw_dict[address])
            )
        # 返回读取到的数据
    ...
    def write_data(self, servo_id, address, content):
        ...
        # 发送请求
        self.send_request(
            self.CODE_WRITE_DATA, struct.pack("<BB", servo_id, address) + content
        )
        # 初始化标志位
        self.servos[servo_id].data_write_success = False
        if self.is_debug:
            logging.info(
                "WRITE DATA 舵机ID={} Address={} Value={}".format(
                    servo_id, address, content
                )
            )
        # 等待数据回传
        self.update(wait_response=True)
    ...
    def response_write_data(self, param_bytes):
        ...
        if self.is_debug:
            logging.info(
                "WRITE DATA 舵机ID={} Address={} Result={}".format(
                    servo_id, address, result
                )
            )
        ...
        return float(
            ...
            / (
                (
                    math.log(
                        10000
                        * (struct.unpack("<H", temp_bytes)[0])
                        / (10000 * (4096 - (struct.unpack("<H", temp_bytes)[0])))
                    )
                    / 3435
                )
                + 1 / (273.15 + 25)
    ...
    def query_servo_monitor(self, servo_id):  # 数据监控
        if self.is_debug:
    ...
    def response_query_servo_monitor(self, param_bytes):
        # 数据解包
        servo_id, voltage, current, power, temp_raw, status, angle, turn = (
            struct.unpack("<BHHHHBih", param_bytes)
        )
        # 舵机的分辨率是0.1度
        angle /= 10.0
        temp = temp_raw
        ...
        if self.is_debug:
            logging.info(
                "舵机电压: {:.2f} V电流: {:.2f}A 功率: {:.2f} W温度: {:.2f} ℃ 状态: {:.2f} 角度: {:.2f} ° 圈数: {:.0f} ".format(
                    voltage / 1000,
                    current / 1000,
                    power / 1000,
                    temp,
    ...
    def stop_on_control_mode(self, servo_id, mode, power):
        """控制模式停止指令
        @param servo_id
                舵机ID
        @param mode
        ...
        method = mode | 0x10
        self.send_request(
            self.CODE_STOP_ON_CONTROL_MODE, struct.pack("<BBH", servo_id, method, power)
        )

        ##同步命令角度模式控制

    def send_sync_angle(self, servo_num, command_data_list):
        """发送同步命令
        @param command_id
                命令编号
        @param servo_num
        ...
        # 构建参数字节流
        param_bytes = struct.pack(
            "<BBB", self.CODE_SET_SERVO_ANGLE, total_data_length, servo_num
        )
    ...
    def send_sync_anglebyinterval(self, servo_num, command_data_list):
        """发送同步命令
        @param command_id
                命令编号
        @param servo_num
        ...
        # 构建参数字节流
        param_bytes = struct.pack(
            "<BBB", self.CODE_SET_SERVO_ANGLE_BY_INTERVAL, total_data_length, servo_num
        )
        for command_data in command_data_list:
            param_bytes += command_data
        # 发送请求
    ...
    def send_sync_anglebyvelocity(self, servo_num, command_data_list):
        """发送同步命令
        @param command_id
                命令编号
        @param servo_num
        ...
        # 构建参数字节流
        param_bytes = struct.pack(
            "<BBB", self.CODE_SET_SERVO_ANGLE_BY_VELOCITY, total_data_length, servo_num
        )
        for command_data in command_data_list:
            param_bytes += command_data
        # 发送请求
    ...
    def send_sync_multiturnangle(self, servo_num, command_data_list):
        """发送同步命令
        @param command_id
                命令编号
        @param servo_num
        ...
        # 构建参数字节流
        param_bytes = struct.pack(
            "<BBB", self.CODE_SET_SERVO_ANGLE_MTURN, total_data_length, servo_num
        )
        for command_data in command_data_list:
            param_bytes += command_data
        # 发送请求
        ...
        ##同步命令多圈角度模式控制(基於加减速時間)

    def send_sync_multiturnanglebyinterval(self, servo_num, command_data_list):
        """发送同步命令
        @param command_id
                命令编号
        @param servo_num
        ...
        param_bytes = struct.pack(
            "<BBB",
            self.CODE_SET_SERVO_ANGLE_MTURN_BY_INTERVAL,
            total_data_length,
            servo_num,
        )
        for command_data in command_data_list:
    ...
    def send_sync_multiturnanglebyvelocity(self, servo_num, command_data_list):
        """发送同步命令
        @param command_id
                命令编号
        @param servo_num
        ...
        param_bytes = struct.pack(
            "<BBB",
            self.CODE_SET_SERVO_ANGLE_MTURN_BY_VELOCITY,
            total_data_length,
            servo_num,
        )
        for command_data in command_data_list:
    ...
    def update(self, is_empty_buffer=False, wait_response=False, timeout=0.01):
        """舵机管理器的定时器回调函数"""
