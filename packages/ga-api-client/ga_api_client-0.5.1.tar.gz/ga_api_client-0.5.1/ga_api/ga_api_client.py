...
class Reader:
    ...
    async def _exec(self, start, end):
        ...
        if 'config' not in self.pipeline:
            self.pipeline['config'] = {}
        self.pipeline['config']['start'] = start
        self.pipeline['config']['end'] = end
        self.pipeline['config']['size'] = self.size
        self.pipeline['config']['series'] = self.series
        if self.fields is not None:
            self.pipeline['config']['fields'] = self.fields
        status, rlt = await self._client._request('POST', f'/record', [200], self.pipeline, self.test_fn)
        logging.info(f'/record status={status} start={_logtime(start)} end={_logtime(end)} series={self.series}')
        if rlt is not None and len(rlt) > 0:
            self.count += len(rlt) - 1 # skip first row, which is header
            # print(rlt)
            return pd.DataFrame(rlt[1:], columns=rlt[0])
        return pd.DataFrame() # empty dataframe is returned if no data


    async def read_data(self, dts, series='full', size=100, fields=None):
