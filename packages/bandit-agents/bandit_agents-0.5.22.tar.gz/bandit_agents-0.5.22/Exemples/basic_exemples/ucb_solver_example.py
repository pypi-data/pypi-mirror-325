...
def ucb_solver_example() -> ndarray[float]:
    ucb_example_logger: logging.Logger = logging.getLogger(__name__)
    ...
    ucb_example_logger.debug(action_keys)
    ...
    # make its decisions
    ucb_solver: UCBSolver = Solvers().ucb_solver(
        action_keys=action_keys,
        optimistic_value=5.0,
        step_size=1e-2,
        confidence=1e-2,
    )
    ucb_example_logger.debug(ucb_solver)
    ...
    ucb_example_logger.debug(actions)
    ...
    with SimulationContext(actions) as simulation:
        ucb_example_logger.debug(simulation)
