...
CURRENT_VERSION = "0.0.5"  # 🎉 New version, new vibes
CACHE_DIR = os.path.expanduser("~/.browser_driver_cache")  # Browser driver cache for quicker setups
MIN_INTERVAL_SECONDS = 30  # Default is 30s, but user can increase ⏳
...
def get_user_input():
    ...
    auto_scroll = input(f"{Fore.CYAN}Enable auto-scroll? (y/n): {Fore.WHITE}").strip().lower() == 'y'


    return website_url, visit_count, visit_interval, browser, proxy, headless, auto_scroll
...
def visit_task(url, visit_count, interval, browser, headless, auto_scroll, proxy):
    ...
    try:
        for visit_number in range(1, visit_count + 1 if visit_count else float('inf')):
...
def check_for_update():
    ...
    try:
        ...
        if latest_version != CURRENT_VERSION:
            print(f"{Fore.YELLOW}New version available: {latest_version}! Check it here: {REPO_URL}")
        else:
...
def start():
    ...
    while True:
        ...
        if confirm == 'y':
            ...
            visit_task(url, count, interval, browser, headless, auto_scroll, proxy)  # ✅ Now passes proxy too
            break
        elif confirm == 'edit':
...
def main():
    ...
    print(f"{Fore.CYAN}Welcome to Auto Website Visitor! Let’s get this automated party started.")
    while True:
        print(AUTHOR_INFO)
