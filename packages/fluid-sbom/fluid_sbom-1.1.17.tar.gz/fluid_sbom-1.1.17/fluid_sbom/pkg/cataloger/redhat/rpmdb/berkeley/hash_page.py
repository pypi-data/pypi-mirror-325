...
from fluid_sbom.pkg.cataloger.redhat.rpmdb.berkeley.hash_page_entry import (
...
class HashPage(BaseModel):
    ...
    @classmethod
    def from_bytes(  # pylint:disable=too-many-locals
        cls,
        data: bytes,
        swapped: bool,
    ) -> "HashPage":
        ...
        if len(data) < expected_size:
            raise ValueError(
                "Data too short, expected at least "
                f"{expected_size} bytes, got {len(data)} bytes",
            )
        ...
        try:
            ...
            raise ValueError("Failed to unpack HashPage") from exc
...
def parse_hash_page(data: bytes, swapped: bool) -> HashPage:
    return HashPage.from_bytes(data, swapped)
...
def hash_page_value_content(
    ...
    if value_page_type != HASH_OF_INDEX_PAGE_TYPE:
        raise ValueError(f"Only HOFFPAGE types supported (got {value_page_type})")

    hash_off_page_entry_buff = page_data[hash_page_index : hash_page_index + HASH_OFF_PAGE_SIZE]

    entry = parse_hash_off_page_entry(hash_off_page_entry_buff, swapped)
    ...
    while current_page_no != 0:
        ...
        if len(current_page_buff) != page_size:
            raise ValueError(f"Failed to read page {current_page_no}: insufficient data")

        current_page = parse_hash_page(current_page_buff, swapped)
...
def hash_page_value_indexes(data: bytes, entries: int, swapped: bool) -> list:
    if entries % 2 != 0:
        raise ValueError(
            f"Invalid hash index: entries should only come in pairs (got {entries})",
        )
    ...
    for idx in range(0, len(hash_index_data), HASH_INDEX_ENTRY_SIZE):
        if (idx - HASH_INDEX_ENTRY_SIZE) % key_value_pair_size == 0:
            ...
            if len(value_bytes) < HASH_INDEX_ENTRY_SIZE:
                raise ValueError(f"Insufficient data at index {idx}")
...
def slice_reader(reader: io.BufferedReader, num: int) -> bytes:
    ...
    if len(data) != num:
        raise ValueError(f"Failed to read {num} bytes, got {len(data)} bytes")
    return data
