...
from fluid_sbom.pkg.cataloger.redhat.rpmdb.rpmdb_interface import RpmDBInterface
...
class RpmNDB(RpmDBInterface):
    ...
    def open(cls, path: str) -> RpmNDB:
        ...
        if len(hdr_data) != NDB_HEADER_STRUCT.size:
            file.close()
            raise InvalidDBFormatError("Failed to read metadata")
        ...
        if (
            ...
            file.close()
            raise InvalidDBFormatError("Invalid or unsupported NDB format")

        if hdr["SlotNPages"] > 2048:
            file.close()
            raise InvalidDBFormatError(f"Slot page limit exceeded: {hdr['SlotNPages']:x}")
        ...
        if len(slots_data) != num_slots * NDB_SLOT_ENTRY_STRUCT.size:
            file.close()
            raise InvalidDBFormatError("Failed to read NDB slot pages")
    ...
    def read(self) -> Generator[bytes, None, None]:
        ...
        for slot in self.slots:
            ...
            try:
                ...
                LOGGER.error("Failed to seek to blob %s", exc)
                return
