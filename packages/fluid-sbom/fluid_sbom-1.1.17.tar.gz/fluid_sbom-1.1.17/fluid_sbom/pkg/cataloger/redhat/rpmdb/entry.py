...
from fluid_sbom.pkg.cataloger.redhat.rpmdb.inet import (
    htonl,
    htonlu,
)
...
def hdrblob_import(blob: HdrBlob, data: bytes) -> list[IndexEntry]:
    ...
    if entry.tag >= RPMTAG_HEADERI18NTABLE:
        ...
        if blob.ril < len(blob.pe_list) - 1:
            ...
            if rdlen < 0:
                raise ValueError("Invalid length of dribble entries")
    ...
    if rdlen != blob.d_length:
        raise ValueError(
            f"The calculated length ({rdlen}) is different from the data"
            f" length ({blob.d_length})",
        )
...
def _parse_and_validate_trailer(
    ...
    try:
        ...
        raise ValueError("Failed to parse trailer") from exc
...
def _process_trailer_and_update_blob(blob: HdrBlob, data: bytes) -> None:
    ...
    if (
        ...
        raise ValueError(f"Invalid region size, region {blob.region_tag}")
...
def hdrblob_init(data: bytes) -> HdrBlob | None:
    try:
        ...
        raise ValueError("Failed to initialize header blob") from exc
...
def _read_pe_list(blob: HdrBlob, data: bytes) -> None:
    ...
    for _ in range(blob.i_length):
        ...
        if len(entry_info_data) < entry_info_size:
            raise ValueError("Failed to read entry info: incomplete data")
...
def header_import(data: bytes) -> list[IndexEntry] | None:
    try:
        ...
        if blob is None:
            raise ValueError("Failed to initialize header blob")

        # Import the index entries from the hdrblob
        index_entries = hdrblob_import(blob, data)
        if index_entries is None:
            raise ValueError("Failed to import header blob")

        return index_entries

    except ValueError as exc:
        raise ValueError("Header import failed") from exc
