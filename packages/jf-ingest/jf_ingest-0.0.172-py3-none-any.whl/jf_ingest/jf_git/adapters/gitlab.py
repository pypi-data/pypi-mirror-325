...
class GitlabAdapter(GitAdapter):
    ...
    def _standardize_gitlab_pull_request_gql(
        gitlab_pr: dict, standardized_repository: StandardizedRepository
    ) -> StandardizedPullRequest:
        """
        ...
        for gitlab_approval in gitlab_pr['approvedBy']['nodes']:
            ...
            standardized_approval = StandardizedPullRequestReview(
                ...
                review_state=str(PullRequestReviewState.APPROVED.value),
            )
        ...
        return StandardizedPullRequest(
            id=get_id_from_gid(gitlab_pr['id'], GitLabGidObjectMapping.MERGE_REQUEST.value),
        ...
        if not self.config.discover_organizations:
            ...
            for api_org in self.client.get_organizations_gql():
                group_id = get_id_from_gid(
                    api_org['groupIdStr'], GitLabGidObjectMapping.GROUP.value
                )
        ...
        if use_gql:
            ...
            for repo in self.client.get_repos_gql(full_path):
                yield StandardizedRepository(
                    id=get_id_from_gid(repo['id'], GitLabGidObjectMapping.PROJECT.value),
                    name=repo['name'],
                    full_name=repo['name'],
                    url=repo['webUrl'],
                    default_branch_sha='',
    ...
    def get_prs(
        ...
        for j, api_pr in enumerate(pr_generator, start=1):
            yield self._standardize_gitlab_pull_request_gql(api_pr, standardized_repo)
            if limit and j >= limit:
                return
