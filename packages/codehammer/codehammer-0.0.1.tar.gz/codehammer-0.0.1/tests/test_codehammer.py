...
def test_invalid_base_directory():
    ...
    with pytest.raises(InvalidBaseDirectoryError):
        forge = CodeHammer(base_dir="non_existent_dir", output_file="output.txt")
        forge.run(["py"])
...
def test_no_files_found(tmp_path):
    ...
    empty_dir.mkdir()
    forge = CodeHammer(base_dir=str(empty_dir), output_file=str(tmp_path / "merged.txt"))
...
def test_forge_prompt_dry_run(tmp_path):
    ...
    forge = CodeHammer(base_dir=str(code_dir), output_file=str(output_file), dry_run=True)
    forge.run(["py"])
...
def test_forge_prompt_force_overwrite(tmp_path):
    ...
    forge_no_force = CodeHammer(base_dir=str(code_dir), output_file=str(output_file), force=False)
    with pytest.raises(OutputFileAlreadyExistsError):
        forge_no_force.run(["py"])

    forge_force = CodeHammer(base_dir=str(code_dir), output_file=str(output_file), force=True)
    forge_force.run(["py"])
...
def test_include_tree(tmp_path):
    ...
    forge = CodeHammer(base_dir=str(code_dir), output_file=str(output_file), include_tree=True, force=True)
    forge.run(["py"])
    merged_content = output_file.read_text()
...
def test_get_directory_tree(tmp_path):
    ...
    forge = CodeHammer(base_dir=str(code_dir))
