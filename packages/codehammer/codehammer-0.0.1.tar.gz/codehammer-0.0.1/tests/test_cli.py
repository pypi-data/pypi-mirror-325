...
def run_cli_command(command, cwd, expect_error=False):
    ...
    result = subprocess.run(
        ["python", "-m", "codehammer.core.cli"] + command,  # ✅ Ensure it runs as module
        cwd=cwd,
        text=True,
        capture_output=True
    )
...
def test_cli_directory_tree(setup_codebase):
    output = run_cli_command(["tree", "--folder", ".","--base-dir", "."], setup_codebase)
    assert "subdir" in output, f"Unexpected output: {output}"


# ✅ Test: Get File Content
def test_cli_get_file_content(setup_codebase):
    output = run_cli_command(["file", "--file", "test.py", '--base-dir', '.'], setup_codebase)
...
def test_cli_get_files_in_folder(setup_codebase):
    output = run_cli_command(["files", "--folder", ".", '--base-dir', '.'], setup_codebase)
...
def test_cli_get_files_recursively(setup_codebase):
    output = run_cli_command(["files_recursive", "--folder", ".", '--base-dir', '.'], setup_codebase)
    assert output, "Output is empty; expected JSON file list."
...
def test_cli_write_file(setup_codebase):
    ...
    run_cli_command(["write", "--file", "output.txt", "--content", "Test Write", '--base-dir', '.'], setup_codebase)
...
def test_cli_combine_files(setup_codebase):
    ...
    run_cli_command(["combine", "--extensions", "py", "--output-file", "combined.txt", '--base-dir', '.'], setup_codebase)
...
def test_cli_exclude_from_combination(setup_codebase):
    ...
    run_cli_command(["combine", "--extensions", "py", "--output-file", "combined_exclude.txt", "--exclude", "test.py", '--base-dir', '.'], setup_codebase)
...
def test_cli_clean_result(setup_codebase):
    result_dir = setup_codebase / ".result"
    run_cli_command(["clean_result", "--exclude-clean", "old_output.txt", '--base-dir', '.'], setup_codebase)
