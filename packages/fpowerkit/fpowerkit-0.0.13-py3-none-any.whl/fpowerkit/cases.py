...
def _get_buses(
        ...
        load_fluctuation:'list[tuple[int,float]]', 
        load_repeat:int,
        load_period:int,
    ):
    if changeable_load:
        if len(load_fluctuation) == 0:
            for i in range(load_repeat):
                for j in range(24):
                    load_fluctuation.append((_DEFAULT_LOAD_SCALE[j][0] + 86400 * i, _DEFAULT_LOAD_SCALE[j][1]))
        else:
            ls = load_fluctuation
            load_fluctuation = []
            for i in range(load_repeat):
                for j in range(24):
                    load_fluctuation.append((ls[j][0] + load_period * i, ls[j][1]))
        if grid_repeat == 1:
            B = [Bus("B"+str(p[0]), _create_load(p[1]/Sb_KVA,load_fluctuation), 
                    _create_load(p[2]/Sb_KVA,load_fluctuation)) for p in LOAD]
        else:
            B = []
            for i in range(grid_repeat):
                B.extend([Bus(f"B{i}_{p[0]}", _create_load(p[1]/Sb_KVA,load_fluctuation), 
                            _create_load(p[2]/Sb_KVA,load_fluctuation)) for p in LOAD])
    else:
        if grid_repeat == 1:
            B = [Bus("B"+str(p[0]), p[1]/Sb_KVA, p[2]/Sb_KVA) for p in LOAD]
        else:
            B = []
            for i in range(grid_repeat):
                B.extend([Bus(f"B{i}_{p[0]}", p[1]/Sb_KVA, p[2]/Sb_KVA) for p in LOAD])
    return B
...
class PDNCases:
    ...
    def IEEE33(
        ...
        return Grid(Sb_MVA, Ub_kV, 
            _get_buses(Sb_MVA*1000, grid_repeat, _IEEE33_LOAD, changeable_load, load_fluctuation, load_repeat, load_period),
    ...
    def IEEE69(
        ...
        return Grid(Sb_MVA, Ub_kV, 
            _get_buses(Sb_MVA*1000, grid_repeat, _IEEE69_LOAD, changeable_load, load_fluctuation, load_repeat, load_period),
