...
def low_voltage_sum(simname, train_e,v_limit,branch_lookup,u_branch_lookup,time_info_sum,branch_name):
    ...
    train_sum = get_train_summary(vsorted_train_e)
    ...
    if time_info_sum[0] == "2":
        csvname = simname + '_time_filter'

    if time_info_sum[0] == "3":
        csvname = simname + '_branch_filter'

    if time_info_sum[0] == "4":
        csvname = simname + '_time_and_branch_filter'

    #write list of low all voltage instances (train steps)
    with open(csvname + '_T_list_below_' + str(int(v_limit))+'.csv','w',newline='') as csvfile:
        writer = csv.writer(csvfile) # create the csv writer
        lock = 0
        row = 1
    ...
    with open(csvname + '_Trains_below_' + str(int(v_limit))+'.csv','w',newline='') as csvfile:
        writer = csv.writer(csvfile) # create the csv writer
        lock = 0
        row = 1
    ...
    with open(csvname+'_branches_below_' + str(int(v_limit))+'.csv','w',newline='') as csvfile:
        writer = csv.writer(csvfile) # create the csv writer
        lock = 0
        row = 1
