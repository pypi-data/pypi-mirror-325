...
class ProtectClient:
    ...
    def protect(
        ...
        # Validate protect_rules
        if not isinstance(protect_rules, list):
            raise TypeError("protect_rules must be a list")

        if not protect_rules:
            raise ValueError("protect_rules cannot be empty")

        valid_metrics = set(self.metric_map.keys())
        valid_types = {"any", "all"}

        for i, rule in enumerate(protect_rules):
        ...
        if len(protect_rules) < BATCH_SIZE:
            BATCH_SIZE = len(protect_rules)
        ...
        for test_case in test_cases:
            for i in range(0, len(protect_rules), BATCH_SIZE):
                ...
                if remaining_time <= 0:
                    ...
                    remaining_rules = [rule["metric"] for rule in protect_rules[i:]]
                    all_uncompleted_rules.extend(remaining_rules)
                    break

                rules_batch = protect_rules[i : i + BATCH_SIZE]
                (
                    messages,
                    completed,
                    uncompleted,
        ...
        ans = {
            ...
            "messages": all_failure_messages[0]
            if all_failure_messages
            else "All checks passed",
            "reasons": all_failure_reasons[0]
            if all_failure_reasons
            else "All checks passed",
            "time_taken": elapsed_time,
        }

        if len(ans["uncompleted_rules"]) == len(protect_rules):
