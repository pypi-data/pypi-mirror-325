...
def apply_module_patches(module):
    ...
    for patch_policy in module_policy:
        ...
        if not settings.is_protect_enabled() and not agent_state.module.assess_enabled:
            continue

        # If only Protect is on and this isn't a protect patch, there's nothing to do.
        if settings.is_protect_enabled() and not agent_state.module.assess_enabled:
            if not patch_policy.is_protect_mode:
                continue
        ...
        try:
            if settings.is_protect_enabled() and patch_policy.is_protect_mode:
...
def register_import_hooks(protect_mode=False):
    ...
    """
    modules_to_patch = registry.get_policy(protect=protect_mode)
