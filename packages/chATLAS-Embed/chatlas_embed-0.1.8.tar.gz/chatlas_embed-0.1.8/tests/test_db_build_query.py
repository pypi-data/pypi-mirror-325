...
def test_filtered_search(populated_vector_store):
    ...
    results = populated_vector_store.search(
        query="Higgs boson discovery", k=2, date_filter="2020-04"
    )

    assert len(results) == 2
    for res in results:  # Check documents new enough
        file_date = datetime.strptime(res[0].metadata["last_modification"], "%Y-%m")
...
def test_document_update(vector_store, test_docs):
    ...
    # updated_content = content.replace("2021-09", current_date)
    ...
    results = vector_store.search(
        ...
        k=1,
        metadata_filters={"last_modification": current_date},
    )
    assert len(results) > 0
    assert "HIGGS BOSON IS A PARTICLE" in results[0][0].page_content
    assert results[0][0].metadata["last_modification"] == current_date
