...
def json_paths_to_jsonl(json_file_paths:list, output_json_path:str) -> None:
    ...
    with open(output_json_path, 'w') as outf:
        ...
        for json_file_path in json_file_paths:
            with open(json_file_path) as f:
                json.dump(json.load(f), outf)
                outf.write('\n')
        info(f'Wrote {len(json_file_paths)} tests to {output_json_path=}')


def json_paths_to_single_json(json_file_paths:list, output_json_path:str) -> None:
    ...
    with open(output_json_path, 'w') as outf:
        ...
        for json_file_path in json_file_paths:
            with open(json_file_path) as f:
                out_json_data['tests'].append(json.load(f))
...
def find_sv_included_files_within_file(filename:str,
    ...
    for fname in found_included_files:
        ...
        if not found and warnings:
            ...
            include_fname = fname
            warning( f'export_helper: {include_fname=} does not exist in any of {know_incdir_paths=},' \
