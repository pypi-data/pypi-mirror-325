...
from heaobject.aws import S3EventName
...
from heaobject.root import Permission, DesktopObjectDict, ViewerPermissionContext, ViewerPermissionContext, Share, DesktopObject
...
@action('heaserver-accounts-awsccount-get-search', rel='hea-search hea-context-menu', path='awsaccounts/{id}/search')
...
async def search_awsaccounts(request: web.Request) -> web.Response:
    ...
    sub = request.headers.get(SUB, NONE_USER)
    context: ViewerPermissionContext[DesktopObject] = ViewerPermissionContext(sub)
    cache_key = (sub, f'id^{request.match_info["id"]}')
    cached_value = request.app[HEA_CACHE].get(cache_key)
    if cached_value is not None:
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        try:
            ...
            bucket_names = [b_item.bucket_id for b_item in await client.get_all_list(request.app,
                                                                                     URL(bucket_url)/volume_id/'bucketitems',
                                                                                     type_=AWSS3BucketItem,
                                                                                     headers={SUB: sub}) if b_item.bucket_id]
            ...
            for sr_dict in search_items:
                sr = AWSS3SearchItemInFolder()
                # share permission always set with viewer. Authorization is established at account level as user is unknown when creating search Item
                share, attr_perms = await asyncio.gather(sr.get_permissions_as_share(context),
                                                        sr.get_all_attribute_permissions(context))
                sr_dict['shares'] = [share.to_dict()]
                permissions.append(share.permissions)
                attribute_permissions.append(attr_perms)
                #doing this because user's volume is unknown until now. search item doesn't know usr who created it
                if volume_id and sr_dict['bucket_id'] and sr_dict['id']:
                    actual_object_uri_base = URL('volumes') / volume_id / 'buckets' /sr_dict['bucket_id']
                    sr_dict['actual_object_id'] = sr_dict['id']

                    if awss3key.is_folder( sr_dict['path']): # is a file
                        project_url = await type_to_resource_url(request, AWSBucket)
                        if await client.has(request.app, URL(project_url)/volume_id/"buckets"/sr_dict['bucket_id']/"awss3projects"/sr_dict['id'], AWSS3Project):
                            sr_dict['actual_object_type_name'] = AWSS3Project.get_type_name()
                            sr_dict['actual_object_uri'] = str(actual_object_uri_base/'awss3projects'/sr_dict['id'])
                        else:
                            sr_dict['actual_object_type_name'] = AWSS3Folder.get_type_name()
                            sr_dict['actual_object_uri'] = str(actual_object_uri_base/'awss3folders'/sr_dict['id'])
                    else:
                        sr_dict['actual_object_uri'] = str(actual_object_uri_base/'awss3files'/sr_dict['id'])
                        sr_dict['actual_object_type_name'] = AWSS3FileObject.get_type_name()
    ...
    return await response.get_all(request=request, data=search_items, permissions=permissions, attribute_permissions=attribute_permissions)
...
async def _listen_for_object_change(app: web.Application):
    ...
    try:
        ...
        await client.post(app, URL(component.base_url) /'awsaccounts'/ 'internal' /'s3event', data={}, headers=headers_)
    except asyncio.TimeoutError:
        logger.debug("Request timed out as expected")
    except Exception as ex:
        logger.debug("Background running s3event aborted", exc_info=ex)


async def _insert_s3_object_from_sqs(request: web.Request, message: dict) -> bool:
    logger = logging.getLogger(__name__)
    records = message['Records'] if 'Records' in message else []
    records_to_idx = []
    responses = []
    try:
        for r in records:
            if r.get('s3'):
                ...
                if 'bucket' in s3 and 'object' in s3:
                    ...
                    search_aws_obj.is_delete_marker = search_aws_obj.event_name == S3EventName.OBJECT_REMOVED.display_name
                    records_to_idx.append(search_aws_obj)

                responses = await asyncio.gather(*[opensearchlib.update(request=request, doc=r) for r in records_to_idx ])

    except Exception as e:
        logger.debug("Failed to insert into index", exc_info=e )
        return False
    return not (len(responses) == 0 or any(resp.status >= 400 for resp in responses))


# S3 event
@routes.post('/awsaccounts/internal/s3event')
async def s3_event_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        async with DesktopObjectActionLifecycle(request=request,
                ...
                prop_vals = [prop.value for prop in await asyncio.gather(*[
                ...
                if len(prop_vals) < 2 or not prop_vals[0] or not prop_vals[1]:
                    logger.debug("Missing role and SQS properties that are required.")
                    return response.status_bad_request(body="Missing role and SQS properties that are required. ")
                aws_cred.role = f"arn:aws:iam::{prop_vals[0]}:role/"
                admin_cred: AWSCredentials = await request.app[HEA_DB].elevate_privileges(request, aws_cred)
                admin_cred.where = 'us-east-1'
                async with SQSClientContext(request=request,
                                            credentials=admin_cred,) as sqs_client:
                    start = time.time()
                    while True:
                        try:
                            resp = await loop.run_in_executor(None,  partial(
                                sqs_client.receive_message,
                                QueueUrl=prop_vals[1],
                                MaxNumberOfMessages=10,
                                WaitTimeSeconds=int(prop_vals[2]) if len(prop_vals) == 3 and prop_vals[2] else 20
                            ))
                            ...
                            for message in messages:
                                ...
                                async def consume_message():
                                    success = await _insert_s3_object_from_sqs(request=request, message=body)
                                    if success:
                            ...
                            if e.response['Error']['Code'] == 'ExpiredToken':
                                ...
                                logger.debug("sqs client expired and will be refreshed")
                            break
                        except Exception as e:
                            logger.debug("Failed to insert into index", exc_info=e)
        ...
        return await response.post(request, None, str(request.url))

    except ClientResponseError as e:
        logging.error('Failed to receive or process message: %s' % str(e))
        activity.status = Status.FAILED
