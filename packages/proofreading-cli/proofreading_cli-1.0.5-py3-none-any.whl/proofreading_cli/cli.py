...
@click.option(
    ...
    help="Specify start date for fetching hits. Format: (YYYY-MM-DDThh:mm:ss).",
    required=True,
)
@click.option(
    "--end-date",
    type=str,
    callback=validate_date,
    help="Specify end date for fetching hits. Format: (YYYY-MM-DDThh:mm:ss)z.",
)
@click.option(
    "--file-system",
    type=str,
...
@click.option(
    ...
    type=bool,
    default=False,
    help="Log request params to gc hit api.",
)
def hits(
    statuses: Tuple[str],
    is_submitted: bool,
    ...
    gc_api_client = ApiClient(config, debug)
