<template>
  <div class="pricing-step bg-white w-full border border-transparent rounded-md">
    ...
    >
      ...
      <div
        v-for="(fee, key) in orderPricing.aml_fees"
        v-if="orderPricing && orderPricing.aml_fees && orderPricing.aml_fees.length > 0"
        :key="key"
        class="pricing-step-content-element flex"
      >
        <div
        ...
        >
          ...
          >
            {{ (fee?.aml_fee?.aml_fee as ITypeReference).name ?? 'AML Fee' }}
            <div class="pricing-step-content-block-body-note hover-wrap contents flex items-center">
              <img
                ...
                class="warn"
              />
              <div class="pricing-step-tooltip right-tooltip">AML Fee</div>
            </div>
          </div>
          <div class="pricing-step-content-element-el flex justify-start items-center w-4/12">
            x {{ parseInt((fee as ISupplierClientFee).quantity_value) }}
          </div>
        </div>
        <div
          class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] el-border-light"
        ...
        <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
          <div class="pricing-step-content-element-el flex w-full justify-start items-center">
            <div class="input-wrap flex pr-[0.75rem]">
              <InputField
                :model-value="removeTrailingZeros((fee as ISupplierClientFee).unit_price_amount)"
                class="w-6/12 mb-0"
                :is-white="true"
                :is-half="true"
                placeholder=" "
              ...
              <SelectField
                ...
                :is-half="true"
                placeholder=" "
                :options="[(fee as ISupplierClientFee).amount_currency?.code]"
                label="description_short"
                :model-value="(fee as ISupplierClientFee).amount_currency?.code"
                :disabled="true"
              />
            </div>
          </div>
          ...
          >
            <InputField
              class="roi-input w-full"
              :model-value="formatNumber((fee as ISupplierClientFee).amount_total)"
              :is-white="true"
              placeholder=" "
              :disabled="true"
            >
              <template #suffix>{{ (fee as ISupplierClientFee).amount_currency?.code }}</template>
            </InputField>
          </div>
        </div>
      </div>
