    ...
    >
      ...
      <div class="status-buttons flex gap-3">
        ...
        <div v-if="currentStep === 1 && isFuel" class="status-buttons flex gap-3">
          ...
          <Button
            v-if="(orderStatus?.progress as IFuelProgress)?.pricing?.is_convert_quote_to_order_available"
            class="button button-green flex items-center gap-2"
            @click="onConvertToOrder"
            >Convert to Order</Button
          >
          <Button
            v-else-if="(orderStatus?.progress as IFuelProgress)?.pricing?.is_proceed_to_compliance_available"
            class="button button-green flex items-center gap-2"
      ...
      <div class="order-content-col w-4/12 flex flex-col gap-1.5">
        ...
        >
          ...
          <div class="order-content-data w-8/12 flex items-center gap-1">
            <div
              class="status sfr px-[0.5rem] py-[0.25rem] border-0 rounded-md cursor-pointer"
              :style="{
                'background-color':
                  orderStatus?.sfr?.sfr_status?.background_color ??
                  order?.sfr?.sfr_status?.background_color ??
                  'rgb(255, 255, 255)',
                color:
                  orderStatus?.sfr?.sfr_status?.text_color ?? order?.sfr?.sfr_status?.text_color
              }"
              @click="redirectToURL(order?.sfr?.url)"
            >
              {{ orderStatus?.sfr?.reference ?? order?.sfr?.reference }}
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="order?.type?.is_gh" class="order-content flex pb-[1rem] px-[1rem] gap-2">
      ...
      <div class="order-content-col w-5/12">
        ...
        <div v-if="order?.sfr" class="order-content-el flex gap-3 items-start">
          ...
          >
            <div
              class="status sfr px-[0.5rem] py-[0.25rem] border-0 rounded-md"
              :style="{
                'background-color':
                  orderStatus?.sfr?.sfr_status?.background_color ??
                  order?.sfr?.sfr_status?.background_color ??
...
import {
  useQueryConvertToOrder,
  useQueryRefreshSupplierPricing,
  useQueryReinstateOrder
...
const { refetch: convertOrder, isFetching: isFetchingConvertOrder } = useQueryConvertToOrder(
  orderId,
  {
    enabled: false,
    retry: false
