...
watch(
  () => [
    ...
    orderStore.order?.type?.is_fuel,
    orderStore.order?.fuel_order?.is_open_release
  ],
  async ([id, step, isFuel, isOpen]) => {
    if ((id && isFuel && step === 2) || (id && isOpen && step === 3)) {
      await fetchCreditExposure(+id);
    }
  }
);
</script>
