...
class Previous:
    def __init__(self, dependencies: Set["Unit"]) -> None:
       self.dependencies = sorted(list(dependencies), key=lambda node: getattr(node, "_ordering_timestamp", 0))

    def __getattr__(self, key: str) -> Union[Schema, List[Schema]]:
        # e.g., previous.score -> (previous Judge unit dependency).output.score
        if len(self.dependencies) == 1 and hasattr((unit := self.dependencies[0]).output, key):
        ...
        if (shortname := key.split('.')[0].lower()) in Unit._registry:
            ...
            matches = list(filter(lambda node: isinstance(node, _type) or node.__class__.__name__ == _type.__name__, self.dependencies))
...
class Propagator:
    ...
    @staticmethod
    def from_fn(fn: Callable[["Unit", Previous, Schema, Schema], Schema]) -> "Propagator":
        return Propagator(fn=fn)

    def __call__(self, unit: "Unit", previous: Previous, source: Schema, output: Schema) -> Schema:
...
class UnitRegistry(ABCMeta):
    ...
    def __new__(mcls, name, bases, namespace, **kwargs) -> Type["Unit"]:
        ...
        if name != 'Unit':
            if "InputSchema" not in namespace and not any(hasattr(base, "InputSchema") for base in bases):
                klass.InputSchema = Schema.inline()

            if "ResponseSchema" not in namespace and not any(hasattr(base, "ResponseSchema") for base in bases):
                raise ConfigurationError(f"Custom Unit '{name}' must define a 'ResponseSchema' class.")

            if "OutputSchema" not in namespace and not any(hasattr(base, "OutputSchema") for base in bases):
        ...
        if name and not name.startswith('_') and name != 'Unit':
            ...
            if shortname in mcls._registry:
                base_logger.warning(f"Unit '{name}' (shortname='{shortname}') name collision in UnitRegistry. Replacing existing unit.")
...
class Unit(Node, Task,
           ModelConfigurable, Promptable,
    ...
    @property
    def char(self) -> str:
        model_selection_policy = self.model_selection_policy or config.DEFAULT_MODEL_SELECTION_POLICY
        _char = f"{self._char}"
    ...
    def copy(self) -> Self:
        ...
        new.data = self.data.copy() # 2-level shallow copy (container reference + all values are copy.copy'd)
    ...
    def _materialize(self, context: MaterializationContext) -> Tuple[Self, MaterializationContext]:
    ...
    def process(self, input: InputSchemaT, response: ResponseSchemaT) -> Union[OutputSchemaT, ResponseSchemaT]:
        return response
...
class Layer(Graph[Node], Node, ModelConfigurable): # type: ignore
    ...
    def __init__(
        ...
        self.how_inner = LinkType.Inner.from_str(inner) if isinstance(inner, str) else inner
        self.how_outer = LinkType.Outer.from_str(outer) if isinstance(outer, str) else outer
    ...
    def copy(self) -> Self:
        copy = type(self)(list(map(lambda node: node.copy(), self.order)), inner=self.how_inner, outer=self.how_outer)
        copy.leaf_idx = self.leaf_idx
        copy.root_idx = self.root_idx
        return copy

    def clone(self) -> Self:
        clone = type(self)(list(map(lambda node: node.clone(), self.order)), inner=self.how_inner, outer=self.how_outer)
            ...
            with self.freeze_all_nodes():
                for i, node in enumerate(list(self.order)):
                    ...
                    with node.freeze(), node.freeze_all_nodes():
                        self.add(materialized := node._materialize(context.copy().add_prefix(f"layer[{i}]"))[0])
    ...
    def link(self, other: Union[Unit, "Layer", "Block"]) -> None: # type: ignore
        if isinstance(other, Layer) and len(self.leaf_nodes) == len(other.root_nodes):
            if self.how_outer == LinkType.Outer.BROADCAST:
                assert len(self.leaf_nodes) == len(other.root_nodes), "Broadcast link requires equal number of leaf and root nodes"
                ...
                with self.freeze(), other.freeze(), self.freeze_leaf_nodes(), other.freeze_root_nodes():
...
class Block(Graph[Union[Unit, Layer]], Node, ModelConfigurable):
    ...
    def _materialize(self, _context: Optional[MaterializationContext]=None, init: bool=True) -> Tuple["Block", MaterializationContext]:
        ...
        def map_to_destination(node: Union[Unit, Layer], context: MaterializationContext) -> Tuple[Union[Unit, Layer], MaterializationContext]:
            if node not in source_to_destination:
                source_to_destination[node], context = node._materialize(context.copy().add_prefix("block"))

            return source_to_destination[node], context

        for source, dependents in list(map(lambda node: (node, list(node.dependents)), self.sort())):
            ...
            for dependent in dependents:
                materialized_dependent, dependent_context = map_to_destination(dependent, source_context) # type: ignore
