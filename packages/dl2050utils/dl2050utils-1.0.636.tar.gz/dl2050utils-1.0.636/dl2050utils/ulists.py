...
async def ulists_insert(db, email, ulist_code, ulist_name=None, ulist_payload=None):
    ...
    if len(rows)!=0:
        row = rows[0]
        res = await db.select('ulist_items', filters=[{'col':'ulist_code', 'val':row['ulist_code']}], limit=1024)
...
async def ulists_update(db, email, ulist_code=None, ulist_name=None, ulist_payload=None):
    ...
    n = await db.update('ulists', 'ulist_code', row, read_only_cols=['ulist_code'])
    return n!=1
...
async def ulists_select(db, email, ulist_code=None):
    ...
    if ulist_code is not None:
        ...
        if len(rows)!=0:
            row = rows[0]
            res = await db.select('ulist_items', filters=[{'ulist_code':row['ulist_code']}], limit=1024)
            row['items'] = oget(res,['data'],[])
            return row
        return None
...
async def ulist_items_insert(db, email, ulist_code, item_name=None, item_payload=None, item_id_1=None, item_id_2=None):
    ...
    """
    # Create the record
    row = {'ulist_code':ulist_code, 'item_name':item_name, 'item_payload':item_payload}
...
async def ulist_items_update(db, email, item_id, ulist_code, item_name=None, item_payload=None, item_id_1=None, item_id_2=None):
    ...
    """
    # Create the record
    row = {'item_id':item_id, 'item_name':item_name, 'item_payload':item_payload}
    ...
    n = await db.update('ulist_items', 'item_id', row, read_only_cols=['item_id'])
    return n!=1
...
async def ulist_items_delete(db, email, item_id):
    ...
    Returns False on success, True on error.
    """
    n = await db.delete('ulist_items', 'item_id', item_id)
    return n!=1
