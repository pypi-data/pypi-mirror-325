    ...
    try:
        ...
        if result:
            ...
            if error[1] == MT5Manager.EnMTAPIRetcode.MT_RET_USR_LOGIN_EXHAUSTED:
                raise raise_mt5_error("USER_LOGIN_EXHAUSTED", "No free logins on server")
            elif error[1] == MT5Manager.EnMTAPIRetcode.MT_RET_USR_LOGIN_PROHIBITED:
                raise raise_mt5_error("USER_LOGIN_PROHIBITED", "Can't add user for non current server")
            elif error[1] == MT5Manager.EnMTAPIRetcode.MT_RET_USR_LOGIN_EXIST:
                raise raise_mt5_error("USER_LOGIN_EXISTS", "User with same login already exists")
            else:
