...
def test_update_notebook_details(
    ...
    Then Infrastucture stored with Mantik API includes the correct path of the
    ...
    mock_update_run_infrastructure = mocker.patch(
    ...
    mock_check_notebook_type.return_value = notebook_type
    ...
    mantik.init(
        project_id=project_id,
        run_id=str(run_id),
        mantik_access_token="test_token",
    )
    ...
    if (
        ...
        mock_update_run_infrastructure.assert_called_once()
        mock_check_notebook_type.assert_called_once()
        mock_update_notebook_source.assert_not_called()
