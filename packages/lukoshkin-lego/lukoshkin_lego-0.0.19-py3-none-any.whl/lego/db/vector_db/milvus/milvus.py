...
class MilvusDBConnector:
    ...
    def __init__(
        ...
        self._pk_is_str = schema.primary_field.dtype == 21  # VARCHAR
        ...
        self._guard_re = re.compile(r"(?<!\\)'", re.IGNORECASE)
    ...
    def register_one(
        self, item: dict[str, Any], use_guards_for_text: bool = False
    ) -> bool:
        """
        ...
        if self._pk_is_str and use_guards_for_text:
    ...
    def _add_guards_for_text(self, text: str) -> str:
        """Escape the text symbols."""
        return self._guard_re.sub(r"\'", text)
