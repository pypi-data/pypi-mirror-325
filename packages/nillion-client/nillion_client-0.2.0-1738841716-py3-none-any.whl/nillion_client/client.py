...
class VmClient:
    ...
    async def _async_init(self):
        ...
        if self.payment_mode == PaymentMode.FROM_BALANCE:
            version = await _invoke_with_retry(membership_service.node_version)(Empty())
    ...
    async def request_quote(self, operation: Operation) -> SignedQuote:
        ...
        result = await _invoke_with_retry(self.leader.payment_service.price_quote)(
            request
        )
        return result
    ...
    async def get_payment_receipt(
        ...
        result = await _invoke_with_retry(self.leader.payment_service.payment_receipt)(
            request
        )
        return result
    ...
    async def invoke_store_values(
        ...
        for member in self._members:
            ...
            response_futs.append(
                _invoke_with_retry(member.values_service.store_values)(request)
            )
    ...
    async def invoke_overwrite_permissions(
        ...
        for member in self._members:
            ...
            response_futs.append(
                _invoke_with_retry(member.permissions_service.overwrite_permissions)(
                    request
                )
            )
    ...
    async def invoke_update_permissions(
        ...
        for member in self._members:
            ...
            response_futs.append(
                _invoke_with_retry(member.permissions_service.update_permissions)(
                    request
                )
            )
    ...
    async def invoke_retrieve_permissions(self, receipt: SignedReceipt) -> Permissions:
        ...
        for member in self._members:
            ...
            response_futs.append(
                _invoke_with_retry(member.permissions_service.retrieve_permissions)(
                    request
                )
            )
    ...
    async def invoke_retrieve_values(
        ...
        async def retrieve_values(receipt, member):
            ...
            response = await _invoke_with_retry(member.values_service.retrieve_values)(
                request
            )
    ...
    async def invoke_delete_values(self, values_id: ValuesId) -> None:
        ...
        for member in self._members:
            ...
            response_futs.append(
                _invoke_with_retry(member.values_service.delete_values)(request)
            )
    ...
    async def invoke_store_program(
        ...
        for member in self._members:
            ...
            response_futs.append(
                _invoke_with_retry(member.programs_service.store_program)(request)
            )
    ...
    async def invoke_compute(
        ...
        for member in self._members:
            ...
            response_futs.append(
                _invoke_with_retry(member.compute_service.invoke_compute)(request)
            )
    ...
    async def invoke_retrieve_compute_results(
        ...
        results = await asyncio.gather(
            *[
                _invoke_with_retry(retrieve_results)(compute_id, member)
                for member in self._members
            ]
        )
    ...
    async def invoke_pool_status(
        ...
        result = await _invoke_with_retry(
            self.leader.leader_queries_service.pool_status
        )(request)
    ...
    async def balance(self) -> AccountBalanceResponse:
        ...
        """
        return await _invoke_with_retry(self.leader.payment_service.account_balance)(
            Empty()
        )
    ...
    async def add_funds(self, amount_unil: int, target_user: UserId | None = None):
        ...
        payments_config = await _invoke_with_retry(
            self.leader.payment_service.payments_config
        )(Empty())
        ...
        await _invoke_with_retry(self.leader.payment_service.add_funds)(request)
