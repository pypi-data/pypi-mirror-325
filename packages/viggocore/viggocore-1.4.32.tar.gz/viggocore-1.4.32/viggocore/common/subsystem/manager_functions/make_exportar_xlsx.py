...
class MakeExportarXsls(operation.Operation):
    def _map_column(self, column):
        ...
        if column.get('total', 'false') == 'true':
            aux['footer'] = '=sum()'
    ...
    def _treat_data_type(self, data, col):
        ...
        if data_type == 'STRING':
            ...
            result = datetime.strftime(data, '%d/%m/%Y HH:MM')
        elif data_type == 'CURRENCY':
            decimals = col.get('decimals', 2)
            result = core_utils.to_decimal(data, decimals)
        elif data_type == 'INT':
            result = int(data)
    ...
    def _formatar_footer(self, col, cell, lin_ini, qtd_linhas, is_to_bold):
        if is_to_bold:
                ...
                for col in cols:
                    ...
                    if (lin == lin_fim):
                        self._formatar_footer(
                            col, cell, lin_ini, qtd_linhas, is_to_bold)
        return
