...
def to_agent(value):
    ...
    agentid = sha256(
        str(value.role.rstrip() + llm["model"]).encode("utf-8")
    ).hexdigest()
    crew = {}
    agents = []
    tasks = []
...
class CrewAIInstrumentation(BaseInstrumentation):
    def _instrument(self, **kwargs):
        ...
        if self.has_overridden_input():
            self._wrap(
                "crewai", "Crew.kickoff", self._beforeKickoff, self._afterKickoff
            )
    ...
    def _afterKickoff(self, kwargs, result):
        if result.json_dict:
            self.write_output("application/json", result.json.encode("utf-8"))
        else:
            self.write_output("text/plain", str(result))
        return result
