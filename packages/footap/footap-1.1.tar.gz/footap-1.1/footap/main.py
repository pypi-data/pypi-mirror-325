...
def parse_args(args=None):
    ...
    # Si on est en mode module (footap) et sans arguments, afficher l'aide
    if args is None and len(sys.argv) == 1 and not os.path.basename(sys.argv[0]).endswith('main.py'):
        parser.print_help()
        sys.exit(1)
...
def main(args=None):
    ...
    # Si on exécute directement le fichier main.py
    if args is None and os.path.basename(sys.argv[0]).endswith('main.py'):
        # Mode exécution directe du script avec valeurs par défaut
        analyze_ball_touch(
