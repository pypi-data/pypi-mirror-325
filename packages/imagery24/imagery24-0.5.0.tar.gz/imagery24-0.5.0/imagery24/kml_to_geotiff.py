...
def _kml_placemarks_to_geodataframe(placemarks: list) -> GeoDataFrame:
    ...
    for placemark in placemarks:
        try:
            ...
            for geometry in placemark.MultiGeometry.getchildren():
                if "Point" in geometry.tag:
                    ...
                    print(f"Unsupported geometry type: {geometry.tag}")
                    continue
...
def _geodataframe_to_geotiff(
    ...
    _draw_text_labels(
        image, gdf, image.info["transform"], font_size=font_size, color=font_color
    )
...
def kml_to_geotiff(
    ...
    _geodataframe_to_geotiff(
        gdf, output_file, resolution, outline_width, font_size, font_color
    )
