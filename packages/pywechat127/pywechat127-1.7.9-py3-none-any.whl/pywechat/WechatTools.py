...
class Tools():
    ...
    def open_wechat(wechat_path:str=None,is_maximize:bool=True):
        ...
        '''
        #最多尝试30次，每次间隔0.4秒，12秒无法打开微信则抛出异常
        max_retry_times=30
        retry_interval=0.4
        #处理登录界面的闭包函数，点击进入微信，若微信登录界面存在直接传入窗口句柄，否则自己查找
        def handle_login_window(max_retry_times=max_retry_times, retry_interval=retry_interval, is_maximize=is_maximize,login_window_handle:int=0):
            counter = 0
            #没有传入登录界面句柄，需要自己查找(此时对应的情况是微信未启动)
            if not login_window_handle:
                Hwnd= FindWindow(Login_window.LoginWindow['class_name'], Login_window.LoginWindow['title'])
                while not Hwnd:
                    Hwnd = FindWindow(Login_window.LoginWindow['class_name'], Login_window.LoginWindow['title'])
                    if Hwnd:
                        break
            ...
            else:#传入了登录界面句柄，说明微信已经启动，但是未登录，登录界面是最小化或者在桌面上
                Hwnd=login_window_handle
            #移动登录界面到屏幕中央
            login_window = Tools.move_window_to_center(Login_window.LoginWindow, Hwnd)
            #点击登录按钮,等待主界面出现并返回
            try:
        ...
        if Tools.is_wechat_running():#微信如果已经打开无需登录可以直接连接
            ...
            if login_window_handle:#微信启动了，但是是登录界面在桌面上，不是主界面
                #传入登录界面句柄，直接处理登录界面
                return handle_login_window(max_retry_times, retry_interval, is_maximize,login_window_handle=login_window_handle)
            ...
            #处理登录界面,不用传入登录界面句柄，因为此时微信未启动，调用handle_login_window函数会自动查找登录界面
