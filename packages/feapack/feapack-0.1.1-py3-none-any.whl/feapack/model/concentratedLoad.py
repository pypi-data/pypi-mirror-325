...
class ConcentratedLoad:
    """Definition of a concentrated (nodal) load."""

    __slots__ = ("_region", "_x", "_y", "_z", "_magnitude")

    @property
    def region(self) -> str:
        """The name of the region in which the load is applied (defined by a node set)."""
        return self._region
    ...
    def __init__(self, region: str, x: float, y: float, z: float) -> None:
        """
        Creates a new concentrated (nodal) load applied in the specified region (defined by the name of a node set).
