...
class TagFormat:
    ...
    def split_tag(self, tagNo: str) -> dict:
        # Change naming pattern for groups to match python re format
        py_pattern = re.sub(r"\(\?<(\w+)>", r"(?P<\1>", self.regex_grouped)

        # Change group number with group description
...
class TagCategory:
    ...
    @staticmethod
    def get_category(inst_code, name="") -> List["TagCategory"]:
...
def _get_category_data(inst_code) -> List[dict]:
