...
class OWCorpusViewer(OWWidget, ConcurrentWidgetMixin):
    ...
    # def convert_markdown_with_syntax_highlighting(self, markdown_text):
    #     """Convertit le texte Markdown en HTML avec syntaxe de code colorée, sans utiliser la bibliothèque 'markdown'."""
    #     # Générer le CSS pour la coloration syntaxique avec Pygments
    ...
    #         key = f"<<CODEBLOCK{code_block_counter}>>"
    #         code_blocks[key] = highlighted_code
    #         code_block_counter += 1
    #         return key
    #
    #     # Remplacer les blocs de code par des marqueurs temporaires
    #     print("Texte échappé :", escaped_text)
    #     processed_text = code_block_pattern.sub(code_block_replacer, escaped_text)
    #     print("Texte après remplacement des blocs de code :", processed_text)
    #
    #     # Traiter les autres éléments Markdown
    #     # Titres
    #     processed_text = re.sub(r'^###### (.*)$', r'<h6>\1</h6>', processed_text, flags=re.MULTILINE)
    #     processed_text = re.sub(r'^##### (.*)$', r'<h5>\1</h5>', processed_text, flags=re.MULTILINE)
    ...
    #
    #     # Gras et italique
    #     processed_text = re.sub(r'\*\*\*(.+?)\*\*\*', r'<b><i>\1</i></b>', processed_text)
    ...
    #
    #     processed_text = re.sub(r'(?:^-\s.*$\n?)+', replace_unordered_list, processed_text, flags=re.MULTILINE)
    #
    #     # Listes ordonnées
    #     def replace_ordered_list(match):
    #         items = match.group(0).split('\n')
    #         list_items = ''.join(f'<li>{item.strip("0123456789. ")}</li>' for item in items if item.strip())
    #         return f'<ol>{list_items}</ol>'
    #
    #     processed_text = re.sub(r'(?:^\d+\.\s.*$\n?)+', replace_ordered_list, processed_text, flags=re.MULTILINE)
    #
    #     # Sauts de ligne
    ...
    #         processed_text = processed_text.replace(key, code_html)
    #
    #     print("Texte final après restauration des blocs de code :", processed_text)
    #     # Ajouter le CSS au début du HTML généré
    #     return css_styles + processed_text
    ...
    def show_docs(self):
        ...
        for doc_count, c_index in enumerate(sorted(self.selected_documents)):
            ...
            for feature in self.display_features:
                ...
                # Si on doit afficher des images, les convertir en balise HTML
                is_image = feature.attributes.get("type", "") == "image"
                if is_image and value != "?":
                    value = os.path.join(feature.attributes.get("origin", ""), value)
                    value = f'<img src="{value}"></img>'

                # Si la valeur est du Markdown, le convertir en HTML
                elif self.is_markdown(value):
                    # Convertir le Markdown en HTML avec coloration syntaxique
                    value = self.convert_markdown_with_syntax_highlighting(value)

                # Sinon, juste remplace les retours à la ligne par des balises <br/>
                else:
                    value = value.replace("\n", "<br/>")
