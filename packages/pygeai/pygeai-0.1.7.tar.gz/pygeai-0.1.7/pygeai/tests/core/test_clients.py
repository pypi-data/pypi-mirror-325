...
class TestGeai(TestCase):
    ...
    def test_create_text_assistant_mocked(self, mock_create_assistant):
        ...
        mock_create_assistant.return_value = {"id": "123", "name": "Text Assistant"}

        assistant = TextAssistant(name="Text Assistant", prompt="Prompt", description="Description")
        llm_settings = LlmSettings(model="GPT-4", temperature=0.7)
        welcome_data = WelcomeData(messages=["Welcome!"])

        response = self.client.create_text_assistant(assistant, llm_settings, welcome_data)
    ...
    def test_create_chat_assistant_mocked(self, mock_create_assistant):
        ...
        mock_create_assistant.return_value = {"id": "456", "name": "Chat Assistant"}

        assistant = ChatAssistant(name="Chat Assistant", prompt="Chat Prompt", description="Description")
        llm_settings = LlmSettings(model="GPT-4", temperature=0.8)
        welcome_data = WelcomeData(messages=["Hello!"])

        response = self.client.create_chat_assistant(assistant, llm_settings, welcome_data)
        self.assertIsInstance(response, AssistantResponse)