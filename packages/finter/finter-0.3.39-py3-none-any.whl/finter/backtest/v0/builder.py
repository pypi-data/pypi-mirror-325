    ...
    def __build_data(self, position: pd.DataFrame) -> DataConfig:
        ...
        if not position.empty and not price.empty:
            ...
            aum = _align_data_with_position(
                money_flow.cumsum(), price_start_date, position, align_columns=False
            )
            if not aum.empty:
...
class DataProcessor:
    ...
    def preprocess_volume_capacity(config: SimulatorConfig, data: DataConfig):
        if config.trade.volume_capacity_ratio == 0:
            ...
            return volume.fillna(0).to_numpy() * config.trade.volume_capacity_ratio
...
if __name__ == "__main__":
    ...
    (
        ...
        .update_trade(initial_cash=1e4, volume_capacity_ratio=0.1)
        .update_execution(core_type="basic", drip="cash")
