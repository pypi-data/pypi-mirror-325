...
class DyntamicFactory:
    ...
    def _make_nested(self, model_name: str, field, default) -> None:
        ...
        schema = {self.ref_template: self.definitions} | self.definitions.get(model_name)
        schema["title"] = schema.get("title", model_name)
        level = DyntamicFactory(schema,
...
def json_schema_to_pydantic_model(schema, **kwargs):
    f = DyntamicFactory(schema, **kwargs)
    return f.make()
