...
def assert_output_equal(expected_output, actual_output):
    if expected_output != actual_output:
        ...
        for i, (expected_line, actual_line) in enumerate(
            zip(chain(iter(expected_output.split("\n")), cycle("")), actual_output.split("\n"))
        ):
            if expected_line != actual_line:
                line_diff = ""
                actual_line = actual_line.ljust(len(expected_line))
                for j, (expected_char, actual_char) in enumerate(
                    zip(chain(iter(expected_line), cycle(" ")), actual_line)
                ):
