...
class Dynamic_Table_Creation:
    ...
    def check_table_exists(self, table_name, connection):
        ...
        try:
            if con_type == 'Oracle':
                cursor.execute(f"SELECT COUNT(*) as CNT FROM USER_TABLES WHERE TABLE_NAME = '{table_name}'")
            else:
                cursor.execute(f"SELECT COUNT(*) as CNT FROM information_schema.tables WHERE table_name = '{table_name.lower()}'")
            count = cursor.fetchone()[0]
            cursor.close()
            logger.log(f"Connection type count::: {count}")
            return count > 0
    ...
    def check_column_exists(self, table_name, column_name, connection):
        ...
        try:
            if con_type == 'Oracle':
                ...
                cursor.execute(f"SELECT COUNT(*) as CNT FROM information_schema.columns WHERE table_name = '{table_name.lower()}' AND column_name = '{column_name.lower()}';")
            count = cursor.fetchone()[0]
            cursor.close()
            return count > 0
        except (cx_Oracle.Error, psycopg2.Error) as error:
