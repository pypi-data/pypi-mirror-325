...
#[pyfunction]
fn init_resample_kline_cluster(
    ws_url: String,
    instrument_ids: Vec<String>,
    freq: String,
    callback: PyObject,
    ...
    init_logging();
    info!("Initializing resample kline cluster...");
    ...
    rt.block_on(async {
        ...
        let mut resample_nodes = HashMap::new();

        // 为每个 instrument 初始化基础节点
        for instrument_id in &instrument_ids {
            info!("Initializing nodes for {}", instrument_id);
            ...
            node.start(Python::with_gil(|py| empty_callback.clone_ref(py))).await.map_err(|e| {
                error!("Failed to start resample kline node: {}", e);
                PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string())
            })?;

            resample_nodes.insert(instrument_id.clone(), Arc::new(node));
        }

        // 创建并启动 cluster
        let cluster = ResampleKlineCluster::new(freq, resample_nodes, ws_client.clone(),);
        cluster.start(callback).await.map_err(|e| {
...
fn rnode(_py: Python, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(init_resample_kline_cluster, m)?)?;
    Ok(())
}
