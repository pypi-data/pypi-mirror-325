...
class OpenSearch(Database):
    ...
    async def update (self, doc: HEAObjectDict, index: str | None = None) -> Optional[str]:
            """
    ...
    async def __batch_operation(self, bulk_gen: AsyncGenerator[Any, None], action_type: str) -> bool:
        ...
        try:
            ...
            if actions:
                ...
                response = await self.__client.bulk(body=bulk_body)
                if response["errors"]:
                    logger.debug(f"Some documents failed during batch {action_type}.")
            success = True
        except RequestError as e:
            raise RuntimeError(f"Error in batch {action_type}: {e.info}")
        finally:
            return success
