...
class BrainRotPlusApi:
    ...
    def get_projects_with_feeds_api_v1_recommendation_projects_with_feeds_get(
        ...
        Retrieve all projects with active BrainLift feeds for the authenticated user. Results are cached for 5 minutes to improve performance.  The endpoint returns projects that: 1. The user has access to 2. Have BrainLift enabled 3. Have active feeds  Results are sorted by last feed update time.
