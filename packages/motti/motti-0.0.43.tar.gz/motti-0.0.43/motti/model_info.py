...
def get_input_output_per_layer(model, model_input):
    ...
    handles = OrderedDict()
    HookData = namedtuple("HookData", ["input", "output"])
    HookDataShape = namedtuple("HookDataShape", ["input_shape", "output_shape"])
    
    def hook(name):
        def hook_fn(module, input, output):
            features[name] = HookData(input, output)
        return hook_fn
...
def summary(model, model_input):
    ...
    for f_key, io_data in features.items():
        ...
        print(f"{f_key}: {input_shape} --> {output_shape}")
        for p_key in mapping[f_key]:
            print(" "*4, f"{p_key}: {parameters[p_key].shape}")
