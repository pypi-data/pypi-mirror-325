...
from relationalai.tools.constants import USE_GRAPH_INDEX
...
class Client():
    ...
    def _manage_packages(self):
        ...
        try:
            ...
            enable_full_package_manager = self._config.get("use_package_manager", False) or self._config.get("compiler.use_monotype_operators", False)
    ...
    def exec_raw(self, code:str, readonly=True, raw_results=True, inputs={}, internal=False, nowait_durable=None, headers={}):
        ...
        return self.query(task, readonly=readonly, raw_results=raw_results, inputs=inputs, internal=internal, nowait_durable=nowait_durable, headers=headers, abort_on_error=not raw_results)
    ...
    def query(self, task:m.Task, rentrant=False, readonly=True, raw_results=False, inputs={}, format="pandas", tag=None, nowait_durable=None, headers={}, internal=False, abort_on_error=True) -> DataFrame|Any:
        ...
        with debugging.span("query", model=self._database, task=task, tag=tag, internal=internal, task_id=task.id, readonly=readonly) as end_span:
            ...
            code += "\ndef output[:__pyrel_debug_watch]: __pyrel_debug_watch"
