...
class AceParser(ArgumentParser, Generic[DataClassType]):
    ...
    def _add_dataclass_arguments(self, dtype: DataClassType) -> None:
        parser = self.add_argument_group(dtype._argument_group_name) if hasattr(dtype, "_argument_group_name") else self  # noqa: SLF001
    ...
    def parse_args_into_dataclasses(
        ...
        if remaining_args:
            class_names = [c.__name__ for c in self.dataclass_types]
            msg = f"Some specified arguments are not used by the AceParser with data classes {class_names}: {remaining_args}"
            raise ValueError(msg)
    ...
    def parse_dict(self, args: Mapping[str, Any], *, allow_extra_keys: bool = False) -> ReturnType:
        ...
        if not allow_extra_keys and unused_keys:
            msg = f"Some keys are not used by the AceParser: {sorted(unused_keys)}"
            raise ValueError(msg)
