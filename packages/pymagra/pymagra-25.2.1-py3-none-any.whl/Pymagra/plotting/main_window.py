...
# Plot a line and check whether a line has already been plotted.
    ...
    def plot_lineaments(self, ax, dfac=1.):
        """
        ...
        for key in self.lineaments.keys():
            ...
            if self.lineaments[key]["type"] in self.legend:
                ...
                ax.plot(x, y, col, label=self.lineaments[key]["type"], ls="--",
                        linewidth=3)
...
# Do triangulation plot
        gci = ax.tricontourf(triang, s, extend="both", vmin=min_col,
                             vmax=max_col, levels=levels, cmap=cmap, norm=norm)
        ax.set_clip_on(True)
...
# Erase actual plot in central widget and plot the new one
    ...
    def plot_sensor_image(self, ax, x, y, s, vmin, vmax, percent, cmap,
                          xlabel, ylabel, title, cbar_title, dec=0.0, dfac=1.):
        """
        Plot map of one sensor by triangulation
...
# Do plot for sensor 1
        if self.grad_data:
            ...
            txt = title
        self.plot_sensor_image(ax[0], x, y,
                               data.sensor1_inter, mincol1, maxcol1,
                               percent, c, xlabel, ylabel, txt,
...
# Do plot for sensor 2
        if self.grad_data:
            if ";" in title[:-1]:
                ...
                txt = title + " Sensor 2"
            self.plot_sensor_image(ax[1], x, y,
                                   data.sensor2_inter, mincol2, maxcol2,
                                   percent, c, xlabel, ylabel, txt,
                                   f"Field strength [{self.unit}]", dec=dec,
                                   dfac=dfac)
# Do plot for gradient
            if grad_flag:
                if ";" in title[:-1]:
                    ...
                    txt = title + " Gradient"
                self.plot_sensor_image(ax[2], x, y,
                                       data.grad_inter, mincolg, maxcolg,
                                       percent, c, xlabel, ylabel, txt,
