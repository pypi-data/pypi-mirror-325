...
class TestPressureVessel:
    ...
    def test_output_function(self):
        ...
        assert capex == approx(
            self.pressure_vessel_instance.compressed_gas_function.cost_kg[5] * capacity,
            tol,
        )
        assert opex == approx(
    ...
    def test_distributed(self):
        ...
        (
            ...
            opex_dist_10,
            energy_kg_dist_10,
            area_footprint_site_10,
            mass_tank_empty_site_10,
            capacity_site_10,
        ...
        (
            ...
            opex_dist_20,
            energy_kg_dist_20,
            area_footprint_site_20,
            mass_tank_empty_site_20,
            capacity_site_20,
        ...
        assert (
            (capex < capex_dist_05)
            and (capex_dist_05 < capex_dist_10)
            and (capex_dist_10 < capex_dist_20)
        ), "capex should increase w/ number of sites"
        assert (
            (opex < opex_dist_05)
            and (opex_dist_05 < opex_dist_10)
            and (opex_dist_10 < opex_dist_20)
        ), "opex should increase w/ number of sites"
        assert (
            (energy_kg == approx(energy_kg_dist_05))
            and (energy_kg_dist_05 == approx(energy_kg_dist_10))
            and (energy_kg_dist_10 == approx(energy_kg_dist_20))
        ), "energy_kg be approx. equal across number of sites"
...
class PlotTestPressureVessel:
    ...
    def plot_size_and_divisions(self):
        ...
        for (
            j,
            div,
        ) in enumerate(divisions):
            for i, capacity in enumerate(capacity_range):
                if div == 1:
                    capex_results[j, i], opex_results[j, i], energy_kg = (
                        self.pressure_vessel_instance.calculate_from_fit(capacity)
                    )
                else:
                    (
                        capex_results[j, i],
        ...
        for j in np.arange(0, len(divisions)):
            ax[0, 0].plot(
                capacity_range / 1e3,
                capex_results[j] / capacity_range,
                label=f"{divisions[j]} Divisions",
            )
            ax[0, 1].plot(
                capacity_range / 1e3,
                opex_results[j] / capacity_range,
                label=f"{divisions[j]} Divisions",
            )
        ...
        for j in np.arange(0, len(divisions)):
            ax[1, 0].plot(
                capacity_range / 1e3,
                capex_results[j] / capex_results[0],
                label=f"{divisions[j]} Divisions",
            )
            ax[1, 1].plot(
                capacity_range / 1e3,
                opex_results[j] / opex_results[0],
                label=f"{divisions[j]} Divisions",
            )
