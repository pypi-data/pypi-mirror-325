...
def run_pipe_array(sections_distance, depth, p_inlet, p_outlet, mass_flow_rate):
    ...
    for i, pipe_string in enumerate(sections_distance):
        ...
        for j, section_length in enumerate(flip(pipe_string)):
            ...
            section_outputs = run_pipe_analysis(
                section_length, m_dot, p_inlet, p_outlet_section, depth, risers=risers
            )
...
def run_pipe_array_const_diam(sections_distance, depth, p_inlet, p_outlet, mass_flow_rate):
    ...
    for i, pipe_string in enumerate(sections_distance):
        ...
        risers = (
            len(pipe_string) + 1
        )  # Wasnt sure on this - is it 1 per turbine + 1 for the storage? - Jamie

        #   get specs and costs for each section
        section_outputs = run_pipe_analysis(
            tot_length, m_dot, p_inlet, p_outlet, depth, risers=risers
        )
...
if __name__ == "__main__":
    sections_distance = [
        [2.85105454, 2.016, 2.016, 2.016, 2.016, 2.016, 2.016, 2.016],
