...
def size_electrolyzer_for_end_use(greenheart_config):
    hybrid_electricity_estimated_cf = greenheart_config["project_parameters"][
    ...
    if "ammonia" in list(greenheart_config.keys()):
        ...
        config = ammonia.AmmoniaCapacityModelConfig(
            input_capacity_factor_estimate=greenheart_config["ammonia"]["capacity"][
                "input_capacity_factor_estimate"
            ],
            feedstocks=feedstocks,
            desired_ammonia_kgpy=greenheart_config["ammonia"]["capacity"][
    ...
    if "steel" in list(greenheart_config.keys()):
        ...
        config = steel.SteelCapacityModelConfig(
            input_capacity_factor_estimate=greenheart_config["steel"]["capacity"][
                "input_capacity_factor_estimate"
            ],
            feedstocks=feedstocks,
    ...
    greenheart_config["electrolyzer"]["sizing"]["hydrogen_dmd"] = (
        hydrogen_production_capacity_required_kgphr
    )
