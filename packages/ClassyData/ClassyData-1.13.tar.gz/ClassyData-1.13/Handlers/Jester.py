...
class Jester(object):
	...
	def _locatePackage(self,name,create=True):
		...
		if name:
			parts = re.split('[\.\/]',name)
		for part in parts:
			package = self.session.query(Package).filter_by(name=part,parent=parent).first()
	...
	def _locateNamedType(self,tipe,name, create=True):
		clasz = self.session.query(tipe).filter_by(name=name).first()
		if not clasz and create:
	...
	def packages(self,package=None):
		pakages = []
		for pakage in self.session.query(Package).filter_by(parent=package):
