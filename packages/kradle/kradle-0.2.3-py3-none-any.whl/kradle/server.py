...
class Kradle:
    ...
    def serve_agent(cls, agent_class, slug, port=None, tunnel=True, host='localhost', hot_reload=True):
        """Start the agent server with API key validation and hot reload support.
        
        Args:
            agent_class: The agent class to serve
            slug: Unique identifier for the agent
            port: Optional port number (default: None, will find free port)
            tunnel: Whether to create a public tunnel (default: True)
            host: Host to bind to (default: 'localhost')
            hot_reload: Enable hot reloading (default: True)
        
        Returns:
            str: The URL where the agent is being served
        """
        instance = cls()
        ...
        agent_url = (f"{instance._tunnel_url}/{slug}" if hasattr(instance, '_tunnel_url') 
                    else f"http://localhost:{instance.port}/{slug}")
