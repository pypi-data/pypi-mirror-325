...
class Particle(Tube):
    ...
    # private own methods
    # --------------------------------------------------------
    def _get_wip_completed_edge_async(self, buffer, context) -> EDGE:
    ...
    async def _wip_feed(self, buffer, context):
        ...
        for wave__, uid, data in self._ordered_streaming(buffer):
            ...
            for edge in self._get_wip_completed_edge_async(buffer, context):
                yield edge
        else:
            # milk?
            pass
...
class XParticle(iGeo, Particle):
    ...
    def _pre_lambda(self, old_value: Walk, new_value: Walk, op, lambda_name):
        ...
        for key, value in new_value.ordered:
            ...
            if isinstance(value, (int, float, FloatWhen)):
                key1 = str(key[-1])
                if re.match(REG_PRIVATE_KEY, key1):
                    continue
    ...
    def _get_wip_completed_edge_sync(self) -> EDGE:
        """
        ...
        for uid in self._wip_known_keys:
            ...
            if not (self._wip_cursor.get(uid)):
                if interval := self._find_bound_interval(uid):
                    ...
                    log.info("Input sources are exhausted, we need to wait until more data is comming")
                    return
        ...
        if all(self._wip_cursor):
            ...
            for uid, stream in self._wip_stream.items():
                ...
                if not holder_keys or holder_keys[-1] < min_requiered_key[-1]:
                    self._milk.add(uid)
                    log.debug("request data for [%s], current: [%s]", uid, holder_keys[-1])
                else:
                    if drop:
                        log.debug("[%s] has drop some values that can't be used for computing", uid)
            ...
            if self._milk:
                ...
                log.debug("we need fill all buffers for [%s] to start computing", min_requiered_key)
                # self._wip_cursor.clear() # TODO: isn't neccesary, but I better for debugging
                return
            else:
                log.debug("all buffers filled for [%s], start computing", min_requiered_key)
                foo = 1
        else:
        ...
        for uid, stream in self._wip_stream.items():
            (k0, k1) = self._wip_cursor[uid] # = (k0, k1) = self._find_bound_interval(uid)
            ...
            for key in keys:
                ...
                log.error("you never must reach this line, buffers must be full filled before computing!")
    ...
    def _get_wip_completed_edge_async(self) -> EDGE:
        """
        1. find the next time mark (t0) from the 1st data received
        2. t0 > datetime of 1st data point
        3. create a record for each different data point in order to check
        ...
        if self.delivery in ("sync", "asap") and all(self._wip_cursor):
            ...
            if self._milk and self.delivery in ("sync", ):
                # let upper level milk extra data for us
                log.debug("Policy: [%s] we need fill all buffers for [%s] to start computing", self.delivery, min_requiered_key)
                # self._wip_cursor.clear() # TODO: isn't neccesary, but I better for debugging
                return
            else:
                log.debug("Policy: [%s] needed buffers filled for [%s], start computing", self.delivery, min_requiered_key)
                foo = 1
        # else:
        #     raise RuntimeError("some sources are not available for computing")
    ...
    async def _wip_feed(self, buffer, context):
        ...
        for edge in self._get_wip_completed_edge_async():
            yield edge
        ...
        async for data in super()._compute(edge, ekeys):
            ...
            for key in self.available_lambdas.intersection(data):
                ...
                for attr, value in sub_data.items():
                    ...
                    if key in ("copy",) or not isinstance(
