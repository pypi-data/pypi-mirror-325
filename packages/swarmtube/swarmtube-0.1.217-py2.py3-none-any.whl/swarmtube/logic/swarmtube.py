    ...
    def _get_wip_completed_edge(self) -> EDGE:
        ...
        if all(self._wip_cursor):
            ...
            for uid, stream in self._wip_stream.items():
                ...
                if holder_keys[-1] < min_requiered_key[-1]:
                    self._milk.add(uid)
                    if key:
                        log.debug("request more data from: [%s] > [%s]", uid, key)
                else:
            ...
            if self._milk:
                ...
                log.debug("we need to reach [%s] key to start computing", min_requiered_key[1])
        ...
        for uid, stream in self._wip_stream.items():
            self._wip_cursor[uid] = (k0, k1) = self._find_bound_interval(uid)
