...
def ordered(items: Iterable[str], indent: int = 0, html: bool = False) -> LineGen:
    ...
    for item in items:
        ...
        if type(item) is str:
            yield "<li>{}</li>".format(item) if html else "{}1. {}".format(
                indent * " ", item
...
def unordered(items: Iterable[str], indent: int = 0, html: bool = False) -> LineGen:
    ...
    for item in items:
        if html:
            yield "<ul style='list-style-type:disc;padding-left:20px;margin-top:0;margin-bottom:0;'>"
        if type(item) is str:
            yield "<li style='margin-top:-0.2em; margin-bottom:-0.2em'>{}</li>".format(
                item
            ) if html else "{}* {}".format(indent * " ", item)
        else:
