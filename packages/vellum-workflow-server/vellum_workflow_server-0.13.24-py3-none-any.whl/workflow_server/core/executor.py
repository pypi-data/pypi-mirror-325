...
def stream_workflow(
    ...
    def call_workflow() -> Generator[dict[str, Any], Any, None]:
        ...
        for event in stream:
            ...
            if event.name.endswith("rejected") and event.body.error.code.name == "INTERNAL_ERROR":
                logger.exception(event.body.error.message)
                event.body.error = WorkflowError(code=WorkflowErrorCode.INTERNAL_ERROR.value, message="Internal Error")
