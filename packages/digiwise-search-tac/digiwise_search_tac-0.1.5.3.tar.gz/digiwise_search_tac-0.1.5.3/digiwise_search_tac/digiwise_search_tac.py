...
def init():
    """
    Load the Excel file from the package and return the 'digiwise_search_SIM SLOT NUMBER' function.
    """
    global _DICTIONARY_CACHE

    # Locate the Excel file inside the package
    with pkg_resources.as_file(pkg_resources.files("digiwise_search_tac").joinpath("MSRPLUS20231218.xlsx")) as path:
        if _DICTIONARY_CACHE is None:
            df = pd.read_excel(path, sheet_name='sh1')
