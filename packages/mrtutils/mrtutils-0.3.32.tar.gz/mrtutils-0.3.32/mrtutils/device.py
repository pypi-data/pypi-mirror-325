...
class RegFieldSpec:
    ...
    def getFieldMaskMacro(self, spacing = 0):
        print(self.reg.pattern)

        if(self.flat):
            ret = self.reg.device.prefix.upper() +"_"+self.name.upper() +"_FIELD_MASK"
        else :
    ...
    def getFieldOffsetkMacro(self, spacing = 0):
        if(self.flat):
            ret = self.reg.device.prefix.upper() +"_"+self.name.upper() +"_FIELD_OFFSET"
        else:
    ...
    def getFieldFlagMacro(self, spacing = 0):
        ...
        if(self.flat):
            ret = self.reg.device.prefix.upper() +"_"+self.name.upper()
        else: 
        ...
        if 'registers' in objDevice:
            ...
            for regItemRaw in regs:
                ...
                if '$n' in regItemRaw['reg_name']:
                    ...
                    regItemRaw['pattern'] = regItemRaw['reg_name']
