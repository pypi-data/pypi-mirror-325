...
class Chat:
    ...
    def reset(self, system=''):
        ...
        self.pids = 0
        self.hx = []
    def __call__(self, inputs, max_new=500, chat_fmt=True, verbose=True, stream=None):
        ...
        for i in range(max_new):
            ...
            if stream and i % 10 == 2:
                ...
                if idx_split > 0:
                    # print(txt[idx_sofar:idx_split], end='', flush=True)
                    f.write(txt[idx_sofar:idx_split])
                    f.flush()
                    idx_sofar = idx_split
            if goon.sum() < 1:
                break
