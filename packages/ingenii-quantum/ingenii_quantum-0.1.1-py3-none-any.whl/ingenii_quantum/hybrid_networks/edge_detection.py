...
class EdgeDetectorBase:
    ...
    This class implements both Pytorch-based and Qiskit-based quantum Hadamard 
    edge detection algorithms for images and 3D volumes.

    Attributes:
        n_dimensions (int): Number of dimensions (2D or 3D).
        size (int): Size of the (NxN) blocks used for partitioning the image.
        backend (str): Computational backend ('torch' for PyTorch, 'aer_simulator' for Qiskit).
        shots (int): Number of shots for Qiskit experiments.
        total_qb (int): Total number of qubits used in the quantum circuit.
        anc_qb (int): Number of ancilla qubits.
        data_qb (int): Number of qubits required for data encoding.
        device (torch.device, optional): PyTorch device ('cpu' or 'cuda'), only for `torch` backend.

    '''
    def __init__(self, n_dimensions: int, size: int, backend: str, shots: int):
        """
        Initializes the EdgeDetectorBase class.

        Args:
            n_dimensions (int): Number of dimensions (2D or 3D).
            size (int): Size of the (NxN) blocks for partitioning the image.
            backend (str): 'torch' for PyTorch or 'aer_simulator' for Qiskit.
            shots (int): Number of shots for Qiskit experiments.
    ...
    def run_box(self, box, tol=1e-3):
        ...
        This function executes a quantum Hadamard-based edge detection 
        algorithm for a given image segment.
        ...
        Returns:
            np.array: Edge-detected version of the input box.
        """
        ...
        with Session(backend=self.backend) as session: # Create a session with such backend
            ...
            passmanager = generate_preset_pass_manager(
                optimization_level=2, backend=self.backend
            )
            qc_noise = passmanager.run(qc)

            pub = (qc_noise,) # Prepare input
            job = sampler.run([pub]) 
    ...
    def run_image_torch(self, data, tol=1e-3, reduce=True, verbose=False):
        ...
        Returns:
            torch.Tensor: Edge-detected image.
        """
        if verbose:
            start_time = time()
...
class EdgeDetector2D(EdgeDetectorBase):
    ...
    This class implements both Qiskit-based and PyTorch-based quantum edge detection 
    algorithms for two-dimensional images.
    ...
    def __init__(self, size, backend='aer_simulator', shots=1000):
        """
        Initializes the 2D Quantum Hadamard Edge Detection algorithm.

        Args:
            size (int): Size of the (NxN) blocks for partitioning the image.
            backend (str, optional): 'torch' for PyTorch or 'aer_simulator' for Qiskit. Defaults to 'aer_simulator'.
            shots (int, optional): Number of shots for Qiskit experiments. Defaults to 1000.
        """
    ...
    def run(self, data, tol=1e-3, reduce=True, verbose=False):
        ...
        Returns:
            np.array/tensor: Edge-detected image
        """
...
class EdgeDetector3D(EdgeDetectorBase):
    ...
    This class implements both Qiskit-based and PyTorch-based quantum edge detection 
    algorithms for three-dimensional images.
    ...
    def __init__(self, size, backend='aer_simulator', shots=100):
        """
        Initializes the 3D Quantum Hadamard Edge Detection algorithm.

        Args:
            size (int): Size of the (NxN) blocks for partitioning the image.
            backend (str, optional): 'torch' for PyTorch or 'aer_simulator' for Qiskit. Defaults to 'aer_simulator'.
            shots (int, optional): Number of shots for Qiskit experiments. Defaults to 100.
        """
