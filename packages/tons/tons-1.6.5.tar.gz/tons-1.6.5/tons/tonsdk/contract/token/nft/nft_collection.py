...
class NFTCollection(Contract):
    ...
    def create_content_cell(self, params) -> Cell:
        ...
        content_cell = Cell()
        content_cell.store_ref(collection_content_cell)
        content_cell.store_ref(common_content_cell)
        return content_cell
    ...
    def create_data_cell(self) -> Cell:
        ...
        cell.store_ref(self.create_content_cell(self.options))
        cell.store_ref(Cell.one_from_boc(self.options['nft_item_code_hex']))
        cell.store_ref(self.create_royalty_cell(self.options))
        return cell
    ...
    def create_mint_body(
        ...
        content_cell.store_ref(uri_content)
        body.store_ref(content_cell)
        return body
    ...
    def create_batch_mint_body(
        ...
        for i, (item_content_uri, new_owner_address) in \
            ...
            uri_content = Cell()
            uri_content.bits.write_bytes(serialize_uri(item_content_uri))
            content.store_ref(uri_content)
            item.store_ref(content)
            deploy_list.store_cell(i + from_item_index, item)
        body.store_ref(deploy_list.end_dict())
        return body
    ...
    def create_edit_content_body(self, params) -> Cell:
        ...
        body.store_ref(self.create_content_cell(params))
        body.store_ref(self.create_royalty_cell(params))
        return body
