...
class JettonWallet(Contract):
    ...
    def create_transfer_body(
            ...
            forward_payload: t.Union[Cell, str] = None,
        ...
        if forward_payload is None:
            ...
            if isinstance(forward_payload, Cell):
                pass
            elif isinstance(forward_payload, str):
                forward_payload = Contract.text_message_to_cell(forward_payload)
            else: 
                raise ValueError("forward_payload expected types:  None | Cell | str")
