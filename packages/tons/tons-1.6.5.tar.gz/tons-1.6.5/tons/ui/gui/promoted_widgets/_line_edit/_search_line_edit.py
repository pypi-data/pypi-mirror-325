...
from tons.ui.gui.utils import qt_exc_handler, get_icon, get_icon_pixmap


# TODO refactor DRY EntityNameLineEdit - make IconLineEdit


class SearchEditProxyStyle(MyLineEditProxyStyle):
    @qt_exc_handler
    def subElementRect(self, element: QStyle.SubElement, option: Optional['QStyleOption'],
        ...
        if element == QStyle.SubElement.SE_LineEditContents:
            assert isinstance(widget, SearchLineEdit)
...
class SearchLineEdit(MyLineEdit):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._icon: QIcon = get_icon('magnifying-glass-solid.svg')
        self.setStyle(SearchEditProxyStyle())
