...
class JettonMinter(Contract):
    ...
    def create_data_cell(self) -> Cell:
        ...
        cell.store_ref(create_offchain_uri_cell(self.options['jetton_content_uri']))
        cell.store_ref(Cell.one_from_boc(self.options['jetton_wallet_code_hex']))
        return cell

    def create_mint_body(self, destination: Address, jetton_amount: int, amount: int = 50000000,
                         query_id: int = 0) -> Cell:
        body = Cell()
    ...
    def create_edit_content_body(self, jetton_content_uri: str, query_id: int = 0) -> Cell:
        ...
        body.bits.write_uint(query_id, 64)  # query_id
        body.store_ref(create_offchain_uri_cell(jetton_content_uri))
        return body
