...
class TestGeometry(TestWithTimer):
    ...
    def test_vbur_lebedev(self):
        ...
        if not np.isclose(vbur, 86.5, atol=0.15):
            print("V_bur =", vbur, "expected:", 86.5)
        self.assertTrue(np.isclose(vbur, 86.5, atol=0.15))

        # 32, 974
        vbur = geom.percent_buried_volume(
            method="lebedev", rpoints=32, apoints=974
        )
        if not np.isclose(vbur, 86.6, atol=0.15):
            print("V_bur =", vbur, "expected:", 86.6)
        self.assertTrue(np.isclose(vbur, 86.6, atol=0.15))

        # 64, 590
        vbur = geom.percent_buried_volume(
            method="lebedev", rpoints=64, apoints=590
        )
        if not np.isclose(vbur, 86.4, atol=0.1):
            print("V_bur =", vbur, "expected:", 86.4)
        self.assertTrue(np.isclose(vbur, 86.4, atol=0.1))

        # 75, 302
        vbur = geom.percent_buried_volume(
            method="lebedev", rpoints=75, apoints=302
        )
        if not np.isclose(vbur, 86.4, atol=0.2):
            print("V_bur =", vbur, "expected:", 86.4)
        self.assertTrue(np.isclose(vbur, 86.4, atol=0.2))

        # 99, 194
        vbur = geom.percent_buried_volume(
            method="lebedev", rpoints=99, apoints=194
        )
        if not np.isclose(vbur, 86.8, atol=0.5):
            print("V_bur =", vbur, "expected:", 86.8)
        self.assertTrue(np.isclose(vbur, 86.8, atol=0.5))
        ...
        if not np.isclose(vbur, 86.6, atol=0.1):
            print("V_bur =", vbur, "expected:", 86.6)
        self.assertTrue(np.isclose(vbur, 86.6, atol=0.1))
