...
from .tools import getip, Ftpserver, Fileserver, HZGTGUI
...
@click.argument('directory', nargs=1, type=click.STRING)
@click.option("-r", "--res", default=getip(-1), type=click.STRING, help="选填- IP地址", show_default=True)
...
@click.option("-s", "--savepath", default=os.path.join(os.path.expanduser("~"), "Downloads"), type=click.STRING, help="必填~ 目录路径")
def dwnl(url, savepath):
    """
    下载模块
...
losf.add_command(dwnl)
losf.add_command(fs)
losf.add_command(gui)
losf.add_command(getip)
