...
class Mysqlop:
    def __init__(self, host: str, port: int, user: str, passwd: str, charset: str = "utf8", logger=None):
        """
        初始化mmysql类
    ...
    def create_table(self, tablename: str, attr_dict: dict, primary_key: list[str] = None,
        ...
        if bool_autoselect:
            self.__selected_table = tablename
            self.__logger.debug(f"已自动选择表{tablename}")
    ...
    def insert(self, tablename: str = '', record: dict = None):
        ...
        if not existing:
            ...
            try:
                ...
                self.__logger.error(f"插入失败: {e.__class__.__name__}: {e}")
                raise Exception(f"插入失败: {e.__class__.__name__}: {e}") from None
        else:
            self.__logger.warning(f"记录 {record} 已存在，未执行插入操作。")
