...
def findmask(file, mission_id):
    ...
    if mask_name is not None:
        ...
        if mask_path.exists():
            mask = np.uint8(pdr.read(mask_path)['IMAGE'])
...
def apply_mask(tiff_image, mask, output_path=None):
    ...
    if len(img.shape) == 2:
        ...
        tifffile.imwrite(output_path, img, photometric='minisblack', extrasamples=['unassalpha'])
    elif img.shape[2] == 3:
        img = np.concatenate([img, mask], axis=2)
        tifffile.imwrite(output_path, img, photometric='rgb', extrasamples=['unassalpha'])
