...
class MarsImageBase:  # noqa: PLR0904
    ...
    def __init__(self, filename):
        ...
        if self.fname.with_suffix('.LBL').exists():
            self._data = pdr.fastread(self.fname.with_suffix('.LBL'))
        else:
            self._data = pdr.fastread(self.fname)
    ...
    def rawtherapee_convert(self, save_path, apply_mask=True):
        ...
        tiff_path = save_dng(self, dng_path, rawtherapee_convert=True, remove_dng=True)
    ...
    def pixel_averaging(self):
        ...
        """
        w = self.metafind(('IMAGE_REQUEST_PARMS', 'PIXEL_AVERAGING_WIDTH'), 1)
        h = self.metafind(('IMAGE_REQUEST_PARMS', 'PIXEL_AVERAGING_HEIGHT'), 1)
