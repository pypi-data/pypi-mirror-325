...
class ArdentClient:
    ...
    def set_config(
        ...
        **config_params: Any
        ...
        Args:
            type: The type of configuration ('mongodb', 'postgreSQL', etc.)
        ...
        if type not in config_types:
            raise ArdentValidationError(f"Invalid configuration type: {type}")
        
        config_info = config_types[type]
        ...
        if missing_params:
            raise ArdentValidationError(
                f"Missing required parameters for {type}: {', '.join(missing_params)}"
            )
        ...
        for param, expected_type in config_info["structure"].items():
            if param in config_params:
                ...
                if not isinstance(value, expected_type):
                    raise ArdentValidationError(
                        f"Invalid type for parameter '{param}' in {type} config. "
                        f"Expected {expected_type.__name__}, got {type(value).__name__}"
                    )

            # Additional structure validation for MongoDB databases
            if type == "mongodb" and param == "databases":
