# -*- coding: utf-8 -*-
"""DataTrails EventsV1 SCRAPI Engine implementation
"""
...
class DatatrailsEventsScrapiEngine(ScrapiEngine):
    """DataTrails Events Native Engine implementation"""
    ...
    def __str__(self) -> str:
        return f"DataTrails SCRAPI Events Engine ({self._url})"
    ...
    def register_signed_statement(self, statement):
        ...
        payload = {
            "trails": ["vCons", "Consent"],
            "attributes": attrs
        }
        try:
            event = self._archivist.post(
    ...
    def check_registration(self, registration_id):
        ...
        try:
            event = self._archivist.get(
                f'${self._url}/v1/${entry_id}'
            )
        except ArchivistBadRequestError as e:
            logging.debug(e)
            return None, encode_problem_details(
...
#FIXME POINT-GET THE EVENT AND DIG OUT THE STATEMENT FROM ATTRIBUTES
