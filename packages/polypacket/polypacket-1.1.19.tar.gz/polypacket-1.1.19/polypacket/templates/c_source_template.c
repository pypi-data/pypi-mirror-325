...
/**
  *@brief initializes ${proto.namespace}_protocol
...
HandlerStatus_e ${proto.namespace}_service_dispatch(${proto.namespace}_packet_t* packet, ${proto.namespace}_packet_t* response)
{

  HandlerStatus_e ${proto.namespace}_status;

  //Dispatch packet
  switch(packet->mDesc->mTypeId)
  {
    case ${proto.namespace.upper()}_PACKET_PING_ID:
      ${proto.namespace}_packet_build(response, ${proto.namespace.upper()}_PACKET_ACK);
      ${proto.namespace}_status = ${proto.namespace}_Ping_handler(packet, response);
      break;
    case ${proto.namespace.upper()}_PACKET_ACK_ID:
      ${proto.namespace}_status = ${proto.namespace}_Ack_handler(packet);
      break;
% for packet in proto.packets:
% if not packet.standard:
    case ${packet.globalName}_ID:
    %if packet.hasResponse:
     ${proto.namespace}_packet_build(response, ${packet.response.globalName});
     ${proto.namespace}_status = ${proto.namespace}_${packet.camel(True)}_handler(packet , response );
    %else:
      ${proto.namespace}_status = ${proto.namespace}_${packet.camel(True)}_handler(packet);
    %endif
      break;
% endif
% endfor
  ...
  if(( ${proto.namespace.upper()}_SERVICE.mAutoAck ) & (!response->mBuilt) && (!(packet->mHeader.mToken & POLY_ACK_FLAG)))
  {
    ${proto.namespace}_packet_build(response, ${proto.namespace.upper()}_PACKET_ACK);
  }
  ...
  if(${proto.namespace}_status == PACKET_NOT_HANDLED)
  {
    ${proto.namespace}_status = ${proto.namespace}_default_handler(packet, response);
  }


  return ${proto.namespace}_status;
}
...
{
  static ${proto.namespace}_packet_t packet;
  static ${proto.namespace}_packet_t response;

  HandlerStatus_e ${proto.namespace}_status = PACKET_NOT_HANDLED;
  ...
  if(poly_service_try_parse(&${proto.service()}, &packet) == PACKET_VALID)
  {
    ...
    ${proto.namespace}_status = ${proto.namespace}_service_dispatch(&packet,&response);

    //If a response has been build and the ${proto.namespace}_status was not set to ignore, we send a response on the intrface it came from
    if(( ${proto.namespace}_status == PACKET_HANDLED) && (response.mBuilt) && ((packet.mHeader.mToken & POLY_ACK_FLAG)==0))
    {
      //set response token with ack flag
			response.mHeader.mToken = packet.mHeader.mToken | POLY_ACK_FLAG;

      ${proto.namespace}_send(packet.mInterface , &response);
    }

    //Clean the packets
    ${proto.namespace}_clean(&packet);
    ${proto.namespace}_clean(&response);
  }
...
{
  ${proto.namespace}_packet_t packet;
  ${proto.namespace}_packet_t response;
  ...
  if(poly_service_parse_json(&${proto.service()}, &packet, req, len) == PACKET_VALID)
  {
    ...
    //If a response has been build and the ${proto.namespace}_status was not set to ignore, we send a response on the intrface it came from
    if(( ${proto.namespace}_status == PACKET_HANDLED) && (response.mBuilt) )
    {
      //set response token with ack flag
			response.mHeader.mToken = packet.mHeader.mToken | POLY_ACK_FLAG;
...
HandlerStatus_e ${proto.namespace}_send(int iface, ${proto.namespace}_packet_t* packet)
{
  ...
  ${proto.namespace}_status = poly_service_spool(&${proto.service()}, iface, packet);

  if(${proto.namespace}_status == PACKET_SPOOLED)
  {
...
/**
  ...
  *@param packet ptr to ${proto.namespace}_packet
...
%endif
%if field.isArray:
uint32_t ${proto.namespace}_get${field.camel()}(${proto.namespace}_packet_t* packet, ${field.getParamType()} val)
{
...
HandlerStatus_e ${proto.namespace}_sendPing(int iface)
{
  ...
  //create packet
  ${proto.namespace}_packet_t packet;
  ${proto.namespace}_packet_build(&packet, ${proto.namespace.upper()}_PACKET_PING);
  ${proto.namespace}_setIcd(&packet,  ${proto.namespace.upper()}_SERVICE_HASH );

  ${proto.namespace}_status = ${proto.namespace}_send(iface,&packet); //send packet
  ${proto.namespace}_clean(&packet); //This will only free the underlying packet if the spooling was unsuccessful
  return ${proto.namespace}_status;
}
...
)
{
  ...
  //create packet
  ${proto.namespace}_packet_t packet;
  ${proto.namespace}_packet_build(&packet,${packet.globalName});

  //set fields
  %for field in packet.fields:
  %if field.isRequired:
  %if field.isArray and not field.isString:
  ${proto.namespace}_set${field.camel()}(&packet, ${field.name}, ${field.name}_len);
  %else:
  ${proto.namespace}_set${field.camel()}(&packet, ${field.name});
  %endif
  %endif
  %endfor
...
/**
  ...
  */
__attribute__((weak)) HandlerStatus_e ${proto.namespace}_Ping_handler(${proto.namespace}_packet_t* ping, ${proto.namespace}_packet_t* ack)
{
  /* Ack token has already been set as ping token with POLY_ACK_FLAG*/
  uint32_t icd_hash = ${proto.namespace}_getIcd(ping);
  ${proto.namespace}_setIcd(ack, ${proto.namespace.upper()}_SERVICE_HASH);
  
  //assert(icd_hash == ${proto.namespace.upper()}_SERVICE_HASH );
...
/**
  ...
  *@return PACKET_HANDLED
  */
__attribute__((weak)) HandlerStatus_e ${proto.namespace}_Ack_handler(${proto.namespace}_packet_t* ack)
{
  return PACKET_HANDLED;
}
...
/**
  ...
  */
__attribute__((weak)) HandlerStatus_e ${proto.namespace}_${packet.camel(True)}_handler(${proto.namespace}_packet_t* ${packet.name})
%else:
/**
  *@brief Handler for receiving ${packet.name} packets
  *@param ${packet.name} incoming ${packet.name} packet
  ...
  *@return handling ${proto.namespace}_status
  */
__attribute__((weak)) HandlerStatus_e ${proto.namespace}_${packet.camel(True)}_handler(${proto.namespace}_packet_t* ${packet.name}, ${proto.namespace}_packet_t* ${packet.response.name})
%endif
{
...
% for field in packet.fields:
  ...
  %if field.isArray:
    %if field.isString:
  //${proto.namespace}_get${field.camel()}(${packet.name}, ${field.name});
    %else :
  //${proto.namespace}_get${field.camel()}(${packet.name}, ${field.name},  );
    %endif
  %else:
  //${field.name} = ${proto.namespace}_get${field.camel()}(${packet.name});
  %endif
% endfor
%if packet.hasResponse:
  /*    Set required Fields in response  */
% for field in packet.response.fields:
  //${proto.namespace}_set${field.camel()}(${packet.response.name}, value );  //${field.desc}
%endfor
%endif
...
/**
  ...
  *@param packet ptr to incoming message
  *@param response ptr to response
  *@return handling ${proto.namespace}_status
  */
__attribute__((weak)) HandlerStatus_e ${proto.namespace}_default_handler( ${proto.namespace}_packet_t * packet, ${proto.namespace}_packet_t* response)
{
