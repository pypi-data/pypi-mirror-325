...
def get_code_diff(commit_hash: str) -> str:
	...
	- If the commit has a parent, return the diff between the parent and the commit.
	- If the commit is the initial commit (i.e. no parent exists), use `git show --format=''` to output only the diff.
	"""
	try:
		# Try to get the parent commit; if this fails, it's likely the initial commit.
		...
		# For an initial commit, use git show with no commit message/metadata
		try:
			diff = subprocess.check_output(
...
async def note(
	...
	try:
		...
		# Get the diff using our helper function.
	...
	# Here we use the branch name as the note identifier.
	note_identifier = branch_name
	try:
