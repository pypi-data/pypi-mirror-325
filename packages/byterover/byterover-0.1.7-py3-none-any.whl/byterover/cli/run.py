...
def get_code_diff(commit_hash: str) -> str:
	"""
	Retrieve the code diff for the given commit hash.

	- If the commit is the initial commit (i.e. it has no parent),
	  use `git show --format=` to output only the diff.
	- Otherwise, return the diff between the parent commit and the commit.
	"""
	try:
		# Retrieve the initial commit hash
	...
	if initial_commit == commit_hash:
		# This is the initial commit; use git show to retrieve the diff only.
		try:
		...
		try:
			# Get the parent commit. Since we know this is not the initial commit,
			# this command should succeed without warnings.
...
async def note(
	...
	If the --commit option is specified, that commit will be processed;
	otherwise, the latest commit is used.
	"""
	...
	try:
		...
		# Use provided commit hash or default to the latest commit.
		...
		# Get the diff using our improved helper function.
		code_diff = get_code_diff(commit_hash)
	...
	note_identifier = branch_name  # using branch name as identifier
	try:
