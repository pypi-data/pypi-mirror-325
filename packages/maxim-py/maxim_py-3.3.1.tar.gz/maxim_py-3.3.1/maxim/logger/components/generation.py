...
class Generation(BaseContainer):
    ...
    def result_(
        ...
        try:
            ...
            logger.error(
                f"Invalid result. You can pass OpenAI/Azure ChatCompletion or Langchain LLMResult,AIMessage,ToolMessage or LiteLLM ModelResponse: {e}"
            )
            raise ValueError(
                f"Invalid result. You can pass OpenAI/Azure ChatCompletion or Langchain LLMResult,AIMessage,ToolMessage or LiteLLM ModelResponse: {e}"
            )
    ...
    @staticmethod
    def convert_result(result: Union[Any, GenerationResult, Dict[str, Any]]):
        try:
            parse_result(result)
            return result
        except Exception:
