...
class LoadingBarWithImagesManimAnimationWrapper(BaseManimAnimationWrapper):
    ...
    def __init__(self, start_percentage: int = 0, end_percentage: int = 100, duration: float = 5, images: list[LoadingBarImage] = None):
        # TODO: This parameter validation could be done
        # by our specific parameter validator (under
        # construction yet)
        exception_messages = []

        if not NumberValidator.is_number_between(start_percentage, 0, 100):
            exception_messages.append('The "start_percentage" parameter must be a valid int between [0, 100]')
        
        if not NumberValidator.is_number_between(end_percentage, 0, 100):
            raise Exception('The "end_percentage" parameter must be a valid int between [0, 100]')
        
        if not NumberValidator.is_number_between(duration, 0, 120, do_include_lower_limit = False):
            raise Exception('"duration" parameter must be a valid float number between (0, 120]')
