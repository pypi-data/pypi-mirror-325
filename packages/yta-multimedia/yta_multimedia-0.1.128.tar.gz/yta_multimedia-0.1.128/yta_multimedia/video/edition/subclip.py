...
class SubClip:
    ...
    def __init__(self, video: Clip):
        ...
        # TODO: Use constants
        self._with_position = [(MOVIEPY_SCENE_DEFAULT_SIZE[0] / 2, MOVIEPY_SCENE_DEFAULT_SIZE[1] / 2) for _ in range(self.number_of_frames)]
        self._effects = []
    ...
    @property
    def number_of_audio_frames(self):
        """
    ...
    @volume.setter
    def volume(self, value: SubClipAttributeModifier):
        """
    ...
    def set_x_movement(self, value: int):
        """
        Set a new x movement value.
        """
        self.x_movement = value

    def set_y_movement(self, value: int):
        """
    ...
    def add_effect(self, effect: 'SEffect'):
        """
...
class VideoProject:
    ...
    def __init__(self, screen_size: tuple[int, int] = (MOVIEPY_SCENE_DEFAULT_SIZE[0], MOVIEPY_SCENE_DEFAULT_SIZE[1])):
...
def _validate_x_movement(x_movement: int):
    if not NumberValidator.is_number_between(x_movement, -MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4, MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4):
        raise Exception(f'The "x_movement" parameter provided is not a number between [{-MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4}, {MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4}].')
    
def _validate_y_movement(y_movement: int):
    if not NumberValidator.is_number_between(y_movement, -MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4, MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4):
        raise Exception(f'The "y_movement" parameter provided is not a number between [{-MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4}, {MOVIEPY_SCENE_DEFAULT_SIZE[0] * 4}].')
