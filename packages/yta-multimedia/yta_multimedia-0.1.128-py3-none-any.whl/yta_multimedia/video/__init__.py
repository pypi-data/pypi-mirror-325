...
class MPVideo:
    ...
    # of frame indexes
    def get_first_n_frames_indexes(self, n: int):
        """
        ...
        issue).
        """
        n = len(self.frames_indexes) if n > len(self.frames_indexes) else n

        return self.frames_indexes[:n]
    
    @staticmethod
    def get_last_n_frames_indexes(self, n: int):
        """
    ...
    def get_first_n_even_frames_indexes(self, n: int):
        """
        ...
        issue).
        """
        n = len(self.even_frames_indexes) if n > len(self.even_frames_indexes) else n

        return self.even_frames_indexes[:n]
    
    def get_last_n_even_frames_indexes(self, n: int):
        """
    ...
    def get_first_n_odd_frames_indexes(self, n: int):
        """
        ...
        issue).
        """
        n = len(self.odd_frames_indexes) if n > len(self.odd_frames_indexes) else n

        return self.odd_frames_indexes[:n]
    
    def get_last_n_odd_frames_indexes(self, n: int):
        """
    ...
    def get_n_random_frames_indexes(self, n: int):
        """
    ...
    @staticmethod
    def wrap_video_with_transparent_background(video: Clip):
        """
        ...
        if video.size != MOVIEPY_SCENE_DEFAULT_SIZE:
    ...
    def concatenate_videos(videos: list[Clip]):
        ...
        videos = [
            ...
            for video in [VideoParser.to_moviepy(video) for video in videos]
        ]
        
        return concatenate_videoclips(videos)

    @staticmethod
    def frame_time_to_frame_index(t: float, fps: float):
        """
    ...
    @staticmethod
    def frame_index_to_frame_time(i: int, fps: float):
        """
...
def get_frame_indexes(duration, fps):
    """
    ...
    """
    return [i for i in range(int(duration * fps + SMALL_AMOUNT_TO_FIX))]

def get_frame_time_moments(duration, fps):
    """
    ...
    """
    return [(1 / fps * i) + SMALL_AMOUNT_TO_FIX for i in range(int(duration * fps + SMALL_AMOUNT_TO_FIX) + 1)][:-1]

def calculate_real_video_duration(video: Clip):
    """
