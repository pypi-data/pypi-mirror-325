...
class QGFile:
    ...
    def delete_lines_in_txt(file_path: str, keyword: str, is_fuzzy=False):
        ...
        print(f"已删除符合条件的行，并保存到文件: {file_path}")
    ...
    def excel_to_file(source_path, to_path, is_sort=False):
        ...
        try:
            ...
            print(f"数据已成功保存到 {to_path} 文件中。")
        except FileNotFoundError:
            print(f"文件 {source_path} 不存在，请检查文件路径是否正确。")
        except Exception as ex:
            print(f"读取 {source_path} 文件时出现解析错误，请确保文件格式正确且数据完整。{ex}")
    ...
    def save_to_excel(file_path, data, is_sort=False):
        try:
            ...
            print(f"保存数据到 {file_path} 文件时出错：{e}")
    ...
    def read_json(file_path):
        try:
            ...
            print(f"文件 {file_path} 不是一个有效的JSON文件，请检查文件内容是否正确。")
            return None
