...
from sources import BaseScraper
...
class PubmedScraper(BaseScraper):
    ...
    async def _fetch_pmid(self, doi: str) -> str | None:
        async with AsyncSession(impersonate='chrome119', verify=False, timeout=15) as client:
            url = f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&term={doi}&retmode=json&field=doi"
            resp = await client.get(url)
            resp.raise_for_status()
            ret = resp.json()
            assert ret['esearchresult']['count'] == 1, f"DOI {doi} not found in Pubmed"
            return ret['esearchresult']['idlist'][0]
        
        
    async def _fetch_info(self, pmid: str):
        async with AsyncSession(impersonate='chrome119', verify=False, timeout=15) as client:
            resp = await client.get(f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=pubmed&id={pmid}")
            resp.raise_for_status()
            return resp.json()

    async def _fetch(self, doi: str):
        try:
            pmid = await self._fetch_pmid(doi)
            if pmid is not None:
                return await self._fetch_info(pmid)
        except Exception as e:
            logger.error(f"Error fetching pmid for {doi}: {e}")
            return None
