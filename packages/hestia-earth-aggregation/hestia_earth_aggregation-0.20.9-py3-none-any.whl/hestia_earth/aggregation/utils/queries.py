...
from .term import DEFAULT_COUNTRY_NAME, _format_country_name, _fetch_countries, _is_global
...
def _country_nodes(product_name: str, start_year: int, end_year: int, country_name: str):
    ...
    nodes = search(
        query=_country_nodes_query(product_name, start_year, end_year, country_name),
        limit=SEARCH_LIMIT,
        fields=['@id', '@type']
    )
...
def _global_nodes(product_name: str, start_year: int, end_year: int):
    nodes = search(
        query=_global_query(product_name, start_year, end_year),
        limit=SEARCH_LIMIT,
...
def _sub_country_nodes(product: dict, start_year: int, end_year: int, region_name: str):
    ...
    sub_regions = search(query=query, limit=SEARCH_LIMIT, fields=['@id', 'name'])
    nodes = [{
