...
def upsample():
    ...
    for i in range(len(config.frames_for_vid) - 1):
        frame1 = tf.convert_to_tensor(config.frames_for_vid[i], dtype=tf.float32) / 255.0
        frame2 = tf.convert_to_tensor(config.frames_for_vid[i + 1], dtype=tf.float32) / 255.0
        ...
        new_frames.extend(interpolated.numpy().astype(np.uint8))
