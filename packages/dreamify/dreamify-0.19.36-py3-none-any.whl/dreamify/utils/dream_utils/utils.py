...
def interpolate_frames(frame1, frame2, num_frames):
    ...
    frame1 = tf.cast(frame1, tf.float32)  # Cast to float32 for interpolation
    frame2 = tf.cast(frame2, tf.float32)  # Cast to float32 for interpolation
    
    interpolated_frames = (1 - alphas[:, None, None, None]) * frame1 + alphas[:, None, None, None] * frame2
    return tf.cast(interpolated_frames, tf.uint8)  # Cast back to uint8
...
def upsample():
    ...
    for i in range(len(config.frames_for_vid) - 1):
        print(type(config.frames_for_vid[i]), config.frames_for_vid[i].dtype)
        
        frame1 = tf.cast(config.frames_for_vid[i], tf.float32)  # Directly cast to float32
        frame2 = tf.cast(config.frames_for_vid[i + 1], tf.float32)  # Directly cast to float32

        new_frames.append(config.frames_for_vid[i])  # Keep original frame
        interpolated = interpolate_frames(frame1, frame2, NUM_FRAMES_TO_INSERT)
        new_frames.extend(interpolated.numpy())  # Add interpolated frames to the list
