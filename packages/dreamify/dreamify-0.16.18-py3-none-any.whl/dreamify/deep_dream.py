...
def run_deep_dream_rolled(
    ...
    result = deprocess(img)
    tf.print("ORIGINAL SHAPE: ", original_shape, "RESULT SHAPE: ", result.shape, "BASE SHAPE: ", base_shape)
    img = tf.image.resize(img, original_shape[:-1])
    img = tf.image.convert_image_dtype(img / 255.0, dtype=tf.uint8)
    show(img)
    return result
...
def main():
    ...
    run_deep_dream_rolled(
        img=original_img, get_tiled_gradients=get_tiled_gradients, steps_per_octave=50, step_size=0.01, original_shape=base_shape
    )
