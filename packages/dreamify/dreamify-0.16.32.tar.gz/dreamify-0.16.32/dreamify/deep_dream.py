...
def run_deep_dream_rolled(
    ...
    original_shape = tf.constant(tf.shape(img)[:-1])
    tf.print(tf.constant(tf.shape(img)))
    tf.print(tf.constant(tf.shape(img)[:-1]))
    tf.print(original_shape.shape)
    ...
    result = deprocess(img)
    tf.print("RESULT SHAPE: ", result.shape, "BASE SHAPE: ", original_shape, "INITIAL SHAPE", initial_shape)
    img = tf.image.convert_image_dtype(img / 255.0, dtype=tf.uint8)
    img = tf.image.resize(img, initial_shape)
    show(img)
    return result
