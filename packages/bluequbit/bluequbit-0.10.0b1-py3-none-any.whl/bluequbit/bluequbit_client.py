...
def _run_circuits_local(circuits, device, shots, pauli_sum, options):
    ...
    try:
        ...
        logger.info("Using local deqart_internal")
    ...
    def _validate_options(options):
        ...
        for k, v in options.items():
            if k not in [
                ...
                "pauli_path_truncation_type",
            ]:
                raise BQSDKUsageError(f"options not supported: {k}")
            supported_tt = [None, "weight", "coeff"]
            if k == "pauli_path_truncation_type" and (v not in supported_tt):
                raise BQSDKUsageError(f"Unsupported truncation type {v}.")
    ...
    def run(
        ...
        *,
        ...
        :param shots: number of shots to run. If device is quantum and shots is None then
                      it is set to 100. For non quantum devices, if None, full
        ...
        :param options: dictionary of options used to configure the run:
                       ...
                       - ``mps_truncation_threshold`` (float): Discard the smallest
                         coefficients for which the sum of their squares is smaller
                         than this threshold.
                         Default: 1e-16

                       - ``pauli_path_truncation_threshold`` (int): threshold for the chosen
                         truncation scheme, either the number of allowed non identity Paulis
                         appearing in any single term in the Pauli decomposition of the
                         evolved observable or an upper bound on the Pauli coefficients of
                         the evolved observable.
                         Default: same as number of qubits as the circuit (i.e. is exact --
                         should give the same result as the statevector simulator)

                       - ``pauli_path_truncation_type`` (str): determines what truncation
                         scheme to apply (value can be either 'weight' or 'coeff')
                         Default: 'weight'
