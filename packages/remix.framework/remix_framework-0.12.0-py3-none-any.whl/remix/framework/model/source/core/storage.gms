...
set pc_storage_reservoirParam
    ...
    nonCircular         "Non-circular storage level | The initial storage level is independant of the last time step's storage level | | boolean | {none} | OEO_00000339:program parameter"
    initialLevel        "Initial storage level | Storage level before first timestep for nonCircular storage |  minimum:0;lessThan:1 | number | {none} | MISSING_TERM:State of charge"
    /;
...
$batinclude %aggregateNodes% storage_reservoirParam(nodesModel,years,storage_techs,"nonCircular")             storage_reservoirParamIn(nodesData,years,storage_techs,"nonCircular")             smax
...
set pc_storage_SoCParam
    /
    SoC             "State of charge | Nominal SoC of this state. NOTE: Incorrect with nonCircular storage as the selfdischarge only works in circular for this implementation | | number | {none} | MISSING_TERM:State of charge"
  ...
  $(sum(years_a$storage_usedTech(nodesModel,years_a,storage_techs,vintage), 1)
    and sum(yearsToCalc
      $(sameas(years, yearsToCalc)
        and yearsToCalc.val >= smin(years_a$storage_availTech(nodesModel,years_a,storage_techs,vintage), years_a.val)
...
storage_reservoirParam(nodesModelToCalc,yearsToCalc,storage_techs,"initialLevel")
    $( storage_reservoirParam(nodesModelToCalc,yearsToCalc,storage_techs,"nonCircular")
        and storage_finiteUnitLimit(nodesModelToCalc,yearsToCalc,storage_techs))
    = sum(nodesData$aggregateNodesModel(nodesData,nodesModelToCalc),
          storage_reservoirParamIn(nodesData,yearsToCalc,storage_techs,"initialLevel")
            * storage_reservoirParamIn(nodesData,yearsToCalc,storage_techs,"unitsUpperLimit"))
    / sum(nodesData$aggregateNodesModel(nodesData,nodesModelToCalc),
            storage_reservoirParamIn(nodesData,yearsToCalc,storage_techs,"unitsUpperLimit"));
...
$iftheni.fcfgdx not %fixedcapsfromgdx%==None
storage_reservoirParam(nodesModel,years,storage_techs,"unitsUpperLimit")
  = sum(vintage,storage_unitsTotal.l(nodesModel,years,storage_techs,vintage));
storage_reservoirParam(nodesModel,years,storage_techs,"unitsLowerLimit")
  = storage_reservoirParam(nodesModel,years,storage_techs,"unitsUpperLimit");
$endif.fcfgdx
...
parameter storage_unitsDelta(nodesModel,years,storage_techs);
...
Eq_storage_losses(timeModelSel(timeModelToCalc),nodesModelSel,yearsSel,storage_techs,vintage,commodity)
    ...
    -(storage_level(timeModelToCalc--1,nodesModelSel,yearsSel,storage_techs,vintage,commodity)
        $(not storage_reservoirParam(nodesModelSel,yearsSel,storage_techs,"nonCircular"))
      + storage_level(timeModelToCalc-1,nodesModelSel,yearsSel,storage_techs,vintage,commodity)
          $(storage_reservoirParam(nodesModelSel,yearsSel,storage_techs,"nonCircular"))
      + (storage_reservoirParam(nodesModelSel,yearsSel,storage_techs,"initialLevel")
          * storage_sizeParam(storage_techs,vintage,commodity,"size")
          * storage_unitsTotal(nodesModelSel,yearsSel,storage_techs,vintage))
          $(storage_reservoirParam(nodesModelSel,yearsSel,storage_techs,"nonCircular")
