...
def calc_vector(dim: int, face_idx: int, xloc: float, yloc: float) -> Vec3TupleType:
    ...
    if face_idx == 0:
        ...
        raise ValueError(f"Unknown face index ({face_idx})")
...
def calc_solid_angle(invdim: float, x: float, y: float) -> float:
    ...
    return (
        math.atan2(x0 * y0, math.sqrt(x02 + y02 + 1))
...
def get_sh_coeffs_from_cube_map(texcubemap: p3d.Texture) -> list[p3d.LVector3]:
    ...
    if texcubemap.x_size != texcubemap.y_size:
        raise RuntimeError(
            "supplied cube map is using unsupported, non-square dimensions"
        )
    if not texcubemap.might_have_ram_image():
        raise RuntimeError(
            "expected might_have_ram_image() to be true on supplied texture"
        )
...
def importance_sample_ggx(
    xi: Vec2TupleType, normal: p3d.LVector3, roughness: float
) -> p3d.LVector3:
...
def geometry_smith(
    normal: p3d.LVector3, view: p3d.LVector3, light: p3d.LVector3, roughness: float
) -> float:
    ndotv = max(normal.dot(view), 0)
    ndotl = max(normal.dot(light), 0)

    return geometry_schlick_ggx(ndotv, roughness) * geometry_schlick_ggx(
        ndotl, roughness
    )


def integrate_brdf(
    ndotv: float, roughness: float, num_samples: int = 1024
) -> p3d.LVector2:
    ndotv = max(ndotv, 0.0001)
    view = p3d.LVector3(math.sqrt(1 - ndotv * ndotv), 0, ndotv)
...
def filter_sample(
    pos: p3d.LVector3, envmap: p3d.TexturePeeker, roughness: float, num_samples: int
) -> p3d.LVector3:
        ...
        for face in range(6):
            for xcoord in range(mipsize):
                for ycoord in range(mipsize):
                    ...
                    struct.pack_into(
                        "fff",
                        typing.cast(memoryview, texdata),
                        offset,
                        result[2],
                        result[1],
                        result[0],
