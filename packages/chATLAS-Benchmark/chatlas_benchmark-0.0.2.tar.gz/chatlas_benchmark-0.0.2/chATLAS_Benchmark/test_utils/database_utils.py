...
def save_benchmark_results(model_name, db_name: str, question_path, scores: dict):
        ...
        for test, scoring in scores.items():
            if test == "LexicalMetrics":
                lexical_run = LexicalMetricsScores(
                    ...
                    exact_match_by_question=scoring[
                        "exact_match_by_question"
                    ].to_json(),
                ...
                print(
                    f"Added LexicalMetrics results to table {LexicalMetricsScores.__tablename__}"
                )
                ...
                semanticSimilarity_run = SemanticSimilarityScores(
                    model_name=model_name,
                    question_path=str(question_path),
                    semanticSimilarity_score=float(
                        scoring["answerSimilarity_score"].iloc[0]
                    ),
                    semanticSimilarity_score_by_question=scoring[
                        "answerSimilarity_score_by_question"
                    ].to_json(),
                )
                session.add(semanticSimilarity_run)
                print(
                    f"Added SemanticSimilarityScores results to table {SemanticSimilarityScores.__tablename__}"
                )
                ...
                documentMatch_run = DocumentMatchScores(
                    ...
                    WeightedDocumentMatch_score=float(
                        scoring["WeightedDocumentMatch"].iloc[0]
                    ),
                )
                session.add(documentMatch_run)
                print(
                    f"Added DocumentMatch scores results to table {DocumentMatchScores.__tablename__}"
                )
        session.commit()
...
def fetch_metrics_results(db_name: str | Path = "benchmark.db") -> pd.DataFrame:
    ...
    try:
        ...
        for table, columns in available_tables.items():
            alias = table[:2]  # Short alias for readability
            if first_table is None:
                first_table = table  # The base table
            else:
                joins.append(
                    f"JOIN {table} AS {alias} ON {first_table}.model_name = {alias}.model_name"
                )

            select_columns.extend([f"{table}.{col}" for col in columns])
