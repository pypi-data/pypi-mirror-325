...
def _format_table(
    ...
    while len(tables_temp) > 0:
        ...
        if cur_table.abort():
            ...
            max(len(cur_table.content), len(cur_table.content[0])) <= min_format or tokens.count(cur_table_str) <= k_max
        ):
...
async def extract_subchunks(
    path: Path,
    function: Callable[[str], str] = (lambda _: ""),
    ...
    if extension == "pdf":
        ...
        match extension:
            case "docx" | "doc" | "odt":
                subchunks, tables, images, new_path = extract_subchunks_docx(
                    path,
                    extension,
                    function=function,
                )
            ...
            case "pptx" | "ppt" | "odp":
                subchunks, tables, images, new_path = extract_subchunks_pptx(
                    path,
                    extension,
                    function=function,
                )
