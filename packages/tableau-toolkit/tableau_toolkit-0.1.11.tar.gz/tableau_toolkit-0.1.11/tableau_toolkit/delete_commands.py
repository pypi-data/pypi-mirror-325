...
@click.option("--delimiter", default="\t", help="Delimiter used in the CSV file")
@click.option(
    "--object-luid-col", default="object_luid", help="Column name for User LUID"
)
...
def users(
    ...
    for row in reader:
        ...
        try:
            ...
            click.echo(
                f"Successfully deleted user: {user_luid} "
            )
        except TSC.ServerResponseError as e:
            click.echo(
                f"Error deleting user {user_luid}: {str(e)}",
                err=True,
            )
        except Exception as e:  # pylint: disable=broad-exception-caught
            click.echo(f"Unexpected error: {str(e)}", err=True)
