...
from astrolabe.utils import get_now_utc, sanitize_datetime
...
class AstrolabeAPI:
    ...
    def post(self, path, data, return_raw_response=False) -> Any:
        ...
        response = requests.post(self.base_url + "/" + path, headers=self._headers(), json=data, timeout=30)
        self._check_for_errors(response)
    ...
    def patch(self, path, data, return_raw_response=False) -> Any:
        ...
        response = requests.patch(self.base_url + "/" + path, headers=self._headers(), json=data, timeout=10)
        self._check_for_errors(response)
    ...
    def put(self, path, data, return_raw_response=False) -> Any:
        ...
        response = requests.put(self.base_url + "/" + path, headers=self._headers(), json=data, timeout=10)
        self._check_for_errors(response)
    ...
    def upload_ephemerides(self, datatype, dataformat, filepath, case_id=None):
        ...
        """
        url = f"{self.base_url}/ephemerides" if case_id is None else f"{self.base_url}/cases/{case_id}/ephemerides"
    ...
    def get_cdms(self, data_format: str, filters: Optional[dict] = None, latest=False) -> list:
        ...
        if "miss_distance" in filters:
            query = (
                query + "&" + self._numeric_comparison_filter_to_query_string("miss_distance", filters["miss_distance"])
            )
        ...
        if "radial_distance" in filters:
            query = (
                query
                + "&"
                + self._numeric_comparison_filter_to_query_string("radial_distance", filters["radial_distance"])
            )

        if "time_to_tca" in filters:
            query = (
                query + "&" + self._duration_comparison_filter_to_query_string("time_to_tca", filters["time_to_tca"])
            )
    ...
    def get_cases(
        ...
        if conjunction_partner is not None:
            query_parameters.append(f"newest_cdm.object2_metadata[operator_organization]={quote(conjunction_partner)}")
    ...
    def get_protocols(self, protocol_type: Optional[str] = None, protocol_status: Optional[str] = None) -> list:
        """
    ...
    def set_up_satellite(self, satellite_definition: dict):
        ...
        for existing_satellite in satellites:
            if "norad_id" in satellite and satellite["norad_id"] == existing_satellite["norad_id"]:
                ...
                break
            if "cospar_id" in satellite and satellite["cospar_id"] == existing_satellite["cospar_id"]:
                satellite_id = existing_satellite["spacecraft_id"]
                break
