...
# Shared library ismini belirleyin: Python extension modülü olarak algılanmasın diye "lib" ekleyelim.
lib_name = "libdot_product.so" if platform.system() != "Windows" else "libdot_product.dll"
...
class BuildLibrary(build_ext):
    def run(self):
        ...
        # Build_ext'in build_lib dizinine shared library dosyasını kopyalayalım.
...
# src/dummy.c dosyası oluşturun (içeriği örneğin "void dummy() {}" olabilir)
