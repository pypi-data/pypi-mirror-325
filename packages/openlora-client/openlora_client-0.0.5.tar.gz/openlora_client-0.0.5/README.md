# openlora Python Client

openlora Python client provides a convenient way of interfacing with a
`openlora` instance running in your environment.
...
```shell
pip install openlora-client
```

### Run

```python
from openlora import Client
...
```python
from openlora import AsyncClient
...
### openledger Inference Endpoints

The openlora client can also be used to connect to [openledger](https://openledger.com/) managed openlora endpoints (including openledger's [serverless endpoints](https://docs.openledger.com/user-guide/inference/serverless_deployments)).

You need only make the following changes to the above examples:

1. Change the `endpoint_url` to match the endpoint of your openledger LLM of choice.
2. Provide your openledger API token in the `headers` provided to the client.
...
endpoint_url = f"https://api.app.openledger.com/v1/llms/{llm_deployment_name}"
headers = {
...
Note that by default openledger will use its internal model repos as the default `adapter_source`. To use an adapter from Huggingface:
