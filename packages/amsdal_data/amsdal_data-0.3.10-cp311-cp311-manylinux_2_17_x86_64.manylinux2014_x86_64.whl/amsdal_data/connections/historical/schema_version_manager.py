...
class HistoricalSchemaVersionManager(metaclass=Singleton):
    def __init__(self) -> None:
        self._cache_scheme_types: dict[SchemaNameType, SchemaTypes] = {}
    ...
    def get_all_schema_versions(self, schema_name: str) -> list[str]:
        ...
        for class_object in self.class_object_classes:
            _data = self._search_classes(
                ...
                only=[
                    glue.FieldReference(
                        field=glue.Field(name=PROPERTIES_FIELD),
                        table_name=class_object[PRIMARY_PARTITION_KEY],
                    ),
                    glue.FieldReference(
            ...
            if _data:
                ...
                for _schema in _data:
                    ...
                    _cached_class[_schema.data[SECONDARY_PARTITION_KEY]] = {
                        prop_name: prop[PROPERTIES_TYPE_FIELD]
                        for prop_name, prop in _schema.data.get(PROPERTIES_FIELD, {}).items()
                    }
    ...
    def resolve_schema_type(self, schema_name: str) -> SchemaTypes:
        if schema_name in (
            METADATA_TABLE,
            REFERENCE_TABLE,
            TRANSACTION_TABLE,
            ...
            # these tables are not versioned
            return SchemaTypes.CORE

        if schema_name not in self._cache_scheme_types:
            # search in object table
            data = next(
                iter(
                    item.get('schema_type', SchemaTypes.CORE)
            ...
            if data:
                ...
                self._cache_scheme_types[schema_name] = SchemaTypes.CORE
    ...
    def _search_classes(
        ...
        result = operation_manager.query_lakehouse(
            glue.QueryStatement(
                only=only,
...
class AsyncHistoricalSchemaVersionManager(metaclass=Singleton):
    ...
    async def resolve_schema_type(self, schema_name: str) -> SchemaTypes:
        if schema_name in (
            METADATA_TABLE,
            REFERENCE_TABLE,
            TRANSACTION_TABLE,
    ...
    async def _search_classes(
        ...
        result = await operation_manager.query_lakehouse(
            glue.QueryStatement(
                only=only,
                table=glue.SchemaReference(name=table_name, version=table_version),
                where=where,
