...
from amsdal_glue_connections.sql.connections.postgres_connection import pg_field_json_transform
from amsdal_glue_connections.sql.connections.postgres_connection import pg_value_json_transform
...
class PostgresHistoricalConnection(glue.PostgresConnection):
    ...
    def query(self, query: glue.QueryStatement) -> list[glue.Data]:
        ...
        _query = self._process_group_by(_transform.transform(PG_METADATA_SELECT_EXPRESSION))
    ...
    def _build_queries_by_version(self, query: glue.QueryStatement) -> list[glue.QueryStatement]:
        if not query.where:
            return self._to_queries_by_version(query.table.version, query)  # type: ignore[union-attr, arg-type]
...
class AsyncPostgresHistoricalConnection(glue_connections.AsyncPostgresConnection):
    def __init__(self) -> None:
        super().__init__()
    ...
    async def query(self, query: glue.QueryStatement) -> list[glue.Data]:
        ...
        _table_transform = AsyncTableNameTransform(_query)
        ...
        for _query_version in queries:
            # Historical table name
            _table_transform = AsyncTableNameTransform(_query_version)
            items.extend(await super().query(await _table_transform.transform()))
    ...
    async def acquire_lock(self, lock: ExecutionLockCommand) -> bool:
        ...
        _q = glue.QueryStatement(
            table=copy(lock.locked_object.schema),
            where=copy(lock.locked_object.query),
        )
        _transform = DataQueryTransform(_q)
        _q = self._process_group_by(_transform.transform(PG_METADATA_SELECT_EXPRESSION))
        _q = await AsyncTableNameTransform(_q).transform()

        lock.locked_object.schema = _q.table  # type: ignore[assignment]
        lock.locked_object.query = _q.where
        ...
        if lock.locked_object.query:
            ...
            _query = (
                f'SELECT * FROM {self._table_name_from_schema_reference(locked_object.schema)} '  # noqa: S608
                f'AS "{locked_object.schema.alias}"'
            )
    ...
    async def query_schema(self, filters: glue.Conditions | None = None) -> list[glue.Schema]:
        _transform = AsyncSchemaQueryFiltersTransform(filters)
    ...
    async def _run_mutation(self, mutation: DataMutation) -> list[glue.Data] | None:
        _transform = AsyncDataMutationTransform(self, mutation)
    ...
    async def run_schema_command(self, command: glue.SchemaCommand) -> list[glue.Schema | None]:
        _executor = AsyncSchemaCommandExecutor(self, command)
    ...
    async def _build_queries_by_version(self, query: glue.QueryStatement) -> list[glue.QueryStatement]:
        if not query.where:
            return await self._to_queries_by_version(query.table.version, query)  # type: ignore[union-attr, arg-type]
        ...
        for _conditions in split_conditions(query.where):
            ...
            for _class_version in _class_versions:
                if _class_version in (glue.Version.ALL, Versions.ALL):
                    _versions = set()
                    for _item_version in await self.schema_version_manager.get_all_schema_versions(query.table.name):  # type: ignore[union-attr]
                        if _item_version or query.table.name in {  # type: ignore[union-attr]
                            TRANSACTION_TABLE,
                            METADATA_TABLE,
                            REFERENCE_TABLE,
                    ...
                    _versions = {await self.schema_version_manager.get_latest_schema_version(query.table.name)}  # type: ignore[union-attr]
                else:
                    _versions = {_class_version}
    ...
    async def _to_queries_by_version(
        ...
        if version in (glue.Version.ALL, Versions.ALL):
            for _class_version in await self.schema_version_manager.get_all_schema_versions(_table.name):
                _query = copy(query)
                _query.table = copy(_table)
                _query.table.version = _class_version
                queries.append(_query)
        elif version in (glue.Version.LATEST, Versions.LATEST):
            _latest_class_version = await self.schema_version_manager.get_latest_schema_version(_table.name)
            _table.version = _latest_class_version
            queries.append(query)
        else:
            queries.append(query)
