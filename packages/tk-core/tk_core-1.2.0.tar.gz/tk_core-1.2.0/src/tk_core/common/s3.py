...
class S3Util:
    ...
    def read_file(self, key: str) -> str:
        if self.enable_prints:
            print(f"Reading from {self.bucket_name}/{self.prefix(key)}")
    ...
    def write_file(self, key: str, data: dict) -> None:
        ...
        """
        if self.enable_prints:
            print(f"Writing to {self.bucket_name}/{self.prefix(key)}")
        self.s3.Object(self.bucket_name, self.prefix(key)).put(Body=data)

    def delete_file(self, key: str) -> None:
        """Deletes a file from S3"""
        if self.enable_prints:
            print(f"Deleting {self.bucket_name}/{self.prefix(key)}")
