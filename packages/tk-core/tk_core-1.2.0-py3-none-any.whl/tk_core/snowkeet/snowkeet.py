...
class Snowkeet:
    ...
    def count_table(self, table_name: str) -> int:
        ...
        count_sql = f"""
            ...
            """  # noqa: S608
        counts_dct = self.execute_sql(count_sql)
    ...
    def get_top_result_filtered(self, table_name: str, filter_var: str, filter_value: str, column_to_get: str) -> bool:
        ...
        """
        query = f"SELECT top 1 {column_to_get} from {table_name} where {filter_var} = '{filter_value}'"  # noqa: S608
    ...
    def execute_sql(self, sql_query_str: str) -> list[Row]:
        """
        Execute a SQL statement in Snowflake.
        """
        return self.session.sql(sql_query_str).collect()
    ...
    def check_if_rows_exist(self, table_name: str, filter_var: str, filter_value: str) -> bool:
        ...
        Returns:
            bool: True if rows exist, False if not.
        """
        query_str = f"""SELECT exists(select top 1 1
         from {table_name} 
        where {filter_var} = '{filter_value}') as exist_check"""  # noqa: S608
        rows = self.execute_sql(query_str)
    ...
    def custom_dml(self, sql: str) -> dict:
        """
