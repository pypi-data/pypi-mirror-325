...
from peepsai.cli.constants import ENV_VARS, MODELS
from peepsai.cli.provider import (
    get_provider_data,
    select_model,
    select_provider,
)
from peepsai.cli.utils import copy_template, load_env_vars, write_env_file
...
def create_folder_structure(name, parent_folder=None):
    ...
    click.secho(
        f"Creating {'peeps' if parent_folder else 'folder'} {folder_name}...",
        fg="green",
        bold=True,
    )
...
def copy_template_files(folder_path, name, class_name, parent_folder):
    ...
    templates_dir = package_dir / "templates" / "peeps"
    ...
    src_template_files = (
        ["__init__.py", "main.py", "peeps.py"] if not parent_folder else ["peeps.py"]
    )
...
def create_peeps(name, provider=None, skip_provider=False, parent_folder=None):
    ...
    click.secho(f"Peeps {name} created successfully!", fg="green", bold=True)
