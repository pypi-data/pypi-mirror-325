...
from peepsai.cli.config import Settings
from peepsai.cli.utils import (
...
class ToolCommand(BaseCommand, PlusAPIMixin):
    """
    A class to handle tool repository related operations for PeepsAI projects.
    """
    ...
    def publish(self, is_public: bool, force: bool = False):
        ...
        console.print(
            f"Successfully published {published_handle} ({project_version}).\nInstall it in other projects with peepsai tool install {published_handle}",
            style="bold green",
        )
    ...
    def login(self):
        ...
        if login_response.status_code != 200:
            console.print(
                "Authentication failed. Verify access to the tool repository, or try `peepsai login`. ",
                style="bold red",
            )
            raise SystemExit
