...
from peepsai import Agent, Peeps, Task
from peepsai.agents.cache import CacheHandler
from peepsai.agents.peeps_agent_executor import PeepsAgentExecutor
from peepsai.agents.parser import AgentAction, PeepsAgentParser, OutputParserException
...
from peepsai.tools.tool_calling import InstructorToolCalling
from peepsai.tools.tool_usage import ToolUsage
from peepsai.tools.tool_usage_events import ToolUsageFinished
from peepsai.utilities import RPMController
from peepsai.utilities.events import Emitter
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_agent_respect_the_max_rpm_set_over_peeps_rpm(capsys):
    from unittest.mock import patch
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_agent_without_max_rpm_respects_peeps_rpm(capsys):
    from unittest.mock import patch
    ...
    # Set peeps's max_rpm to 1 to trigger RPM limit
    peeps = Peeps(agents=[agent1, agent2], tasks=tasks, max_rpm=1, verbose=True)
...
def test_agent_remembers_output_format_after_using_tools_too_many_times():
    ...
    peeps = Peeps(agents=[agent1], tasks=tasks, verbose=True)
...
def test_agent_use_specific_tasks_output_as_context(capsys):
    ...
    peeps = Peeps(agents=[agent1, agent2], tasks=tasks)
...
def test_agent_step_callback():
    ...
    with patch.object(StepCallback, "callback") as callback:
        ...
        tasks = [essay]
        peeps = Peeps(agents=[agent1], tasks=tasks)
...
def test_agent_count_formatting_error():
    ...
    parser = PeepsAgentParser(agent=agent1)
...
def test_agent_human_input():
    ...
    with patch.object(
        PeepsAgentExecutor, "_ask_human_input", side_effect=ask_human_input_side_effect
    ) as mock_human_input:
        # Execute the task
        output = agent.execute_task(task)
...
"""
    ...
    with patch.object(PeepsAgentExecutor, "_format_prompt") as mock_format_prompt:
...
@patch("peepsai.agent.PeepsTrainingHandler")
def test_agent_training_handler(peeps_training_handler):
    ...
    peeps_training_handler().load.return_value = {
    ...
    peeps_training_handler.assert_has_calls(
        [mock.call(), mock.call("training_data.pkl"), mock.call().load()]
    )


@patch("peepsai.agent.PeepsTrainingHandler")
def test_agent_use_trained_data(peeps_training_handler):
    task_prompt = "What is 1 + 1?"
    agent = Agent(
        role="researcher",
        goal="test goal",
...
def test_agent_max_retry_limit():
    ...
    with patch.object(
        PeepsAgentExecutor, "invoke", wraps=agent.agent_executor.invoke
    ) as invoke_mock:
...
def test_handle_context_length_exceeds_limit():
    ...
    with patch.object(
        ...
        with patch.object(
            PeepsAgentExecutor, "_handle_context_length"
...
def test_agent_with_knowledge_sources():
    ...
    with patch(
        "peepsai.knowledge.storage.knowledge_storage.KnowledgeStorage"
    ) as MockKnowledge:
...
def test_agent_with_knowledge_sources_works_with_copy():
    ...
    with patch(
        "peepsai.knowledge.source.base_knowledge_source.BaseKnowledgeSource",
        autospec=True,
...
def test_peeps_agent_executor_litellm_auth_error():
    """Test that PeepsAgentExecutor handles LiteLLM authentication errors by raising them."""
    from litellm.exceptions import AuthenticationError

    from peepsai.agents.tools_handler import ToolsHandler
    from peepsai.utilities import Printer
