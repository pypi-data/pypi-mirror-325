...
def create_flow(name):
    ...
    (project_root / "src" / folder_name / "peepz").mkdir(parents=True)
    ...
    # Copy peeps folders
    for peeps_folder in peeps_folders:
        src_peeps_folder = templates_dir / "peepz" / peeps_folder
        dst_peeps_folder = project_root / "src" / folder_name / "peepz" / peeps_folder
        if src_peeps_folder.exists():
            for src_file in src_peeps_folder.rglob("*"):
                if src_file.is_file():
                    relative_path = src_file.relative_to(src_peeps_folder)
                    dst_file = dst_peeps_folder / relative_path
                    dst_file.parent.mkdir(parents=True, exist_ok=True)
                    process_file(src_file, dst_file)
        else:
            click.secho(
                f"Warning: Peeps folder {peeps_folder} not found in template.",
                fg="yellow",
            )
