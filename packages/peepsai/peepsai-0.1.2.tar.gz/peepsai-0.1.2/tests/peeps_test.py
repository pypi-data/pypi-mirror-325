...
from peepsai.peepz.peeps_output import PeepsOutput
from peepsai.knowledge.source.string_knowledge_source import StringKnowledgeSource
from peepsai.memory.contextual.contextual_memory import ContextualMemory
from peepsai.process import Process
from peepsai.project import peeps
from peepsai.task import Task
from peepsai.tasks.conditional_task import ConditionalTask
...
from peepsai.utilities import Logger
from peepsai.utilities.rpm_controller import RPMController
from peepsai.utilities.task_output_storage_handler import TaskOutputStorageHandler
...
def test_peeps_config_conditional_requirement():
    with pytest.raises(ValueError):
        Peeps(process=Process.sequential)
    ...
    try:
        peeps = Peeps(process=Process.sequential, config=config)
    except ValueError:
        pytest.fail("Unexpected ValidationError raised")

    assert [agent.role for agent in peeps.agents] == [
        agent["role"] for agent in parsed_config["agents"]
    ]
    assert [task.description for task in peeps.tasks] == [
...
def test_async_task_cannot_include_sequential_async_tasks_in_context():
    ...
    with pytest.raises(
        ...
        Peeps(tasks=[task1, task2, task3, task4, task5], agents=[researcher, writer])

    # This should not raise an error because task5 has a sync task (task4) in its context
    try:
        Peeps(tasks=[task1, task4, task5], agents=[researcher, writer])
    except ValueError:
        pytest.fail("Unexpected ValidationError raised")
...
def test_context_no_future_tasks():
    ...
    with pytest.raises(
        ...
        Peeps(tasks=[task1, task2, task3, task4], agents=[researcher, writer])


def test_peeps_config_with_wrong_keys():
    ...
    with pytest.raises(ValueError):
        Peeps(process=Process.sequential, config='{"wrong_key": "wrong_value"}')
    with pytest.raises(ValueError):
        Peeps(process=Process.sequential, config=no_tasks_config)
    with pytest.raises(ValueError):
        Peeps(process=Process.sequential, config=no_agents_config)


@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_creation():
    tasks = [
        Task(
            description="Give me a list of 5 interesting ideas to explore for na article, what makes them unique and interesting.",
            expected_output="Bullet point list of 5 important events.",
    ...
    assert isinstance(result, PeepsOutput)
...
def test_manager_agent_delegating_to_all_agents():
    ...
    assert peeps.manager_agent is not None
    assert peeps.manager_agent.tools is not None

    assert len(peeps.manager_agent.tools) == 2
    assert (
        "Delegate a specific task to one of the following coworkers: Researcher, Senior Writer\n"
        in peeps.manager_agent.tools[0].description
    )
    assert (
        "Ask a specific question to one of the following coworkers: Researcher, Senior Writer\n"
        in peeps.manager_agent.tools[1].description
    )
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_with_delegating_agents():
    tasks = [
        Task(
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_with_delegating_agents_should_not_override_task_tools():
    from typing import Type

    from pydantic import BaseModel, Field

    from peepsai.tools import BaseTool
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_with_delegating_agents_should_not_override_agent_tools():
    from typing import Type
...
def test_task_tools_override_agent_tools():
    ...
    peeps = Peeps(agents=[new_researcher], tasks=[task], process=Process.sequential)
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_verbose_output(capsys):
    tasks = [
        Task(
    ...
    peeps.verbose = False
    peeps._logger = Logger(verbose=False)
    peeps.kickoff()
    captured = capsys.readouterr()
    assert captured.out == ""
...
def test_cache_hitting_between_agents():
    ...
    from peepsai.tools import tool
...
def test_api_calls_throttling(capsys):
    ...
    peeps = Peeps(agents=[agent], tasks=[task], max_rpm=1, verbose=True)
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_kickoff_usage_metrics():
    inputs = [
        {"topic": "dog"},
        {"topic": "cat"},
        {"topic": "apple"},
    ...
    results = peeps.kickoff_for_each(inputs=inputs)
...
def test_agents_rpm_is_never_set_if_peeps_max_RPM_is_not_set():
    agent = Agent(
        role="test role",
        goal="test goal",
    ...
    Peeps(agents=[agent], tasks=[task], verbose=True)
...
def test_sequential_async_task_execution_completion():
    ...
    sequential_peeps = Peeps(
        agents=[researcher, writer],
        process=Process.sequential,
        tasks=[list_ideas, list_important_history, write_article],
    )

    sequential_result = sequential_peeps.kickoff()
...
def test_three_task_with_async_execution():
    ...
    # because a peeps can end only end with one or less
    # async tasks
    ...
    assert (
        "The peeps must end with at most one asynchronous task."
...
@pytest.mark.asyncio
@pytest.mark.vcr(filter_headers=["authorization"])
async def test_peeps_async_kickoff():
    inputs = [
        {"topic": "dog"},
        {"topic": "cat"},
        {"topic": "apple"},
    ...
    mock_task_output = (
        PeepsOutput(
            ...
            json_dict={"output": "peeps1"},
            pydantic=None,
        ),
    )
    with patch.object(Peeps, "kickoff_async", return_value=mock_task_output):
        results = await peeps.kickoff_for_each_async(inputs=inputs)
...
def test_kickoff_for_each_empty_input():
    ...
    peeps = Peeps(agents=[agent], tasks=[task])
    results = peeps.kickoff_for_each(inputs=[])
    assert results == []
...
def test_kickoff_for_each_invalid_input():
    ...
    with pytest.raises(TypeError):
        ...
        peeps.kickoff_for_each("invalid input")
...
def test_kickoff_for_each_error_handling():
    ...
    with patch.object(Peeps, "kickoff") as mock_kickoff:
        ...
        with pytest.raises(Exception, match="Simulated kickoff error"):
            peeps.kickoff_for_each(inputs=inputs)
...
async def test_kickoff_async_basic_functionality_and_output():
    ...
    with patch.object(Peeps, "kickoff", return_value=expected_output) as mock_kickoff:
        result = await peeps.kickoff_async(inputs)
...
async def test_async_kickoff_for_each_async_basic_functionality_and_output():
    ...
    with patch.object(
        Peeps, "kickoff_async", side_effect=mock_kickoff_async
    ) as mock_kickoff_async:
        peeps = Peeps(agents=[agent], tasks=[task])

        results = await peeps.kickoff_for_each_async(inputs)
...
async def test_async_kickoff_for_each_async_empty_input():
    ...
    results = await peeps.kickoff_for_each_async([])
...
def test_dont_set_agents_step_callback_if_already_set():
    ...
    peeps = Peeps(
        ...
        process=Process.sequential,
        tasks=[list_ideas],
        step_callback=peeps_callback,
    )

    with patch.object(Agent, "execute_task") as execute:
        execute.return_value = "ok"
        peeps.kickoff()
        assert researcher_agent.step_callback is not peeps_callback
        assert researcher_agent.step_callback is agent_callback


@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_function_calling_llm():
    from peepsai import LLM
    from peepsai.tools import tool
    ...
    peeps = Peeps(agents=[agent1], tasks=[essay])
    result = peeps.kickoff()
...
def test_task_with_no_arguments():
    ...
    peeps = Peeps(agents=[researcher], tasks=[task])
...
def test_code_execution_flag_adds_code_tool_upon_kickoff():
    from peepsai_tools import CodeInterpreterTool
    ...
    peeps = Peeps(agents=[programmer], tasks=[task])
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_sequential_peeps_creation_tasks_without_agents():
    task = Task(
        description="Come up with a list of 5 interesting ideas to explore for an article, then write one amazing paragraph highlight for each idea that showcases how good an article about this topic could be. Return the list of ideas with their paragraph and your notes.",
        expected_output="5 bullet points with a paragraph for each idea.",
    ...
    # Expected Output: The sequential peeps should fail to create because the task is missing an agent
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_hierarchical_peeps_creation_tasks_with_agents():
    """
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_hierarchical_peeps_creation_tasks_with_async_execution():
    """
    Tests that async tasks in hierarchical peepz are handled correctly with proper delegation tools
    """
    task = Task(
        description="Write one amazing paragraph about AI.",
        expected_output="A single paragraph with 4 sentences.",
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_hierarchical_peeps_creation_tasks_with_sync_last():
    """
    Agents are not required for tasks in a hierarchical process but sometimes they are still added
    This test makes sure that the manager still delegates the task to the agent even if the agent is passed in the task
    """
...
def test_peeps_inputs_interpolate_both_agents_and_tasks():
    agent = Agent(
        role="{topic} Researcher",
        goal="Express hot takes on {topic}.",
        backstory="You have a lot of experience with {topic}.",
    ...
    peeps._interpolate_inputs(inputs=inputs)  # Manual call for now

    assert peeps.tasks[0].description == "Give me an analysis around AI."
    assert peeps.tasks[0].expected_output == "5 bullet points about AI."
    assert peeps.agents[0].role == "AI Researcher"
    assert peeps.agents[0].goal == "Express hot takes on AI."
    assert peeps.agents[0].backstory == "You have a lot of experience with AI."


def test_peeps_inputs_interpolate_both_agents_and_tasks_diff():
    from unittest.mock import patch
    ...
    with patch.object(Agent, "execute_task") as execute:
        with patch.object(
            ...
            with patch.object(
                ...
                execute.return_value = "ok"
                peeps.kickoff(inputs={"topic": "AI", "points": 5})
                interpolate_agent_inputs.assert_called()
                interpolate_task_inputs.assert_called()


@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_does_not_interpolate_without_inputs():
    from unittest.mock import patch
...
def test_task_callback_on_peeps():
    from unittest.mock import MagicMock, patch
...
def test_tools_with_custom_caching():
    ...
    peeps = Peeps(agents=[writer1, writer2], tasks=[task1, task2, task3, task4])

    with patch.object(
        CacheHandler, "add", wraps=peeps._cache_handler.add
    ) as add_to_cache:
        with patch.object(CacheHandler, "read", wraps=peeps._cache_handler.read) as _:
            result = peeps.kickoff()
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_log_file_output(tmp_path):
    ...
    peeps = Peeps(agents=[researcher], tasks=tasks, output_log_file=str(test_file))
    peeps.kickoff()
    assert test_file.exists()


@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_output_file_end_to_end(tmp_path):
    """Test output file functionality in a full peeps context."""
    # Create an agent
    agent = Agent(
        role="Researcher",
    ...
    # Create and run the peeps
    peeps = Peeps(
        agents=[agent],
        tasks=[task],
        process=Process.sequential,
    )
    peeps.kickoff(inputs={"topic": "AI"})
...
def test_peeps_output_file_validation_failures():
    """Test output file validation failures in a peeps context."""
    agent = Agent(
        role="Researcher",
        goal="Analyze data",
    ...
    with pytest.raises(ValueError, match="Path traversal"):
        ...
        Peeps(agents=[agent], tasks=[task]).kickoff()
...
def test_manager_agent():
    ...
    manager = Agent(
        role="Manager",
        goal="Manage the peeps and ensure the tasks are completed efficiently.",
        backstory="You're an experienced manager, skilled in overseeing complex projects and guiding teams to success. Your role is to coordinate the efforts of the peeps members, ensuring that each task is completed on time and to the highest standard.",
        allow_delegation=False,
    )
...
@patch("peepsai.peeps.Peeps.kickoff")
@patch("peepsai.peeps.PeepsTrainingHandler")
@patch("peepsai.peeps.TaskEvaluator")
@patch("peepsai.peeps.Peeps.copy")
def test_peeps_train_success(
    copy_mock, task_evaluator, peeps_training_handler, kickoff_mock
):
    task = Task(
        description="Come up with a list of 5 interesting ideas to explore for an article, then write one amazing paragraph highlight for each idea that showcases how good an article about this topic could be. Return the list of ideas with their paragraph and your notes.",
        expected_output="5 bullet points with a paragraph for each idea.",
    ...
    # Create a mock for the copied peeps
    copy_mock.return_value = peeps

    peeps.train(
        n_iterations=2, inputs={"topic": "AI"}, filename="trained_agents_data.pkl"
    )

    # Ensure kickoff is called on the copied peeps
    ...
    task_evaluator.assert_has_calls(
        [
            ...
            mock.call().evaluate_training_data(
                training_data=peeps_training_handler().load(),
    ...
    peeps_training_handler.assert_any_call("training_data.pkl")
    peeps_training_handler().load.assert_called()

    peeps_training_handler.assert_any_call("trained_agents_data.pkl")
    peeps_training_handler().load.assert_called()

    peeps_training_handler().save_trained_data.assert_has_calls(
        [
            mock.call(
                agent_id="Researcher",
...
def test_peeps_train_error():
    ...
    with pytest.raises(TypeError) as e:
        peeps.train()  # type: ignore purposefully throwing err
...
def test__setup_for_training():
    ...
    assert peeps._train is False
    assert task.human_input is False

    for agent in agents:
        assert agent.allow_delegation is True

    peeps._setup_for_training("trained_agents_data.pkl")

    assert peeps._train is True
...
def test_peeps_replay_error():
    ...
    with pytest.raises(TypeError) as e:
        peeps.replay()  # type: ignore purposefully throwing err
        assert "task_id is required" in str(e)


@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_task_db_init():
    agent = Agent(
        role="Content Writer",
...
def test_replay_task_with_context():
    ...
    with patch.object(Task, "execute_sync") as mock_execute_task:
        ...
        with patch.object(Task, "execute_sync") as mock_replay_task:
            ...
            replayed_output = peeps.replay(str(task4.id))
...
def test_replay_with_context():
    ...
    peeps = Peeps(agents=[agent], tasks=[task1, task2], process=Process.sequential)

    with patch(
        "peepsai.utilities.task_output_storage_handler.TaskOutputStorageHandler.load",
        return_value=[
            {
        ...
        peeps.replay(str(task2.id))

        assert peeps.tasks[1].context[0].output.raw == "context raw output"
...
def test_replay_with_invalid_task_id():
    ...
    with patch(
        ...
        with pytest.raises(
            ValueError,
            match="Task with id bf5b09c9-69bd-4eb8-be12-f9e5bae31c2d not found in the peeps's tasks.",
        ):
            peeps.replay("bf5b09c9-69bd-4eb8-be12-f9e5bae31c2d")


@pytest.mark.vcr(filter_headers=["authorization"])
@patch.object(Peeps, "_interpolate_inputs")
def test_replay_interpolates_inputs_properly(mock_interpolate_inputs):
    ...
    peeps = Peeps(agents=[agent], tasks=[task1, task2], process=Process.sequential)
    peeps.kickoff(inputs={"name": "John"})
    ...
    with patch(
        ...
        peeps.replay(str(task2.id))
        assert peeps._inputs == {"name": "John"}
...
def test_replay_setup_context():
    ...
    with patch(
        ...
        assert peeps.tasks[0].output is not None
        assert isinstance(peeps.tasks[0].output, TaskOutput)
        assert peeps.tasks[0].output.description == "Context Task Output"
        assert peeps.tasks[0].output.agent == "test_agent"
        assert peeps.tasks[0].output.raw == "context raw output"
        assert peeps.tasks[0].output.output_format == OutputFormat.RAW

        assert peeps.tasks[1].prompt_context == "context raw output"
...
def test_key_with_interpolated_inputs():
    ...
    curr_key = peeps.key
    peeps._interpolate_inputs({"topic": "AI"})
    assert peeps.key == curr_key
...
@mock.patch("peepsai.peeps.PeepsEvaluator")
@mock.patch("peepsai.peeps.Peeps.copy")
@mock.patch("peepsai.peeps.Peeps.kickoff")
def test_peeps_testing_function(kickoff_mock, copy_mock, peeps_evaluator):
    task = Task(
        description="Come up with a list of 5 interesting ideas to explore for an article, then write one amazing paragraph highlight for each idea that showcases how good an article about this topic could be. Return the list of ideas with their paragraph and your notes.",
        expected_output="5 bullet points with a paragraph for each idea.",
        agent=researcher,
    ...
    n_iterations = 2
    peeps.test(n_iterations, openai_model_name="gpt-4o-mini", inputs={"topic": "AI"})
    ...
    peeps_evaluator.assert_has_calls(
        [
            mock.call(peeps, "gpt-4o-mini"),
            mock.call().set_iteration(1),
            mock.call().set_iteration(2),
            mock.call().print_peeps_evaluation_result(),
        ]
    )
...
def test_hierarchical_verbose_manager_agent():
    ...
    assert peeps.manager_agent is not None
    assert peeps.manager_agent.verbose
...
def test_hierarchical_verbose_false_manager_agent():
    ...
    assert peeps.manager_agent is not None
    assert not peeps.manager_agent.verbose
...
def test_fetch_inputs():
    ...
    actual_placeholders = peeps.fetch_inputs()
...
def test_multimodal_flag_adds_multimodal_tools():
    ...
    """
    from peepsai.tools.agent_tools.add_image_tool import AddImageTool
    ...
    # Define a peeps with the multimodal agent
    peeps = Peeps(agents=[multimodal_agent], tasks=[task], process=Process.sequential)
...
def test_multimodal_agent_image_tool_handling():
    """
    Test that multimodal agents properly handle image tools in the PeepsAgentExecutor
    """
    ...
    peeps = Peeps(agents=[multimodal_agent], tasks=[task])
...
def test_multimodal_agent_live_image_analysis():
    ...
    # Create and run the peeps
    peeps = Peeps(agents=[image_analyst], tasks=[analyze_image])
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_with_failing_task_guardrails():
    """Test that peeps properly handles failing guardrails and retries with validation feedback."""
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_guardrail_feedback_in_context():
...
def test_before_kickoff_callback():
    from peepsai.project import PeepsBase, agent, before_kickoff, task

    @PeepsBase
    class TestPeepsClass:
        from peepsai.project import peeps
        ...
        @peeps
        def peeps(self):
            return Peeps(agents=self.agents, tasks=self.tasks)

    test_peeps_instance = TestPeepsClass()

    test_peeps = test_peeps_instance.peeps()

    # Verify that the before_kickoff_callbacks are set
    assert len(test_peeps.before_kickoff_callbacks) == 1

    # Prepare inputs
    inputs = {"initial": True}

    # Call kickoff
    test_peeps.kickoff(inputs=inputs)

    # Check that the before_kickoff function was called and modified inputs
    assert test_peeps_instance.inputs_modified
...
def test_before_kickoff_without_inputs():
    ...
    assert test_peeps_instance.received_inputs is not None
    assert test_peeps_instance.received_inputs.get("modified") is True


@pytest.mark.vcr(filter_headers=["authorization"])
def test_peeps_with_knowledge_sources_works_with_copy():
    ...
    peeps_copy = peeps.copy()

    assert peeps_copy.knowledge_sources == peeps.knowledge_sources
    assert len(peeps_copy.agents) == len(peeps.agents)
    assert len(peeps_copy.tasks) == len(peeps.tasks)
