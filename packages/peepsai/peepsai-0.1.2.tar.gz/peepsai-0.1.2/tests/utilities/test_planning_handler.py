...
from peepsai.utilities.planning_handler import (
    PeepsPlanner,
...
class InternalPeepsPlanner:
    ...
    def peeps_planner(self):
        ...
        return PeepsPlanner(tasks, None)

    @pytest.fixture
    def peeps_planner_different_llm(self):
        tasks = [
            Task(
                description="Task 1",
                expected_output="Output 1",
        ...
        return PeepsPlanner(tasks, planning_agent_llm)

    def test_handle_peeps_planning(self, peeps_planner):
        ...
        with patch.object(Task, "execute_sync") as execute:
            ...
            result = peeps_planner._handle_peeps_planning()
            assert peeps_planner.planning_agent_llm == "gpt-4o-mini"
            assert isinstance(result, PlannerTaskPydanticOutput)
            assert len(result.list_of_plans_per_task) == len(peeps_planner.tasks)
            execute.assert_called_once()

    def test_create_planning_agent(self, peeps_planner):
        agent = peeps_planner._create_planning_agent()
        assert isinstance(agent, Agent)
        assert agent.role == "Task Execution Planner"

    def test_create_planner_task(self, peeps_planner):
        ...
        task = peeps_planner._create_planner_task(planning_agent, tasks_summary)
    ...
    def test_create_tasks_summary(self, peeps_planner):
        tasks_summary = peeps_planner._create_tasks_summary()
    ...
    @patch('peepsai.knowledge.storage.knowledge_storage.chromadb')
    def test_create_tasks_summary_with_knowledge_and_tools(self, mock_chroma):
        ...
        planner = PeepsPlanner([task], None)
    ...
    def test_handle_peeps_planning_different_llm(self, peeps_planner_different_llm):
        with patch.object(Task, "execute_sync") as execute:
            execute.return_value = TaskOutput(
                description="Description",
                agent="agent",
            ...
            result = peeps_planner_different_llm._handle_peeps_planning()

            assert peeps_planner_different_llm.planning_agent_llm == "gpt-3.5-turbo"
            assert isinstance(result, PlannerTaskPydanticOutput)
            assert len(result.list_of_plans_per_task) == len(
                peeps_planner_different_llm.tasks
            )
            execute.assert_called_once()
