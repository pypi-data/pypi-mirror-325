...
from peepsai.cli.deploy.main import DeployCommand
from peepsai.cli.utils import parse_toml


class TestDeployCommand(unittest.TestCase):
    @patch("peepsai.cli.command.get_auth_token")
    @patch("peepsai.cli.deploy.main.get_project_name")
    @patch("peepsai.cli.command.PlusAPI")
    ...
    def test_display_deployment_info(self):
        with patch("sys.stdout", new=StringIO()) as fake_out:
            ...
            self.assertIn("Deploying the peeps...", fake_out.getvalue())
    ...
    @patch("peepsai.cli.deploy.main.DeployCommand._display_deployment_info")
    ...
    @patch("peepsai.cli.deploy.main.fetch_and_json_env_file")
    @patch("peepsai.cli.deploy.main.git.Repository.origin_url")
    @patch("builtins.input")
    def test_create_peeps(self, mock_input, mock_git_origin_url, mock_fetch_env):
        ...
        self.mock_client.create_peeps.return_value = mock_response

        with patch("sys.stdout", new=StringIO()) as fake_out:
            self.deploy_command.create_peeps()
            self.assertIn("Deployment created successfully!", fake_out.getvalue())
            self.assertIn("new-uuid", fake_out.getvalue())

    def test_list_peepz(self):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = [
            {"name": "Peeps1", "uuid": "uuid1", "status": "active"},
            {"name": "Peeps2", "uuid": "uuid2", "status": "inactive"},
        ]
        self.mock_client.list_peepz.return_value = mock_response

        with patch("sys.stdout", new=StringIO()) as fake_out:
            self.deploy_command.list_peepz()
            self.assertIn("Peeps1 (uuid1) active", fake_out.getvalue())
            self.assertIn("Peeps2 (uuid2) inactive", fake_out.getvalue())

    def test_get_peeps_status(self):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"name": "InternalPeeps", "status": "active"}
        self.mock_client.peeps_status_by_name.return_value = mock_response

        with patch("sys.stdout", new=StringIO()) as fake_out:
            self.deploy_command.get_peeps_status()
            self.assertIn("InternalPeeps", fake_out.getvalue())
            self.assertIn("active", fake_out.getvalue())

    def test_get_peeps_logs(self):
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = [
        ...
        self.mock_client.peeps_by_name.return_value = mock_response

        with patch("sys.stdout", new=StringIO()) as fake_out:
            self.deploy_command.get_peeps_logs(None)
            self.assertIn("2023-01-01 - INFO: Log1", fake_out.getvalue())
            self.assertIn("2023-01-02 - ERROR: Log2", fake_out.getvalue())

    def test_remove_peeps(self):
        mock_response = MagicMock()
        mock_response.status_code = 204
        self.mock_client.delete_peeps_by_name.return_value = mock_response

        with patch("sys.stdout", new=StringIO()) as fake_out:
            self.deploy_command.remove_peeps(None)
            self.assertIn(
                "Peeps 'test_project' removed successfully", fake_out.getvalue()
            )
    ...
    def test_parse_toml_python_311_plus(self):
        ...
        python = "^3.11"
        peepsai = { extras = ["tools"], version = ">=0.51.0,<1.0.0" }
        """
    ...
    @patch(
        ...
        dependencies = ["peepsai"]
        """,
    )
    def test_get_project_name_python_310(self, mock_open):
        from peepsai.cli.utils import get_project_name
    ...
    def test_get_peepsai_version(self):
        from peepsai.cli.version import get_peepsai_version

        assert isinstance(get_peepsai_version(), str)
