...
from peepsai.knowledge.source.peeps_docling_source import PeepsDoclingSource
from peepsai.knowledge.source.csv_knowledge_source import CSVKnowledgeSource
from peepsai.knowledge.source.excel_knowledge_source import ExcelKnowledgeSource
from peepsai.knowledge.source.json_knowledge_source import JSONKnowledgeSource
from peepsai.knowledge.source.pdf_knowledge_source import PDFKnowledgeSource
from peepsai.knowledge.source.string_knowledge_source import StringKnowledgeSource
from peepsai.knowledge.source.text_file_knowledge_source import TextFileKnowledgeSource


@pytest.fixture(autouse=True)
def mock_vector_db():
    """Mock vector database operations."""
    with patch("peepsai.knowledge.storage.knowledge_storage.KnowledgeStorage") as mock:
...
def test_pdf_knowledge_source(mock_vector_db):
    ...
    pdf_path = current_dir / "peepsai_quickstart.pdf"
    ...
    mock_vector_db.query.return_value = [
        {"context": "peepsai create peeps latest-ai-development", "score": 0.9}
    ]

    # Perform a query
    query = "How do you create a peeps?"
    results = mock_vector_db.query(query)

    # Assert that the correct information is retrieved
    assert any(
        "peepsai create peeps latest-ai-development" in result["context"].lower()
        for result in results
    )
    mock_vector_db.query.assert_called_once()
...
def test_docling_source(mock_vector_db):
    docling_source = PeepsDoclingSource(
        file_paths=[
...
def test_multiple_docling_sources():
    ...
    docling_source = PeepsDoclingSource(file_paths=urls)
