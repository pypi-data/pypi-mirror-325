...
from peepsai.utilities.peeps_json_encoder import PeepsJSONEncoder
from peepsai.utilities.errors import DatabaseError, DatabaseOperationError
        ...
        try:
            ...
            """,
                    (
                        ...
                        json.dumps(output, cls=PeepsJSONEncoder),
                        task_index,
                        json.dumps(inputs, cls=PeepsJSONEncoder),
                        was_replayed,
                    ),
                )
                conn.commit()
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for key, value in kwargs.items():
                    fields.append(f"{key} = ?")
                    values.append(
                        json.dumps(value, cls=PeepsJSONEncoder)
