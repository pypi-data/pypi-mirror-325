...
from peepsai.tasks.guardrail_result import GuardrailResult
from peepsai.tasks.output_format import OutputFormat
from peepsai.tasks.task_output import TaskOutput
from peepsai.telemetry.telemetry import Telemetry
from peepsai.tools.base_tool import BaseTool
from peepsai.utilities.config import process_config
from peepsai.utilities.converter import Converter, convert_to_model
...
class Task(BaseModel):
    ...
    def _execute_core(
        ...
        if not agent:
            raise Exception(
                f"The task '{self.description}' has no agent assigned, therefore it can't be executed directly and should be executed in a Peeps using a specific process that support that, like hierarchical."
            )

        self.start_time = datetime.datetime.now()
        self._execution_span = self._telemetry.task_started(peeps=agent.peeps, task=self)
        ...
        if self._execution_span:
            self._telemetry.task_ended(self._execution_span, self, agent.peeps)
    ...
    def interpolate_inputs_and_add_conversation_history(
        ...
        if "peeps_chat_messages" in inputs and inputs["peeps_chat_messages"]:
            conversation_instruction = self.i18n.slice(
                "conversation_history_instruction"
            )

            peeps_chat_messages_json = str(inputs["peeps_chat_messages"])

            try:
                peeps_chat_messages = json.loads(peeps_chat_messages_json)
            except json.JSONDecodeError as e:
                print("An error occurred while parsing peeps chat messages:", e)
                raise

            conversation_history = "\n".join(
                f"{msg['role'].capitalize()}: {msg['content']}"
                for msg in peeps_chat_messages
