...
def method_calls_peeps(method: Any) -> bool:
    """
    Check if the method contains a call to `.peeps()`.

    Parameters
    ----------
    method : Any
        The method to analyze for peeps() calls.

    Returns
    -------
    bool
        True if the method calls .peeps(), False otherwise.

    Notes
    -----
    Uses AST analysis to detect method calls, specifically looking for
    attribute access of 'peeps'.
    """
    try:
    ...
    class PeepsCallVisitor(ast.NodeVisitor):
        """AST visitor to detect .peeps() method calls."""
        def __init__(self):
            self.found = False

        def visit_Call(self, node):
            if isinstance(node.func, ast.Attribute):
                if node.func.attr == "peeps":
                    self.found = True
            self.generic_visit(node)

    visitor = PeepsCallVisitor()
    visitor.visit(tree)
...
def add_nodes_to_network(
    ...
    for method_name, (x, y) in node_positions.items():
        ...
        elif method_calls_peeps(method):
            node_style = node_styles["peeps"]
        else:
