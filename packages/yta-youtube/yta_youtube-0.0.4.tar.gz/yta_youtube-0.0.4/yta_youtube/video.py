...
class YoutubeVideo:
        ...
        if not hasattr(self, '_audio_formats') or self._audio_formats is None:
            ...
            for format in self._formats:
                if (
                    ...
                    language_key = format['language'] if format['language'] is not None else YoutubeVideoLanguage.NO_LANGUAGE.value
                    # audio_format.ar = {}
    ...
    def _get_subtitles(
        ...
        language = (
            ...
            if language == YoutubeVideoLanguage.DEFAULT else
            language.value
        )
        ...
        if format in YoutubeSubtitleFormat.special_values():
            format = (
                YoutubeSubtitleFormat.formats_ordered_by_quality()
                if format == YoutubeSubtitleFormat.BEST
                else YoutubeSubtitleFormat.formats_ordered_by_quality()[::-1]
            )[0]

        return with_language[format]
