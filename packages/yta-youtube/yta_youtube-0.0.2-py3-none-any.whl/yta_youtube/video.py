...
and work faster.
"""
from yta_youtube.dataclasses import YoutubeVideoScene, YoutubeVideoAudioFormat, YoutubeVideoVideoFormat, YoutubeVideoChapter
from yta_youtube.enums import AudioFormatExtension, VideoFormatExtension, VideoFormatQuality, AudioFormatQuality
...
class YoutubeVideo:
    ...
    @property
    def language(self) -> YoutubeLanguage:
        """
        ...
        """
        # "language": "en",
        return YoutubeLanguage.to_enum(self.data['language'])
    ...
    def _automatic_subtitles(self) -> Union[dict, None]:
        ...
        """
        if self._automatic_subtitles is None:
            subtitles = self.data['automatic_captions']
            self._automatic_subtitles = defaultdict(dict)
            ...
            for language, subtitle_elements in subtitles.items():
                for subtitle_element in subtitle_elements:
                    ...
                    if not 'protocol' in subtitle_element:
                        self._automatic_subtitles[language][subtitle_element['ext']] = subtitle_element

        return self._automatic_subtitles
    ...
    @property
    def automatic_subtitles_languages(self) -> list[YoutubeLanguage]:
        """
        ...
        return list([
            YoutubeLanguage.to_enum(language)
    ...
    def _subtitles(self) -> Union[dict, None]:
        ...
        if self._subtitles is None:
            subtitles = self.data['subtitles']
            self._subtitles = defaultdict(dict)
    
            for language, subtitle_elements in subtitles.items():
                for subtitle_element in subtitle_elements:
                    self._subtitles[language][subtitle_element['ext']] = subtitle_element
    ...
    @property
    def subtitles_languages(self) -> list[YoutubeLanguage]:
        """
    ...
    def audio_formats(self) -> any:
        ...
        if self._audio_formats is None:
            ...
            for format in self._formats:
                if not format.get('width') and not format.get('height'):
                    # Ensure the language key exists in audio_formats
                    language_key = format['language']
                    # audio_format.ar = {}
    ...
    def get_audio_format(
        self,
        language: YoutubeLanguage = YoutubeLanguage.DEFAULT,
        extension: AudioFormatExtension = AudioFormatExtension.HIGHEST,
        quality: AudioFormatQuality = AudioFormatQuality.HIGHEST
        ...
        def get_audio_format_from_language(
            audio_formats: dict,
            ...
            (level 0).
            """
            # TODO: The default audio format is not just the
            # first one available, it is the one with which
            # the video was uploaded
            language_key = (
                # TODO: Handle special default language
                # TODO: By now I'm using the first one as default
                list(audio_formats.keys())[0]
                if language == YoutubeLanguage.DEFAULT
                else language.value
            )

            return audio_formats.get(language_key, None)

        with_language = get_audio_format_from_language(self._audio_formats, language)
        
        if with_language is None:
            # No audio available for that language
            return None
        
        def get_audio_format_from_extension(
            audio_formats: dict,
            ...
            if extension in AudioFormatExtension.special_values():
                ...
                formats_ordered_by_quality = (
                    ...
                    if extension == AudioFormatExtension.HIGHEST
        ...
        with_extension = get_audio_format_from_extension(with_language, extension)

        if with_extension is None:
            # No audio available with that extension for that language
            return None
        
        def get_audio_format_from_quality(
            audio_formats: dict,
            ...
            if quality in AudioFormatQuality.special_values():
                ...
                formats_ordered_by_quality = (
                    ...
                    if extension == AudioFormatQuality.HIGHEST
                ...
                quality_key = extension.value

            return audio_formats.get(quality_key, None)

        with_quality = get_audio_format_from_quality(with_extension, quality)
    ...
    def video_formats(self) -> dict:
        ...
        """
        if self._video_formats is None:
            self._video_formats = {}
            ...
            for format in self._formats:
                if format.get('aspect_ratio') and format.get('format_note') not in ['storyboard', 'Premium']:
    ...
    def get_video_format(
        self,
        quality: VideoFormatQuality = VideoFormatQuality.HIGHEST,
        extension: VideoFormatExtension = VideoFormatExtension.HIGHEST,
        ...
        def get_video_format_from_quality(
            video_formats: dict,
            ...
            if quality in VideoFormatQuality.special_values():
                ...
                formats_ordered_by_quality = (
                    ...
                    if extension == VideoFormatQuality.HIGHEST
        ...
        with_quality = get_video_format_from_quality(self._video_formats, quality)

        def get_video_format_from_extension(
            video_formats: dict,
            ...
            if extension in VideoFormatExtension.special_values():
                ...
                formats_ordered_by_quality = (
                    ...
                    if extension == VideoFormatExtension.HIGHEST
        ...
        with_extension = get_video_format_from_extension(with_quality, extension)
    ...
    def get_automatic_subtitles(
        ...
        filename = self._get_filename(f'automatic_subtitles_{language}.{format}')

        if not FileValidator.file_exists(filename):
            filename = Downloader.download_file(subtitles_info['url'], filename).filename
    ...
    def get_subtitles(
        ...
        if not YoutubeLanguage.to_enum(language) in self.subtitles_languages:
            # No subtitles for that language
            return None
        ...
        if subtitles_info is None:
            # No subtitles available for that language in
            # that specific format
            return None
        
        # This name will allow us persist the file and return
        # it the next time we need it
        filename = self._get_filename(f'subtitles_{language}.{format}')
    ...
    def download_audio(
        ...
        audio_format = self.get_audio_format(language, extension, quality)

        if audio_format is None:
            raise Exception('No "audio" available for the given language, extension and quality.')
        
        output_filename = Output.get_filename(output_filename, {format})

        # First is persistance filename, then output
        filename = self._get_filename(f'audio_{language}.{format}')

        if not FileValidator.file_exists(filename):
            filename = Downloader.download_file(audio_format['url'], filename).filename
    ...
    def __init__(
        ...
        # TODO: Maybe initialize the most demanding properties
        # here if we need
    ...
    def _get_filename(self, filename: str) -> str:
        ...
        will use the video id to concatenate an build
