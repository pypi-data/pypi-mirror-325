...
def create(config, plandir, cluster, overrides, dnsconfig=None):
    ...
    if sno:
        ...
        if 'ipv6' in overrides and not overrides['ipv6']:
            nic_name = overrides.get('sno_nic', 'ens3')
            nm_data = f"[connection]\ntype=ethernet\ninterface-name={nic_name}\n[ipv6]\nmethod=disabled\n"
            sno_files.append({'path': f"/etc/NetworkManager/system-connections/{nic_name}.nmconnection",
                              'data': nm_data, 'mode': '0600'})
        if sno_files:
