...
def asdf_tree_from_filenames(
    ...
    if len(datasets) == 1:
        ...
        # or else the tiles are in the wrong order, hence the transpose.
        datasets_arr = np.array(datasets, dtype=object).reshape(mosaic_shape, order="F").T
    ...
    **kwargs,
    ...
    with resources.as_file(
        ...
        with asdf.AsdfFile(tree, custom_schema=schema_path, **kwargs) as afile:
            ...
            # This is also failing with the following error in asdf-maker and instrument pipeline tests
            # asdf._jsonschema.exceptions.ValidationError: <Helioprojective Frame (...)> is not of type 'object'
            # afile.validate()
            yield afile
