...
# --------------------------------------------------------------------
# 4. Fetch HubSpot List Records (Pagination) 
#    with offset, limit, order_by, order
# --------------------------------------------------------------------
@assistant_tool
async def fetch_hubspot_list_records(
    ...
    Supports offset, limit, order_by, and order via repeated calls and skipping records on the client side.
    (Note: The membership endpoint doesn't inherently support reorder-by-property, so we do local skipping and
    optionally a local sort after fetching.)
    """
    ...
    # We can emulate local sorting if desired. 
    # The membership endpoint doesn't accept sorting by property. 
    # If you truly need order_by from the API side, use the "search" endpoint.
    ...
    async with aiohttp.ClientSession() as session:
        # 1) Page through membership to get contact IDs
        while True:
            if count_collected >= limit:
                break
        ...
        # 2) Optionally sort by 'order_by' locally. But we must fetch the contacts first. 
        #    For that, we do a batch read for each chunk of IDs.
        batch_read_url = "https://api.hubapi.com/crm/v3/objects/contacts/batch/read"

        # We'll fetch all contact property names:
        ...
        # 3) If we want local sorting by 'order_by', do it here:
        #    We'll look inside each record at "properties" -> { order_by: val }, then sort.
        #    Note that values might be missing or None. 
        if order_by:
...
# --------------------------------------------------------------------
# 5. List All CRM Lists (Pagination)
#    with offset, limit, order_by, order
# --------------------------------------------------------------------
@assistant_tool
async def list_all_crm_lists(
    ...
    HUBSPOT_ACCESS_TOKEN = get_hubspot_access_token(tool_config)
    if payload is None:
        # This is the typical shape for the "crm/v3/lists/search" endpoint. 
        payload = {
            "listIds": [],
            "offset": 0,  # HubSpot's internal offset
            "query": "",
            "count": 0,
            "processingTypes": [],
            "additionalProperties": [],
            "sort": "HS_LIST_NAME"  # By default
        }
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            async with session.post(url, headers=headers, json=payload) as response:
                ...
                if (not has_more) or (count_collected >= limit):
                    break
        ...
        if order_by:
            reverse_sort = (order.lower() == "desc")
            all_results.sort(key=lambda x: x.get(order_by, ""), reverse=reverse_sort)
...
# --------------------------------------------------------------------
# Placeholder for your real "assistant_tool" decorator
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Utility: Retrieve HubSpot Access Token (Bearer)
# --------------------------------------------------------------------
def get_hubspot_access_token(tool_config: Optional[List[Dict]] = None) -> str:
    """
    Retrieves the HubSpot access token from the provided tool configuration or environment.
    """
    if tool_config:
        hubspot_config = next(
            (item for item in tool_config if item.get("name") == "hubspot"), None
        ...
        if hubspot_config:
            config_map = {
                c["name"]: c["value"]
                for c in hubspot_config.get("configuration", [])
                if c
            }
            HUBSPOT_ACCESS_TOKEN = config_map.get("apiKey")
        else:
...
# --------------------------------------------------------------------
# 1) Fetch HubSpot Lead Info (Custom Object: 'leads', V3)
# --------------------------------------------------------------------
@assistant_tool
