...
async def guess_email_with_hunter(
    ...
    Fix applied: We handle the case where score might be None or not numeric, defaulting to 0.
    """
    ...
    # If you want to handle middle_name and the API supports it, you could append it:
...
async def check_email_validity(
    ...
    # Identify which providers are available
    ...
    for provider in priority:
        if provider in provider_names:
            ...
            # Overwrite final_result with the latest
            final_result = result
            # If we get "high" confidence, stop.
...
async def guess_email(
    ...
    """
    Attempt to guess the email using the provider(s) indicated in the tool_config, in priority order.
    If the first guess is "high" confidence, we stop. Otherwise we move on.
    """
    ...
    for provider in priority:
        if provider in provider_names:
            ...
            result = await guess_func(first_name, last_name, domain, middle_name, tool_config)
            final_result = result
...
async def process_email_properties(
    ...
    if email:
        ...
        # If not valid or confidence < "high", we attempt to guess
        ...
        # No existing email => must guess a new one
