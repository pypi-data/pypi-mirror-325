...
""",
    ...
    def execute(self, args: argparse.Namespace) -> None:
        ...
        if (args.list_pipelines or args.operation == "list") and run_context:
...
def list_pipelines(run_context: ProjectRunContext) -> None:
    print_entity_list(
        ...
        [
            ...
            for name, config in run_context.config.get("pipelines", {}).items()
        ],
    )
...
def add_project_pipeline(
    ...
    if source_name not in run_context.config.sources.keys():
        fmt.warning(
            "Source %s does not exist in project, your pipeline will "
            "reference a non-existing source." % source_name
        )
    if destination_name not in run_context.config.destinations.keys():
        fmt.warning(
