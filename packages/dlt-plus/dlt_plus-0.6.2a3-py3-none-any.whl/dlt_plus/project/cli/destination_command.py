        ...
        if args.destination_command == "list":
            print_entity_list(
                ...
                [
                    ...
                    for name, config in project_run_context.config.get("destinations", {}).items()
                ],
            )
...
def add_project_destination(
    ...
    if not fmt.confirm("Do you want to proceed?", default=True):
        exit(0)
    add_destination(project_run_context, destination_name, destination_type, dataset_name)
