...
from ..run_context import ProjectRunContext, project_from_args, ProjectRunContextNotAvailable
from dlt_plus.project.run_context import switch_context
...
def project_from_args_with_cli_output(
    ...
    try:
        ...
        if not allow_no_project:
            ...
            raise
        fmt.note("No project found. Running outside of a project context.")
    return run_context
...
def init_project(project_dir: str, name: str = None) -> ProjectRunContext:
    """
    ...
    # write some files
    _write_project_yaml(project_dir, project_yaml)

    project_run_context = switch_context(project_dir)
    ...
    for fname in ["secrets.toml", "config.toml"]:
        with resources.open_text("dlt_plus.project._templates.project.dlt", fname) as file:
            with open(
    ...
    # copy gitignore file
    with resources.open_text("dlt_plus.project._templates.project", "gitignore") as file:
        with open(os.path.join(project_dir, ".gitignore"), "w", encoding="utf-8") as f:
            f.write(file.read())

    # create requirements file with base dependencies
    with open(os.path.join(project_dir, REQUIREMENTS_FILE_NAME), "w", encoding="utf-8") as f:
        from dlt_plus.version import __version__ as dlt_plus_version
        from dlt.version import __version__ as dlt_version

        f.write(f"dlt=={dlt_version}\n")
        f.write(f"dlt-plus=={dlt_plus_version}\n")

    # check that we can switch to the project context
...
def add_source(
    ...
    # NOTE: for some reason this import is needed to make the sql_database source work
    from dlt.sources.sql_database import sql_database  # noqa: F401
    ...
    source_spec = SourceReference.find(source_type).ref.SPEC
...
def add_destination(
    ...
    destination_ref = DestinationReference.find(destination_type)
