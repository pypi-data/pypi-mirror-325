...
class ProjectCommand(SupportsCliCommand):
    ...
    def configure_parser(self, parser: argparse.ArgumentParser) -> None:
        ...
        init_parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Create a pip package instead of a flat project",
        )
        init_parser.add_argument(
            "--force",
            action="store_true",
        ...
        if args.project_command == "list":
            ...
            if args.project_command == "clean":
                ...
                if args.config_command == "validate":
                    success = validate_config(run_context.config, args.profile)
                    if not success:
                        exit(1)

                elif args.config_command == "show":
                    show_config(run_context.config, args.format, args.section)
            else:
...
def init_command(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if run_context.config.project_dir == cwd and not args.force:
            fmt.error(
    ...
    if args.package:
        fmt.error("Package creation is not implemented yet, please omit the --package flag.")
        raise CliCommandException()
    ...
    run_context = init_project(cwd, args.project_name if args.project_name else None)
...
def show_info(run_context: ProjectRunContext) -> None:
    ...
    fmt.echo("Default profile: %s" % fmt.bold(run_context.config.settings["default_profile"]))
...
def clean_local_data(run_context: ProjectRunContext, args: argparse.Namespace) -> None:
    ...
    for cache_name in config.caches:
        fmt.echo("Will delete local data for cache %s" % fmt.style(cache_name, fg="green"))
        cache = factory.create_cache(cache_name)
        cache.wipe()
...
def validate_config(config: Project, strict: bool = False) -> bool:
    try:
...
def show_config(config: Project, format: str, section: Optional[str] = None) -> None:
    ...
    if section:
        ...
        # add project section
        config_to_show["project"] = config.settings  # type: ignore
