...
class Agent:
    ...
    terminating_tools: Sequence[str] = field(
        default_factory=lambda: [send_message.__name__], repr=False
    )
    ...
    async def step(
        ...
        if (
            ...
            and response.content[0].name in self.terminating_tools
        ):
    ...
    async def run(
        ...
        while True:
            ...
            if (
                isinstance(last_tool_call, ToolCall)
                and last_tool_call.name in self.terminating_tools
            ):
                break
