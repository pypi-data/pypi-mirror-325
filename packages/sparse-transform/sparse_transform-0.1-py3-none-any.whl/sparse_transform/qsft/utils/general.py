...
def gwht(x: np.ndarray, q: int, n: int) -> np.ndarray:
    ...
    n (int): Dimension of the transform.
...
def nth_roots_unity(n: int) -> np.ndarray:
    ...
    Parameters:
    n (int): The order of the roots.

    Returns:
    np.ndarray: Array of nth roots of unity.
    """
...
def near_nth_roots(ratios: np.ndarray, q: int, eps: float) -> bool:
    ...
    Parameters:
    ratios (np.ndarray): Array of ratios.
    q (int): Base of the transform.
    eps (float): Tolerance.

    Returns:
    bool: True if the ratios are near the nth roots of unity, False otherwise.
    """
...
def qary_vec_to_dec(x: np.ndarray, q: int = 2) ->  Union[np.int64, np.int32, object]:
    ...
    Parameters:
    x (np.ndarray): q-ary vector.
    q (int): Base of the vector.
...
def dec_to_bin(x: int, num_bits: int) -> np.ndarray:
    """
...
def binary_ints(m: int) -> np.ndarray:
    ...
    Returns:
    np.ndarray: Matrix of binary representations.
    """
...
def angle_q(x: np.ndarray, q: int) -> np.ndarray:
    ...
    Parameters:
    x (np.ndarray): Complex array.
    q (int): Base of the angle.

    Returns:
    np.ndarray: q-ary angle array.
    """
...
def qary_ints(m: int, q: int, dtype: type = int) -> np.ndarray:
    ...
    Parameters:
    m (int): Length of the vectors.
    q (int): Base of the vectors.
    dtype (type): Data type of the vectors.

    Returns:
    np.ndarray: Matrix of q-ary vectors.
    """
...
def qary_ints_low_order(m: int, q: int, order: int) -> np.ndarray:
    ...
    m (int): Length of the vectors.
    q (int): Base of the vectors.
    order (int): Order of the vectors.
...
def base_ints(q: int, m: int) -> np.ndarray:
    """
    Returns a matrix where row 'i' is the base-q representation of 'i', for 'i' from 0 to q ** m - 1.

    Parameters:
    q (int): Base of the representation.
    m (int): Length of the representation.

    Returns:
    np.ndarray: Matrix of base-q representations.
    """
...
def random_signal_strength_model(sparsity: int, a: float, b: float) -> np.ndarray:
    ...
    Parameters:
    sparsity (int): Sparsity of the signal.
    a (float): Minimum magnitude.
    b (float): Maximum magnitude.

    Returns:
    np.ndarray: Random signal strength model.
    """
...
def random_skewed_signal_strength_model(sparsity: int, a: float, b: float, skewed: float) -> np.ndarray:
    ...
    a (float): Minimum magnitude.
    b (float): Maximum magnitude.
    skewed (float): Skewness of the signal.

    Returns:
    np.ndarray: Random skewed signal strength model.
    """
    x_0 = a
    x_1 = b
...
def best_convex_underestimator(points: np.ndarray) -> np.ndarray:
    """
    Finds the best convex underestimator for a set of points.

    Parameters:
    points (np.ndarray): Input points.

    Returns:
    np.ndarray: Convex underestimator.
    """
...
def sort_qary_vecs(qary_vecs: np.ndarray) -> np.ndarray:
    ...
    Parameters:
    qary_vecs (np.ndarray): Input q-ary vectors.

    Returns:
    np.ndarray: Sorted q-ary vectors.
    """
