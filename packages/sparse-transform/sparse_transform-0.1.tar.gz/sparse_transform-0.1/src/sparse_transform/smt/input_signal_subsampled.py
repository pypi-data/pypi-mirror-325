from sparse_transform.smt.utils import binary_ints, load_data, save_data, bin_vec_to_dec, fmt, dec_to_bin_vec
from sparse_transform.smt.input_signal import Signal
from sparse_transform.smt.query import get_Ms_and_Ds
...
class SubsampledSignal(Signal):
            ...
            for j in range(len(self.Ds[i])):
                ...
                if self.foldername and transform_file.is_file():
                    ...
                    if self.foldername and sample_file.is_file():
                        ...
                        query_indices = self._get_smt_query_indices(self.Ms[i], self.Ds[i][j], dec=False)
    ...
    def _get_smt_query_indices(self, M, D_sub, dec=True):
        """
