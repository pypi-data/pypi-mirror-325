...
def get_Ms_simple(n: int, b: int, **kwargs: Any) -> List[np.ndarray]:
    ...
    Returns:
    List[np.ndarray]: List of subsampling matrices.
    """
    Ms = []
...
def get_Ms(n: int, b: int, q: int, method: str = "simple", **kwargs: Any) -> List[np.ndarray]:
    ...
    b (int): Number of columns.
    q (int): Base of the transform.
    method (str): Method to use for generating matrices.
    kwargs (Any): Additional keyword arguments.
...
def get_D_identity(n: int, **kwargs: Any) -> np.ndarray:
    ...
    Returns:
    np.ndarray: Identity delay matrix.
    """
...
def get_D_random(n: int, **kwargs: Any) -> np.ndarray:
    ...
    Returns:
    np.ndarray: Random delay matrix.
    """
    q=kwargs.get("q")
...
def get_D_joint_coded(n: int, **kwargs: Any) -> np.ndarray:
    ...
    Returns:
    np.ndarray: Joint coded delay matrix.
    """
    t = kwargs.get("t")
...
def get_D_nso(n: int, D_source: np.ndarray, **kwargs: Any) -> List[np.ndarray]:
    ...
    Parameters:
    n (int): Number of rows.
    D_source (np.ndarray): Source delay matrix.
    kwargs (Any): Additional keyword arguments.

    Returns:
    List[np.ndarray]: NSO-SPRIGHT delay matrix.
    """
...
def get_D_channel_identity(n: int, D: np.ndarray, **kwargs: Any) -> List[np.ndarray]:
    ...
    Returns:
    List[np.ndarray]: Channel identity delay matrix.
    """
    q = kwargs.get("q")
    return [D % q]
...
def subsample_indices(M: np.ndarray, d: np.ndarray) -> np.ndarray:
    ...
    Parameters:
    M (np.ndarray): Subsampling matrix.
    d (np.ndarray): Subsampling offset.

    Returns:
    np.ndarray: Subsample indices.
    """
...
def get_bch_decoder(n: int, t: int, dectype: str = "hard", chase_depth: Union[int, None] = None) -> Callable:
    ...
    Parameters:
    n (int): Number of rows.
    t (int): Number of errors to correct.
    dectype (str): Type of decoding.
    chase_depth (Union[int, None]): Depth of chase decoding.

    Returns:
    Callable: BCH decoder.
    """
    if dectype == "hard":
