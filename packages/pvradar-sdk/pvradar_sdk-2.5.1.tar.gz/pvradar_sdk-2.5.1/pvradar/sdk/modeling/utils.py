...
from .basics import AggFunctionName, DataType
...
def resample_series(
    ...
    *,
    ...
    if not from_freq:
        from_freq = pd.to_timedelta(np.diff(series.index).min())
    ...
    result = _pure_resample_series(series, scaling, freq, agg, is_upsampling, interpolate=interpolate)
...
def convert_by_attrs(value: Any, param_attrs: Mapping[str, Any]) -> Any:
