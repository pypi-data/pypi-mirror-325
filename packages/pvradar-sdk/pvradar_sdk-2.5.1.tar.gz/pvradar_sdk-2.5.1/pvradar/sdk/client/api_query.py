...
class Query(JSONEncoder):
    ...
    @classmethod
    def from_site_environment(cls, location: Location, interval: pd.Interval) -> Self:
        return cls(period=_to_period(interval), location=_to_lat_lon(location), tz=location.tz)
...
def _to_lat_lon(location: Location) -> dict[str, float]:
    return {
