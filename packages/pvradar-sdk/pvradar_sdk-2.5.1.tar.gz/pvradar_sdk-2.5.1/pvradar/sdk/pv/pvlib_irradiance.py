...
from ..modeling.decorators import pvradar_resource_type, set_unit
import pandas as pd
import numpy as np
import pvlib
...
def tracker_orientation_table(
    ...
    tracker_orientation_table = pvlib.tracking.calc_surface_orientation(
...
def surface_tilt_angle(context: ModelContext, array: ArrayDesign):
    if isinstance(array.structure, TrackerStructureDesign):
        orientation_table = context.run(tracker_orientation_table)  # TODO: keep=True?
        return orientation_table['surface_tilt']

    else:
        orientation_table = context.run(fixed_structure_orientation_table)  # TODO: keep=True?
        return orientation_table['surface_tilt']
