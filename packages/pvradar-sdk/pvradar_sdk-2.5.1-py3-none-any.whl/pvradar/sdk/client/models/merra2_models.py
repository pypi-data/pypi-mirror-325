...
def _merra2_3h_to_1h(df: SeriesOrDf) -> SeriesOrDf:
    index = df.index
    assert index is not None
    assert index.inferred_type == 'datetime64'
    start_datetime = df.index[0]
    assert isinstance(start_datetime, pd.Timestamp)
    end_datetime = index[-1].replace(hour=23)  # type: ignore
...
def merra2_particle_volume_concentration(
    *,
    ...
    result = _merra2_3h_to_1h(result)
...
def merra2_relative_humidity(
    *,
    ...
    result = _merra2_3h_to_1h(merra2_aerosol_mixing_table['RH'])
    return result
