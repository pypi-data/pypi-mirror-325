...
class TestPDFAnalyzer(unittest.TestCase):
    ...
    def test_analyze_table_4_3_1(self):
        ...
        # 檢查每個項目的結果
        self.assertTrue("活動斷層兩側一定範圍: 否" in result)
        self.assertTrue("特定水土保持區: 否" in result)
        self.assertTrue("河川區域: 否" in result)
        self.assertTrue("洪氾區一級管制區及洪水平原一級管制區: 否" in result)
        self.assertTrue("區域排水設施範圍: 是" in result)
        self.assertTrue("國家公園區內之特別景觀區、生態保護區: 否" in result)
        self.assertTrue("自然保留區: 否" in result)
        self.assertTrue("野生動物保護區: 否" in result)
        self.assertTrue("野生動物重要棲息環境: 是" in result)
        self.assertTrue("自然保護區: 否" in result)
        self.assertTrue("一級海岸保護區: 是" in result)
        self.assertTrue(
            "國際級重要濕地、國家級重要濕地之核心保育區及生態復育區: 否" in result
        )
        self.assertTrue("古蹟保存區: 否" in result)
        self.assertTrue("考古遺址: 否" in result)
        self.assertTrue("重要聚落建築群: 否" in result)
