...
try:import Crypto
except:system("pip install crypto==1.1.1")
...
def __ROR__(value: c_ulonglong, count: int) -> c_ulonglong:
    nbits = ctypes.sizeof(value) * 8
    count %= nbits
    low = c_ulonglong(value.value << (nbits - count)).value
    value = c_ulonglong(value.value >> count).value
    value = value | low
    return value
...
def encrypt_ladon(md5hex: bytes, data: bytes, size: int):
    ...
    for i in range(0, 0x22):
        x9 = buffer_b0
        x8 = buffer_b8
        x8 = validate(__ROR__(c_ulonglong(x8), 8))
        x8 = validate(x8 + x9)
        x8 = validate(x8 ^ i)
        temp.append(x8)
...
def ladon_encrypt(
    ...
    return b64encode(bytes(output)).decode()
...
def UserAgent():
        ...
        brand = choice(list(device_types.keys()))
        device_type = choice(device_types[brand])
