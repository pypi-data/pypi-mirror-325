...
def _get_compose_files(
    mode: Mode, prod: bool = False, network: Network = Network.HOST
) -> List[ValidPath]:
...
def build(mode: Mode, clean: bool, services: List[str]):
    ...
    if config.mode == Mode.LOW_FIDELITY:
        ...
        docker = DockerClient(
            compose_files=_get_compose_files(mode=config.mode, prod=False, network=config.network),
...
def up(spin: bool, build: bool, services: List[str]):
    ...
    os.environ["MISSIM_MODE"] = mode.value
    ...
    docker = DockerClient(
        compose_files=_get_compose_files(mode, prod=prod, network=config.network),
        compose_project_directory=get_project_root(),
    )
    if spin:
        if mode == "standalone":
...
@click.option("--ros-domain-id", type=int, help="ROS domain ID", default=0)
...
def configure(ros_domain_id: int, mode: Mode, prod: bool, default: bool):  # type: ignore
    """Configure MIS-SIM"""

    if default:
        config = MissimConfig()
        config.ros_domain_id = ros_domain_id
        config.mode = mode
        config.prod = prod
...
def generate():  # type: ignore
    ...
    docker.compose.run("missim_core", ["npx", "-y", "ros-typescript-generator"])
