...
def readExcel2dataframe(sheetname,filepath,skiprows,nrows=0):
        ...
        if event.xdata is not None and event.ydata is not None:
            ...
            if event.inaxes == ax_twin or event.inaxes == ax:
                if ax_twin is None: #鼠标点坐标显示为主坐标轴
                    ...
                    if line_flag: #值为True
                        #y1_min, y1_max = ax.get_yaxis().get_data_interval() #取主y轴的坐标范围
                    ...
                    if dis_main < dis_twin: #主轴近
                        ...
                        # 如果line_flag为True，则需要更新重绘鼠标点位置的垂直辅助观察虚线
                        if line_flag:  # 值为True
                            #y1_min, y1_max = ax_twin.get_yaxis().get_data_interval()  # 取副y轴的坐标范围
                            y1_min, y1_max = ax_twin.get_ylim()
