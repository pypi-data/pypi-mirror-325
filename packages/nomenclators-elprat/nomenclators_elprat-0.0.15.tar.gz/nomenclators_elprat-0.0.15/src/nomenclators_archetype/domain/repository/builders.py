...
COMPARISON_OPERATIONS = [GT_OPERATION, GT_OPERATION, LT_OPERATION,
                         LTE_OPERATION, EQ_OPERATION, NEQ_OPERATION]

LIKE_OPERATION = "LIKE"
IN_OPERATION = "IN"
NOT_IN_OPERATION = "NOTIN"

LITERAL_OPERATIONS = [IN_OPERATION, NOT_IN_OPERATION, LIKE_OPERATION]
...
class RequiredElementError(Exception):
    """RequiredElementError exception Class"""
...
class QueryBuilder():
    ...
    def build(self):
        ...
        if self._grouping_by:
            query = self.apply_group_query(_query=query)
    ...
    def apply_filter_query(self, query):
        ...
        if not self._filters:
            raise RequiredElementError(
                "The entity filters for queries builder is not defined")
