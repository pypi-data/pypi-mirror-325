...
from nomenclators_archetype.domain.repository.builders import LOGICAL_OPERATIONS, COMPARISON_OPERATIONS, LITERAL_OPERATIONS
...
class QueryBuilderImpl(QueryBuilder):
    ...
    def apply_filter_query(self, query):
        ...
        for key, value in self._filters.items():
            ...
            # for now, we are going to ignore the logical operations
            if column_name is not None and column_name == "" and operator in LOGICAL_OPERATIONS:
                ...
                # self.apply_filter_operations(value)
                continue

            # for now, we are going to ignore the comparing operations
            if column_name is not None and operator in COMPARISON_OPERATIONS:
                continue

            # for now, we are going to ignore the comparing operations
            if column_name is not None and operator in LITERAL_OPERATIONS:
                continue
    ...
    def apply_group_query(self, _query=None):
        ...
        if self._session is not None:
            ...
            query = _query.group_by(*group_columns)
        ...
        if self._pageable is not None and self._pageable.sort:
            for (sort_field_name, sort_orientation) in self._pageable.sort:
                ...
                try:
                    ...
                    if column is None:
                        raise ValueError(
                            f"For persistence class {self._model.__name__ if self._model else 'Undefined class'}, invalid column name: {sort_field_name}")
                    ...
                    raise ValueError(
                        f"For persistence class {self._model.__name__ if self._model else 'Undefined class'}, error accessing attribute '{sort_field_name}'") from e
