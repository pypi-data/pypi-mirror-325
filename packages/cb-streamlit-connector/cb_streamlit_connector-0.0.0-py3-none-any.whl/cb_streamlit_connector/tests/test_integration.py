...
# def test_query_consistency(connection):
    # Insert 100 documents asynchronously one after the another thena also start the query on those 100 docs and see if the results match
    # for i in range(100):
    #     connection.insert_document(f"{i}",{f"key_{i}": f"value_{i}"} )
        
    # QueryOptions(scan_consistency= QueryScanConsistency.AT_PLUS)
