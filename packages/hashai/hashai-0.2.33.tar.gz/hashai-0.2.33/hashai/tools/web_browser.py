...
class WebBrowserTool(BaseTool):
    ...
    description: str = Field(
        "Highly advanced universal web automation tool with advanced element identification, AJAX waiting, modal dismissal, multi-tab and dropdown support, UI labeling (with index logging), and custom JS injection.",
    ...
    # Maximum overall attempts to complete the plan from the point of failure.
    ...
    def execute(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        This version:
         - Generates a plan via the LLM.
         - Automatically inserts a UI-labeling task after the first navigation task.
         - Then processes tasks one by one; if a task fails, it retries that task (up to overall max attempts).
         - When interacting with UI elements via advanced fallback, logs the index (from the UI mapping) of the element.
        """
        driver = None
        ...
        try:
            ...
            # Automatically insert a UI-labeling step after the first navigation task.
            nav_index = None
            ...
            # Process tasks with overall retry logic.
            i = 0
            overall_attempts = 0
            while i < len(plan.tasks) and overall_attempts < self.max_overall_attempts:
                task = plan.tasks[i]
                action = task.get("action", "").lower()
                handler = action_map.get(action)
                if not handler:
                    logger.warning(f"Unsupported action '{action}' at index {i}. Skipping.")
                    results.append({
                        "action": action,
                        "success": False,
                ...
                logger.info(f"Executing task index {i}: {task.get('description', action)}")
                start_time = time.time()
                result = self._execute_with_retries(driver, task, handler)
                elapsed = time.time() - start_time
                result["elapsed"] = elapsed
                result["index"] = i
                logger.info(f"Task index {i} ('{action}') completed in {elapsed:.2f} seconds.")
                results.append(result)
                
                if not result.get('success', False):
                    logger.error(f"Task at index {i} failed: {result.get('message')}")
                    overall_attempts += 1
                    logger.info(f"Retrying task at index {i} (overall attempt {overall_attempts+1}/{self.max_overall_attempts})")
                    # Do not increment i so that the same task is retried.
                else:
                    i += 1
...
"""
    ...
    def _advanced_find_element(self, driver: webdriver.Chrome, keyword: str) -> Optional[webdriver.remote.webelement.WebElement]:
        ...
        If a UI mapping exists from a previous label_ui call, attempts to match the element and logs its index.
        """
        ...
        if best_match and best_ratio > 0.5:
            # If UI mapping is available, try to match the element and log its index.
            if self.ui_mapping:
                for item in self.ui_mapping:
                    # Compare tag and trimmed text (case-insensitive).
                    try:
                        ...
                        if candidate_tag == item.get("tag", "").upper() and candidate_text == item.get("text", "").strip():
                            logger.info(f"Advanced fallback interacting with element at UI mapping index {item.get('index')}.")
                            break
                    except Exception:
                        continue
        ...
        try:
            ...
            if "invalid element state" in error_message.lower():
                ...
                try:
                    ...
                    element.clear()
                    element.send_keys(text)
                    return {"action": "type", "success": True, "message": f"Typed '{text}' into element after removing readonly."}
                except Exception as e2:
                    logger.warning(f"Attempt to remove readonly attribute failed: {e2}")
                    try:
                        driver.execute_script("arguments[0].value = arguments[1];", element, text)
                        return {"action": "type", "success": True, "message": f"Set value via JS: '{text}'."}
                    except Exception as e3:
                        logger.error(f"JavaScript value injection failed: {e3}")
    ...
    def _handle_label_ui(self, driver: webdriver.Chrome, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        on interactive elements. Returns a JSON mapping of the labeled elements.
        This function is called after navigation to provide a visual mapping of the page.
        """
        try:
            script = """
            (function() {
                var elements = document.querySelectorAll("input, button, a, select, textarea");
                var mapping = [];
