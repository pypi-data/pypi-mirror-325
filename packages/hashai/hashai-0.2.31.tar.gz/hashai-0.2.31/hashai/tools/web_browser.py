...
class WebBrowserTool(BaseTool):
    ...
    description: str = Field(
        "Highly advanced universal web automation tool with advanced element identification, AJAX waiting, modal dismissal, multi-tab and dropdown support, UI labeling, and custom JS injection.",
    ...
    def execute(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Automatically insert the UI-labeling step if it's not already present.
            if not any(task.get("action", "").lower() == "label_ui" for task in plan.tasks):
                plan.tasks.insert(0, {
                    "action": "label_ui",
                    "selector": "",
                    "value": "",
                    "description": "Automatically label UI elements for improved accuracy."
                })
    ...
    def _handle_label_ui(self, driver: webdriver.Chrome, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            (function() {
                ...
                var result = JSON.stringify(mapping);
                return result;
            })();
            """
            result = driver.execute_script(script)
