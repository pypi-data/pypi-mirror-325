...
class WebBrowserTool(BaseTool):
    ...
    description: str = Field(
        "Highly advanced universal web automation tool with advanced element identification, AJAX waiting, modal dismissal, multi-tab and dropdown support, and custom JS injection.",
    ...
    def execute(self, input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for task in plan.tasks:
                # Dismiss modals/overlays before each action.
...
{{
    "tasks": [
        {{
            "action": "navigate|click|type|wait|wait_for_ajax|scroll|hover|screenshot|switch_tab|execute_script|drag_and_drop|select",
...
"""
    ...
    def _get_synonyms(self, keyword: str) -> List[str]:
        ...
        The method sends a prompt asking for a comma-separated list of synonyms.
        ...
        response = self.llm.generate(prompt=prompt)
        # Try to extract comma-separated words.
    ...
    def _handle_typing(self, driver: webdriver.Chrome, selector: str, text: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        If send_keys fails due to an invalid element state (for example, if the field is readonly),
        attempt to remove the readonly attribute or set the value via JavaScript.
        """
        try:
            element = WebDriverWait(driver, self.default_timeout).until(
        ...
        # Try normal clear and send_keys first.
        try:
            element.clear()
