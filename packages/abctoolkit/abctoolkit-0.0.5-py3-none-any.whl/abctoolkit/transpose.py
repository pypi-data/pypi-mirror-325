...
from abctoolkit.utils import extract_metadata_and_parts, extract_global_and_local_metadata, find_all_abc
...
def transpose_an_abc_text(abc_text_lines, des_key):
    ...
    if global_metadata_dict['K'] == 'none':
        global_metadata_dict['K'] = 'C'
    for symbol in local_metadata_dict.keys():
        if 'perc' in local_metadata_dict[symbol]['V']:
            ...
            local_metadata_dict[symbol]['K'] = global_metadata_dict['K']
    ...
    for line in metadata_lines:
        if line.startswith('K:'):
            key = line[2:].strip()
            if key != 'none':
                transposed_keymode = lookup_new_keymode_to_transpose(key, global_metadata_dict['K'], des_key)[0]
            else:
    ...
    for symbol, part_text in part_text_dict.items():
        ...
        if part_ori_key == 'none':
            ...
            part_des_key = lookup_new_keymode_to_transpose(part_ori_key, global_metadata_dict['K'], des_key)[1]
...
def transposed_abc_path_list(args):
    abc_path_list, des_folder = args

    for file_idx in trange(len(abc_path_list)):
        abc_path = abc_path_list[file_idx]
        ...
        try:
            ...
            for key in Key2index.keys():
                ...
                transposed_abc_path = os.path.join(des_folder, key, transposed_abc_name + '.abc')
...
def batch_transpose(ori_folder, des_folder):
    ...
    for i in range(num_cpu):
        start_idx = int(math.floor(i * len(abc_path_list) / os.cpu_count()))
        end_idx = int(math.floor((i + 1) * len(abc_path_list) / os.cpu_count()))
        arg_lists.append((abc_path_list[start_idx:end_idx], des_folder))

    pool = Pool(processes=os.cpu_count())
    pool.map(transposed_abc_path_list, arg_lists)
...
if __name__ == '__main__':
    ...
    # with open(r"D:\Research\Projects\MultitrackComposer\dataset\03_abc\meta\00000ec8a66b6bd2ef809b0443eeae41.abc", 'r', encoding='utf-8') as f:
    #     abc_text_lines = f.readlines()
    #
    # print('X:1')
    ...
    # print('')
    #
    # transposed_abc_text = transpose_an_abc_text(abc_text_lines, 'B')
    #
    # print('X:2')
    # print(transposed_abc_text)
    # print('')

    # sample_and_test(r'D:\Research\Projects\MultitrackComposer\dataset\06_abc_text-filtered\musescoreV2')

    # count = 0
    # for abc_path in find_all_abc(r'D:\Research\Projects\MultitrackComposer\dataset\03_abc\musescoreV2'):
    #     count += 1
    #     if count % 10000 == 0:
    #         print(count)
    #     with open(abc_path, 'r', encoding='utf-8') as f:
    ...
    #
    #     for symbol in local_metadata_dict.keys():
    #         if 'K' in local_metadata_dict[symbol].keys() and local_metadata_dict[symbol]['K'] == 'none':
    #             if part_text_dict[symbol].strip().startswith('[K:') and not 'perc' in local_metadata_dict[symbol]['V']:
    #                 print(abc_path)
