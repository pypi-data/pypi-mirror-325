...
class FovusApiValidator:  # pylint: disable=too-few-public-methods
    ...
    def validate(self):
        ...
        self._validate_is_subject_to_available_resources()
    ...
    def _validate_is_subject_to_available_resources(self):
        if (
            Config.is_gov()
            and self.payload[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS].get(IS_SUBJECT_TO_AVAILABLE_RESOURCES)
            is not None
        ):
            print(
                "isSubjectToAvailableResources is no longer supported in the Gov environment. Ignoring this value. Use"
                " isHybridStrategyAllowed instead."
            )
        ...
        if (
            ...
            print("Autofilling 'isSubjectToAvailableResources' with default value of False.")
            self.payload[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS][IS_SUBJECT_TO_AVAILABLE_RESOURCES] = False

    def _validate_is_hybrid_strategy_allowed(self):
        if (
            not Config.is_gov()
            and self.payload[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS].get(IS_HYBRID_STRATEGY_ALLOWED) is not None
        ):
            print(
                "isHybridStrategyAllowed is currently only supported in the Gov environment. Ignoring this value. Use"
                " isSubjectToAvailableResources instead."
            )

        if (
            Config.is_gov()
            and self.payload[PAYLOAD_CONSTRAINTS][PAYLOAD_JOB_CONSTRAINTS].get(IS_HYBRID_STRATEGY_ALLOWED) is None
        ):
            print("Autofilling 'isHybridStrategyAllowed' with default value of False.")
