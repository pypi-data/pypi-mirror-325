...
from octostar.api.ontology import relationship_mappings
...
class NifiOTMRelationshipProxy(object):
    def __init__(self, os_entity_uid_from, os_entity_type_from, os_entity_uid_to, os_entity_type_to, os_relationship_name, drop_on_output=False):
        self.record = {
...
class NifiContextManager(object):
    ...
    def receive_input(self, json_data, processor_name) -> List["NifiEntity"]:
    ...
    def sync_entities(self):
        ...
        for entity in entities:
            if entity.sync_params.get(NifiContextManager.SyncFlag.FETCH_RELATIONSHIPS):
                ...
                import logging
                logging.critical(f"RELATIONSHIPS: {rels_to_fetch}")
                ...
                fetch_concept_relationships[concept_name] = fetch_concept_relationships[concept_name].union(set(rels_to_fetch))
        ...
        if fetch_relationships_entities:
            relationship_mappings_info = relationship_mappings.sync_detailed(client=self.client).parsed.data.to_dict()
            expanded_entities, _ = safe_async_run(expand_entities(
            ...
            for entity in entities:
                ...
                if found_entities:
                    for (rel, target) in found_entities[1]:
                        is_mtm = bool(rel.get('os_entity_uid_from') and rel.get('os_entity_uid_to'))
                        rel_type = 'mtm' if is_mtm else 'otm'
                        child_entity, child_rel = entity.add_child_entity(
                            target['os_workspace'], target['os_concept'], target,
                            os_relationship_name=rel['os_relationship_name'], os_relationship_type=rel_type
                        )
                        child_entity.record = {**child_entity.record, **target}
                        child_entity.record['entity_id'] = child_entity.record['os_entity_uid']
                        child_entity.uid = child_entity.record['os_entity_uid']
                        child_entity.request['is_temporary'] = False
                        child_entity.request['entity_timestamp'] = target.get('os_last_updated_at')
                        if not is_mtm:
                            child_rel.record['os_entity_uid_to'] = child_entity.record['os_entity_uid']
                        else:
                            child_rel.record = {**child_rel.record, **rel}
                            child_rel.record['entity_id'] = child_rel.record['os_entity_uid']
                            child_rel.uid = child_rel.record['os_entity_uid']
                            child_rel.request['is_temporary'] = bool(rel.get('os_entity_uid'))
                            child_rel.request['entity_timestamp'] = rel.get('os_last_updated_at')
        # CLEAN SYNC PARAMS
        for entity in entities:
            entity.sync_params = {}
...
class NifiEntity(object):
    def __init__(self, context, request, record, all_independent_uids, children=[], contents=None):
        ...
        full_entity_children = [NifiEntity(
            self.context,
            c['request'],
            c['record'],
            all_independent_uids,
        ...
        def decorator(func):
            ...
            def nifi_func(task, body, processor_name, *args, **kwargs):
                with NifiContextManager(body) as nifi_context:
                    ...
                    out = nifi_context.send_output(entity_batches, processor_name)
