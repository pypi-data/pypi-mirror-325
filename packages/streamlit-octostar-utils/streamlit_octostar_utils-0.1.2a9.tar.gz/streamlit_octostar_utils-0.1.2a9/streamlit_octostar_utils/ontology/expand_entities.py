...
REL_FETCHED_FIELDS = '`os_relationship_name`,`os_relationship_label`,`os_entity_uid_from`,`os_entity_uid_to`,`os_entity_uid`,`os_entity_type_from`,`os_entity_type_to`'
TARGETS_FETCHED_FIELDS = '`os_entity_uid`,`os_concept`,`entity_label`'
...
def _left_join(left_list, right_list, left_keys, right_keys):
    ...
    left_groups = groupby(left_list, key=lambda item: _make_composite_key(item, left_keys))
    right_groups = groupby(right_list, key=lambda item: _make_composite_key(item, right_keys))
...
async def expand_otm(expanded_entities, entities_by_concept, ontology_rels, concepts_to_otm_rels, metrics, client, timeout, limit):
    ...
    for concept_name, rels in concepts_to_otm_rels.items():
        for rel_name in rels:
            ...
            source_properties_values = [[entity[p] for p in source_properties] for entity in entities_by_concept[concept_name]]
            source_properties_values = [values for values in source_properties_values if all(value is not None for value in values)]
            if source_properties_values:
                target_prop_names = rel['target_properties'].split(",")
                target_prop_names = ",".join(["`" + prop + "`" for prop in target_prop_names])
                source_properties_values = ",".join(["(" + ",".join(["'" + value + "'" for value in values]) + ")" for values in source_properties_values])
                query = f"""SELECT {TARGETS_FETCHED_FIELDS} FROM `etimbr`.`{rel['target_concept']}` WHERE ({target_prop_names}) IN ({source_properties_values}) LIMIT {limit}"""
                otm_queries.append({
    ...
    otm_stream = multiquery_ontology.streaming(
        sql_queries={query_id: query_data['query'] for query_id, query_data in otm_queries.items()},
    ...
    for query_id, data in all_results.items():
        ...
        data = _left_join(
            [e[0] for e in expanded_entities.values() if e[0]['os_concept'] == query_data['concept_name']], data,
            query_data['source_properties'], query_data['target_properties']
        )
        data = {e[0]['os_entity_uid']: (e[0], {r['os_entity_uid']: ({'os_relationship_name': query_data['relationship_name']}, {r['os_entity_uid']: r}) for r in e[1]}) for e in data}
        for entity_id, entities in data.items():
            ...
            for sub_entity_id, sub_entities in entities[1].items():
                if sub_entity_id not in expanded_entities[entity_id][1]:
                    expanded_entities[entity_id][1][sub_entity_id] = [sub_entities[0], {}]
                expanded_entities[entity_id][1][sub_entity_id][1].update(sub_entities[1])
    metrics.exec_times['otm'] = time.time() - start_time
    return expanded_entities

async def expand_mtm_mixed(expanded_entities, entities_by_concept, ontology, concepts_to_mixed_rels, ontology_rels, metrics, client, timeout, limit):
    start_time = time.time()
    mixed_queries = []
    match_patterns = {}
    os_workspace_relationship_fields = [prop['property_name'] for prop in ontology['concepts']['os_workspace_relationship']['allProperties']]
    for concept_name, rels in concepts_to_mixed_rels.items():
        if concept_name not in match_patterns:
            match_patterns[concept_name] = []
        match_patterns[concept_name].extend([entity['os_entity_uid'] for entity in entities_by_concept[concept_name]])
    for concept_name, entity_ids in match_patterns.items():
        ...
        for rel_name in rels:
            ...
            relationship_fields = ",".join([f"`{rel_name}[{target_concept}]_" + prop  + "` AS " + "`rel__" + prop + "`" for prop in os_workspace_relationship_fields])
            target_concept_fields = [prop['property_name'] for prop in ontology['concepts'][rel['target_concept']]['allProperties']]
            target_concept_fields = ",".join([f"`{rel_name}[{target_concept}]." + prop + "` AS " + "`tgt__" + prop + "`" for prop in target_concept_fields])
            mixed_queries.append({
                'query': f"SELECT {relationship_fields}, {target_concept_fields}, `os_entity_uid` FROM `dtimbr`.`{concept_name}` WHERE `os_entity_uid` IN ({entity_ids}) LIMIT {limit}"
            })
    ...
    local_stream = multiquery_ontology.streaming(
        sql_queries={query_id: query_data['query'] for query_id, query_data in mixed_queries.items()},
        client=client, timeout=timeout/2.0
    )
    ...
    for query_id, data in all_results.items():
        ...
        if not data:
            continue
        data = _left_join(
            [e[0] for e in expanded_entities.values()], data,
            'os_entity_uid', 'os_entity_uid'
        )
        data = {e[0]['os_entity_uid']: (e[0], {r['os_entity_uid']: ({}, {r['os_entity_uid']: r}) for r in e[1]}) for e in data}
        for entity_id, entities in data.items(): ### CHECK
            if entity_id not in expanded_entities:
                expanded_entities[entity_id] = [entities[0], {}]
            for sub_entity_id, sub_entities in entities[1].items():
                if sub_entity_id not in expanded_entities[entity_id][1]:
...
async def expand_mtm_local(expanded_entities, entities_by_concept, ontology_rels, concepts_to_local_rels, metrics, client, timeout, limit):
    start_time = time.time()
    local_queries = {
        'from': [],
        'to': []
    ...
    for concept_name, entity_ids in match_patterns.items():
        ...
        inverse_names = [ontology_rels[rel_name]['inverse_name'] for rel_name in rel_names]
        ...
        local_queries['from'].append({
            'query': f"SELECT {REL_FETCHED_FIELDS} FROM `timbr`.`os_workspace_relationship` WHERE `os_relationship_name` IN ({rel_names}) AND `os_entity_uid_from` IN ({entity_ids}) LIMIT {limit}"
        })
        local_queries['to'].append({
            'query': f"SELECT {REL_FETCHED_FIELDS} FROM `timbr`.`os_workspace_relationship` WHERE `os_relationship_name` IN ({inverse_names}) AND `os_entity_uid_to` IN ({entity_ids}) LIMIT {limit}"
        })
    local_queries = {
        **{'from_' + str(i): local_queries['from'][i] for i in range(len(local_queries['from']))},
        **{'to_' + str(i): local_queries['to'][i] for i in range(len(local_queries['to']))},
    }
    metrics.n_queries += len(local_queries)
    local_stream = multiquery_ontology.streaming(
        sql_queries={query_id: query_data['query'] for query_id, query_data in local_queries.items()},
        client=client, timeout=timeout/2.0
    )
    all_results = await _get_stream_result(local_stream)
    middle_entities = []
    for query_id, data in all_results.items():
        ...
        data = _left_join(
            [e[0] for e in expanded_entities.values()], data, 'os_entity_uid', 'os_entity_uid_' + query_id[0]
        )
        inverse_direction = 'to' if query_id[0] == 'from' else 'from'
        data = {e[0]['os_entity_uid']: (e[0], {r['os_entity_uid']: (r, {}) for r in e[1]}) for e in data}
        middle_entities.extend([(inverse_direction, r) for e in data.values() for r in e[1].values()])
        for entity_id, entities in data.items():
            if entity_id not in expanded_entities:
                expanded_entities[entity_id] = [entities[0], {}]
            for sub_entity_id, sub_entities in entities[1].items():
    ...
    for concept_name, entities in middle_entities.items():
        ...
        local_queries_2.append({
            'query': f"SELECT {TARGETS_FETCHED_FIELDS} FROM `timbr`.`{concept_name}` WHERE `os_entity_uid` IN ({entity_ids}) LIMIT {limit}"
        })
    ...
    local_stream_2 = multiquery_ontology.streaming(
        sql_queries={query_id: query_data['query'] for query_id, query_data in local_queries_2.items()},
        client=client, timeout=timeout/2.0
    )
    ...
    for query_id, data in all_results.items():
        ...
        if not data:
            continue
        data = _left_join(
            middle_entities, data, 'entity_id', 'os_entity_uid'
        )
        for entry in data:
...
async def expand_entities(entities, ontology, relationship_mappings_info, client, relationship_names_by_entity_type=None, batch_size = 10000, avg_limit_per_entity = 20, timeout=10.0):
    ...
    entity_batches = [ordered_entities[i:i+batch_size] for i in range(0, len(ordered_entities), batch_size)]
    expanded_entities = {}
    for batch in entity_batches:
        new_entities = await expand_entities_batch(batch, ontology, relationship_mappings_info, relationship_names_by_entity_type, metrics, client, avg_limit_per_entity, timeout)
    ...
    expanded_entities = [(e[0], [(r[0], list(r[1].values())[0]) for r in e[1].values()]) for e in expanded_entities.values()]
    return expanded_entities, metrics

async def expand_entities_batch(entities, ontology, relationship_mappings, relationship_names_by_entity_type, metrics, client, avg_limit_per_entity = 20, timeout=10.0):
    ...
    concepts_to_rels = {cn: c['relationships'] for cn, c in ontology['concepts'].items()}
        ...
        if relationship_names_by_entity_type:
            ...
            for cn_name in relationship_names_by_entity_type.keys():
                if is_child_concept(concept_name, cn_name, ontology):
                    relationship_names = relationship_names.union(set(relationship_names_by_entity_type[cn_name]))
    ...
    expanded_entities = await expand_otm(expanded_entities, entities_by_concept, ontology_rels, concepts_to_otm_rels, metrics, client, timeout, limit)
    # LOCAL MTM QUERIES: fetch relationships directly from the os_workspace_relationship table, then fetch target entities
    expanded_entities = await expand_mtm_local(expanded_entities, entities_by_concept, ontology_rels, concepts_to_local_rels, metrics, client, timeout, limit)
    # MIXED MTM QUERIES: query via timbr to make the most of JOINs
    expanded_entities = await expand_mtm_mixed(expanded_entities, entities_by_concept, ontology, concepts_to_mixed_rels, ontology_rels, metrics, client, timeout, limit)
    return expanded_entities
