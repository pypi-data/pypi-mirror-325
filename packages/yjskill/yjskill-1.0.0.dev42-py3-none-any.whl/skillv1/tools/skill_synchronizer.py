...
class SkillSynchronizer(metaclass=ABCMeta):
    ...
    def __sync_skill(self, target: dict):
        ...
        if not ok:
            ...
            # skill task metric
            self.__create_metric(task_name=self.skill_task_name,
        ...
        create_skill_req = result.get("create_skill_request", default=None)
        create_skill_extra_data = result.get("extra_data", default=None)
    ...
    def run(self):
        ...
        ok, msg, self.targets = self.list_targets(self.tar)
        if not ok:
            self.__mark_job_failed(reason=msg["reason"],
                                   message=msg["error"][:500])
            return
        ...
        for target in self.targets:
            ...
            self.__create_event(reason=reason, message=reason)
