...
class SkillEdgeSynchronizer(skill_synchronizer.SkillSynchronizer):
    ...
    def __release_skill(self,
        ...
        """
        device_hub_name = extra_data["device_hub_name"]
        device_name = extra_data["device_name"]
        workspace_id = self.skill_version_name.workspace_id
    ...
    def preprocess_sync_skill(self, target: dict):
        ...
        # 盒子状态改为下发中
        ok, err = self.__update_device_status(client=device_client,
        ...
        graph = sync_skill.build_skill_graph(
            origin_graph=skill.graph,
            replace={skill.workspace_id: target_workspace})

        create_skill_req = skill_api.CreateSkillRequest(
            workspaceID=target_workspace,
            localName=skill.local_name,
            ...
            tags=skill.tags,
            graph=graph,
            artifact=skill.graph.get('artifact', default=None),
            imageURI=skill.image_uri,
            defaultLevel=skill.default_level,
            alarmConfigs=skill.alarm_configs)

        result = {}
        result["create_skill_request"] = create_skill_req
        extra_data = {}
        extra_data["device_name"] = target.get("local_name", default=None)
        result["extra_data"] = {}
        return True, {}, result
    ...
    def postprocess_sync_skill(self, skill: skill_api.Skill, target: dict):
        ...
        if artifact is not None:
            artifact_version = artifact.get('version', default=None)
        if artifact_version is not None:
            extra_data = {}
            extra_data["device_hub_name"] = target.get(
                "device_hub_name", default=None)
            extra_data["device_name"] = target.get("local_name", default=None)
