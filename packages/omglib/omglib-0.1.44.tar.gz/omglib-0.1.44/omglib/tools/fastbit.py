# These packages are imported from original outdated OMEGAPy-Project omg package lib
from .listtools import listing as LT
import math
...
def Qp(number):
    if type(number) == UnderSquareRoot:
        ...
        return number if len(str(number).split('.')[-1])>5 else float("nan")
    else:
        return float("nan")
