...
from elog_cli.elog_management_backend_client.api.entries_controller import get_full,search
...
class ElogApi():
    ...
    def list_entries(self, limit:Optional[int] = 10, context:Optional[int] = None, logbooks:Optional[list[int]] = None, anchor:Optional[str] = None, end_date:datetime = None) -> list[EntrySummaryDTO]:
        """List all logbooks"""
        client = self.get_authenticated_client()
        all_logbook_result: Response[ApiResultResponseListLogbookDTO] = search.sync_detailed(
            client=client,
            authorization_cache=AuthorizationCache(),
            limit=limit if not None else 10,
            context_size=context if not None else 10,
            anchor=anchor if not None else Unset,
            end_date=end_date if not None else Unset,
        )
        if all_logbook_result.status_code == 200:
            if all_logbook_result.parsed.error_code == 0:
                return all_logbook_result.parsed.payload
