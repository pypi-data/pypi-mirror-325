...
def  get_current_shift_end_date(elog_api: ElogApi)->datetime:
    current_dt = datetime.now()
    logbooks:list[LogbookDTO] = elog_api.list_logbooks()
...
@click.option("--current-shift", type=bool, show_default=True, default=True, is_flag=True, help="Show entries using as maximum date time the current shift end date time.")
@click.pass_context
def list_entries(ctx, interactive: bool, current_shift: bool):
    """List all entries."""
    ...
    def main(stdscr):
        ...
        def fetch_entries_page(count:int, anchor:str = None, next:bool=True):
            ...
            stdscr.refresh()
            try:
                new_page = elog_api.list_entries(limit=count, anchor=anchor, end_date=end_date) if next else elog_api.list_entries(context=count, anchor=anchor, end_date=end_date)
                return new_page
    ...
    try:
        ...
        if current_shift:
            end_date = get_current_shift_end_date(elog_api)
