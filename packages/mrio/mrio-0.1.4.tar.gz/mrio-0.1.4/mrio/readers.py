...
class DatasetReader:
    ...
    def _repr_attributes(self, max_items: int = 6) -> str:
        ...
        for key, value in self.attrs.items():
            if isinstance(value, (list, tuple)):                
                # Convert timestamps to strings if the key is md:time_start
                if key == "md:time_start":
                    converted_values = []
                    for ts in value:
                        if ts % 86400 == 0:  # Check if timestamp has only day precision
                            date_str = datetime.fromtimestamp(ts).strftime("%Y-%m-%d")
                        else:
                            date_str = datetime.fromtimestamp(ts).strftime("%Y-%m-%d %H:%M:%S")
                        converted_values.append(date_str)
                    value_repr = self._repr_value(converted_values, 3)
                else:
                    value_repr = self._repr_value(value, 3)
    ...
    def __repr__(self) -> str:
        ...
        attrs_repr = self._repr_attributes(max_items=4)
        ...
        # Convert bbox to lat/lon if crs is available
        bbox = f"[{', '.join(f'{x:.4f}' for x in self.bounds)}]"
        transform = f"[{', '.join(f'{x:.4f}' for x in self.transform[:6])}]"        

        bbox_geo = None    
        if self.crs:
            try:
                # Split bounds into x/y coordinates
                minx, miny, maxx, maxy = self.bounds
                
                # Create transformer with explicit axis order
                transformer = Transformer.from_crs(self.crs, "EPSG:4326", always_xy=True)
                
                # Transform corners separately
                ...
                bbox_geo = f"[{minlon:.6f}, {minlat:.6f}, {maxlon:.6f}, {maxlat:.6f}]"
            except Exception as e:
                bbox_geo = f"[Error transforming coordinates: {str(e)}]"
