...
class Collection:
    ...
    def FilterBounds(
        self: T, min_x: float, min_y: float, max_x: float | None = None, max_y: float | None = None
    ) -> T:
    ...
    def _get_selection_indices(
        ...
        if self._query.date_range:
            ...
            dates = [
                datetime.fromtimestamp(int(date)) for date in self.dataset.attrs["md:time_start"]
            ]
            date_indices = tuple(
    ...
    def _calculate_bounds(self) -> tuple[tuple[slice, slice], rio.Affine]:
        ...
        if self._is_bbox_geo(query_bounds):
            # Convert geographic bounds to pixel coordinates            
            TRANSFORMER = Transformer.from_crs("epsg:4326", self.dataset.crs, always_xy=True)
            min_x, min_y = TRANSFORMER.transform(*query_bounds[:2])
            max_x, max_y = TRANSFORMER.transform(*query_bounds[2:])
    ...
    @staticmethod
    def _is_bbox_geo(bbox: tuple[float, float, float, float]) -> bool:
        """Check if bbox coordinates are in geographic format (lat/lon)."""
        minx, miny, maxx, maxy = bbox
        return all(abs(x) <= 180 and abs(y) <= 90 
