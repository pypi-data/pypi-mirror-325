    ...
    try:
        with open(outfile, "wb") as ff:
            while True:
                ...
                ff.write(chunk)
                dl_size += len(chunk)
                await _update_progress(advance=len(chunk))
...
async def download_to_file(progress, session, url, filename, file_size, headers, download_dir):
    """Download url contents to file.

    Provides a quick and 'lean' implementation that drops global variables, shares sessions
    and avoids redudent logics for marginally better performance.
    """
    _retry = 0
    while _retry < NGC_CLI_DOWNLOAD_RETRIES:
        _dl_size = 0
        try:
            outfile = os.path.join(download_dir, filename)
            ngc_file_url_resp = await session.get(
                xfer_utils.use_noncanonical_url(f"{url}?" + urlencode({"path": filename})), headers=headers
            )
            ngc_file_url_resp.raise_for_status()
            presign_url_dict = await ngc_file_url_resp.json()
            presign_url = presign_url_dict["urls"][0]

            logger.debug("File direct URL: %s", presign_url)
            mkdir_path(os.path.dirname(outfile))

            start = time.time()
            file_content_resp = await session.get(xfer_utils.use_noncanonical_url(presign_url))
            file_content_resp.raise_for_status()
            with open(outfile, "wb") as ff:
                while True:
                    chunk = await file_content_resp.content.read(NGC_CLI_TRANSFER_CHUNK_SIZE)
                    if not chunk:
                        break
                    ff.write(chunk)
                    _dl_size += len(chunk)
                    await progress.advance(len(chunk), 0)
                if file_content_resp.content.at_eof() and _dl_size != file_size:
                    raise NgcException("mismatch file size")
                await progress.advance(0, 1)
            end = time.time()
            logger.debug("time for %s: %d", url, end - start)
            return
        except Exception as _e:  # pylint: disable=broad-except
            await progress.advance(0 - _dl_size, 0)
            logger.debug("Error downloading '%s': %s, %s", filename, type(_e), _e)
            if _retry == NGC_CLI_DOWNLOAD_RETRIES:  # pylint: disable=no-else-return
                logger.debug(
                    "Retry download_to_file %s exhausted, input %s, %s, %s",
                    NGC_CLI_DOWNLOAD_RETRIES,
                    url,
                    filename,
                    download_dir,
...
async def _direct_download_files(
    ...
    file_size_tuple = [(k, v) for k, v in files_with_size.items()]
    trace_config = aiohttp.TraceConfig()
    trace_config.on_request_start.append(on_request_start)
    ssl_context = ssl.create_default_context(cafile=certifi.where())
    connector = aiohttp.TCPConnector(ssl=ssl_context)

    if NGC_CLI_EXPERIMENTAL_DOWNLOAD:
        print("experimental download is enabled")

        async with aiohttp.ClientSession(trace_configs=[trace_config], connector=connector, trust_env=True) as session:
            progress_monitor_task = asyncio.create_task(progress.monitor_progress())
            semaphore = asyncio.Semaphore(value=NGC_CLI_MAX_CONCURRENCY)
            coroutines = [
                download_to_file(progress, session, url, filename, size, auth_header, download_dir)
                for filename, size in file_size_tuple
            ]

            async def _func(coroutine):
                async with semaphore:
                    try:
            ...
            await asyncio.gather(*[_func(c) for c in coroutines])
            progress_monitor_task.cancel()
            await asyncio.gather(*[progress_monitor_task], return_exceptions=False)
...
def direct_download_files(  # noqa: D103
    ...
    if not NGC_CLI_EXPERIMENTAL_DOWNLOAD:
    ...
    progress = AsyncTransferProgress(callback_func=callback, total_bytes=size, total_count=len(dl_files_with_size))
