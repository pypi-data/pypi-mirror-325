...
class GSHAPA3:
    ...
    def compute_target_global_gene_enrichment_score(self, target_class, absolute=True):
        """
    ...
    def pval_gene_set_enrichment(self,global_gene_enrichment_score, set_of_genes, n_tests=10000, absolute=True):
        ...
        def sampling(x, size, n_tests):
            return self.pseudo_random_generator.choice(x.values,[size,n_tests]).sum(axis=0)
    ...
    def explain_gene_sets(self, 
        ...
        """
        global_gene_enrichment_score = self.compute_target_global_gene_enrichment_score(target_class,absolute)
        genesets_shap = {}
        genesets_shap_pvals = {}
        ...
        self.global_feature_importance = global_gene_enrichment_score.sort_values(ascending=False)
        return res.sort_values("SHAP values",ascending=False)

    def isolation_forest_gene_selection(self,absolute,**isolation_forest_parameters):
        '''
        Apply the Isolation Forest outlier detection algorithm on the average genes' Shap values to detect outstanding genes.
        Params:
            absolute (True): If absolute SHAP values are considered (in this case only genes with mean SHAP values above the average are kept, otherwise low and high SHAP values are considered as outliers)
        ...
        outliers = isofo.fit_predict(self.gene_importance.values)
        if not absolute:
            selected = (outliers == -1)
        else:
            selected = np.logical_and(self.global_feature_importance>self.global_feature_importance.mean(), outliers == -1)
