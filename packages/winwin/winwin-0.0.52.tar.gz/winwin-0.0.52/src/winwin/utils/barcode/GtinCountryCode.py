...
def load_iso3166():
    ...
    iso3166_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'iso3166.csv')
    with open(iso3166_file, newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',', quotechar='|')
        next(reader)  # skip csv header
        for row in reader:
            yield row[0], row[1], row[3], row[4], row[5]


def make_iso3166():
    iso3166 = {}
    for i in load_iso3166():
        for j in range(1, 5):
            iso3166[i[j]] = i[0]
    return iso3166
...
def get_country_code(barcode: str) -> str:
    barcode_refix = int(barcode.strip().zfill(13)[:3])
