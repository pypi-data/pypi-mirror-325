...
使用示例5：
    from extexsion import exe, lnk#ExeIcon,LnkIcon是一样的
    with exe('path/to/your/exe.exe','path/to/your/icon.ico') as file:
        file.icon()#将指定的exe文件图标转为指定的路径的ico文件.

    with lnk('path/to/your/lnk.lnk','path/to/your/icon.ico') as file:
        file.icon()#将指定的lnk文件图标转为指定的路径的ico文件.
...
from .main import (
    ...
    ctrlc, curani, size, screenshot, brightness
)
...
with ExeIcon('D:/exe.exe','D:/icon.ico')as file:
    file.get_icon()
    file.icon()

with LnkIcon('D:/lnk.lnk','D:/icon.ico')as file:
    file.get_icon()
    file.icon()
"""