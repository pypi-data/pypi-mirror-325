...
async def prepare_db(ctx):
    from psycopg_pool import AsyncConnectionPool

    pool: AsyncConnectionPool = ctx.pool
    await pool.open()
...
async def test_utils(ctx: SqlContext):
    async with ctx.use_db():
        ...
        # delete moons
        await ctx.run_query(query=Delete(model=MoonLocation), params={"pk": 1}, exec=True)
        await ctx.run_query(query=Delete(model=Moon), params={"pk": 1}, exec=True)
        await ctx.run_query(query=Delete(model=Galaxy), params={"pk": 1}, exec=True)
