...
from dbox.ctx import use_type
from dbox.sql.ctx import SqlGenContext

count = 0
DEFAULT_CTX = SqlGenContext()
...
def jinja_magic(line: str, cell: str = None):
    ...
    ctx = use_type(SqlGenContext, default=DEFAULT_CTX)
    template = ctx.get_template(template_string=cell.lstrip())
    if ns.parse_only:
