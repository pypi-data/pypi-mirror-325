...
@click.option('--gpu', type=click.Choice(['t4', 'a100','a100xl']), default='t4', 
...
def create(vcpus, template, gpu):
    if template == 'webui-forge' and gpu == 't4':
        ...
        if not click.confirm(click.style("Do you want to continue running this template with a T4 GPU?", fg="cyan")):
            return
    with DelayedProgress(
        SpinnerColumn(spinner_name="dots", style="white"),
        TextColumn("[white]{task.description}"),
...
def robust_ssh_connect(ip, keyfile, max_wait=120, interval=5, username="ubuntu"):
    ...
    raise RuntimeError(f"Failed to establish SSH connection to {ip} within {max_wait} seconds. Last error: {last_error}")
                ...
                try:
                    ...
                    try:
                        ...
                        commands = [
                            ...
                            f'sudo curl -L https://storage.googleapis.com/client-binary/client_linux_x86_64 -o {remote_path}',
                            f'sudo chown ubuntu:ubuntu {remote_path}',  # Ensure proper ownership
                            f'sudo chmod 755 {remote_path}',
                            f'[ -f "{remote_path}" ] || (echo "Failed to download binary" && exit 1)',  # Verify download
                            f'sudo ln -s {remote_path} /etc/thunder/libthunder.so',  # Create new symlink
                            f'echo "/etc/thunder/libthunder.so" | sudo tee /etc/ld.so.preload',
                            f'stat -c "%s %a" {remote_path}'  # Show both size and permissions
                        ]
                    ...
                    if error:
                        ...
                        try:
                            ...
                            with ssh.open_sftp() as sftp:
                                ...
                                with sftp.file(remote_token_path, 'w') as f:
                                    f.write(token)
                            # Force write the symlink
                            _, stdout, stderr = ssh.exec_command(f"sudo ln -sf {remote_config_path} /etc/thunder/config.json")
                            _, stdout, stderr = ssh.exec_command(f"sudo ln -sf {remote_token_path} /etc/thunder/token")
