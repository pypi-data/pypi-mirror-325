...
from asf_snow.utils.download import url_download
...
def s1_img_search(area: shapely.geometry.Polygon, dates: Tuple[str, str], flightdir: str = 'both') -> pd.DataFrame:
    """
    ...
    # get results from asf_search in date range and geometry and flight direction
...
def coarsen(da, x=3, y=3):
    bounds = da.rio.bounds()
    gt = list((da.rio.transform()).to_gdal())
    # gt = [float(i) for i in da.spatial_ref.GeoTransform.split()]
    # coarsen da
    da_des = da.coarsen(x=3, boundary='trim').mean().coarsen(y=3, boundary='trim').mean()

    # get the GeoTrannsform for da_des
    _, yn, xn = da_des.shape
    xres = (bounds[2] - bounds[0]) / xn
    yres = (bounds[3] - bounds[1]) / yn
    gt_des = f'{gt[0]} {xres} {gt[2]} {gt[3]} {gt[4]} {-yres}'

    # update the attr
    da_des.spatial_ref.attrs['GeoTransform'] = gt_des

    return da_des


def coarsen_by_reproject(da, x=3, y=3):
    _, row, col = da.shape
    dst_row = int(row / x)
    dst_col = int(col / y)
    wkt = da.spatial_ref.crs_wkt
    da = da.rio.reproject(dst_crs=wkt, shape=(dst_row, dst_col), resampling=Resampling.average)
    return da
...
def download_hyp3(jobs: sdk.jobs.Batch, area: shapely.geometry.Polygon, outdir: str, clean = True) -> Dict[str, xr.DataArray]:
    ...
    for job in tqdm(jobs, desc = 'Downloading S1 images'):
        ...
        for name, url in urls.items():
            ...
            if url_download(url, join(outdir, f'{name}.tif'), verbose=False):
...
def files2dataarray(filelist: list, area: shapely.geometry.Polygon, outdir: str, clean=True) -> Dict[str, xr.DataArray]:
    """ Read rtc Sentinel-1 images and convert them into dict of [str, xr.DataArray].
    Args:
    filelist
    outdir: directory to save tif files.
    clean: clean up tiffs after creating DataArray [default: True]
    ...
    # check file type of the context in the filelist
    # grab first granule name for reprojecting matching
    if filelist[0].find(".zip") > 0:
        filetype = "zip"
        first_granule = Path(filelist[0]).name.split(".zip")[0]
    else:
        filetype = "tif"
        first_granule = Path(filelist[0]).name.split("_VV.tif")[0]


    # loop through jobs
    for job in tqdm(filelist, desc=f'read the S1 {filetype} files'):
        # capture url from job description
        # u = job.files[0]['url']
        ...
        # granule = job.job_parameters['granules'][0]
        if filetype == "zip":
            granule = Path(job).name.split(".zip")[0]
            # create dictionary to hold cloud url from .zip url
            # this lets us download only VV, VH, inc without getting other data from zip
            urls = {}
            urls[f'{granule}_VV'] = job.replace('.zip', '_VV.tif')
            urls[f'{granule}_VH'] = job.replace('.zip', '_VH.tif')
            urls[f'{granule}_inc'] = job.replace('.zip', '_inc_map.tif')
        else:
            granule = Path(job).name.split("_VV.tif")[0]
            urls = {}
            urls[f'{granule}_VV'] = job.replace('_VV.tif', '_VV.tif')
            urls[f'{granule}_VH'] = job.replace('_VV.tif', '_VH.tif')
            urls[f'{granule}_inc'] = job.replace('_VV.tif', '_inc.tif')
        ...
        for name, url in urls.items():
            ...
            if filetype == "zip":
                file = extractfromzipfile(Path(job), Path(url))
            else:
                file = Path(url)

            try:
                # open image in xarray
                img = rxa.open_rasterio(file, masked=True)
        ...
        # coarsen to correct resolution (90 m), must do in linear space not logarithmic dBs.
        # VV and VH are power scale in the da.
        da = coarsen(da, x=3, y=3)
...
def combine_s1_images(dataArrays: Dict[str, xr.DataArray]) -> xr.Dataset:
    ...
    for granule, da in tqdm(dataArrays.items(), desc = 'Combining Sentinel-1 dataArrays'):
        ...
        # set frame number
        frame_number = granule_metadata.properties['frameNumber']
        ...
        # add frame number as indexable parameter
        da = da.assign_coords(frame_number = ('time', [frame_number]))
    ...
    # since the resolution to 90 has been done in either download_hyp3 or files2dataarray functions
    # it shouldn't be done here, only set the resolution attribute to 90m
