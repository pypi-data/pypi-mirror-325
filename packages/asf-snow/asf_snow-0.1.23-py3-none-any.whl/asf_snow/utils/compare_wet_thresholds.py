...
def calc_cr_with_spicy_wet_mask(nc_ds, mode:str='all'):
    mk1 = np.isnan(nc_ds.snow_depth)

    # mk2 = nc_ds.wet_snow.astype(np.bool)
    mk2 = nc_ds.wet_snow_flag.astype(np.bool)
    ...
    if mode == 'dry':
        mk2 = nc_ds.wet_snow_flag.astype(np.bool)
        #mk2 = nc_ds.perma_wet.astype(np.bool)
        mk = np.logical_or(mk1, mk2)
    elif mode == 'wet':
        mk2 = ~nc_ds.wet_snow_flag.astype(np.bool)
        #mk2 = ~nc_ds.perma_wet.astype(np.bool)
...
def analyze_s1_snotel(s1_file, snotel_file, lon, lat, aoi: None, res: None, stdate: None, eddate: None,
                          mode: 'all'):
    # read the nc_ds
    ...
    sd_mean, vh_mean, vv_mean, cr_mean, wet_snow_percent = calc_cr_with_spicy_wet_mask(nc_ds, mode=mode)
    ...
    # get rid of nan snow_depth rows
    merged = merged[~merged['snow_depth_y'].isnull()]

    return merged


def draw_wet_snow_with_threshold(filelist, snotel_file, lon, lat, aoi: None, res: None, stdate: None, eddate: None,
                          mode: 'all', outfile: None):
    ...
    for i in range(num):
        my_dict[lines[i]] = analyze_s1_snotel(lines[i], snotel_file, lon, lat, aoi=aoi, res=res,
                                           stdate=stdate, eddate=eddate, mode=mode)


    # draw_wet_snow_with_thresholds(dict)
    # set global parameters for the figure
    params = {'legend.fontsize': 20, 'legend.handlelength': 2,
              'axes.titlesize': 22,
              'axes.labelsize': 20,
    ...
    fig, ax = plt.subplots(nrows=num, ncols=1)
    fig.set_figwidth(30)
    fig.set_figheight(30)
    fig.suptitle('Wet Snow Percentage', fontsize=24)

    # get the val with key with n3d0
    ref_key = [key for key,val in my_dict.items() if "n3d0" in key][0]
    ref_val = my_dict[ref_key]

    i = 0
    for key, val in my_dict.items():
        lbl = key.split("_")[-1].split(".")[0]
        lbl = lbl.replace('n','-').replace('d','.')
        r = val['wet_snow_percent'].corr(ref_val['wet_snow_percent'])
        r = float("{:.2f}".format(r))
        # ax.plot(val['date'], val['wet_snow_percent'], label=f'{lbl}, {r}', color=colors[i])
        ax[i].hist(val['wet_snow_percent']/max(val['wet_snow_percent']), bins=10, label=f'{lbl}, {r}')
...
def main():
    ...
    parser.add_argument('--s1list', type=str,  required=True)
    ...
    #investigate_s1_snotel(args.s1list, args.snotelfile, args.lon, args.lat, args.aoigeojsonfile, args.res,
    #                      args.stdate, args.eddate, args.mode, args.outfile)

    draw_wet_snow_with_threshold(args.s1list, args.snotelfile, args.lon, args.lat, args.aoigeojsonfile, args.res,
                          args.stdate, args.eddate, args.mode, args.outfile)
