...
def calc_delta_gamma_remapping_fcf(dataset: xr.Dataset, B: float = 0.5, ll: float = 0.3, ul: float = 0.6,
                                   inplace: bool = False) -> Union[None, xr.Dataset]:
    """
    Calculate change in combined gamma parameter (VV and cross-ratio) between
    current time step and previous.

    delta-gamma(i, t) = (ul -(ul - ll)*FCF) * delta-gamma-CR (i, t) + (ll + (ul - ll)*FCF) * B * gamma-VV(i, t)

    with:
    ll, ul are the range for remapping fcf [0,1] to [ll, ul], this limits the impact of fcf to delta_gamma.
    FCF - being the PROBA-V fraction forest cover (ranging from 0 to 1).
    B - fitting parameter tested over 0 to 1 in steps of 0.1 and optimized at 0.5.
    ...
    dataset['deltaGamma'] = (ul - (ul-ll)*dataset['fcf']) * dataset['deltaCR'] + \
                            (ll + (ul-ll)*dataset['fcf']) * B * dataset['deltaVV']
    if not inplace:
        return dataset
