...
def _setup_convolver_c(convolver_c) -> None:
    """Setup the imported C function. Important to avoid segfaults.
    We keep the convolver as an argument to allow importing this function elsewhere
    """
...
class Grid:
    ...
    def get_l_from_params(self) -> float:
        ...
        See :class:`pyoggrid.LogGrid.Framework.Solver` for more information.

        Reference:
            Campolina & Mailybaev 2020, *Fluid dynamics on logarithmic lattices*
        """
        if self.l_params.get("plastic", False):  # lambda=plastic
            return 1.324717957244746025
        a, b = self.l_params["a"], self.l_params["b"]
        if a is None and b is None:  # lambda=2
            return 2
        # lambda^b-lambda^a=1
        assert int(a) == a and int(b) == b, "a, b should be integers"
    ...
    def __init__(
        ...
        self.maths = Maths(self, n_threads)
...
class Maths:
    ...
    Attributes:
        convolve_batch (Callable): Convolve a list of convolutions at once. More efficient that calling :func:`convolve` several times. Use as ``convolve_batch([(f0,g0), (f1,g1), ...])``
    """
    ...
    def generate_convolution_kernel(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        kernel_signs_k0_ax0 = np.array(
            [(1, 1)] + [(1, -1) for _ in range((kernel_offsets_k0_ax0.shape[0] - 1) // 2)] + [(-1, 1) for _ in range((kernel_offsets_k0_ax0.shape[0] - 1) // 2)]
        )
        if self.grid.D > 1:
    ...
    def _convolve_batch_V_inner(self, fgs: list[tuple[np.ndarray, np.ndarray]], V: int) -> np.ndarray:
    ...
    def _convolve_batch_V(self, fgs: list[tuple[np.ndarray, np.ndarray]]) -> np.ndarray:
        ...
        with self.grid.time_tracker("convolution"):
            ...
            if N_V > 0:
                res_maxV = self._convolve_batch_V_inner(fgs[: N_V * max_V], max_V)
            if diff_V > 0:
                res_diffV = self._convolve_batch_V_inner(fgs[-diff_V:], diff_V)

            res = np.concatenate((res_maxV, res_diffV)).reshape((len(fgs),) + fgs[0][0].shape)
        return res

    def _convolve_batch_list_slower(self, fgs: list[tuple[np.ndarray, np.ndarray]]) -> np.ndarray:
    ...
    def _convolve_batch_list(self, fgs: list[tuple[np.ndarray, np.ndarray]]) -> Iterable[np.ndarray]:
        """from couples ``((f0, g0), (f1, g1), ...)``, computes their convolution
...
class Physics:
    ...
    def energy(self) -> float:
        ...
        match self.grid.D:
            case 3:
                u = self.grid.maths.rot3D_inv(self.grid.field("wx", "wy", "wz")) if "wx" in self.grid.fields else self.grid.field("ux", "uy", "uz")
            case 2:
