...
configs = [
    triton.testing.Benchmark(
        ...
        x_vals=list(range(32, 1024, 32)),  # Different values for n to benchmark
...
def benchmark(n, mode, dtype, provider):
    ...
    if provider == "trifast_compiled":
        fn = lambda: torch.compile(triangle_attention)(q, k, v, bias, mask)
    if provider == "ds4s":
