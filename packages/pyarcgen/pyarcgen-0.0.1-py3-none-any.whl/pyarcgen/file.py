...
def removeEmptyFiles(directory, extensions = [], debug: bool = DEBUG):
    ...
    if debug == True:
        print("[pyarcgen -> file.py] removeEmptyFiles - directory:: ", directory)
    extensions = list(
        filter(
            lambda ext: (
                type(ext) == str and
    ...
    if directory is not None :
        if debug == True:
            print("[pyarcgen -> file.py] removeEmptyFiles - os.listdir(directory):: ", os.listdir(directory))
            print("[pyarcgen -> file.py] removeEmptyFiles - extensions:: ", extensions)
        i = 0
