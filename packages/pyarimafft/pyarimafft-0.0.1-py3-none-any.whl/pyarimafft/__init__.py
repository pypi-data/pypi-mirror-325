...
class model:
    ...
    def add_exog(self,exog):
        ...
        if exog.shape[0] != self.seasonal_feature_train.shape[1]+self.seasonal_feature_future.shape[1]:
            return "Exog length does not match train + future horizon length "
        exog_train = exog[0:self.seasonal_feature_train.shape[1]]
        exog_train = exog_train.T
        exog_future = exog[self.seasonal_feature_train.shape[1]:self.seasonal_feature_train.shape[1]+self.seasonal_feature_future.shape[1]]
