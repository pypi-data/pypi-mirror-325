...
SIGNING_KEY: bytes = settings.EVERYSK_SIGNING_KEY.encode()
SEPARATOR: bytes = b':'
...
def sign(data: str | bytes | bytearray, hash_name: str = 'sha1') -> bytes:
    ...
    digest = hmac.new(SIGNING_KEY, data, hash_name).hexdigest().encode()
...
def unsign(signed_data: bytes, hash_name: str = 'sha1') -> bytes | bytearray:
    ...
    if hmac.compare_digest(digest, hmac.new(SIGNING_KEY, data, hash_name).hexdigest().encode()):
        return data
