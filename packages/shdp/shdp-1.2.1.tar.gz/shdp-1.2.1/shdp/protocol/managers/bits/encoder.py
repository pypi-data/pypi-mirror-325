...
class BitEncoder(Generic[R]):
    ...
    def reverse_bits_in_bytes(self, input: BitVec[R]) -> BitVec[R]:
...
class FrameEncoder(Generic[R]):
    ...
    def encode(self, frame: "EventEncoder[R]") -> Result[BitVec[R], Error]:
        ...
        logging.debug(
            f"[\x1b[38;5;227mSHDP\x1b[0m] Sent: "
            f"In-size {data_size}b / {(data_size + 8 - (data_size % 8)) // 8}B, "
            f"out-size {len(self.encoder.frame)}b / {len(self.encoder.frame) // 8}B"
        )
