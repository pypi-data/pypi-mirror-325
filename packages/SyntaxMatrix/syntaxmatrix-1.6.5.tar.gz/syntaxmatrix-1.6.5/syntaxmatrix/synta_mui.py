...
from syntaxmatrix import db  # This is our SQLite module for admin persistence

# Define two built-in themes.
DEFAULT_THEMES = {
    "light": {
        "background": "#f4f7f9",
        "text_color": "#333",
...
class SyntaxMUI:
    def __init__(self, title="SyntaxMatrix", host="127.0.0.1", port=5000, theme="light"):
        ...
        # Initialize dynamic pages from SQLite.
        db.init_db()
        ...
        # Set theme (use one of the built-in themes or a custom dict)
        ...
        # Setup routes (index, pages, admin).
        self.setup_routes()

    def set_theme(self, theme):
        """Set the UI theme. Accepts a key ('light', 'dark') or a custom dict."""
        if theme in DEFAULT_THEMES:
            self.theme = DEFAULT_THEMES[theme]
        elif isinstance(theme, dict):
            self.theme = theme
    ...
    def columns(self, *components):
        """Return HTML that arranges provided components side by side (equal width columns)."""
    ...
    def setup_routes(self):
        ...
        def home():
            ...
            # Auto-scroll: when page loads, scroll chat container to bottom.
            scroll_js = """
            <script>
            ...
            <head>
              ...
              <style>
                ...
                #widget-container {{
                    ...
                    margin: 0 auto;
                    padding: 15px;
                    background: #cccccc;  /* Dimmer than white */
