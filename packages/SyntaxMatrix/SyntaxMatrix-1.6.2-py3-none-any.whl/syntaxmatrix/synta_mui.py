...
class SyntaxMUI:
    def __init__(self, title="SyntaxMatrix", host="127.0.0.1", port=5000):
        ...
        # Dynamic pages for the admin panel.
        self.pages = {"Home": "Welcome to SyntaxMatrix!"}

        # Widgets: text inputs and buttons.
    ...
    def setup_routes(self):
        ...
        def home():
            if request.method == "POST":
                # Process form submissions (for text inputs & buttons).
            ...
            # Build navigation menu, chat history, and widget form.
            nav_html = self._generate_nav()
            chat_html = self._render_chat_history()
            widget_html = self._render_widgets()

            # Inject a small script that auto-scrolls to the bottom.
            scroll_js = """
            <script>
            ...
            # Assemble the page HTML with improved styles.
            page_html = f"""
            <!DOCTYPE html>
            <html>
            <head>
              ...
              <style>
                ...
                #chat-history {{
                    width: 90%;
                    max-width: 800px;
                    margin: 80px auto 120px auto;  /* leave space for pinned widget */
                    padding: 20px;
                    background: #eef2f7;
                    border-radius: 8px;
        ...
        def view_page(page_name):
            if page_name in self.pages:
                return f"<h1 style='text-align:center;'>{page_name}</h1><div style='max-width:800px;margin:20px auto;padding:20px;background:#fff;border-radius:8px;'>{self.pages[page_name]}</div>"
    ...
    def _render_widgets(self):
        ...
        for key, widget in self.widgets.items():
            if widget["type"] == "text_input":
                ...
                # Each button is placed in its own form so callbacks are triggered independently.
                widget_html += f"""
                <div style="margin-right:10px;">
                    <form method="POST">
                        <button type="submit" name="{key}" value="clicked" style="padding:12px 25px; font-size:1em; border:none; border-radius:4px; background:#007acc; color:#fff; cursor:pointer;">
                            {widget["label"]}
                        </button>
                    </form>
                </div>
                """
        # Wrap the widgets in a flex container for horizontal layout.
        return f"""<form method="POST" style="display:flex; align-items:center;">{widget_html}</form>"""
