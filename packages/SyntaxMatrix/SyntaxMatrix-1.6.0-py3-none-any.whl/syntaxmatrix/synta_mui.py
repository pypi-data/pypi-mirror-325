...
class SyntaxMUI:
    def __init__(self, title="SyntaxMatrix", host="127.0.0.1", port=5000):
        ...
        # Set up routes (index, admin, etc.)
        self.setup_routes()

    def setup_routes(self):
        """Define routes for home, dynamic pages, and admin panel."""
        @self.app.route("/", methods=["GET", "POST"])
        def home():
            # Process widget submissions on POST
            ...
            # Build navigation, chat history, and widget form HTML
            nav_html = self._generate_nav()
            chat_html = self._render_chat_history()
            widget_html = self._render_widgets()
            
            # Use our preferred CSS styling (you can tweak these values as needed)
            page_html = f"""
            <!DOCTYPE html>
            <html>
            <head>
    ...
    def _render_chat_history(self):
        """Generate HTML for chat history from session."""
        chat_html = ""
    ...
    def _render_widgets(self):
        ...
        for key, widget in self.widgets.items():
            if widget["type"] == "text_input":
                ...
                # Note: each button is wrapped in its own form to trigger callbacks separately.
                widget_html += f"""
    ...
    # Public API functions for widget registration
    ...
    def write(self, content):
        """Append arbitrary HTML content to a buffer (for injection)."""
