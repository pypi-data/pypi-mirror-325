...
from jh_scrapyd.interfaces import IEggStorage, IPoller, ISpiderScheduler
from jh_scrapyd.utils import local_items
from jh_scrapyd import is_debug, get_config
...
class Jobs(PrefixHeaderMixin, resource.Resource):
    ...
    def prepare_pending(self):
        limit = get_config(option='tab_pending_limit', default=-1)
        return "\n".join(
            self.prepare_row(
                {
            ...
            for project, queue in self.root.poller.get_queues().items()
            for message in queue.list(limit)
        )
    ...
    def render_GET(self, txrequest):
        ...
        pending_total_count = self.root.poller.get_queues_count()
        content = dedent(
            f"""\
            <!DOCTYPE html>
            <html lang="en">
...
{indent(self.prepare_headers(), "                        ")}
                    ...
                    <tbody>
                        <tr>
                            <th colspan="{len(self.headers)}">Pending({pending_total_count})</th>
                        </tr>
