...
class TestYouTubeIntegration(unittest.TestCase):
    ...
    def test_youtube_url_validation(self):
        ...
        self.assertTrue(
            mantis.utils.is_youtube_url(valid_url),
            "Valid YouTube URL not recognized"
        )
        self.assertFalse(
            mantis.utils.is_youtube_url(invalid_url),
            "Invalid URL incorrectly identified as YouTube URL"
        )

    @unittest.skipIf(
        os.getenv("CI") == "true",
        "Skipping long-running YouTube download test in CI"
    )
