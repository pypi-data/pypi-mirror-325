...
from nbclass.create import make_insert_sql, make_batch_sql, make_update_sql
...
class MysqlDB:
    ...
    def __init__(self, ip, port, db, user_name, user_pass):
        ...
        try:
            ...
            log.debug("连接到mysql数据库 %s : %s  instance ID: %d" % (ip, db, id(self)))
    ...
    @auto_retry
    def find(self, sql, limit=0, to_json=True, conver_col=True) -> list:
        """
        @summary:
        无数据： 返回()
