...
Functions:
    ...
    chisquare_params_flat_or_unflat()
...
def chisquare(params_3D, dico = None):
    ...
    for i_obs in range(nb_obs): # loop on each observation
        ...
        if type_obs == 'total_intensity':    
            ...
            try:
                res_full_map = vip.psfsub.pca_fullfr.pca(cube_diff, pa_array, ncomp=nb_modes, mask_center_px=iwa, imlib='opencv', full_output=False)
            except: 
