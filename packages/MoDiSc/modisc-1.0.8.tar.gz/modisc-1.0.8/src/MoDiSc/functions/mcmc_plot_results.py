...
def make_chains_plot(dico):
    ...
    nb_iter, nb_walkers, nb_dim_mcmc = chains.shape[0], chains.shape[1], chains.shape[2]
    ...
    if nb_dim_mcmc > 1:
        ...
        axarr[nb_dim_mcmc - 1].tick_params(axis='x', labelsize = 8 * mcmc_fig_size_factor) # * mcmc_chains_binning)    # update params size for the x axis
        axarr[nb_dim_mcmc - 1].set_xlabel('Iterations', fontsize = 8 * mcmc_fig_size_factor) # * mcmc_chains_binning)  # update labels size for the x axis
        axarr[nb_dim_mcmc - 1].set_xlim([0, nb_iter])

     # Case: there is only one parameter fitted
    else: 
        axarr.set_ylabel(params_labels_flat[0].replace('!', ' '), fontsize = 8 * mcmc_fig_size_factor)# * mcmc_chains_binning)
        axarr.tick_params(axis='y', labelsize = 8 * mcmc_fig_size_factor)# * mcmc_chains_binning)

        for j in range(nb_walkers): # loop on each walker
            axarr.plot(chains[:, j, 0], linewidth = 1/2 * mcmc_fig_size_factor) # plot its chain # mcmc_chains_binning/2
        ...
        axarr.set_xlabel('Iterations', fontsize=8 * mcmc_fig_size_factor) # * mcmc_chains_binning) # update labels size for the x axis
...
def make_corner_plot(dico):
    ...
    if nb_dim_mcmc > 1 and cornerplot_add_text_annot: # defined only if more than one parameter was fitted
        ...
        fig.gca().annotate(text_annot, xy=(0.55, 0.99), xycoords="figure fraction", xytext=(-20, -10), textcoords="offset points", ha="center", va="top", fontsize = 16 * fig_size_factor)
    
    plt.rcParams.update({
        "axes.titlesize"  : 8 * fig_size_factor,
        "axes.labelsize"  : 8 * fig_size_factor,
        "xtick.labelsize" : 8 * fig_size_factor,
        "ytick.labelsize" : 8 * fig_size_factor,
        })

    plt.savefig(os.path.join(mcmc_resultdir, f'cornerplot_{suffix}_{mcmc_chains_burnin}burnin_{cornerplot_sigma}sigma.pdf'))
    plt.close()
