# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/04_nucleotide_downloader.ipynb.

# %% auto 0
__all__ = ['hello_world', 'download_summary', 'download_genomes_from_summary', 'download_genomes_from_taxon', 'split_fasta',
           'nucleotide_download', 'cli']

# %% ../nbs/04_nucleotide_downloader.ipynb 6
...
# %% ../nbs/04_nucleotide_downloader.ipynb 8
from ncbi_datasets_util import (
    core,
)

# %% ../nbs/04_nucleotide_downloader.ipynb 17
def hello_world(name1: str, name2: str) -> str:
    return f"Hello {name1} and {name2}!"

# %% ../nbs/04_nucleotide_downloader.ipynb 19
### Functions
...
def split_fasta(fasta_file: str, output_folder: str) -> None:
    fasta = core.Fasta.from_file(fasta_file)

    return none


def nucleotide_download(taxon: str, output_file: str, format="fasta") -> None:
    cmd = f"esearch -db nucleotide -query '{taxon}[organism]' | efetch -format {format} > {output_file}"
    print(f"Downloading from ncbi with Entrez Direct. Command:")
    print(cmd)
    os.system(cmd)
    return None

# %% ../nbs/04_nucleotide_downloader.ipynb 21
...
def cli(
    ...
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    output_fasta = os.path.join(output_folder, "nucleotide_sequences.fasta")
    nucleotide_download(taxon, output_fasta)
