...
def GridReal_is_order_valid(*args):
    return _flumy.GridReal_is_order_valid(*args)

def GridReal_get_format(filename):
    return _flumy.GridReal_get_format(filename)

def GridReal_get_format_label(format):
    return _flumy.GridReal_get_format_label(format)
...
# Get the domain size
  ...
  if (zb == None):
    zb = dom.getZrefGeo() # Initial grid elevation
  if (zt == None):
...
def showVPC(simu, dz, zmin=None, zmax=None, color=0, title="VPC", filepng=None):
  ...
  if (simu.getAge() <= 0):
    ...
    return
  if (zmin is None):
    zmin = simu.getDomain().getZrefGeo() # Initial grid elevation
  if (zmax is None):
...
# Get 3D regular block
  ...
  fac_arr,gr_arr,age_arr = simu.getBlock(dz, zb=zmin, nz=nz)
...
def sliceFromGrid(grid, section = 0, index = 0,
  ...
  block: 3D numpy array of facies, grain size or age values
  section: 0: X-section, 1: Y-section, 2: Z-slice
  index: Index of the section (0-based)
  colorscale: See plotly.graph_objects.Surface
  cmin: See plotly.graph_objects.Surface
  cmax: plotly.graph_objects.Surface
  """
  ...
  slice = go.Surface(x=x, y=y, z=z, surfacecolor=val,
                     coloraxis='coloraxis', colorscale=colorscale,
                     cmin = cmin, cmax = cmax)
  return slice
