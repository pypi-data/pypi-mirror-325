...
class Table(LanceTable):
    ...
    def add_columns(
        ...
        **kwargs,
        ...
        udf_id = uuid.uuid4().hex

        # TODO(rmeng): this should come from the client or config
        _image_prefix = "us-docker.pkg.dev/lancedb-devland/dataflow-devland/lance-beam"

        packager = DockerWorkspacePackager(additional_pip_dependencies=["prefect"])
        image_tag = f"{_image_prefix}:{udf_id}"

        _LOG.info("Building UDF image: %s", image_tag)
        packager.build(image_tag, platform="linux/amd64")
        ...
        # TODO: Add a local fireup test to make sure:
        # 1. the docker and the pickle and load
        # 2. when a batch is passed, the UDF can be executed correctly
            ...
            "new_columns": [
                {
                    ...
                    "virtual_column": {
                        "image": image_tag,
                        "udf": base64.b64encode(udf_pickle).decode("utf-8"),
                        "data_type": {
                            "type": str(udf.data_type),
                        },
        ...
        result = client.post(
            f"v1/table/{self.name}/add_columns/",
            data=req_data,
            content_type="application/json",
        )

        _LOG.debug("Add columns result: %s", result)

    def alter_columns(self, *alterations: Iterable[dict[str, Any]]) -> None:
        self._ltbl.alter_columns(*alterations)
