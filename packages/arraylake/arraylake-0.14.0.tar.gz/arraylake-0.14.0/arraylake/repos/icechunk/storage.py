...
from arraylake.types import BucketResponse, S3Credentials
...
def _get_credential_type(credentials: S3Credentials | None, credential_refresh_func: Callable | None) -> CredentialType:
...
def _get_icechunk_storage_obj(
    ...
    if bucket_config.platform in ("s3", "s3c", "s3-compatible", "minio"):
        if s3_credentials and credential_refresh_func:
        ...
        return icechunk.s3_storage(
            ...
            expires_after=s3_credentials.expiration.replace(tzinfo=timezone.utc) if s3_credentials and s3_credentials.expiration else None,
