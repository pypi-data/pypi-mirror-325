...
def main():
    ...
    msg = "T %18s %s %s" % (
        locale.format_string("%d", get_total_size('.'), grouping=True),
        '',
        '.'
    )
    print(msg)
    ...
    for root, dirs, files in os.walk(path):
        ...
        for name in files:
            try:
                ...
                if verbose == -1:
                    print("Can't read '%s' file" % join(root, name))
                pass
...
def rec_tree_traverse(curr_dir, indent, verbose):
    ...
    for file_or_dir in dfList:
        if os.path.isfile(file_or_dir):
            ...
            if verbose == -1:
                msg = "  %18s %s %s" % (
                    locale.format_string("%d", getsize(file_or_dir), grouping=True),
                    ' ' * indent,
                    file_or_dir
                )
                print(msg)
    ...
    if verbose == -1:
        msg = "  %18s %s %s" % (
            locale.format_string("%d", files_subtotal, grouping=True),
            ' ' * indent,
            curr_dir
        )
        print(msg)
