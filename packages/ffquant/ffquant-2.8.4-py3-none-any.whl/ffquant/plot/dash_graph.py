...
def get_order_pnl_stats(order_info_list: list, position_list: list = [], timeframe="Minutes", compression=1, debug=False):
    closed_trades = []
    ...
    for order_item in order_info_list:
        ...
        if is_close_order:
            ...
            if pos_price is not None and pos_price != 0:
                ...
                stdout_log(f"[PNL] order_dt_str: {order_dt_str}, pnl_return: {pnl_return}")
            else:
...
def init_table_callback(app, debug=False):
    ...
    def update_metrics_table(n, data):
        ...
        if length > 0:
            ...
            completed_order_num = len([item for item in orders if item['order_status'] == "Completed"])
            limit_order_num = len([item for item in orders if item['exec_type'] == "Limit"])
            completed_limit_order_num = len([item for item in orders if item['exec_type'] == "Limit" and item['order_status'] == "Completed"])

            long_positions = [item['position'] for item in backtest_data["positions"] if item['position'] > 0]
            max_long_position = max(long_positions) if len(long_positions) > 0 else 0
            short_positions = [item['position'] for item in backtest_data["positions"] if item['position'] < 0]
            ...
            reward_risk_ratio, win_rate, avg_return = get_order_pnl_stats(
                order_info_list=orders,
                position_list=backtest_data["positions"],
                timeframe=backtest_data["timeframe"],
                compression=backtest_data["compression"],
                debug=debug)
            if debug:
...
def init_graph_callback(app, debug=False):
    ...
    def update_graph(n, data):
        ...
        for item in backtest_data["klines"]:
        ...
        # Handle buy points
        annotations = []
        for item in backtest_data["orders"]:
            if item['order_status'] == "Completed" and item['order_type'] == "Buy":
                ...
                for annotation in annotations:
                    if annotation['x'] == item['datetime'] and (annotation['arrowcolor'] == "purple" or annotation['arrowcolor'] == "orange" or annotation['arrowcolor'] == "red" or annotation['arrowcolor'] == "green"):
                        current_count += 1
                close_price = None
                for kline in backtest_data["klines"]:
                    if kline['datetime'] == item['datetime']:
                        close_price = kline['close']
                        break

                last_pos = None
                for i in range(0, len(backtest_data["positions"])):
                    if backtest_data["positions"][i]['datetime'] == item['datetime']:
                        if i > 0:
                            last_pos = backtest_data["positions"][i - 1]
                        else:
                            last_pos = backtest_data["positions"][i]
                        break
                ...
                if item['is_close_pos']:
                    ...
                    hovertext = f"{hovertext}, 平空, 订单ID: {item['order_id']}, 价格: {item['execute_price']}, 数量: {item['execute_size']}, 盈利: {round(realized_pnl, 2)}, 原因: {item['message']}"
                else:
                    hovertext = f"{hovertext}, 开多, 订单ID: {item['order_id']}, 价格: {item['execute_price']}, 数量: {item['execute_size']}, 原因: {item['message']}"
        ...
        for item in backtest_data["orders"]:
            if item['order_status'] == "Completed" and item['order_type'] == "Sell":
                ...
                if item['is_close_pos']:
                    ...
                    hovertext = f"{hovertext}, 平多, 订单ID: {item['order_id']}, 价格: {item['execute_price']}, 数量: {item['execute_size']}, 盈利: {round(realized_pnl, 2)}, 原因: {item['message']}"
                else:
                    hovertext = f"{hovertext}, 开空, 订单ID: {item['order_id']}, 价格: {item['execute_price']}, 数量: {item['execute_size']}, 原因: {item['message']}"
        ...
        for item in backtest_data["positions"]:
