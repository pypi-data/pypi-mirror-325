...
def wash_orders(orders, pickle_dict):
    ...
    for item in orders:
        ...
        washed_orders.append(tmp_order_dict)
    
    pickle_dict['orders'] = washed_orders
    return pickle_dict

# 实时和回测都把数据序列化为pickle
def prepare_data_for_pickle(strategy_name, timeframe="Minutes", compression=1, riskfree_rate=0.01, use_local_dash_url=False, version="1.0.1", debug=False):
    pickle_dict = {}
    ...
    pickle_dict["buysells"] = global_backtest_data.buysells
...
def serialize_backtest_data(strategy_name, timeframe="Minutes", compression=1, riskfree_rate=0.01, use_local_dash_url=False, version="1.0.1", backtest_data_dir=None, debug=False):
    ...
    start_dt_str = global_backtest_data.klines[0]['datetime'].replace(" ", "_").replace(":", "-")
    end_dt_str = global_backtest_data.klines[-1]['datetime'].replace(" ", "_").replace(":", "-")
    pkl_file_name = f"{strategy_name}_{start_dt_str}_to_{end_dt_str}.pkl"
