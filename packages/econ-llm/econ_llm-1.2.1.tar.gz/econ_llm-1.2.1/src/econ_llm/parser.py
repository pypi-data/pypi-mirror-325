...
def visualize(data: dict):
    ...
    for log in data:
        ...
        if agent_data["agent_role"] == "responder":
            # Parse the rounds
            rounds = {elem["Round"]: elem["Offer for You"] for elem in agent_data["previous_rounds"]}
            # Make 'Accept' green and 'Reject' red
            colors = ["g" if elem["Your Decision"] == "Accept" else "r" for elem in agent_data["previous_rounds"]]
            # Plot the rounds
