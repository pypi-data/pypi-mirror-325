...
def get_fund_dv(db, rep, page_size, begin_code='000000', code=None, name_like=''):
    ...
    def get_fund_dv_one(code, rep, page_size=9999, times=1):
        try:
            ...
            log.info('get_fund_dv_one', code, len(rss), len(vals_all), times)
    ...
    if thread_num == 1:
        ...
        for fund in funds:
            if len(vals_all) >= 300000:
                ...
                code = fund[0]
                tp.run_wait(get_fund_dv_one, (code, rep, page_size))
        tp.wait()
        num = db.run(sql, vals_all)
        log.info('get_fund_dv', num)
        vals_all.clear()
...
def get_zf(db):
    ...
    def get_zf_one(db, code):
        ...
        log.info('get_zf', code, len(rss), len(zfs))
    ...
    for fund in funds:
        if len(zfs) >= 1000000:
            ...
            tp.run_wait(get_zf_one, (db, fund[0]))
    tp.wait()
    num = db.run(sql, zfs)
    log.info('get_zf', num)
    zfs.clear()
...
def get_val(db):
    ...
    def get_val_one(db, code):
        ...
        log.info('get_val', code, len(rss), len(vals))
    ...
    for fund in funds:
        if len(vals) >= 1000000:
            ...
            tp.run_wait(get_val_one, (db, fund[0]))
    tp.wait()
    num = db.run(sql, vals)
    log.info('get_val', num)
    vals.clear()
...
def get_hc(db):
    ...
    def get_hc_one(db, code):
        ...
        codes.append(code)
        log.info('get_hc', code, len(rows), len(rs_list))
    ...
    for fund in funds:
        if len(rs_list) >= 1000000:
            ...
            tp.run_wait(get_hc_one, (db, fund[0]))
    tp.wait()
    num = db.run(hc_sql, rs_list)
    log.info('get_hc', num)
    rs_list.clear()
...
def get_nh(db):
    ...
    def get_nh_one(db, code):
        ...
        codes.append(code)
        log.info('get_nh', code, len(rows), len(rs_list))
    ...
    for fund in funds:
        if len(rs_list) >= 1000000:
            ...
            tp.run_wait(get_nh_one, (db, fund[0]))
    tp.wait()
    num = db.run(nh_sql, rs_list)
    log.info('get_nh', num)
    rs_list.clear()
...
def get_xp(db):
    ...
    for fund in funds:
        if len(rs_list) >= 100000:
            ...
            tp.run_wait(get_xp_one, (db, fund[0]))
    tp.wait()
    num = db.run(xp_sql, rs_list)
    log.info('get_xp', num)
    rs_list.clear()
