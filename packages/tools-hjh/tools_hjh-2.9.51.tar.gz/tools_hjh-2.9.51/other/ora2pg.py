...
drop_fk 删除PG端已存在的外键
pg2ora_copy_data 用于从PG同步数据到Oracle，需要表提前建好，is_clear参数会生效，不支持并行，tables中不支持写*，但可以写多个表和where条件，没有检查点，不会对数
...
def main():
    ...
    elif run_mode == 'pg2ora_copy_data':
        pg2ora_copy_data()
...
def get_table_map_list(src_db):
    ...
    for table_mess in tables.split('[--split--]'):
        ...
        if src_table == '*' and src_db_type == 'oracle':
...
def get_data_from_oracle(table_map, mess, table_scn, col_desc, table_metadata=None, page=None, page_rn=None):
    ...
    try:
        tp = ThreadPool(1)
        tid = tp.run(ready)
        src_db, dst_db, once_num, rs, is_exists_clob, is_exists_blob, merge_col, cols_for_copy = tp.get_result(tid, timeout=10)
    except Exception as _:
        ...
        log.warning('到Oracle获取数据失败', mess, '获取Oracle连接超时，自动重试')
        return task
    ...
    try: 
        ...
        while True:
            ...
            if len(rows_new) == 0:
                ...
                tp.run(save_to_pg, (dst_db, table_map, rows_new, rows_size, cols_for_copy, my_mess, select_time, table_scn, is_exists_blob))
...
def pg2ora_copy_data():
    # 获取连接
    src_db = DBConn(src_db_type, src_ip, src_port, src_database, src_read_username, src_read_password, poolsize=max_page_num + 1)
    dst_db = DBConn(dst_db_type, dst_ip, dst_port, dst_database, dst_username, dst_password, poolsize=save_parallel + 1)
    table_map_list = get_table_map_list(src_db)
    
    for table_map in table_map_list:
        src_schema = table_map[0].lower()
        src_table = table_map[1].lower()
        dst_schema = table_map[2].upper()
        dst_table = table_map[3].upper()
        src_where = table_map[4]
        
        if is_clear == 'true':
            truncate_table_sql = 'truncate table ' + dst_schema + '."' + dst_table + '"'
            dst_db.run(truncate_table_sql)
            log.info('Oracle执行SQL成功', truncate_table_sql)
        
        select_sql = 'select * from ' + src_schema + '.' + src_table + ' where ' + src_where
        
        if src_where == '1=1':
            src_where = ''
                    
        qrs = dst_db.run('select * from ' + src_schema + '.' + src_table + ' where 1=2')
        col_desc = qrs.get_cols_description()
        wenhaos = ''
        for _ in col_desc:
            wenhaos = wenhaos + '?,'
        wenhaos = wenhaos[0:-1]
        insert_sql = 'insert into ' + dst_schema + '."' + dst_table + '" values(' + wenhaos + ')'
        qrs.close()
        ...
        while True:
            ...
            if len(rows) == 0:
                ...
                try:
                    ...
                    write_speed = '∞'
                log.info('写入Oracle成功', mess, src_where, num, '大小=' + str(round(rows_size, 2)) + 'MB', '读速=' + read_speed + 'MB/s', '写速=' + write_speed + 'MB/s')
