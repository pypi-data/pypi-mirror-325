...
from .resource_abc import CamelAlias, Ref, Resource
...
class ImageResource(BaseModel, Resource):
    ...
    def _pull_commands(self, tag):
        return [["docker", "pull", "--platform", "linux/amd64", self.source_image]]
...
class JobResource(BaseModel, Resource):
    ...
    argument_definitions: Dict[str, CommandlineArg | EnvironmentArg] = {}
    ...
    def update(self, client: Client, old_state):
        ...
        desired = self.model_dump(mode="json", exclude_none=True, exclude={"job_id"}, by_alias=True)
        effective = remote | desired
        remote_args = remote["argumentDefinitions"]
        effective_args = {
            argKey: remote_args.get(argKey, {}) | argValue
...
class ScheduleResource(BaseModel, Resource):
    ...
    use_as_auth: identity.UserResource | identity.UserRef | None = None

    @field_serializer("use_as_auth", when_used="always")
    ...
    def update(self, client, old_state):
        ...
        remote = self.read(client, old_state)
        desired = self.model_dump(mode="json", exclude_none=True, by_alias=True, exclude={"schedule_id"})
        effective = remote | desired
