...
def delete_directory(
    ...
    if directory_exists(
        ...
        for path in paths:
            remove_object(minio_client, bucket_name, path)
...
def download_object_file(
    ...
    minio_client.fget_object(bucket_name, object_name, file_path=file_path)
    return True
...
def download_object_bytes(
    ...
    object_name = normalize_minio_object_name(object_name)
    response: BaseHTTPResponse = download_object(minio_client, bucket_name, object_name)
...
def upload_object_file(
    ...
    if overwrite or not already_exists:
        ...
        minio_client.fput_object(bucket_name, object_name, os.path.join(file_path))
        return True
...
def remove_object(minio_client: Minio, bucket_name: str, object_name: str) -> bool:
...
def copy_file_from_bucket_to_bucket(
    ...
    response: BaseHTTPResponse = download_object(
        origin_minio_client, origin_minio_bucket, origin_path
    )
...
def download_json(
    minio_client: Minio, bucket_name: str, object_name: str, encoding="utf-8"
) -> str:
