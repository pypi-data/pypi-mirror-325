...
class PyLinkForwardModel:
    ...
    def read_observation(self, case="calib"):
        ...
        if case.lower() == "valid":
            ...
            if self.mc_ref_file is None and isinstance(self.mc_reference, pd.DataFrame):
                return self.mc_reference
            if isinstance(self.mc_reference, dict) and bool(self.mc_reference):
    ...
    def delete_hdf5(self):
        ...
        """
        filename = f"ExpDesign_{self.name}.hdf5"
    ...
    def _store_simulations(self, c_points, all_outputs, nan_idx, key_str, prev_run_no):
        ...
        with h5py.File(hdf5file, "a") as file:
            ...
            new_c_points = np.delete(c_points, nan_idx, axis=0)
            grp_x = file.create_group("EDX") if not hdf5_exist else file.get("EDX")
            if prev_run_no == 0 and key_str == "":
