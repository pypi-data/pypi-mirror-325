...
from ydata.profiling.correlations import pandas_auto_compute  # noqa: F401
...
from ydata._licensing import profiling_report
...
class ProfileReport(_ProfileReport):
    ...
    def __init__(
        ...
        config_file: Path | str = __default_config,
        lazy: bool = True,
    ...
    def compare(
        self, other: "ProfileReport", config: Settings | None = None, config_file: str | None = None
    ) -> "ProfileReport":
        from ydata.profiling.compare_reports import compare
        config = config if config is not None else Settings.from_file(
            config_file if config_file is not None else self.__default_comparison_config
        )
        report = compare([self, other], config)
        return report
