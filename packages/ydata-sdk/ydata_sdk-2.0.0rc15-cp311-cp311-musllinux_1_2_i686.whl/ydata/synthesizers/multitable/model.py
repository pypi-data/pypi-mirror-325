...
class MultiTableSynthesizer(BaseModel):
    ...
    def sample(
        ...
        """Sample from a trained synthesizer.

        Args:
            n_samples (float | None): percentage of the original database to sample
            ...
            if_exists ({'fail', 'replace', 'append'}): defines the write behavior when the table already exists. Defaults to 'append'
                ...
                the persistence can fail leadind to inconsistences in the database
            random_state: random generator or seed for the synthesizer fit

        Note:
            When this method receives a connector, it persists all generated tables and return an empty dataset.

        Returns:
            synthetic MultiDataset
        """
        _n_samples = n_samples if n_samples is not None else 1.0
        composite_keys = self._get_user_defined_composite_keys()
        ...
        for i, (table, previous_table) in enumerate(self._tables_order):
            ...
            if self.is_attribute_table(table):
                ...
                sample_data = model.sample(model.nrows)
