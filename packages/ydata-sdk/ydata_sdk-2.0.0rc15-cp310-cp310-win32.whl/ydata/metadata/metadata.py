...
class Metadata:
    ...
    def __validate_dataset_type(self, dataset_type, dataset_attrs):
        if dataset_type == DatasetType.TIMESERIES and self.__partition is False:
            assert dataset_attrs, "A Timeseries dataset requires dataset_attrs to include at least the sortbykey attribute."

            if dataset_attrs.sortbykey is None or len(dataset_attrs.sortbykey) == 0:
                raise InvalidMetadataInputError("Timeseries dataset requires sortbykey attribute.")
    ...
    def __set_dataset_type(self, dataset_type,
        ...
        try:
            ...
            self.__validate_dataset_type(dataset_type, dataset_attrs)
    ...
    def get_numerical_columns(self):
        ...
        return [
            col
            for col, var_type in self._cols_dtypes.items()
            if var_type in _NUMERICAL_VARTYPES
        ]
    ...
    def dataset_attrs(self, attrs: dict | DatasetAttr | None = None):
        ...
        #If dataset_attrs is None, this is expected to clean the Metadata dataset_atts
        if attrs:
