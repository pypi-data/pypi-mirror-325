...
def build_world_and_block_def_for_single_layout(layout, layoutDb=None, pathFinderFactory=None):
    '''
...
def build_world_and_block_defs_for_layout_combination(
        start_layout, end_layout, direction, layoutDb=None,
        path_finder_factory=None):
    if layoutDb is None:
        layoutDb = LayoutDatabase()

    world, startBlockDef = build_world_and_block_def_for_single_layout(
        start_layout, layoutDb, path_finder_factory)

    if start_layout.kind in ('fwd', 'bck'):
        i, j = 2, 1
    elif start_layout.kind == 'top':
        i, j = 1, 1
    else:
        i, j = 1, 2
    ...
    end_layout.applyTo(endBlockDef)
    world.map.layout.setupComplete(pathFinderFactory=path_finder_factory)
...
class PathFindingStoreBuilder(object):
    ...
    def calculateDiscretisationLocations(self, regionName):
        ...
        world, block = build_world_and_block_def_for_single_layout(layout)
        ...
        for i in range(lowerBound, n + 1):
            ...
            for j in range(-m, m + 1):
                ...
                if not any(world.map.get_candidate_polygons(
    ...
    def calculateSingleRegionTransitions(self, regionName):
        ...
        layout = self.regions[regionName]
        world, block = build_world_and_block_def_for_single_layout(
    ...
    def setupComboWorld(self, combo):
        ...
        return build_world_and_block_defs_for_layout_combination(
            layout1, layout2, combo.direction,
            path_finder_factory=self.buildPathFinder)
