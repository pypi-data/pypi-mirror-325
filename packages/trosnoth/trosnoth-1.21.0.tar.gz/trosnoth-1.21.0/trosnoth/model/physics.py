...
class PhysicsObject(Protocol):
    '''
    Useful for type-checking: these things can be treated as a unit by
    WorldPhysics, which calculates collisions etc.
    '''
...
class CollisionEllipse(CollisionShape):
    ...
    def checkLedgeCollision(self, start, vector, polygon):
        ...
        '''
        vectorDist = (vector[0] ** 2 + vector[1] ** 2) ** 0.5
        for x1, y1, x2, y2 in polygon.get_edges():
...
class WorldPhysics(PhysicsConstants):
    ...
    def getCollision(
            self, unit: PhysicsObject, vector, *, ignoreLedges=False,
            collide_with_zone_boundaries=False):
        '''
        ...
        collisions = []
        for polygon in self.get_candidate_polygons(unit, vector):
            if ignoreLedges and polygon.is_ledge():
                continue
            if polygon.zone_boundary_polygon:
                if not collide_with_zone_boundaries:
                    continue

                unit_room = self.world.rooms.get_at(unit.pos)
                if unit_room and polygon.zone_def == unit_room.defn:
                    continue
    ...
    def get_candidate_polygons(self, unit: PhysicsObject, vector):
        x0, y0, x1, y1 = unit.collision_shape.getMotionBounds(unit.pos, vector)
        return self.world.map.get_candidate_polygons(x0, y0, x1, y1)

    def checkCollision(self, unit: PhysicsObject, vector, polygon):
        '''
        :return: a Collision instance, or None
        '''
        shape = unit.collision_shape
        start = unit.pos
        if polygon.is_ledge():
    ...
    def getMotion(
            self, unit: PhysicsObject, vector, *,
            ignoreLedges=False,
            collide_with_zone_boundaries=False):
        '''
        ...
        collision = self.getCollision(
            unit, vector,
            ignoreLedges=ignoreLedges,
            collide_with_zone_boundaries=collide_with_zone_boundaries,
        )
        if collision:
