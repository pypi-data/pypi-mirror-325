...
async def session():
    ...
    dependency_provider.dependency_overrides[get_client_session] = get_mock
...
async def broker():
    ...
    br.subscriber("to_send", internal_exchange())(sending_message)
    br.subscriber("fetch_object", internal_exchange())(fetch_object)
...
async def test_fetch_invalid(broker, data):
...
async def test_fetch(broker, session, test_actor):
    expected = {"key": "value"}
    ...
    import cattle_grid.dependencies.globals as g

    g.lookup = AsyncMock(return_value=MagicMock(result=None, uri="http://object"))

    response = await broker.publish(
        {"actor": test_actor.actor_id, "uri": "http://object"},
        routing_key="fetch_object",
        exchange=internal_exchange(),
        rpc=True,
    )
...
async def test_to_send_no_inbox(broker, session, test_actor):
    Result = namedtuple("http_result", ["status", "raise_for_status", "text"])
...
async def test_to_send(broker, session, test_actor):
    Result = namedtuple("http_result", ["status", "raise_for_status", "text"])
...
async def test_resolve_inbox_no_inbox():
    actor = AsyncMock()
    actor.get = AsyncMock(return_value={})

    result = await resolve_inbox(actor, "target")
