...
async def test_account():
    account = await create_account("name", "pass")
...
def test_signin(test_client, test_account):
    ...
    response = test_client.get(
        "/account/actors", headers={"Authorization": f"Bearer {data['token']}"}
    )

    assert response.status_code == 200


@pytest.mark.parametrize("endpoint", ["/account/actors", "/settings"])
