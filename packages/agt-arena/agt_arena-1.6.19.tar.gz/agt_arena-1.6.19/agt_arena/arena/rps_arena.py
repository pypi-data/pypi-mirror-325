...
class RPSDockerArena():
    def __init__(self, num_rounds=10, submission_ids=[], setupTimeout: int = 5, totalMoveTimeout: float = 0.5, grace_time_per_move: float = 0.05, port = 5000):
    ...
    def build_and_run_container(bot_id, port, delay = 5, skip_image = False):
    ...
    def get_bot_move(self, bot_url):
        try:
            response = requests.post(f"{bot_url}/get_action", timeout=self.totalMoveTimeout)
            if response.status_code == 200:
    ...
    def run_match(self, p1_bot_id: int, p2_bot_id: int) -> dict:
        ...
        _, p1_bot_url = RPSDockerArena.build_and_run_container(p1_bot_id, self.port, skip_image=True)
        _, p2_bot_url = RPSDockerArena.build_and_run_container(p2_bot_id, self.port, skip_image=True)
    ...
    def run_tournament(self):
        ...
        bot_stats = {
            bot_id: existing_stats.get(
                bot_id, 
                {'name': self.bot_names[bot_id], 'total_utility': 0.0, 'rounds': 0}
            )
