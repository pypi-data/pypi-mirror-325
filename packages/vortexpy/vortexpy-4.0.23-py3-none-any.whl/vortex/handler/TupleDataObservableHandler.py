...
class TupleDataObservableHandler:
    ...
    def tupleSubscribers(self) -> List[TupleSelector]:
        # This is a bit costly
        return [
            TupleSelector.fromJsonStr(jsonStr)
            for jsonStr in self._observerDataByTupleSelector
        ]
    ...
    def _createVortexMsg(self, filt, tupleSelector: TupleSelector) -> Deferred:
        ...
        try:
            ...
            logger.error(
                f"createVortexMsg"
                f" TupleSelector = {tupleSelector.toJsonStr()},"
                f" ObservableFilt = {self._filt}"
            )
            logger.exception(e)
            raise
        if not vortexMsg:
    ...
    def notifyOfTupleUpdate(self, tupleSelector: TupleSelector) -> None:
        ...
        send new data to its observers.
