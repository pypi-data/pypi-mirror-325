...
class RudderStackDatasetConfig(BaseModel):
    """
    RudderStackDatasetConfig
    """ # noqa: E501
    name: StrictStr
    schema_type: StrictStr
    __properties: ClassVar[List[str]] = ["name", "schema_type"]

    @field_validator('schema_type')
    def schema_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['RUDDERSTACK']):
            raise ValueError("must be one of enum values ('RUDDERSTACK')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of RudderStackDatasetConfig from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of RudderStackDatasetConfig from a dict"""
        if obj is None:
            return None
