...
CONNECTORSINNER_ANY_OF_SCHEMAS = ["DatasetConfig", "FileConfig"]

class ConnectorsInner(BaseModel):
    """
    ConnectorsInner
    """

    # data type: FileConfig
    anyof_schema_1_validator: Optional[FileConfig] = None
    # data type: DatasetConfig
    anyof_schema_2_validator: Optional[DatasetConfig] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[DatasetConfig, FileConfig]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: Set[str] = { "DatasetConfig", "FileConfig" }
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = ConnectorsInner.model_construct()
        error_messages = []
        # validate data type: FileConfig
        if not isinstance(v, FileConfig):
            error_messages.append(f"Error! Input type `{type(v)}` is not `FileConfig`")
        else:
            return v

        # validate data type: DatasetConfig
        if not isinstance(v, DatasetConfig):
            error_messages.append(f"Error! Input type `{type(v)}` is not `DatasetConfig`")
        else:
            return v

        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in ConnectorsInner with anyOf schemas: DatasetConfig, FileConfig. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[FileConfig] = None
        try:
            instance.actual_instance = FileConfig.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        # anyof_schema_2_validator: Optional[DatasetConfig] = None
        try:
            instance.actual_instance = DatasetConfig.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into ConnectorsInner with anyOf schemas: DatasetConfig, FileConfig. Details: " + ", ".join(error_messages))
        else:
            return instance
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], DatasetConfig, FileConfig]]:
