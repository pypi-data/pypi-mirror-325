...
class ProxyPoolDownloaderMiddleware:
    def __init__(self, settings: BaseSettings, spider):
        ...
        if retry_order is not None and proxy_pool_order is not None and proxy_pool_order <= retry_order:
            raise ValueError(f"'scrapy_proxy_pool.middlewares.ProxyPoolDownloaderMiddleware':{proxy_pool_order}必须大于'scrapy.downloadermiddlewares.retry.RetryMiddleware':{retry_order}")
        # 必须重写"获取代理IP"的方法
