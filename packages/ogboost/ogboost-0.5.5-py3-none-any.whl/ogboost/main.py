...
class GradientBoostingOrdinal(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        (Docstring omitted for brevity — original text remains.)
        """
        ...
        for p in range(self.n_estimators):
            ...
            if hasattr(base_learner_this_iter, 'random_state'):
    ...
    def _fit_cv(self, X, y, sample_weight):
        """
        K-fold cross-validation at each boosting iteration for early stopping,
        maintaining both training and validation latent vectors per fold:
        - fm['g_train']:  array for fold's training data
        - fm['g_holdout']: array for fold's validation data (accumulated each iteration)
        """
        ...
        for train_idx, val_idx in kf.split(X, y):
            ...
            # For local train set
            fm['base_learner'] = clone(self.base_learner)
            if hasattr(fm['base_learner'], 'random_state'):
                fm['base_learner'].set_params(random_state=self.random_state)

            # 'g_train' is length == len(train_idx)
            y_train_fold = y[train_idx]
            fm['g_train'] = np.zeros(len(train_idx), dtype=float)
            fm['g_train'][:] = self._initialize_g(y_train_fold)

            # 'theta' is fold-specific
            fm['theta'] = self._initialize_thresholds(
                y_train_fold, n_class=self._n_class, laplace_smoothing=True
            )

            # For local validation set:
            # 'g_holdout' is length == len(val_idx)
            fm['g_holdout'] = np.zeros(len(val_idx), dtype=float)
            # Initially, zeros or could call _initialize_g(y[val_idx]) if you want a non-zero start
        ...
        for iteration in range(self.n_estimators):
            ...
            for fm in fold_models:
                ...
                if self.subsample < 1.0:
                    ...
                    chosen_local = self._get_random_state().choice(
                        local_indices,
                ...
                if hasattr(fold_learner, 'random_state'):
            ...
            if hasattr(base_learner_full, 'random_state'):
    ...
    def plot_loss(self, show=True, return_fig=False, **kwargs):
        ...
        ax1.set_title(kwargs.get('title_loss', 'Log-Loss Over Iterations'))
        ax1.set_xlabel(kwargs.get('xlabel_loss', 'Iteration'))
        ax1.set_ylabel(kwargs.get('ylabel_loss', 'Log-Loss'))
        ax1.legend()
        ...
        if loss_improvement_g is not None and loss_improvement_theta is not None:
            ...
            ax2.set_title(kwargs.get('title_improvement', 'Log-Loss Improvement'))
            ax2.set_xlabel(kwargs.get('xlabel_improvement', 'Iteration'))
            ax2.set_ylabel(kwargs.get('ylabel_improvement', 'Improvement'))
            ax2.legend()
