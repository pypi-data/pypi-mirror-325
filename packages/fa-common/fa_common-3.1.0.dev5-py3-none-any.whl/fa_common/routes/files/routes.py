...
@router.get("", response_model=list[FileDB])
async def get_files(
    onlyMine=True,
    limit: int = 10,
    offset: int = 0,
    ...
    return await service.get_files_for_user(
        user=current_user,
        owner_only=onlyMine,
        offset=offset,
        limit=limit,
...
async def file_streamer(file_bytes):
    for _i in range(10):
...
async def get_raw_table(
    ...
    sort_ascending: bool = Query(True, description="Sort in ascending order, false for descending"),
    ...
    if sort_by is not None:
        sort = service.TableSort(column=sort_by, ascending=sort_ascending)

    response = await service.get_table_from_file(
        file=file, offset=offset, limit=limit, sort=sort, separator=separator, sheet=sheet, return_format=response_format
    )
...
async def delete_file(file_id: str, current_user: UserDB = Depends(get_current_app_user)):
    ...
    return Message(message=f"File {file.id} deleted successfully")
