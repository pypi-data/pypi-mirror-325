...
def bike(idx: int = 0):
    ...
    if idx==0: code = """
    df = pd.read_csv('regression/bike_cnt.csv')
#instant просто инкрементируется как id, dteday не нужен, уже есть признаки 
df = df.drop(columns=['instant', 'dteday'])
df.head()

cat_cols = ['season', 'yr', 'weekday', 'mnth', 'holiday', 'weekday', 'workingday', 'weathersit']
num_cols = df.drop(columns=cat_cols + ['cnt']).columns
...
X_processed = preprocessor.fit_transform(X).toarray() """
    ...
    elif idx==1: code = """ X_train, X_test, y_train, y_test = train_test_split(X_processed, y, test_size=0.2, random_state=42)
...
test_loader = DataLoader(test_dataset, batch_size=64) """
    ...
    elif idx==2: code = """class MLP_REG(nn.Module):
    ...
    elif idx==3: code= """model = MLP_REG(X_train.shape[1], 1)
lr = 1e-3
...
for epoch in range(num_epochs):
    ...
    elif idx==4: code="""plt.figure(figsize=(10, 6))
...
def bank(idx: int = 0):
    if idx==0: code = """df['deposit'] = df['deposit'].apply(lambda x: 0 if x == 'no' else 1)
cat_cols = ['job', 'marital', 'education', 'default', 'housing', 'loan', 'contact',
           'month', 'campaign', 'poutcome']
num_cols = df.drop(columns=cat_cols + ['deposit']).columns
...
model = MLP_CLF(X_train.shape[1], 1)
lr = 1e-4
optimizer = optim.AdamW(model.parameters(), lr, weight_decay=1e-4)
