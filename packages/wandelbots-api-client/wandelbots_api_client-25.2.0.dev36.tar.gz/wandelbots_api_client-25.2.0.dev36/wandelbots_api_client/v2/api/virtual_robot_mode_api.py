...
class VirtualRobotModeApi:
    ...
    async def set_emergency_stop(
        ...
        Activates or releases the Emergency Stop on the virtual robot controller.  Activating the Emergency Stop stops the execution of all motions. The stop is ecexuted on physical controllers immediately not gracefully and not on paths. Due to restricted physical behaviour of the virtual robot controller, the Emergency Stop is executed on paths. To return to normal operation the Emergency Stop needs to be released.  Use [getCurrentMotionGroupState](getCurrentMotionGroupState) to get the Emergency Stop state regardless of the controller type. There the Emergency Stop state is visible as the `safety_state`.  **Note:** The Emergency Stop state can only be changed via API when using virtual robot controllers. 
