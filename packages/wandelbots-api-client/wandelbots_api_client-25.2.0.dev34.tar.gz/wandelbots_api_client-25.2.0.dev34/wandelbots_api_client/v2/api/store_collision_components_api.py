...
class StoreCollisionComponentsApi:
    ...
    async def get_default_link_chain(
        self,
        cell: Annotated[str, Field(strict=True, description="Unique identifier addressing a cell in all API calls. ")],
        motion_group_model: Annotated[MotionGroupModel, Field(description="Unique identifier for the model of a motion group, e.g. `UniversalRobots_UR10e`.  Get the `model` of a configured motion group with [getOptimizerConfiguration](getOptimizerConfiguration). ")],
        _request_timeout: Union[
            None,
            Annotated[StrictFloat, Field(gt=0)],
            Tuple[
        ...
        :type motion_group_model: MotionGroupModel
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
    ...
    def _get_default_link_chain_serialize(
        ...
        if motion_group_model is not None:
            _path_params['motion-group-model'] = motion_group_model.value
        # process the query parameters
        # process the header parameters
        # process the form parameters
        # process the body parameter
