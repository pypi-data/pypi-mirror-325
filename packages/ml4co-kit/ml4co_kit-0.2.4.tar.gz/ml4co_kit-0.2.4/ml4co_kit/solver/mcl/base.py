...
class MClSolver(SolverBase):
    ...
    def _check_nodes_label_not_none(self, ref: bool):
        ...
        message = (
            f"``{msg}`` cannot be None! You can use solvers based on ``MClSolver`` "
            "like ``MClGurobiSolver`` or use methods including ``from_graph_data``, "
    ...
    def from_gpickle_result(
        ...
        if cover:
            ...
            graph: MClGraphData = self.graph_data[0]
    ...
    def to_gpickle_result_folder(
        ...
        if gpickle_save_dir is not None:
            ...
            for idx in iterative_execution(range, samples, write_msg, show_time):
                ...
                # graph_data -> nx.Graph
                graph: MClGraphData = self.graph_data[idx]
    ...
    def evaluate(self, calculate_gap: bool = False):
        if calculate_gap:
            ...
            gap_list = list()
            for graph in self.graph_data:
                snn, rsnn, gap = graph.evaluate(calculate_gap=True)
                snn_list.append(snn)
            ...
            snn_list = list()
            for graph in self.graph_data:
                snn = graph.evaluate(calculate_gap=False)
                snn_list.append(snn)
            snn_avg = np.average(np.array(snn_list))   
            return snn_avg
