...
class MVCDataGenerator(NodeGeneratorBase):
    def __init__(
        ...
        num_threads: int = 1,
        data_type: str = "er",
        solver: Union[SOLVER_TYPE, MVCSolver] = SOLVER_TYPE.GUROBI,
        train_samples_num: int = 128000,
        val_samples_num: int = 1280,
        test_samples_num: int = 1280,
        save_path: pathlib.Path = "data/mvc",
        filename: str = None,
        # args for generate
        ...
        # filename
        if filename is None:
            filename = f"mvc_{data_type}-{nodes_num_min}-{nodes_num_max}"
        ...
        # super args
        super(MVCDataGenerator, self).__init__(
