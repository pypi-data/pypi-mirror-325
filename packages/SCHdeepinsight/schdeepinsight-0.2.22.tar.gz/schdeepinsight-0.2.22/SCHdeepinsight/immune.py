...
class immune:
    ...
    def scale_and_fill_with_mean(self, query):
        # Convert sparse matrix to dense if needed and transpose
        if issparse(query.X):
        ...
        # Calculate mean value after normalization
        mean_value = sample.values.mean()
        
        # Find missing genes and fill with mean value
        excluded_genes = list(set(self.gene_list) - set(sample.columns))
        blank_dataframe = pd.DataFrame(np.full((len(sample), len(excluded_genes)), mean_value),
                                        index=sample.index, columns=excluded_genes)
        
        # Concatenate and reorder columns according to reference gene list
