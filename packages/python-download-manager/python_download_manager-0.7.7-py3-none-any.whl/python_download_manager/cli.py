...
from .database import DownloadTask
from .download_manager import DownloadManager

# 工作目录
__work_dir = Path(tempfile.gettempdir(), ".pdm")
__download_manager = None
...
def add_download_task(url: str, filename: str = None, save_dir: str = None):
    ...
    if save_dir is None:
        save_dir = "."
    __download_manager.add_download_task(url, filename, save_dir)


def start_download():
    __download_manager.start_download()
    __download_manager.wait()
...
def __delete(status: str, task_ids: list[int], forever: bool):
    ...
    if len(task_ids) == 0:
        ...
        user_cancelled = input("Press enter directly to cancel this operation: ") != "Yes"
        if user_cancelled:
            print("Cancel delete.")
            return
        task_ids = None
    ...
    if forever:
        ...
        if user_cancelled:
            ...
            return
    if status == "waiting":
        deleted_tasks = __download_manager.delete_download_tasks(task_ids, forever)
    elif status == "downloading":
        deleted_tasks = __download_manager.delete_downloading_tasks(task_ids, forever)
    elif status == "deleted":
        deleted_tasks = __download_manager.delete_deleted_tasks(task_ids)
    elif status == "completed":
        deleted_tasks = __download_manager.delete_completed_tasks(task_ids)
...
def __list(status: str):
    if status == "waiting":
        tasks = __download_manager.get_all_download_task()
    elif status == "downloading":
        tasks = __download_manager.get_all_downloading_task()
    elif status == "deleted":
        tasks = __download_manager.get_all_deleted_task()
    elif status == "completed":
        tasks = __download_manager.get_all_completed_task()
    print_tasks(tasks)
...
def cli():
    """命令行接口"""
    global __download_manager
    # 初始化下载管理器
    __download_manager = DownloadManager(__work_dir)
    # 参数解析器
    ...
    delete_parser.add_argument("-f", "--forever", action="store_true", help="是否永久删除")
