...
class DatabaseManager:
    ...
    @classmethod
    def initialize(cls, database_dir: Union[str, Path] = None):
        if cls._initialized:
            return
        if not database_dir:
            database_dir = ".pdm"
    ...
    @synchronized(_write_lock)
    @database.atomic()
    def delete_completed_tasks(task_ids: list[int] = None) -> list[CompletedDownloadTaskViewModel]:
        """批量删除已完成的下载任务，如果传入的参数为None，则删除表中的所有数据"""
        if task_ids is None:
            tasks = list(CompletedDownloadTask.select())
            CompletedDownloadTask.truncate_table()
        else:
            tasks = list(CompletedDownloadTask.select().where(CompletedDownloadTask.id << task_ids))
            CompletedDownloadTask.delete().where(CompletedDownloadTask.id << task_ids).execute()
        return [CompletedDownloadTaskViewModel.from_model(task) for task in tasks]

    @staticmethod
    @synchronized(_write_lock)
    @database.atomic()
    def delete_deleted_tasks(task_ids: list[int] = None) -> list[DeletedDownloadTaskViewModel]:
        """批量删除回收站中的下载任务，如果传入的参数为None，则删除表中的所有数据"""
        if task_ids is None:
            tasks = list(DeletedDownloadTask.select())
            DeletedDownloadTask.truncate_table()
        else:
            tasks = list(DeletedDownloadTask.select().where(DeletedDownloadTask.id << task_ids))
            DeletedDownloadTask.delete().where(DeletedDownloadTask.id << task_ids).execute()
        return [DeletedDownloadTask.from_model(task) for task in tasks]

    @staticmethod
    @synchronized(_write_lock)
    @database.atomic()
    def delete_download_tasks(task_ids: list[int] = None, forever: bool = False) -> list[DownloadTaskViewModel]:
        ...
        if not forever:
            deleted_tasks = [DeletedDownloadTask.from_model(task).__data__ for task in tasks]
    ...
    @synchronized(_write_lock)
    @database.atomic()
    def delete_downloading_tasks(task_ids: list[int] = None, forever: bool = False) -> list[DownloadingTaskViewModel]:
    ...
    # @staticmethod
    # @synchronized(_write_lock)
    # def delete_deleted_download_task(
    #     deleted_download_task: DeletedDownloadTaskViewModel,
    # ) -> DeletedDownloadTask:
    #     """删除某个回收站中的任务"""
    #     DeletedDownloadTask.delete_by_id(deleted_download_task.id)
    #     return deleted_download_task

    # @staticmethod
    # @synchronized(_write_lock)
    # def delete_completed_download_task(
    #     completed_download_task: CompletedDownloadTaskViewModel,
    # ) -> CompletedDownloadTaskViewModel:
    #     """删除某个已完成的任务"""
    #     CompletedDownloadTask.delete_by_id(completed_download_task.id)
    #     return completed_download_task
    ...
    @staticmethod
    def get_all_download_task() -> list[DownloadTaskViewModel]:
        """获取所有下载任务"""
        tasks = DownloadTask.select()
        return [DownloadTaskViewModel.from_model(task) for task in tasks]

    @staticmethod
    def get_all_downloading_task() -> list[DownloadingTaskViewModel]:
        tasks = DownloadingTask.select()
        return [DownloadingTaskViewModel.from_model(task) for task in tasks]

    @staticmethod
    def get_all_deleted_download_task() -> list[DeletedDownloadTaskViewModel]:
        """获取所有已删除的下载任务"""
        tasks = DeletedDownloadTask.select()
        return [DeletedDownloadTaskViewModel.from_model(task) for task in tasks]

    @staticmethod
    def get_all_completed_download_task() -> list[CompletedDownloadTaskViewModel]:
        """获取所有已完成的下载任务"""
    ...
    def start_download_task(
        ...
        chunk_models = Chunk.select().where(Chunk.downloading_task == downloading_task.id)
