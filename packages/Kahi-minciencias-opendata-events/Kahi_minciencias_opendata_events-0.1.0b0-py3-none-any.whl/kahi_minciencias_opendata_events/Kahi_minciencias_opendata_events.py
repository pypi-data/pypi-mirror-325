...
from kahi_minciencias_opendata_events.process_one import process_one
from mohan.Similarity import Similarity


class Kahi_minciencias_opendata_events(KahiBase):

    config = {}

    def __init__(self, config):
        """
        Constructor for the Kahi_minciencias_opendata_events class.
        ...
        config : dict
            ...
            - minciencias_opendata_events: a dictionary with the following keys:
        ...
        if "es_index" in config["minciencias_opendata_events"].keys() and "es_url" in config["minciencias_opendata_events"].keys() and "es_user" in config["minciencias_opendata_events"].keys() and "es_password" in config["minciencias_opendata_events"].keys():  # noqa: E501
        ...
        self.task = config["minciencias_opendata_events"]["task"] if "task" in config["minciencias_opendata_events"].keys(
        ) else None
        self.insert_all = config["minciencias_opendata_events"]["insert_all"] if "insert_all" in config["minciencias_opendata_events"].keys(
        ) else False
        self.thresholds = config["minciencias_opendata_events"]["thresholds"] if "thresholds" in config["minciencias_opendata_events"].keys(
        ) else None
        self.n_jobs = config["minciencias_opendata_events"]["num_jobs"] if "num_jobs" in config["minciencias_opendata_events"].keys(
        ) else 1
        self.verbose = config["minciencias_opendata_events"]["verbose"] if "verbose" in config["minciencias_opendata_events"].keys(
        ) else 0
    ...
    def check_databases_and_collections(self):
        ...
        """
        try:
            with MongoClient(self.config["minciencias_opendata_events"]["database_url"]) as client:
                db_name = self.config["minciencias_opendata_events"]["database_name"]
                collection_name = self.config["minciencias_opendata_events"]["collection_name"]
    ...
    def process_opendata(self):
        ...
        """
        client = MongoClient(
            self.config["minciencias_opendata_events"]["database_url"])
        db = client[self.config["minciencias_opendata_events"]["database_name"]]
        opendata = db[self.config["minciencias_opendata_events"]
                      ["collection_name"]]
