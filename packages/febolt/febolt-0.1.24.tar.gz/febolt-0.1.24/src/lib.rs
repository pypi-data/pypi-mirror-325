...
// -----------------------------------------------------------------------------
// Approximate covariance from L-BFGS history
// -----------------------------------------------------------------------------
// Correct covariance approximation implementation
fn approximate_covariance(history: &VecDeque<(Array1<f32>, Array1<f32>)>) -> Array2<f32> {
    let k = history.front().map_or(0, |(s, _)| s.len());
    let mut inv_hess = Array2::eye(k);

    for (s, y) in history.iter().rev() {
        let rho = 1.0 / s.dot(y);
        let sy = s.dot(&inv_hess);
        
        // Convert to 2D arrays for proper matrix multiplication
        let s_col = s.view().insert_axis(Axis(1));  // Shape: [k, 1]
        let sy_col = sy.view().insert_axis(Axis(0)); // Shape: [1, k]
        let update = rho * s_col.dot(&sy_col);
        
        inv_hess = inv_hess + update;
    }
...
struct Probit<'a> {
    ...
    xbeta_buf: RefCell<Array1<f32>>,
    hess_buf: RefCell<Array2<f32>>,
}
...
impl<'a> Probit<'a> {
    fn new(endog: ArrayView1<'a, f32>, exog: ArrayView2<'a, f32>) -> Self {
        ...
        Probit {
            endog,
            exog,
            xbeta_buf: RefCell::new(Array1::zeros(n)),
            hess_buf: RefCell::new(Array2::zeros((k, k))),
        }
    }
    ...
    // Update xbeta to reuse buffer
    fn xbeta(&self, params: &ArrayView1<f32>) -> Array1<f32> {
        let mut xbeta = self.xbeta_buf.borrow_mut();
        *xbeta = self.exog.dot(params);
        xbeta.clone()
    }
    ...
    fn score(&self, xbeta: &ArrayView1<f32>, grad: &mut Array1<f32>) {
        ...
        // Modified hessian to reuse buffer
    fn hessian(&self, xbeta: &ArrayView1<f32>) -> Array2<f32> {
        let mut hess = self.hess_buf.borrow_mut();
        self.hessian_impl(xbeta, &mut hess);
        hess.clone() // Return a clone of the computed Hessian
    }

    // Hessian computed using BLAS’s single‑precision rank‑1 update (sger)
fn hessian_impl(&self, xbeta: &ArrayView1<f32>, hess: &mut Array2<f32>) {
    let weights = self.endog
        .iter()
        .zip(xbeta.iter())
        .map(|(y, xb)| {
            let q = 2.0 * y - 1.0;
            ...
            let cdf = self.cdf(z);
            let ratio = pdf / cdf;
            (ratio * (ratio + q * z)).max(0.0)
        })
        .collect::<Vec<f32>>();

    let sqrt_weights = Array1::from_vec(weights.iter().map(|w| w.sqrt()).collect());
    let weighted_exog = &self.exog * &sqrt_weights.view().insert_axis(Axis(1));
...
struct LbfgsState {
    ...
    workspace: LineSearchWorkspace,  // Add workspace to state
}
...
impl LbfgsState {
    fn new(k: usize) -> Self {
        LbfgsState {
            x: Array1::zeros(k),
            g: Array1::zeros(k),
            history: VecDeque::with_capacity(M_LIMIT),
            workspace: LineSearchWorkspace::new(k),
        }
    }
}
...
fn lbfgs_optimize(
    ...
    max_iter: usize,
    tol: f32,
) -> (Array1<f32>, f32, usize, bool, VecDeque<(Array1<f32>, Array1<f32>)>) {
    let mut state = LbfgsState::new(init_param.len());

    state.x.assign(&init_param);
    compute_initial_gradient(obj, &state.x, &mut state.g);

    let mut f_val = -Probit::new(obj.endog.view(), obj.exog.view())
        .loglike(&obj.exog.dot(&state.x).view());
    ...
    while iter < max_iter && !converged {
        ...
        let p = compute_search_direction(&state.g, &state.history);

        // Perform line search to find step size
        let (_step, x_new, f_new, g_new) = line_search(
            obj, 
            &state.x, 
            f_val, 
            &state.g, 
        ...
        if s.dot(&y) > 1e-10 * s.norm() * y.norm() { // Curvature condition
            if state.history.len() == M_LIMIT {
                state.history.pop_front();
            }
            state.history.push_back((s, y));
        }

        // Update variables
        state.x.assign(&x_new);
        state.g.assign(&g_new);
        f_val = f_new;
        iter += 1;

        // Check convergence (relative gradient norm)
        converged = state.g.norm() < tol * (1.0 + f_val.abs());
    }

    (state.x, f_val, iter, converged, state.history)
}

// Helper: Compute initial gradient and function value
fn compute_initial_gradient(
    obj: &ProbitObjective, 
    x: &Array1<f32>, 
    grad: &mut Array1<f32>  // Added output parameter
) -> f32 {
    let xbeta = obj.exog.dot(x);
    let probit = Probit::new(obj.endog.view(), obj.exog.view());
    let f_val = -probit.loglike(&xbeta.view());
    probit.score(&xbeta.view(), grad);
    f_val
}
...
fn compute_search_direction(
    ...
    for ((s, y), a) in history.iter().zip(alpha.iter().rev()) {
        ...
        q.scaled_add(a - beta, s);  // Removed extra parentheses
    }

    -q // Search direction is the negative of q
}

// Replace existing line_search function with this
struct LineSearchWorkspace {
    grad_buf: Array1<f32>,
    x_new_buf: Array1<f32>,
}

impl LineSearchWorkspace {
    fn new(n: usize) -> Self {
        LineSearchWorkspace {
            grad_buf: Array1::zeros(n),
            x_new_buf: Array1::zeros(n),
        }
    }
}
...
fn line_search(
    ...
    for _ in 0..20 {
        workspace.x_new_buf.assign(&(x + &(p * alpha)));
        let f_new = compute_initial_gradient(obj, &workspace.x_new_buf, &mut workspace.grad_buf);
        
        // Armijo condition
        if f_new > f_val + c1 * alpha * blas_dot(g, p) {
            return zoom(obj, x, f_val, g, p, alpha_prev, alpha, f_prev, f_new, &g_prev, workspace); // Fix: pass workspace directly
        }
        
        // Wolfe condition
        if blas_dot(&workspace.grad_buf, p) < c2 * blas_dot(g, p) {
            alpha_prev = alpha;
            f_prev = f_new;
            std::mem::swap(&mut g_prev, &mut workspace.grad_buf);
            alpha *= 2.0;
        } else {
            return (alpha, workspace.x_new_buf.clone(), f_new, workspace.grad_buf.clone());
        }
    }
    (alpha, workspace.x_new_buf.clone(), f_prev, g_prev)
}
...
fn zoom(
    ...
    let c2 = 0.9;
    let mut alpha = 1.0;
    let mut g_current = g_lo.clone();
    let mut f_new = f_lo; // Declare f_new here

    for _ in 0..20 {
        // Cubic interpolation
        alpha = cubic_interpolate(alpha_lo, alpha_hi, f_lo, f_hi, blas_dot(g_lo, p), blas_dot(&g_current, p));
        
        workspace.x_new_buf.assign(&(x + &(p * alpha)));
        f_new = compute_initial_gradient(obj, &workspace.x_new_buf, &mut workspace.grad_buf);

        if f_new > f_val + c1 * alpha * blas_dot(g, p) || f_new >= f_lo {
            alpha_hi = alpha;
            f_hi = f_new;
        } else {
            if blas_dot(&workspace.grad_buf, p) >= c2 * blas_dot(g, p) {
            ...
            alpha_lo = alpha;
            f_lo = f_new;
            g_current.assign(&workspace.grad_buf);
        }
    }
    (alpha, workspace.x_new_buf.clone(), f_new, workspace.grad_buf.clone())
}
fn cubic_interpolate(a: f32, b: f32, fa: f32, fb: f32, dfa: f32, dfb: f32) -> f32 {
    let d1 = dfa + dfb - 3.0 * (fa - fb) / (a - b);
    let d2 = if d1 * d1 > dfa * dfb {
        d1.signum() * (d1 * d1 - dfa * dfb).sqrt()
    } else {
        0.0
    };
    b - (b - a) * (dfb + d2 - d1) / (dfb - dfa + 2.0 * d2)
}
// -----------------------------------------------------------------------------
...
fn robust_covariance(
    ...
    if cluster.is_none() {
        // Compute weights for all observations
        let weights = endog
            .iter()
            .zip(xbeta.iter())
            .map(|(y, xb)| {
        ...
        // Compute M = X^T * W * X, where W is a diagonal matrix of weights
        let weighted_exog = exog * &weights.view().insert_axis(Axis(1));
        let M = weighted_exog.t().dot(&weighted_exog);

        // Compute robust covariance: H^{-1} * M * H^{-1}
        ...
        if cluster.ncols() != 1 {
            // Non-clustered robust covariance
            for i in 0..nobs {
            ...
            // Accumulate M from cluster scores
...
fn model_fit_lbfgs<'a>(
    ...
    let (optimized_params, _f_val, iterations, converged, history) =
        lbfgs_optimize(&obj, Array1::zeros(exog.ncols()), max_iter, tol);

    let xbeta = exog.dot(&optimized_params);
    let probit = Probit::new(obj.endog.view(), obj.exog.view());
    let loglike = probit.loglike(&xbeta.view());

    let cov_final = if robust || cluster_vars.is_some() {
        let cluster_view = cluster_vars
            .map(|obj| {
                let arr_2d = obj.downcast::<PyArray2<f32>>()?;
                Ok::<ArrayView2<f32>, PyErr>(unsafe { arr_2d.as_array() })
            })
            .transpose()?;

        // Compute proper Hessian-based covariance
        let hess = probit.hessian(&xbeta.view());
        let h_inv = hess.inv().unwrap_or_else(|_| Array2::eye(exog.ncols()));
        robust_covariance(
            &exog.view(),
            &xbeta.view(),
            &endog,
            &h_inv.view(),
            cluster_view  // Use converted view here
        )
    } else {
        // Use L-BFGS history approximation
        approximate_covariance(&history)
    };
