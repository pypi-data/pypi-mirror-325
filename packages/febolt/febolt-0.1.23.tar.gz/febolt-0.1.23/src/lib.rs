...
impl LbfgsState {
    fn new(k: usize) -> Self {
        LbfgsState {
            x: Array1::zeros(k),
            g: Array1::zeros(k),
            x_new: Array1::zeros(k),
            g_new: Array1::zeros(k),
...
fn lbfgs_optimize(
    ...
    while iter < max_iter && !converged {
        ...
        let (_step, x_new, f_new, g_new) = line_search(obj, &state.x, &f_val, &state.g, &p);
...
fn line_search(
    ...
    for _ in 0..max_backtrack {
        let x_new = x + &(p * step);
        let mut grad_buf = Array1::zeros(x.len());
        let f_new = compute_initial_gradient(obj, &x_new, &mut grad_buf);
        
        let armijo = f_new <= *f_val + c1 * step * g.dot(p);
        let wolfe = grad_buf.dot(p) >= c2 * g.dot(p);

        if armijo && wolfe {
            return (step, x_new, f_new, grad_buf);
        }
        step /= 2.0;
    }
