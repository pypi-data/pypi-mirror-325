...
TOP_BACKBONE_LEVEL, LEVEL = 5, 2
ONNX_VERSION, ONNX_FILE_NAME = 18, "anomaly_detection.onnx"


@pytest.fixture()
def model() -> AnomalyDetection:
    return AnomalyDetection(
        in_channels=[3] + [NUM_CHANNELS for _ in range(TOP_BACKBONE_LEVEL)],
        level=LEVEL,
    )
...
def backbone_output() -> List[Tensor]:
    return [torch.randn((BATCH_SIZE, 3, HEIGHT, WIDTH))] + [
        ...
        for _ in range(1, TOP_BACKBONE_LEVEL + 1)
    ]
...
def test_forward(model: AnomalyDetection, backbone_output: List[Tensor]) -> None:
    anomaly_map = model(backbone_output)
    assert tuple(anomaly_map.shape) == (BATCH_SIZE, HEIGHT, WIDTH)


def test_training_step(
    model: AnomalyDetection, backbone_output: List[Tensor], targets: List[List[str]]
) -> None:
...
@pytest.fixture()
def onnx_model(model: AnomalyDetection, backbone_output: List[Tensor]) -> None:
    torch.onnx.export(
...
def test_onnx_inference(
    onnx_model, model: AnomalyDetection, backbone_output: List[Tensor]
) -> None:
    model.eval()
