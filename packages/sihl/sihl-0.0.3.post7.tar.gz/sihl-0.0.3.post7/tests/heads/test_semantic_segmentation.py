...
ONNX_VERSION, ONNX_FILE_NAME = 18, "semantic_segmentation.onnx"


@pytest.fixture()
def model() -> SemanticSegmentation:
    return SemanticSegmentation(
        in_channels=[3] + [NUM_CHANNELS for _ in range(LEVEL)], num_classes=NUM_CLASSES
    )
...
def test_forward(model: SemanticSegmentation, backbone_output: List[Tensor]) -> None:
    score_map, class_map = model(backbone_output)
    assert tuple(score_map.shape) == (BATCH_SIZE, HEIGHT, WIDTH)
    assert tuple(class_map.shape) == (BATCH_SIZE, HEIGHT, WIDTH)


def test_training_step(
    model: SemanticSegmentation, backbone_output: List[Tensor], targets: Tensor
) -> None:
...
@pytest.fixture()
def onnx_model(model: SemanticSegmentation, backbone_output: List[Tensor]) -> None:
    torch.onnx.export(
...
def test_onnx_inference(
    onnx_model, model: SemanticSegmentation, backbone_output: List[Tensor]
) -> None:
    model.eval()
