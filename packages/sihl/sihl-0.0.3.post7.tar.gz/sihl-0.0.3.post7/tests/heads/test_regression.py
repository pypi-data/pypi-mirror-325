...
LOWER_BOUND, UPPER_BOUND, LEVEL = -2.0, 10.0, 5
ONNX_VERSION, ONNX_FILE_NAME = 18, "regression.onnx"


@pytest.fixture()
def model() -> Regression:
    return Regression(
...
@pytest.fixture()
def targets() -> Tensor:
    return torch.rand((BATCH_SIZE,)) * (UPPER_BOUND - LOWER_BOUND) + LOWER_BOUND


def test_forward(model: Regression, backbone_output: List[Tensor]) -> None:
    values = model(backbone_output)
    assert tuple(values.shape) == (BATCH_SIZE,)


def test_training_step(
    model: Regression, backbone_output: List[Tensor], targets: Tensor
) -> None:
...
@pytest.fixture()
def onnx_model(model: Regression, backbone_output: List[Tensor]) -> None:
    torch.onnx.export(
...
def test_onnx_inference(
    onnx_model, model: Regression, backbone_output: List[Tensor]
) -> None:
    model.eval()
