...
This file used to live in pytango at `tango/databaseds/db_access/beacon.py`.

It has been moved into bliss and converted to migrated to 3.

The git history for this file can be found here:
...
def _filter(pattern: str, lst: typing.List[str]) -> typing.List[str]:
    """
    ...
    Arguments:
        ...
        lst: A list of string identifiers without special characters
...
class beacon:
    ...
    def _parse_list(self, lst, indexing_flag):
        try:
            lst = lst.raw_list
    ...
    def _index_tango(self, v):
        ...
        if personal_name is not None:
            ...
            if server is None:
                self._error(
                    "_index_tango(personal_name=%) didn't specify server key (executable name)",
                    personal_name,
                )
                return
    ...
    def add_device(self, server_name, dev_info, klass_name, alias=None):
        self._info(
            "add_device(server_name=%s, dev_info=%s, klass_name=%s, alias=%s)",
            server_name,
            dev_info,
            klass_name,
            alias,
    ...
    def _get_class_attribute(self, klass_name, attr_name):
        self._info(
            "_get_class_attribute(klass_name=%s,attr_name=%s)", klass_name, attr_name
        )
    ...
    def get_device_alias_list(self, alias):
        alias_list = [
            node.get("alias") for node in list(self._tango_name_2_node.values())
        ]
    ...
    def get_device_attribute_list(self, dev_name, attribute):
        prop_attr_device = self._get_property_attr_device(dev_name)
        return _filter(attribute, list(prop_attr_device.keys()))
    ...
    def get_device_domain_list(self, wildcard):
        filtered_names = _filter(
            wildcard,
            [n.get("tango_name") for n in list(self._tango_name_2_node.values())],
        )
    ...
    def get_device_list(self, server_name, class_name):
        if server_name == "*":
            r_list = list()
            for server_node in list(self._personal_2_node.values()):
                device_list = server_node.get("device")
    ...
    def get_device_wide_list(self, wildcard):
        return _filter(wildcard, list(self._tango_name_2_node.keys()))

    @_info
    def get_device_member_list(self, wildcard):
        wildcard = wildcard.lower()
        filtered_names = _filter(wildcard, list(self._tango_name_2_node.keys()))
        return list(set([x.split("/")[-1] for x in filtered_names]))
    ...
    def get_server_list(self, wildcard):
        return _filter(wildcard, list(self._personal_2_node.keys()))
    ...
    def get_server_name_list(self, wildcard):
        res = list(
            set(
                _filter(
                    wildcard,
                    [x.split("/")[0] for x in list(self._personal_2_node.keys())],
                )
            )
        )
        res.sort()
    ...
    def put_attribute_alias(self, attribute_name, attr_alias_name):
        ...
        if attr_alias_info is not None:
            self.warn_stream(
                "DataBase::DbPutAttributeAlias(): this alias exists already "
            )
            th_exc(
                db_errors.DB_SQLError,
