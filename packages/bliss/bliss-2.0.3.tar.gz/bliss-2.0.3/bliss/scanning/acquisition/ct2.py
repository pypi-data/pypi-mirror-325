...
class CT2AcquisitionMaster(AcquisitionMaster):
    ...
    def __init__(
        ...
        self.wait_new_point_signal = wait_new_point_signal
    ...
    def prepare(self):
        ...
        if hasattr(device, "enable_new_point_signal"):
            # hasattr for backward compatibility with old CT2 server
            device.enable_new_point_signal = self.wait_new_point_signal
        device.read_all_triggers = self.read_all_triggers
    ...
    def wait_ready(self):
        if self.device.enable_new_point_signal:
...
class CT2VarTimeAcquisitionMaster(CT2AcquisitionMaster):
    ...
    def __init__(self, device, acq_expo_time=tuple(), **kwargs):
        kwargs.pop("npoints", None)
        kwargs.pop("prepare_once", None)
        kwargs.pop("start_once", None)

        if isinstance(acq_expo_time, Iterable):
            self._acq_expo_times = list(acq_expo_time)
        else:
        ...
        self._channel_time = AcquisitionChannel(
            f"{self.name}:acq_time", numpy.double, (), unit="s"
        )
...
class CT2CounterAcquisitionSlave(IntegratingCounterAcquisitionSlave):
    ...
    def start_device(self):
        ...
        if not self._event_connected:
            ...
            event.connect(
                self.device._master_controller.server, ErrorSignal, self._on_error
            )
    ...
    def _on_error(self, err):
        if self._reading_task:
            name = self.device._master_controller.name
            self._reading_task.kill(RuntimeError(f"on {name}: {err}"))

    def reading(self):
        read_all_triggers = self.device._master_controller.read_all_triggers
        from_index = 0
        ...
        while (
            ...
            if from_index > 0 or not read_all_triggers:
                self.__buffer.clear()
            ...
            if from_index == 0 and read_all_triggers:
                if data_len > 1:
                    data[0] = data[1]
                    self.__buffer.clear()
                else:
    ...
    def _emit_new_data(self, data):
        super()._emit_new_data(
            [
                c.convert(v.astype(float))
                for c, v in zip(self.device.counter_indexes, data)
            ]
        )
