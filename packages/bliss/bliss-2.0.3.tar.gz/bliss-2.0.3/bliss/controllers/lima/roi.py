...
class RoiStatCounter(IntegratingCounter):
    ...
    def scan_metadata(self) -> dict:
        ...
        metadata.update(
            {self.roi_name: self._counter_controller.get(self.roi_name).to_dict()}
        )
        return metadata
...
class RoiCounters(IntegratingCounterController):
    ...
    def _initialize(self):
        ...
        self._raw_params = settings.OrderedHashObjSetting(raw_params_settings_name)
    ...
    def get_rois(self):
        """return active rois as a list"""
        return list(self._active_rois.values())
    ...
    def __info__(self):
        ...
        if self._stored_names:
            ...
            for name, roi in self._active_rois.items():
                tab.add_line([name, str(roi), "Enabled"])

            for name, roi in self._inactive_rois.items():
...
class RoiProfileController(IntegratingCounterController):
    ...
    def get_values(self, from_index, *counters):
        ...
        cnt_ids = [self._roi_ids[cnt.name] for cnt in counters]
        cnt_size = [cnt.shape[0] for cnt in counters]

        profiles = list()
        nb_read = list()

        for cid, size in zip(cnt_ids, cnt_size):
            spec = self._proxy.readImage([int(cid), int(from_index)])

            if len(spec):
                num_of_spec = len(spec) // size
                spec.shape = (num_of_spec, size)

                profiles.append(spec)
                nb_read.append(num_of_spec)
            else:
                # if no profiles ready yet for that counter then return
                # and let the 'reading' polling call this function again
                return blank
        # keep same number of spectrums
        nb_keep = min(nb_read)
        for idx in range(len(profiles)):
            profiles[idx] = profiles[idx][0:nb_keep, :]
...
class RoiCollectionController(IntegratingCounterController):
    ...
    def _initialize(self):
        ...
        loaded_file_setting_name = "%s_loaded_file_name:%s" % (
            self.fullname,
            self._current_config.get(),
        )
        self._loaded_rois_file_setting = settings.SimpleSetting(
            loaded_file_setting_name, default_value=""
        )
    ...
    def _set_roi(self, name, roi_values):
        ...
        # prevent modification if a rois collection file is loaded
        fname = self._loaded_rois_file_setting.get()
        if fname:
            msg = f"Cannot modify rois list when a rois file is loaded ({fname})\n"
            msg += "Use '.clear()' to unload current file or update the file and reload it."
    ...
    def clear(self):
        self._remove_rois(self._stored_names.copy())
        self._loaded_rois_file_setting.set("")
    ...
    def __info__(self):
        ...
        if nb_rois:
            ...
            if fname:
                txt += f" (from file '{fname}')"
            txt += f"\n{len(self._inactive_rois)} are currently disabled"
            return "\n".join([header, txt])
        else:
    ...
    def load_rois_from_file(self, fpath, separator=" "):
        rois_dict = {}
        with open(fpath) as f:
            for line in f:
                line = line.strip()
                if line:
                    (name, x, y, w, h) = line.split(separator)
                    rois_dict[name] = [x, y, w, h]

        self.clear()
        for k, v in rois_dict.items():
            self._set_roi(k, v)
        self._loaded_rois_file_setting.set(fpath)
