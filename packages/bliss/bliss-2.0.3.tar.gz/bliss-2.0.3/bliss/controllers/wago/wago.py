...
Every Wago PLC is normally assembled with a central unit (Wago PLC Ethernet/IP
type 750-842) plus a variable numbers of addons as needed.
Addon modules usually has one or more input/output that can be plugged to the
outside world.

Naming Convention:

* PHYSICAL_MODULES: (int) hardware modules with I/O that you physically
  compose to obtain the physical configuration, we will number them from 0
  (first hardware module) to n.
* PHYSICAL_CHANNELS: (int) I/O inside one PHYSICAL_MODULE, we will number
  from 0 (first channel) to n.
...
* LOGICAL_DEVICE_KEY: (int) an enumeration of logical devices where first
  logical device will have device key 0, second logical device will have
  device key 1 and so on.
* LOGICAL_CHANNEL: (int) every Logical Device can have multiple channels that
  are numbered from 0. One logical channel will map to a Physical Channel
...
Example taken from a configuration:
    ...
    Explanation:
        ...
        * foh2ctrl has 4 logical channel from 0 to 3, situated in the physical
          module n.0, respectively on physical channels from 0 to 3
        * foh2pos has 4 logical channel from 0 to 1, two logical channels are
          on physical module n.1 (channels n.0 and n.2) and two
          on physical module n.2 (channels n.0 and n.2)
        * sain2 has 2 logical channels on physical module n.1, on
          channels n.1 and n.3
        * sain6 has 1 logical channel on physical module n.2 on channel n.1
        * sain8 has 1 logical channel on physical module n.2 on channel n.3
...
name: wceudev01
plugin: bliss
description: test eth module
module: wago.wago
class: Wago
main_module: 750-8001  # new ethernet module
modbustcp:
    url: wceudev01
mapping:
  - type: 750-508  # 2 digital outputs
    logical_names: do011,do012
  - type: 750-411  # 2 digital inputs
    logical_names: di011, di012
  - type: 750-403  # 4 digital inputs
    logical_names: di013, di014 , di015, di016
  - type: 750-478
    logical_names: ai011, ai012 # 2 analog input    
  - type: 750-550
    logical_names: ao011, ao012 # 2 analog output
  - type: 750-469
    logical_names: th011, th012 # 2 thermocouples type K
  - type: 750-517
    logical_names: relay011,relay012
    
counter_names: do011, do012, di011, di012, di013, di014, ai011, ai012, ao011,ao012, th011, th012,relay011,relay012
...
MODULES_CONFIG = {
    ...
    "750-8001": [0, 0, 0, 0, 2, 2, "cpu", "Wago PLC Ethernet/IP"],
    ...
    "750-497": [0, 0, 8, 0, 8, 8, "fs10", "8 Channel Analog Input"],
    ...
    "750-553": [0, 0, 0, 4, 4, 4, "fs20", "4 Channel 0/20mA Output"],
    ...
    "750-1406": [16, 0, 0, 0, 16, 16, "digital", "16 Channel Digital Input 24V"],
...
def get_channel_info(module_name, module_channel=0, extended_mode=False):
    ...
    if module_name == "750-404":
        ...
        if extended_mode:
            ...
            if module_channel in (1, 3, 5, 7):
                ...
                msg = f"Module channel n.{module_channel} not available for "
                msg += f"Wago module {module_name} with "
                msg += f"extended_mode={extended_mode}"
    ...
    try:
        ...
        msg = f"Module channel n.{module_channel} not available for "
        msg += f"Wago module {module_name} with extended_mode={extended_mode}"
...
class TangoWago:
    ...
    def get(self, *logical_names, **kwargs):
        ...
        for name in logical_names:
            try:
                result.append(getattr(self.comm, name))
    ...
    def set(self, *args):
        ...
        attr_set = []
        attr_list = self.comm.get_attribute_list()
        attr_to_write = self.modules_config._channels_to_write(*args)
        for _dev, _val in attr_to_write:
            if _dev in attr_list:
                attr_set.append(_dev)
                setattr(self.comm, _dev, _val)
        # This way of setting is not needed, but kept just in case???
        if not attr_set or len(attr_set) != len(attr_to_write):
            try:
                ...
                raise KeyError("Incorrect channel name") from err
...
class ModulesConfig:
    def __init__(
        ...
        Args:
            mapping_str (str): a comma separated string containing the vendor
                               code for the module followed by names of
                               logical channels.
        Example:
        ...
        if (
            main_module not in MODULES_CONFIG
            and MODULES_CONFIG[main_module][READING_TYPE] != "cpu"
        ):
            raise RuntimeError(f"{main_module} module is not a CPU module")

        self.__modules = [
            main_module
        ]  # first element is the Ethernet Controller Module
    ...
    def create_memory_table(self):
        ...
        Example:
            ...
            DeviceInfo(logical_device='encpsb', logical_channel=0,
                       module_name='750-630', physical_module_number=2,
                       physical_module_channel=0,
                       info=ModConf(digi_in=0, digi_out=0, ana_in=2, ana_out=0,
                                    n_channels=1, n_channels_ext=1,
                                    reading_type='ssi24',
                                    description='24 bit SSI encoder',
                                    reading_info={'reading_type': 'ssi',
                                    'type': 'ANA_IN', 'bits': 24},
                                    writing_info={}))
        ...
        for phys_mod_num, (module_name, logical_devices) in enumerate(
            ...
            #
            # Example
            # [DeviceInfo(logical_device='foh2ctrl', logical_channel=0,
            # module_name='750-504', physical_module_number=0,
            # physical_module_channel=0),
            # DeviceInfo(logical_device='foh2ctrl', logical_channel=1,
            # module_name='750-504', physical_module_number=0,
            # physical_module_channel=1), ...

            if module_name == "750-404":  # 32 bit counter
                # Input process image has 2 words counter value and 1 byte
                # status, total of 3 words
                # Output process image has 2 words counter setting value and
                # 1 byte control, total of 3 words
                if self.__extended_mode:
                    (
                        counter_value,
                        counter_status,
                        counter_setting_value,
                        counter_control,
                    ) = filled_channels
    ...
    def parse_mapping_str(mapping_str: str):
        ...
        Args:
            mapping_str(str): string containing PLC's attached modules info
        Returns:
        ...
        for _line in splitlines(mapping_str):
            line = _line.replace(":", ",")
            items = [item.strip() for item in [_f for _f in line.split(",")]]
            if items:
    ...
    def from_config_tree(cls, config_tree: dict):
        ...
        if config_tree.get("mapping"):
            for module in config_tree["mapping"]:
                ...
                mapping.append(f"{module_type},{logical_names}")
        extended_mode = config_tree.get("extended_mode", False)
        main_module = config_tree.get("main_module")
    ...
    def devhard2log(self, array_in):
        ...
        try:
            ...
            raise RuntimeError("Invalid offset") from err
    ...
    def _channels_to_write(self, *args):
        """Args should be list or pairs: channel_name, value
        or a list with channel_name, val1, val2, ..., valn
        or a combination of the two
        Args:
        ...
        Returns:
            list of lists: [[logical_device_key1, logical_channel1, value1,
                             logical_channel2, value2],
                            [logical_device_key2, logical_channel1, value1],
                           ...]
    ...
    def _resolve_write(self, *args):
        channels_to_write = self._channels_to_write(*args)
        out_array = []
...
class WagoController:
    ...
    def connect(self):
        ...
        reply_firmware = self.client_read_input_registers(0x2010, "H")
        self.firmware["version"] = reply_firmware
        if not self.coupler:
            # get firmware date and version
            if self.order_nu != 8001:
                reply_time = struct.pack(
                    "8H", *self.client_read_input_registers(0x2021, "8H")
                )
                reply_date = struct.pack(
                    "8H", *self.client_read_input_registers(0x2022, "8H")
                )

            else:
                reply_time = struct.pack(
                    "8H", *self.client_read_input_registers(0x2080, "8H")
                )
                reply_date = struct.pack(
                    "8H", *self.client_read_input_registers(0x2089, "8H")
                )

            self.firmware["time"] = "/".join(
                (x.decode("utf-8") for x in reply_time.split(b"\x00") if x)
            )
            self.firmware["date"] = "/".join(
                (x.decode("utf-8") for x in reply_date.split(b"\x00") if x)
            )
    ...
    def update_read_table(self):
        ...
        if total_digi_in > 0:
            digi_in_reading = self.client_read_discrete_inputs(0, total_digi_in)

            value_table["DIGI_IN"] = digi_in_reading

        if total_digi_out > 0:
            if self.modules_config.modules[0] == "750-8001":
                digi_out_reading = self.client_read_coils(0x0, total_digi_out)
            else:
        ...
        if total_ana_out > 0:
            if self.modules_config.modules[0] == "750-8001":
                ana_out_reading = self.client_read_holding_registers(
                    0x0, total_ana_out * "H"
                )
            else:
    ...
    def get(self, *logical_names, convert_values=True, flat=True, cached=False):
        ...
        if not cached or (time.perf_counter() - self.last_read) > self.polling_time:
        ...
        for name in logical_names:
            ...
            for chann in channels_to_read:
                value = (
                    []
                )  # normally is a single value, with encoder could be two values
    ...
    def devreadnocachedigi(self, key):
        # Doing a digital read on an analog channel gives the raw bit value
        # (not converted in voltage, temperature ...)
    ...
    def devwccomm(self, args, sleep_time=0.1):
        ...
        Args:
            ...
            sleep_time(float): delay between writing requests and reading
        ...
        Note: as the logic was implemented through reverse engineering,
              some parts could be not accurate.
        """
        ...
        data = []
        data.append(0xA5A5)
        # print(f"command in wago.py = {command}")
        ...
        while True:
            ...
            try:
                (
                    ...
                    command_executed,
                    registers_to_read,
                ) = self.client_read_input_registers(
                    addr, "H" * size, timeout=self.timeout
                )
            except Exception:
                log_debug(
        ...
        for logical_device in self.modules_config.read_table.keys():
            for logical_channel in self.modules_config.read_table[
                ...
                (
                    ...
                    physical_module,
                    physical_channel,
                ) = self.modules_config.devlog2hard(
    ...
    def _check_mapping(module1: str, module2: str) -> bool:
        ...
        descr_mod_isinput = "Input" in descr_mod
        descr_mod_isoutput = "Output" in descr_mod
    ...
    def client_read_discrete_inputs(self, *args, **kwargs):
        with self.lock:
...
class Wago(SamplingCounterController):
    ...
    def __interlock_load_config(self):
        try:
            ...
            try:
                ...
                raise RuntimeError("Interlock config parsing error") from exc
    ...
    def __info__(self):
        ...
        try:
            ...
            if "DOES NOT match" in self.controller.status():
                repr_ += "\n\n** WARNING: Given mapping DOES NOT match Wago attached modules **"
                repr_ += (
    ...
    def interlock_upload(self, ask=True):
        ...
        try:
            ...
            raise AttributeError(
                "Interlock configuration is not found in Beacon"
            ) from err
        try:
            ...
            are_equal, _ = compare(self._interlocks_on_beacon, interlocks_on_plc)
        if are_equal:
    ...
    def interlock_to_yml(self):
        ...
        except MissingFirmware as err:
            raise MissingFirmware("No ISG Firmware loaded into wago") from err
