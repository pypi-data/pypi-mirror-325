...
_motion_timing_logger = logging.getLogger("bliss.motion.timing")


@contextmanager
def motion_bench(msg: str):
    now = time.perf_counter()
    try:
        yield
    finally:
        elapsed = int((time.perf_counter() - now) * 1000)
        _motion_timing_logger.info(f"TIMING: {msg:20s} {elapsed:6d} ms")
...
class GroupMove:
    ...
    def _fill_motions_dict(
        ...
        _motion_logger.info(
            f"=== ENTER level {level}: inputs={[x.name for x in axis_pos_dict]} found={[x.name for x in axes_motions]} pending={[x.name for x in remaining_axes]}"
        )
        ...
        for axis, target_pos in axis_pos_dict.items():
            ...
            if motion is None:  # motion can be None if axis is not supposed to move
                ...
                _motion_logger.info(f" Motion for axis {axis.name} is None")
                continue

            elif axes_motions.get(axis):  # a motion for that axis already exist
                cur_motion = axes_motions.get(axis)
                _motion_logger.info(f" Motion for axis {axis.name} already exist")
                ...
                _motion_logger.info(f" Found motion for axis {axis.name}")

            if isinstance(axis, CalcAxis):
                _motion_logger.info(f" Axis {axis.name} is CalcAxis")
                step1_ok = True
                param_motions = []
                ...
                for param in axis.controller.params:
                    if (
                        ...
                        _motion_logger.info(
                            f" Axis {axis.name} has a param {param.name} with motion "
                        )
                        if not axes_motions.get(
                            ...
                            step1_ok = False
                            _motion_logger.info(
                                f" But param {param.name} motion is not known yet, axis {axis.name} added to pending list"
                            )
                ...
                if step1_ok:
                    ...
                    for pseudo in axis.controller.pseudos:
                        if pseudo in moving_axes:
                            _motion_logger.info(f" Found involved pseudo {pseudo.name}")
                            if not axes_motions.get(pseudo):
                                step2_ok = False
                                _motion_logger.info(
                                    f" But pseudo {pseudo.name} motion is not known yet, axis {axis.name} added to pending list"
                                )
                                break
                            else:
                    ...
                    if step2_ok:
                        ...
                        for pseudo in axis.controller.pseudos:
                            if remaining_axes.pop(pseudo, None):
                                _motion_logger.info(
                                    f" pseudo {pseudo.name} has been cleared from pending list"
                                )
                        motions = pseudo_motions + param_motions
                        _motion_logger.info(
                            f" Computing reals motions for controller {axis.controller.name} from motions of {[x.axis.name for x in motions]}"
                        )
                        ...
                        # discard this calc axis if all its dependencies are not moving
                        dependencies = set(real_move_dict.keys())
                        if not (dependencies - not_moving_axes):
                            not_moving_axes.add(axis)
                            _motion_logger.info(
                                f" Discarding motion of {axis.name} because {[x.name for x in dependencies]} are not moving"
                            )
                            del axes_motions[axis]

        _motion_logger.info(f"=== EXIT level {level}")
    ...
    def _find_sub_axes(self, axis_pos_dict, found_axes):
        ...
        for axis in axes_list:
            if isinstance(axis, CalcAxis):
                ...
                dependencies_pos_dict = axis.controller._get_real_axes_move_dict(
    ...
    def _resolve_motions_tree(self, axis_pos_dict, relative, polling_time):
        ...
        found_axes = []
        # === Find sub axes (requires absolute positions)
        self._find_sub_axes(
            {
        ...
        # === DEBUG ==========================
        _motion_logger.info(f"Axes involved in motion: {[x.name for x in found_axes]}")
        # ====================================
        ...
        while remaining_axes:
            _motion_logger.info(
                f"=== DEALING with remaining axes {[x.name for x in remaining_axes]}"
            )
    ...
    def move(
        ...
        # ============ NOTES ABOUT MOTION MECHANISMS ==================================================
        # 1) _resolve_motions_tree:
        #    Obtain 'motions_dict' (motion objects ordered by controller).
        #    It includes all CalcAxis dependencies, down to physical axes.
        #    Axes already in place are filtered.
        #
        # 2) cancel move if motions_dict is empty after filtering
        #
        # 3) for remaining motions:
        ...
        #    - "internal_move_done" triggers the _real_axis_move_done() callback on CalcController
        #    - _set_moving_state() is called on pseudos only if all their reals are not moving
            ...
            try:
                ...
                for motion in all_motions:
                    ...
                    if target_pos is not None:
                        restore_axes[motion.axis] = motion.axis._set_position
                ...
                for ax, setpos in restore_axes.items():
                    ax._set_move_done()
    ...
    def _end_of_move(self, _=None):
        with motion_bench("end_of_move"):
            ...
            try:
                ...
                def parallel_set_move_done():
                    tasks = [gevent.spawn(ax._set_move_done) for ax in axl]
                    gevent.joinall(tasks)

                with contextlib.ExitStack() as final_tasks:

                    # final_tasks callbacks will be called in reverse order
                    ...
                    final_tasks.callback(parallel_set_move_done)
        ...
        try:
            ...
            with capture_exceptions(raise_index=0) as capture:
                ...
                if not self._interrupted_move:
                    ...
                    with capture():
                        with motion_bench("do_backlash_move"):
