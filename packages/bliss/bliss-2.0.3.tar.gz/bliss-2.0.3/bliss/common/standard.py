...
from bliss.common.logtools import user_print  # noqa: F401
...
WhereAll = namedtuple(
    "WhereAll", "axis disabled error unit user_position dial_position"
)
WhereMotor = namedtuple(
    "WhereMotor",
    "axis disabled error unit user_position user_high_limit user_low_limit offset, dial_position dial_high_limit dial_low_limit",
)
...
def iter_axes_position(*axes, **kwargs):
    ...
    for axis in global_map.get_axis_objects_iter(*axes):
        ...
        disabled_state = axis.disabled or "DISABLED" in axis_state
        error = None
...
def mvr(*args):
    ...
    Example::
        ...
        >>> mv(th, 180, chi, 90)
    """
    _move(list(grouped(args, 2)), relative=True)
...
def mvd(*args):
    ...
    Arguments are interleaved axis and respective relative target position.
    """
    _move(list(grouped(args, 2)), dial=True)
...
def mvdr(*args):
    ...
    Arguments are interleaved axis and respective relative target position.
    """
    _move(list(grouped(args, 2)), relative=True, dial=True)
...
def _move(
    ...
    """
    Move a list of axis or group at a specific position.

    Arguments:
        axis_pos_list: List of axis object and target position, or list of
                       group and target position
    ...
    if isinstance(axis_pos_list, (dict, collections.UserDict)):
        motor_pos = axis_pos_list
    else:
        if len(axis_pos_list) == 2 and is_motor_group(axis_pos_list[0]):
            group = axis_pos_list[0]
            axis_pos_list = axis_pos_list[1]
        motor_pos = dict()
        for m, p in axis_pos_list:
