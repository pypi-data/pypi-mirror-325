...
from bliss.shell.formatters import ansi as _ansi
...
class _UserNamespaceMeta(type):
    """This metaclass forces all derived classes to have __slots__"""
...
class UserNamespace(metaclass=_UserNamespaceMeta):
    def __new__(cls, **kwargs):
        cls = types.new_class(cls.__name__, (cls,), {"metaclass": _UserNamespaceMeta})
        ...
        cls.__slots__ = tuple(kwargs)
        return super(UserNamespace, cls).__new__(cls)

    def __add__(self, other):
        if isinstance(other, UserNamespace):
            return self.__class__(**{**self._asdict(), **other._asdict()})
        elif isinstance(other, Mapping):
            return self.__class__(**{**self._asdict(), **other})
        else:
            raise TypeError
...
class _IterableNamespaceMeta(_UserNamespaceMeta, abc.ABCMeta):
    """This is a new metaclass, grouping base classes meta, in order to solve the metaclass conflict"""

    pass


class IterableNamespace(Sequence, UserNamespace, metaclass=_IterableNamespaceMeta):
...
def is_sametype(val1, val2):
    if is_basictype(val1) and is_basictype(val2) and (type(val1) is type(val2)):
        return True
...
def _float_format(template_number):
    try:
        digits = int(numpy.ceil(-numpy.log10(template_number)))
    except ZeroDivisionError:
        digits = 0
    except TypeError:
        print(f"template_number must be a number, not a {type(template_number)}")
    return f".0{digits}f"
...
# For compatibility with BLISS <= 2.0
ColorTags = _ansi.ColorTags
PURPLE = _ansi.PURPLE
ORANGE = _ansi.ORANGE
CYAN = _ansi.CYAN
DARKCYAN = _ansi.DARKCYAN
BLUE = _ansi.BLUE
GREEN = _ansi.GREEN
YELLOW = _ansi.YELLOW
RED = _ansi.RED
