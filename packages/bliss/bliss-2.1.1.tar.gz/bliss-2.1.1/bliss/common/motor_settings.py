...
class AxisSettings:
    ...
    def _create_channel(self, setting_name):
        ...
        chan = Channel(chan_name, self_updates=False)
    ...
    def _get_last_local(self, setting_name):
        """Returns local cache related to the motion.

        This allows to get the values without direct access to the hardware.
        and even with `NoSettingsAxis`.

        This is used by the live display during the motion.
        """
        if setting_name == "state":
        ...
        if setting_name == "dial_position":
            return self.__prev_dial
        raise KeyError(f"Property {setting_name} is not available")
    ...
    def set(self, *settings_values):
        ...
        for setting_name, value in events_dict.items():
            ...
            if setting_name not in disabled_settings.names:
    ...
    def clear(self, setting_name, clear_cache=True):
        disabled_settings = self._disabled_settings
        ...
        try:
            self._beacon_channels[setting_name]._set_raw_value(None)
        except KeyError:
            pass
    ...
    def disable_cache(self, setting_name, flag=True):
        ...
        self.clear(setting_name, clear_cache=False)
