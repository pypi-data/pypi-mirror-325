...
class PI_E753(pi_gcs.Communication, pi_gcs.Recorder, Controller):
    ...
    """
    ANALOG INPUT - activation is required when PI controller is used as a MOCO actuator
    """

    @object_method(types_info=("None", "None"))
    def enable_analog_input(self, axis):
        self.command("CCL 1 advanced")
        self.command("SPA 1 0x6000500 2")

    @object_method(types_info=("None", "None"))
    def disable_analog_input(self, axis):
        self.command("CCL 1 advanced")
        self.command("SPA 1 0x6000500 0")
    ...
    def __info__(self):
        ...
        the PC (no DHCP server is present), OR there is no DHCP server answer
        (for instance you did not provide a MAC address in your DHCP server configuration)
        the current IP address of the E-753 will be as follows:
        ...
        mac = ifc[1].split("=")[1]
        ip = ifc[0].split("=")[1]
        ipmode = ifc[2].split("=")[1]

        info_str = "CONTROLLER:\n"
        info_str += f"     ID: {idn}\n"
        info_str += f"     MAC address: {mac}\n"
        info_str += f"     IP address: {ip}\n"
        _start_mode = "use IPADR" if ipmode == "0" else "use default -> 192.168.0.1"
        info_str += f"     IP start: IPSTART={ipmode}({_start_mode})\n"
    ...
    def start_wave(
        ...
        Arguments:
            wavetype: can be `LIN` for a linear or `SIN` for a sinusoidal.
            offset: amplitude motor displacement
