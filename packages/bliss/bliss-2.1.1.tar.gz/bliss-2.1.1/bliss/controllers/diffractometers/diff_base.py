...
class Diffractometer(BlissController, HasMetadataForScan):
    ...
    def __init__(self, config):
        ...
        global_map.register(self, children_list=[self._scc])
    ...
    def geo_limits_sync_with_reals(self):
        """Synchronize geo_limits to current real axes limits.
        Useful when axes limits are larger than current geo limits.
        Still, geo limits will be limited to the broadest range [-360, 360].
        """

        self._motor_calc.update_limits(force_sync_with_reals=True)
    ...
    def get_axis(self, axis_tag):
        ...
        if motor_name is None:
            raise ValueError("Unknown axis name [{0}]".format(axis_tag))
