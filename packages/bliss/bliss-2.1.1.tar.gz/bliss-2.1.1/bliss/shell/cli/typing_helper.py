...
if typing.TYPE_CHECKING:
    from bliss.repl.cli.bliss_repl import BlissRepl


def full_ns(repl):
    return {**builtins.__dict__, **repl.get_globals(), **repl.get_locals()}
...
def get_obj_from_text(text, ns):
    ...
    if text_split[0] in ns:
        ...
        for subtext in text_split[1:]:
            try:
                obj = safe_getattr(obj, subtext)
            except AttributeError:
                # subtext is not an attribute
                return
            except Exception:
                return
        return obj
...
class TypingHelper(object):
    def __init__(self, blissrepl: BlissRepl):
        self.validator = PythonValidator()
        self.add_helper_key_binding(blissrepl)
        self.blissrepl: BlissRepl = blissrepl

    @property
    def is_active(self):
        return cli_config.typing_helper_active
    ...
    def add_helper_key_binding(self, repl):
        ...
        def _(event):
            ...
            cli_config.typing_helper_active = not cli_config.typing_helper_active
