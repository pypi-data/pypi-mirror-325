...
class StandardScanAcquisitionChain(AcquisitionChain):
    def __init__(self, parallel_prepare: bool = True):
        super().__init__(parallel_prepare=parallel_prepare)
        self._sw_timer_master: SoftwareTimerMaster | None = None

    @property
    def timer(self) -> SoftwareTimerMaster:
        return self._sw_timer_master

    @timer.setter
    def timer(self, value: SoftwareTimerMaster):
        if isinstance(value, SoftwareTimerMaster):
            self._sw_timer_master = value
...
class DefaultAcquisitionChain:
    ...
    def set_settings(self, settings_list):
        ...
        for device_settings in settings_list:
            ...
            if master is not None:
                if hasattr(master, "_get_default_chain_counter_controller"):
                    master = master._get_default_chain_counter_controller()

                elif not isinstance(master, CounterController):
                    msg = "default_chain config: master object must be a CounterController or implement '_get_default_chain_counter_controller',"
            ...
            if isinstance(device, Counter):
                ...
                controller = device
            elif hasattr(device, "_get_default_chain_counter_controller"):
                controller = device._get_default_chain_counter_controller()
            else:
                msg = "default_chain config: device object must be a CounterController, a Counter or implement '_get_default_chain_counter_controller'"
    ...
    def get(self, scan_pars, counter_args, top_master=None):
        ...
        chain = StandardScanAcquisitionChain()
        ...
        for node in topnodes:
            ...
            # print(f"====== for {node.controller} {node.controller.counters._fields} in topnodes")
