...
from bliss.flint.widgets.viewer.rois.lima_arc_roi import LimaArcRoi
from bliss.flint.widgets.viewer.rois.lima_vprofile_roi import LimaVProfileRoi
from bliss.flint.widgets.viewer.rois.lima_hprofile_roi import LimaHProfileRoi
from bliss.flint.widgets.viewer.rois.lima_rect_roi import LimaRectRoi
...
class ShapeSelector(Selector):
    def __init__(self, parent: qt.QWidget | None = None):
        ...
        self._result = None
        self._selection: tuple[tuple[float, float], ...] = ()
        self._itemId = "%s-%s" % (self.__class__.__name__, id(self))
        self._shape: str = ""
        self._validButton: bool = False
        self._cancelButton: bool = False
    ...
    def setValidButton(self, enabled: bool):
        self._validButton = enabled

    def setCancelButton(self, enabled: bool):
        self._cancelButton = enabled
    ...
    def selection(self):
        ...
        :return: Nx2 (x, y) coordinates or an empty tuple (or True/False).
        """
    ...
    def done(self, result):
        """Closes the selection and sets its result to `result`."""
        self.reset()
        self._result = result
        self.stop()
    ...
    def start(self):
        ...
        if self._cancelButton:
            ...
            action.triggered.connect(self._cancel)
            action.setObjectName("interaction-cancel")
            action.setIcon(icons.getQIcon("flint:icons/action-cancel"))
            action.setToolTip("Cancel")
            self._toolbar.addAction(action)

        if self._validButton:
            action = qt.QAction(self)
            action.triggered.connect(self._valid)
            action.setObjectName("interaction-validate")
            action.setIcon(icons.getQIcon("flint:icons/action-valid"))
            action.setToolTip("Validate")
            self._toolbar.addAction(action)
...
class ShapesSelector(Selector):
    def __init__(self, parent: qt.QWidget | None = None):
        ...
        self.__initialShapes: list[lima_roi._BaseRoi | mosca_rois.McaRoi] = []
        self.__timeout = None
        self.__dock = None
        self.__roiWidget = None
        self.__selection = None
        self.__kinds: list[RegionOfInterest] = []
        self.__mapping = {
            "rectangle": RectangleROI,
            "lima-rectangle": LimaRectRoi,
            "lima-arc": LimaArcRoi,
            "lima-vertical-profile": LimaVProfileRoi,
            "lima-horizontal-profile": LimaHProfileRoi,
            "mosca-range": MoscaRangeRoi,
        }
        self.__deviceName = None

    def setKinds(self, kinds: list[str]):
        self.__kinds.clear()
        for kind in kinds:
    ...
    def setInitialShapes(
        self,
        initialShapes: Sequence[lima_roi._BaseRoi | mosca_rois.McaRoi] | None = None,
    ):
        if initialShapes is None:
            initialShapes = []
        self.__initialShapes = list(initialShapes)
    ...
    def __roisToGui(self, shapes: Sequence[object]) -> Sequence[RegionOfInterest]:
        rois = []
        for shape in shapes:
            roi = roi_helper.roiToGui(shape)
            if roi is None:
                _logger.error("Unknown ROI kind '%s'", type(shape))
                continue
            rois.append(roi)
        return rois
    ...
    def start(self):
        ...
        roiWidget.selectionCancelled.connect(self.__selectionCancelled)
        dock.show()
...
class MaskImageSelector(Selector):
    def __init__(self, parent: PlotWidget | None = None):
        assert isinstance(parent, PlotWidget)
        super(MaskImageSelector, self).__init__(parent=parent)
        self.__timeout = None
        self.__selection = None
        self.__dock: MaskToolsDockWidget | None = None
        self.__initialMask: numpy.ndarray | None = None
    ...
    def __selectionCancelled(self):
        if not self.__started:
            return
        assert self.__dock is not None
        self.stop()
        self.__selection = None
        self.selectionFinished.emit()
