...
class FlintApi:
    ...
    def get_bliss_version(self) -> str:
        ...
        return str(bliss.release.version)
    ...
    def get_live_plot_detector(
        ...
        workspace = self.__flintModel.workspace()
        for iwidget, widget in enumerate(workspace.widgets()):
            if not isinstance(widget, interfaces.HasScan) or not isinstance(
                widget, interfaces.HasPlotModel
            ):
                # Skip widgets which does not display scans (like profile)
                continue
            if not isinstance(widget, interfaces.HasScan) or not hasattr(
                widget, "deviceName"
            ):
                # Skip widgets which does not display scans (like profile)
                continue
    ...
    def test_active(self, plot_id, qaction: str):
    ...
    def is_plot_exists(self, plot_id: str) -> bool:
        """Deprecated API in BLISS 1.10

        Use `plot_exists` instead.
        """
        return self.plot_exists(plot_id)

    def _get_plot_class(self, class_name: str):
        """Returns a plot class from a user class name."""
        try:
        ...
        if not issubclass(class_obj, qt.QWidget):
            raise ValueError(f"Class name {class_name} is not a Qt Widget class")
        return class_obj

    def plot_exists(self, plot_id: str, class_name: str | None = None) -> bool:
        ...
        if isinstance(plot_id, str) and plot_id.startswith("live:"):
            ...
            if class_name is not None:
                class_obj = self._get_plot_class(class_name)
                if not isinstance(custom_plot.widget(), class_obj):
                    raise RuntimeError(
                        f"The existing plot `{plot_id}` use an unexpected type"
                    )
    ...
    def update_user_data(
        self, plot_id, unique_name: str, channel_name: str, ydata: numpy.ndarray
    ):
    ...
    def set_displayed_channels(self, plot_id, channel_names, role: str | None = None):
        """Deprecated since BLISS 1.12

        setDisplayedChannels can be called directly from the plot.
        """
        return self.run_method(
            plot_id,
            "setDisplayedChannels",
            [],
            {"channel_names": channel_names, "role": role},
        )

    def get_displayed_channels(self, plot_id):
        """Deprecated since BLISS 1.12

        getDisplayedChannels can be called directly from the plot.
        """
        return self.run_method(plot_id, "getDisplayedChannels", [], {})
    ...
    def request_select_shapes(
        self,
        plot_id,
        initial_shapes: Sequence[lima_roi._BaseRoi | mosca_rois.McaRoi] = (),
        kinds: str | list[str] = "rectangle",
        timeout=None,
    ) -> str:
        """
        ...
        if isinstance(plot, plot_item_model.PLOTS_WITH_DEVICENAME):
    ...
    def request_select_data_index(self, plot_id: str) -> str:
        """
        Request the selection of a point.
        ...
        Return:
            ...
            The event result is a index of the data selection. A point.
            If nothing is selected, None is returned.
        """
        from bliss.flint.viewers.live_scatter.data_index_selector import (
            DataIndexSelector,
        )

        plot = self._get_plot_widget(plot_id)
        silxPlot = plot._silxPlot()
        selector = DataIndexSelector(silxPlot)
        return self.__request_selector(plot_id, selector)

    def request_select_shape(
        self, plot_id: str, shape: str, valid: bool = False, cancel: bool = False
    ) -> str:
        """
        ...
        if valid:
            selector.setValidButton(True)
        if cancel:
            selector.setCancelButton(True)
        return self.__request_selector(plot_id, selector)

    def request_select_mask_image(
        self,
        plot_id: str,
        initial_mask: numpy.ndarray | None = None,
