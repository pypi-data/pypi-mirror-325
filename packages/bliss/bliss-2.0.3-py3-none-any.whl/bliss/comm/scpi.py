...
from bliss.comm.gpib import Gpib, TangoDeviceServerAscii
from bliss.comm.util import get_interface
from bliss.comm.exceptions import CommunicationError, CommunicationTimeout
from bliss.common.logtools import log_debug_data, log_exception
...
class Commands(object):
    ...
    *kwargs* should also be SCPI command expressions; *kwargs* values should be
    ...
    def get_max_command(self, cmd_name):
        cmd_expr = self.get_command_expression(cmd_name)
        return self.command_expressions[cmd_expr]["max_command"]

    def get_min_command(self, cmd_name):
        cmd_expr = self.get_command_expression(cmd_name)
        return self.command_expressions[cmd_expr]["min_command"]
...
class SCPI:
    ...
    def __init__(self, interface, commands=COMMANDS, **kwargs):
        ...
        self._cmd_ack = kwargs.get("cmd_ack", False)
        self._contexts = []
    ...
    def read(self, *msgs, **kwargs):
        ...
        """
        if self._contexts:
            self._contexts[-1]["commands"].extend(msgs)
            return
        raw = kwargs.get("raw", False)
        kwargs.setdefault("eol", self._eol)
        kwargs.setdefault("strict_query", self._strict_query)
    ...
    def write(self, *msgs, **kwargs):
        ...
        if self._cmd_ack:
            self.interface.write_readline(msg.encode())
        else:
    ...
    def get_syst_err(self):
        return self["SYST:ERR"]
...
class BaseSCPIDevice:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(interface, Gpib):
            if interface.gpib_type == Gpib.GpibType.TANGO_DEVICE_SERVER:
                # ASCII commands need a specific Tango GPIB client,
                # which calls the 'good' commands to send/receive ASCII strings
                # so, we change interface 'raw_handler' to the good handler
                global_map.unregister(interface)
                interface._raw_handler = TangoDeviceServerAscii(
                    interface, **interface._gpib_kwargs
                )
                interface._raw_handler.init()
