...
def plot_image(
    ...
    Arguments:
        ...
        selected: If true the plot will be selected when created
        closeable: If true the plot will be closeable
        side_histogram: If true a side histogram are displayed
    """
    flint = flint_proxy.get_flint()
    p = flint.get_plot(
        "image",
    ...
    p.side_histogram_displayed = side_histogram
    if data is not None:
        p.set_data(data, origin=origin, scale=scale)
    return p


def plot_image_with_histogram(*args, **kwargs):
    """Compatibility with BLISS <= 1.8"""
    deprecated_warning(
        kind="function",
        name="plot_image_with_histogram",
        reason="`plot_image` can be used with `side_histogram=True/False`. It will be removed in near future",
        since_version="1.9.0",
        ...
        if flint is not None:
            try:
                try:
                    ...
                    plot._plotselect(channel_names)
            except Exception:
...
def meshselect(*counters):
    ...
    * flint display (bliss/flint/plot1d.py)
    ...
    if flint_proxy.check_flint():
        ...
        if flint is not None:
            try:
                plot = flint.get_live_plot("default-scatter")
                if plot is not None:
                    plot._plotselect(channel_names)
            except Exception:
                FLINT_LOGGER.error("Error while executing meshselect", exc_info=True)
    ...
    if flint_proxy.check_flint():
        ...
        if flint is not None:
            try:
                ...
                FLINT_LOGGER.error("Error while executing replot", exc_info=True)
...
def display_motor(
    ...
    if is_bliss_shell() and scan_display_params.motor_position:
        try:
            ...
            if not silent:
                print(
                    "The object %s have no obvious channel. Plot marker skipped."
                    % (axis,)
                )
            channel_name = None
        if channel_name is not None:
            try:
                f = get_flint()
                plot = f.get_live_plot("default-curve")
