        ...
        if line:
            if interlock_parse_relay_line(line) is not None:
                ...
                (
                    ...
                    iflags,
                    description,
                ) = interlock_parse_relay_line(line)
                logical_device_key = modules_config.devname2key(logical_device)
                flags = iflags
...
def _interlock_channel_info_from_plc(
    ...
    if len(get_conf_output) > 6:
        info["type"]["type"] = word_to_2ch(get_conf_output[4])
        info["dac"] = get_conf_output[6]
        info["dac_scale"] = two_short_to_float(get_conf_output[7], get_conf_output[8])
        info["dac_offset"] = two_short_to_float(get_conf_output[9], get_conf_output[10])
                    ...
                    if ck in ("low_limit", "high_limit"):  # ,"dac", "dac_scale"):
                        if isinstance(val1, (int, float)):
                            ...
                            try:
                                ...
                                pass
                        if isinstance(val2, (int, float, numpy.int64)):
                            val2 = to_signed(val2)
...
def interlock_download(
    ...
    for i in range(1, registered_inst + 1):
        ...
        for j in range(1, n_of_channels + 1):
            ...
            if is_monitor(flags):
                interlock_channel_info["dac"] += 1
                interlock_channel_info["dac"] = "dac" + str(
                    interlock_channel_info["dac"]
                )
    ...
    for interlock in interlock_list:
        ...
        for channel in interlock["channels"]:
            ...
            if not channel["type"]["digital"]:
                params += (
                    to_unsigned(channel["low_limit"]),
                    to_unsigned(channel["high_limit"]),
                )
                type_ = channel["type"]["type"]

                params += ((ord(type_[0]) << 8) + ord(type_[1]),)

            if is_monitor(channel["flags"]):
                dac_name = channel["dac"]
                dac_number = int(dac_name[3]) - 1
                params += (dac_number,)
                dac_scale0, dac_scale1 = float_to_2ushort(channel["dac_scale"])
                params += (dac_scale0, dac_scale1)
                dac_offset0, dac_offset1 = float_to_2ushort(channel["dac_offset"])
                params += (dac_offset0, dac_offset1)
...
def interlock_show(wc_name: str, interlock_info: list):
    ...
    for info in sorted(interlock_info, key=lambda i: i["num"]):
        num, description, logical_device = (
            info["num"],
            info["description"],
            info["logical_device"],
            # info["logical_device_channel"], ## Apparently always 0. So not needed ?
        )
        ...
        out += f"    Alarm relay = {logical_device} {bchstring}".ljust(79)
        out += f"[{value}]\n"
        out += f"    State = {state}\n"
        ...
        for ch in channels:
            ...
            out += chline.ljust(79) + f"[{ch_value}]\n"
        out += "\n"
