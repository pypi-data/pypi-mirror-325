...
class UndulatorAxis(NoSettingsAxis):
    """
    NoSettingsAxis -> Settings of undulators axes are maneged by tango device server.
    """
    ...
    def sync_hard(self):
        ...
        if "DISABLED" in state:
            ...
            log_warning("undulator %s is disabled, no position update", self.name)
        else:
            super().sync_hard()
