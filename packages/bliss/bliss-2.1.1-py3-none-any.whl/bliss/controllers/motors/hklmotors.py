...
class HKLMotors(CalcController):
    ...
    def update_limits(self, *args, force_sync_with_reals=False):
        ...
        for axis in self.reals:
            ...
            if name != "energy":
                ...
                if (
                    not self.diffracto.geometry.is_axis_limits_initialized(name)
                    or force_sync_with_reals
                ):
                    geo_limits[name] = max(-360, axis_limits[0]), min(
                        360, axis_limits[1]
                    )
                else:
                    geo_limits[name] = (
                        max(-360, max(axis_limits[0], geo_limits[name][0])),
                        min(360, min(axis_limits[1], geo_limits[name][1])),
                    )
