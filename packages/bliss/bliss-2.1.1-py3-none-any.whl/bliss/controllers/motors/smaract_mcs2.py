...
class PositionerType(enum.IntEnum):
    # SR1908 rotation supported by any other stage type (open loop), i.e. SR20
    SL_S1SS = 300  # linear positioner former S fo MCS1, SLCxxxxs
    SR_S1S5S = 309  # small rotary positioner, former SR20 for MCS1
    SR_S1S6S = 312  # small rotary positioner, former SR2812, SR5714
    SR_D1S6S = 313  # small rotary positioner, former SR2812ds1
    SG_D1S1S = 325  # Hopefull SGO = CGO-60 ?
    SG_D1S2S = 328  # S/CGO-77.5 ?
    SR_S1L2S = 354  # SR-4011l
    SR_D1L2S = 355  # SR-4011dl


RotaryPositioners = (
    PositionerType.SR_S1S5S,
    PositionerType.SR_S1S6S,
    PositionerType.SR_D1S6S,
    PositionerType.SR_S1L2S,
    PositionerType.SR_D1L2S,
)
...
class Positioner:
    ...
    @property
    def open_loop_step_frequency(self):
        return int(self.get_property(":STEP:FREQ"))

    @open_loop_step_frequency.setter
    def open_loop_step_frequency(self, value):
        """Step frequency = speed, default frequency is 1000Hz"""
        if value < 1 or value > 20000:
            raise RuntimeError("Step Frequency must be between 1 and 20000Hz")

        self.set_property(f":STEP:FREQ {value}")

    @property
    def open_loop_step_amplitude(self):
        return int(self.get_property(":STEP:AMPL"))

    @open_loop_step_amplitude.setter
    def open_loop_step_amplitude(self, value):
        """Step amplitude = step size, default amplitude is 65535 = 100V"""
        if value < 1 or value > 65535:
            raise RuntimeError("Step Amplitude must be between 1 and 65535")

        self.set_property(f":STEP:AMPL {value}")
...
class SmarAct_MCS2(Controller):
    ...
    def initialize_hardware_axis(self, axis):
        ...
        if status.SENSOR_PRESENT:
            # force closed-loop absolute versus relative position
            self.write(":MMOD 0", axis.positioner.channel)
        else:
            # otherwise enable open loop movements
            self.write(":MMOD 4", axis.positioner.channel)
        self._open_loop = not status.SENSOR_PRESENT
    ...
    def get_axis_info(self, axis):
        ...
        if self._open_loop:
            amplitude = axis.positioner.open_loop_step_amplitude
            amplitude_v = round(amplitude / 65535 * 100, 1)
            info_str += "       Open Loop Movement:\n"
            info_str += f"         Step Frequency: {axis.positioner.open_loop_step_frequency} Hz"
            info_str += f"    Step Amplitude: {amplitude} = {amplitude_v}V\n"
    ...
    def start_one(self, motion):
        ...
        # If we are in open loop always move by the delta
        if self._open_loop:
            channel.move_absolute(motion.delta)
            self._current_position = motion.target_pos
        else:
