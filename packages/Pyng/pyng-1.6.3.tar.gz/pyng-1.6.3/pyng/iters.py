...
def blocks_from(iterable, bufsize):
    """
    Partition the input iterable into a sequence of tuples, each up to length
    bufsize. Specifically, for q, r = divmod(len(iterable), bufsize), the
    first q blocks are each of length bufsize. There will be a final block of
    length r, but only if r is nonzero.
    """
    step_iter = iter(iterable)
    block = tuple(itertools.islice(step_iter, bufsize))
    while len(block) == bufsize:
        yield block
        block = tuple(itertools.islice(step_iter, bufsize))
    if block:
        yield block
