...
#import cgitb                   # conditional, see below
import sys
...
class Retry(object):
    """
    Retry presents yet another retry paradigm. First instantiate a Retry
    object, binding the callable and its arguments, then iterate over fails().
    The body of the 'for' loop is ... whatever you want to do between retries:
    log a message, sleep(), whatever. Or count failures and break the loop.

    When you drop out of the 'for' loop, the return value of the call is in
    the Retry instance's 'result' property. If the last call still raised an
    exception, accessing 'result' re-raises the same exception.

    If the callable succeeds on the first call, of course the loop will
    iterate 0 times, and 'result' will contain its return value.

    Iterate over zip(range(n), fails()) to constrain the number of retries.

    If you access 'result' without calling fails(), Retry directly calls the
    bound callable and either returns its result or raises the exception -- no
    retries. In other words, unless you call fails():

    Retry(func, *args, **kwds).result
    is equivalent to
    functools.partial(func, *args, **kwds)()
    """
    def __init__(self, func, *args, **kwds):
        """
        Bind a callable and its arguments to call, potentially multiple times,
        as long as it raises (specified) exception(s).

        func: any Python callable you (might) need to retry calling
        args: positional arguments to pass to func
        kwds: keyword arguments to pass to func
        """
        self.callable = functools.wraps(func)(functools.partial(func, *args, **kwds))
        # handle the case when 'result' is accessed without ever calling
        # 'fails()'
        self._result = self.callable

    def fails(self, exc=Exception):
        """
        Each time you request an item from the fails() generator, it retries
        a call to the callable bound in the constructor. The peculiar thing
        about fails() is that if the bound callable succeeds, iteration
        terminates. If it fails (raises 'exc'), fails() yields the caught
        exception object.

        exc: an Exception subclass (default Exception), or a tuple of such
             Exception subclasses, that fails() should catch and yield

        If the bound callable raises an exception NOT caught by 'except exc',
        the exception simply propagates. In other words, the 'exc' parameter
        lets the caller distinguish exceptions that can reasonably be retried
        from those that must fail right away.

        Usage:

        retry = Retry(myfunc, 'abc', special=True)
        for i, exc in zip(range(3), retry.fails(WhoopsError)):
            print(f'call {i+1} failed, sleeping before retry')
            time.sleep(1)
        # Here either myfunc() returned a result, or it raised WhoopsError 3
        # successive times (per the range() call above). If it returned a
        # result, the following access will obtain it. If it raised
        # WhoopsError every time, this statement will re-raise WhoopsError.
        value = retry.result

        Neither __init__() nor fails() supports any equivalent to our
        retry_func(when=) parameter because it's unnecessary. retry_func()
        calls when(exc) to discover whether it should continue looping or
        propagate the exception to its caller. With Retry, your loop body can
        directly examine the yielded exception object 'exc' and, if it should
        fail instead of retrying, 'raise exc'.
        """
        # keep on retrying as long as caller is willing to resume:
        # constraining the iteration is up to caller
        while True:
            try:
                result = self.callable()
            except exc as x:
                # if caller chooses not to resume, result should re-raise
                self._result = functools.partial(self._raise, x)
                yield x
            else:
                # okay, accessing result attribute should return this
                self._result = lambda: result
                # once callable() succeeds, we're done
                break
        # once we've succeeded, calling fails() again is a usage error --
        # point it out to the user
        self.callable = functools.partial(
            self._raise,
            Error('Retry callable succeeded, fails() is done'))

    @property
    def result(self):
        # result property is syntactic sugar for calling _result()
        return self._result()
...
def _raise_from(throw, cause):
    ...
    originally intended for compatibility with Python 2.
    """
...
def exception_default(exc, default, func, *args, **kwds):
    """
    Like suppress() for functions with return values. If calling
    func(*args, **kwds)
    raises 'exc' (which may be a tuple of Exception subclasses), return
    'default' instead of letting the exception propagate.

    Example:

    data = exception_default(OSError, None, lambda: open(filename).read())
    """
    try:
        return func(*args, **kwds)
    except exc:
...
try:
    ...
    class cgitb_Hook(exceptlink):
        ...
        def __init__(self, *args, **kwds):
            ...
            self.handle = cgitb.Hook(*args, **kwds)
