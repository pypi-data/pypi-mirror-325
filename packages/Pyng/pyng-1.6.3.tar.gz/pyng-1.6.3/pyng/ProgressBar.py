...
##         print title
    ...
    def setlabel(self, newstr):
        """
        The difference between label() and setlabel() is that label() assumes
        it will be interleaved with set() calls, and that only when the bar
        must advance (that is, only when set() is called) should we redisplay
        the bar. setlabel() is for use when you want the label redisplayed
        right away, before the next set() call.
        """
        self.label(newstr)
        self.display()
...
class _FakeProgressBarImpl(ProgressBarImplBase):
    ...
    """
    def __init__(self, title, maxval, label, plain=False):
        if not plain:
