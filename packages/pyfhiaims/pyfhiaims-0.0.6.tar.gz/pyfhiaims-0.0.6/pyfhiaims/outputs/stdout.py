...
class AimsImage:
    ...
    def from_spec(cls, ionic_spec: dict[str, Any]):
        ...
        if geometry.lattice_vectors is not None and spec.get("is_metallic", False):
            spec["total_energy"] = results["total_energy"]
            results["total_energy"] = spec.get("corrected_total_energy")

        results["n_scf_iter"] = len(scf)
        results["magmom"] = spec.get("spin", None)
        results["fermi_energy"] = spec.get("chemical_potential", None)
        results["stress"] = spec.get(
            "analytical_stress", spec.get("numerical_stress", None)
        )
        results["dipole"] = spec.get("total_dipole_moment", None)
        results["absolute_dipole"] = spec.get("absolute_dipole_moment", None)
    ...
    def __post_init__(self):
        ...
        if self._geometry.lattice_vectors is not None:
