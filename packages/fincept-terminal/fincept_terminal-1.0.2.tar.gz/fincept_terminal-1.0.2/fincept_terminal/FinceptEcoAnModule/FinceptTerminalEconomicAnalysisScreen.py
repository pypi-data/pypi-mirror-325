...
class DataGovINtab(VerticalScroll):
    ...
    async def on_button_pressed(self, event: Button.Pressed):
        ...
        if button_id == "next_resource_page":
            ...
            if self.current_resource_id:
                ...
                success = await self.download_resource_data(self.current_resource_id, file_name)
                if success:
    ...
    async def download_resource_data(self, resource_id: str, output_file: str) -> bool:
        """
        Download the complete data for a resource and save it to the specified file.
        ...
        try:
            ...
            if output_file.endswith(".csv"):
                await asyncio.to_thread(data.to_csv, output_file, index=False)
            elif output_file.endswith(".json"):
                await asyncio.to_thread(data.to_json, output_file, orient="records", indent=4)
            elif output_file.endswith(".xlsx"):
                await asyncio.to_thread(data.to_excel, output_file, index=False, engine="openpyxl")
            else:
