...
from src.constants import beta_version_percentage
...
def select_llm_client(params, input_doc_type):
    if input_doc_type in beta_version_percentage.keys():
        if random.random() < beta_version_percentage[input_doc_type]:
            return params["LlmClientBeta"]
...
async def extract_data_by_doctype(
    ...
    llm_client = select_llm_client(params, input_doc_type)
