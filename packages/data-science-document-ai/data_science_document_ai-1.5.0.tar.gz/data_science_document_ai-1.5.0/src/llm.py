...
class LlmClient:
    ...
    async def ask_gemini(self, prompt: str, document: str = None, response_schema: dict = None):
        """Ask the Gemini model a question.
        ...
        try:
            ...
            model_response = await cache_on_disk(self.geminy_client.generate_content_async,
                contents=inputs,
    ...
    async def get_unified_json_genai(self, prompt, document=None, response_schema=None, model="gemini"):
    ...
    async def ask_chatgpt(self, prompt: str, document=None, response_schema=None):
        ...
        completion = await cache_on_disk(self.chatgpt_client.chat.completions.create,
            model="gpt-4o",
            temperature=0.1,
            messages=[
                {
