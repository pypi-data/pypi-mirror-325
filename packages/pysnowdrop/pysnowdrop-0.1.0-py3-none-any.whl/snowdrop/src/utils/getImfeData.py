...
def getModelSpecifications(json_path=None,xl_path=None,fpe_file=None,bHist=False,freq=None,aggr_freq=None,m_aggr=None,save=False,debug=False):
    ...
    ### Equations
    eqs_expressions = list()
    eqs_ids = list(); eqs_labels = list(); eqs_sector = list()
    df = m["equations"]
    ...
    if bHist:
        ...
        for v in lvars:
            if v in exog_data:
                ...
                ser = pd.Series([default_value]*len(f_range),f_range)
        ...
        if bOneVarEquation:
            ...
            eqs = [eqs[i] for i in range(n_eqs) if not i in indx]
...
def instantiate_model(name,eqs,var,params,exogenous,exog_data,assumpt_data,calib,
    ...
    if bHist: 
        ...
        if not forecast_range_start is None and not forecast_range_end is None:
            ...
            T = 2+len(forecast_rng)
            rng = [[forecast_range_start,1,1],[forecast_range_end,10,1]]
            periods = list(range(1,1+T))
            options = {"T":T,"periods":periods,"frequency":freq,"range":rng}
        elif len(df) > 0:
            dates = df.index if rng is None else rng
...
if __name__ == '__main__':
    ...
    fpe_file = os.path.join(working_dir,"snowdrop/data/IMFE/GUY_Pilot.fpe")
    #fpe_file = os.path.join(working_dir,"snowdrop/data/IMFE/Fin.fpe")
    fpe_file = os.path.join(working_dir,"snowdrop/data/IMFE/ALB.fpe")
    fpe_file = os.path.join(working_dir,"snowdrop/data/IMFE/Fin.fpe")
                     
    model,lfunc,lvars,largs,one_var,exog,one_params,one_var_eqs,frcst_rng  =  \
        get_model(fpe_file=fpe_file,bHist=False,freq="monthly",aggr_freq="quarterly",debug=False)
