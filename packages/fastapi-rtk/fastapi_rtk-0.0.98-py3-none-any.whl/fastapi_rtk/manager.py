...
class UserManager(IDParser, BaseUserManager[User, int]):
    ...
    async def create(
        ...
        """
        Modified version of the default `create` method to add roles to the user during creation and to allow user_create as a dict. `safe` is now unused.
    ...
    async def update(
        ...
        """
        Modified version of the default `update` method to also receive user_update as a dict. `safe` is now unused.
