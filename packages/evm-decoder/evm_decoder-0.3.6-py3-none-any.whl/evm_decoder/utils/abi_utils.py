...
def decode_log(params: List[Dict[str, Any]], log: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Create param_names mapping for both indexed and non-indexed parameters
    param_names = {}
    for i, param in enumerate(indexed_params):
        param_names[param['name']] = i
    for i, param in enumerate(non_indexed_params):
        param_names[param['name']] = i + len(indexed_params)

    return IndexableEventLog(decoded_topics, decoded_data, param_names)
