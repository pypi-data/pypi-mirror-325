...
__device__ bool doublepush_move(
    ...
    const unsigned char player_1st_row = boards[TURN][env] == WHITE ? 6 : 1;
    const unsigned char player_3rd_row = boards[TURN][env] == WHITE ? 4 : 3;
    ...
    const bool is_action_ok = (
        ...
        (boards[clamp(0,63,source + ((+8) * boards[TURN][env] + (-8) * (1-boards[TURN][env])))][env] == EMPTY) // intermediate cell is empty
    );
...
__global__ void doublepush_kernel(
    ...
    if (env < boards.size(1)) result[env] = doublepush_move(env, boards, actions);
}
