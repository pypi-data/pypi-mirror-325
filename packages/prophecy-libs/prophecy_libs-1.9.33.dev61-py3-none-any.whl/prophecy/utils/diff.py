...
class DataFrameDiff:
    ...
    def create_joined_df(cls, df1, df2, key_columns, value_columns):
        ...
        left_value_exprs = [
            F.col(f"left.{col}").alias(col) if col in df1.columns else F.lit(None).alias(col)
            for col in value_columns
        ]
        right_value_exprs = [
            F.col(f"right.{col}").alias(col) if col in df2.columns else F.lit(None).alias(col)
            for col in value_columns
        ]
            ...
            "data": {
                ...
                "commonData": {
                    ...
                    "columnComparisons": {
                        ...
                        for col in set(expected_df.columns).intersection(set(generated_df.columns)).intersection(set(value_columns))
                    },
