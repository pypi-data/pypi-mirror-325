        ...
        if filter:
            for k, v in vars(filter).items():
                ...
                if join_model and join_source_col and join_target_col:
                    ...
                    continue
                # We don't need column for custom filter builder
                ...
                if mode == GraphemyFilterMode.EXACT:
                    ...
                    filter_builder = declared_filters[k].get("filter_builder")
                    if callable(filter_builder):
                        stmt, count_stmt = filter_builder(stmt, count_stmt, v, cls)
