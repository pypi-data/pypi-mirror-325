...
class ARTOFTransformer:
    ...
    def __clip_t(self, raw_data, energy_scaler):    
        """
    ...
    def get_axis_and_bins(self, load_as: str, metadata: Metadata) -> tuple:
        ...
        match load_as:
            ...
            case 'spherical':
                ...
                energy_bins = int((end_energy - begin_energy)/0.5 + 1) if metadata.general.acquisitionMode == 'sweep' else 101
                theta_max = metadata.lensmode.maxTheta
                return ['E_eV', 'phi_rad', 'theta_rad'], [[begin_energy, end_energy, energy_bins], [-np.pi, np.pi, 201],[0, theta_max, 201]] 
            case _:
