import functools
from pydantic import BaseModel, Field
from langchain.tools import StructuredTool
from langchain_community.document_loaders import WebBaseLoader
from tolstoy_agents.utils import handle_exceptions

class WebBaseLoaderScraper:

    def __init__(self,  session=None):
        self.session = session

    def scrape(self, link) -> str:
        """
        This Python function scrapes content from a webpage using a WebBaseLoader object and returns the
        concatenated page content.
        
        Returns:
          The `scrape` method is returning a string variable named `content` which contains the
        concatenated page content from the documents loaded by the `WebBaseLoader`. If an exception
        occurs during the process, an error message is printed and an empty string is returned.
        """
        try:
            loader = WebBaseLoader(link)
            loader.requests_kwargs = {"verify": False}
            docs = loader.load()
            content = ""

            for doc in docs:
                content += doc.page_content

            return content

        except Exception as e:
            print("Error! : " + str(e))
            return ""


class ToolInput(BaseModel):
    url: str = Field(..., description=(
        "url link to web page from which you want get content."
        )
    )


@handle_exceptions
def web_content_retriever(url: str) -> str:
    loader = WebBaseLoaderScraper()
    content = loader.scrape(url)
    return content


def web_content_retriever_factory() -> StructuredTool:
    return StructuredTool.from_function(
        func=web_content_retriever,
        name="web_content_retriever",
        description= (
            "Use this tool to get content from web url."
        ),
        args_schema=ToolInput,
        return_direct=False
    )
