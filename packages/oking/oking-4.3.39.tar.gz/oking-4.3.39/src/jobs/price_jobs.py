...
def job_products_prices_list(job_config_dict: dict):
    with lock:
        ...
        try:
            ...
            split_price_lists = {code: [] for code in set([d.price_list_code for d in db_price_list_products])}
            ...
            for k, v in split_price_lists.items():
                ...
                if not api_okVendas.put_price_lists_products(k, products_file):
                    ...
                    for product in v:
                        if protocol_semaphore_item(job_config_dict, db_config, product.price_list_code, product.erp_code):
                            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), False,
                                     f'Produto {product.erp_code} da lista de preco {product.price_list_code} protocolado no banco semaforo',
                                     'info', 'PRECO')
                        else:
                            send_log(job_config_dict.get('job_name'), job_config_dict.get('enviar_logs'), True,
                                     f'Falha ao protocolar o produto {product.erp_code} da lista de preco {product.price_list_code}',
                                     'warning', 'PRECO')
            if src.print_payloads:
