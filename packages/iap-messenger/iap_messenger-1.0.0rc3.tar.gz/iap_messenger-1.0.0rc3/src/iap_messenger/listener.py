...
import json
from iap_messenger.config import Config
...
class MsgListener():
    ...
    async def _process_data(self, msgs: list[Message]):
        ...
        for msg in msgs:
            ...
            if self.decode:
                data, error = decode(msg.Raw, msg._content_type, self.inputs[msg.From])
                if error:
            ...
            try:
                ...
                if self.is_batch:
                    ...
                    for uid in _uids:
                        for msg in msgs:
                            if msg.uid == uid:
                                asyncio.create_task(self.send_msg(self.error_queue, uid, msg._source, "failed to process data", "failed to process data"))
                else:
