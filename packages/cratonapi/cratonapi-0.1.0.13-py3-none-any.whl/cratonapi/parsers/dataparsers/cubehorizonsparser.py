...
def parse(message: bytes) -> np.ndarray:
    ...
    if uid == 0 or (operation == 2 and request == 18 and size == 12):
    ...
    for horizon_num in range(horizons_count):
        horizon_name_symbols_count = struct.unpack("<H", message[18 + offset: 20 + offset])[0]
        horizon_name_bytes = message[20 + offset: 20 + offset + horizon_name_symbols_count]
