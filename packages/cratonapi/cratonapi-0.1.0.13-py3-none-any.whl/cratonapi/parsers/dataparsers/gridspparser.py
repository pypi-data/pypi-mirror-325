...
def parse(message: bytes) -> np.ndarray:
    ...
    if uid == 0 or (operation == 2 and request == 16 and size == 12):
    ...
    n_x, n_y, x_min, x_max, y_min, y_max, z_min, z_max, blank_code, ch_cnt = struct.unpack(
        "<2H6dfB", message[20:77]
    )
    offset = 0
    ...
    for data_num in range(ch_cnt):
        ...
        offset += 4 * n_x * n_y
        grid_list[data_num] = Grid(n_id=grid_sp_identifier,
                                   n_x=n_x,
                                   n_y=n_y,
                                   x_min=x_min,
                                   x_max=x_max,
