...
def test(
    ...
    queries = np.random.uniform(0, 100, size=(n_query_points, data_dim)).astype(
        np.float32
    )
    if has_sklearn:
        with Timer() as sk_init:
            nn = neighbors.NearestNeighbors(
                n_neighbors=n_neighbors, algorithm="auto", metric=metric, radius=radius
            )
            nn.fit(data)
    ...
    assert len(kd_res_dist) == len(queries)
    assert len(kd_res_idx) == len(queries)
    ...
    if has_sklearn:
        ...
        if output and search_type == "knn":
            ...
            print(
                "IDX diff: {} / {}".format(
                    (kd_res_idx != sk_res_idx).sum(), kd_res_idx.size
                )
            )
...
def test_radius():
    ...
    nn.fit(np.array([[1.0], [2.0], [3.0], [4.0]]).reshape((-1, 1)))
    ...
    distances, indices = nn.radius_neighbors(
        np.array([[1.5]]).reshape((-1, 1)), radius=0.1
    )
...
def test_radius_arg_passing():
    ...
    index = np.array([[1.0], [2.0], [3.0], [4.0]]).reshape(-1, 1)
    nn.fit(index)
...
def test_consistency_with_sklearn():
    test(
        data_dim=4,
        n_index_points=1000000,
        n_query_points=50000,
        n_neighbors=10,
        metric="l2",
        output=True,
    )
