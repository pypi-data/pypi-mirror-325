...
import alphagram
from alphagram import __version__, __license__
from alphagram import enums
from alphagram import raw
from alphagram import utils
from alphagram.crypto import aes
from alphagram.errors import CDNFileHashMismatch
from alphagram.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    BadRequest
)
from alphagram.handlers.handler import Handler
from alphagram.methods import Methods
from alphagram.session import Auth, Session
from alphagram.storage import FileStorage, MemoryStorage
from alphagram.types import User, TermsOfService
from alphagram.utils import ainput
...
class Client(Methods):
    """Alphagram Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~alphagram.Client.export_session_string` before stopping the client to get a session string you can
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where Alphagram will store the session files.
            Defaults to the parent directory of the main script.

        plugins (``dict``, *optional*):
            Smart Plugins settings as dict, e.g.: *dict(root="plugins")*.

        parse_mode (:obj:`~alphagram.enums.ParseMode`, *optional*):
    ...
    APP_VERSION = f"Alphagram {__version__}"
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to Alphagram (version {__version__})")
        print(f"Alphagram is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~alphagram.enums.ParseMode`):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.alphagram.org/start/auth")
    ...
    async def get_file(
        ...
        async with self.get_file_semaphore:
            ...
            except alphagram.StopTransmission:
                raise
            except Exception as e:
                log.exception(e)
            finally:
