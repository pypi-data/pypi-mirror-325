...
class CdmFee(BaseModel):
    """
    Fees for organisations, users, and cases.
    """ # noqa: E501
    fee_type: Optional[CdmFeeFeeType] = CdmFeeFeeType.INVALID_FEE_TYPE
    value: Optional[StrictStr] = Field(default=None, description="Value of Fee (Percentage or Currency depending on Type).  For currency values, this field has an accuracy of exactly 2 decimal places. E.g. 250.00 represents £250  For percentage values, the value must be in our standard 5dp percentage format. NOTE: Previously we supported another format for percentages where values ranged from 0.00 meaning 0% to 1.00 meaning 100%. That format is no longer supported.")
    __properties: ClassVar[List[str]] = ["fee_type", "value"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmFee from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmFee from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "fee_type": obj.get("fee_type") if obj.get("fee_type") is not None else CdmFeeFeeType.INVALID_FEE_TYPE,
            "value": obj.get("value")
        })
        return _obj
