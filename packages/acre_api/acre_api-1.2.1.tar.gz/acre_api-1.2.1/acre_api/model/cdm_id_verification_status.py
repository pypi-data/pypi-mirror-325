...
class CdmIDVerificationStatus(BaseModel):
    """
    IDVerificationStatus contains verifications from different sources.
    """ # noqa: E501
    eidvs: Optional[List[CdmEIDV]] = None
    kbas: Optional[List[CdmKBA]] = None
    __properties: ClassVar[List[str]] = ["eidvs", "kbas"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmIDVerificationStatus from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in eidvs (list)
        _items = []
        if self.eidvs:
            for _item_eidvs in self.eidvs:
                if _item_eidvs:
                    _items.append(_item_eidvs.to_dict())
            _dict['eidvs'] = _items
        # override the default output from pydantic by calling `to_dict()` of each item in kbas (list)
        _items = []
        if self.kbas:
            for _item_kbas in self.kbas:
                if _item_kbas:
                    _items.append(_item_kbas.to_dict())
            _dict['kbas'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmIDVerificationStatus from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "eidvs": [CdmEIDV.from_dict(_item) for _item in obj["eidvs"]] if obj.get("eidvs") is not None else None,
            "kbas": [CdmKBA.from_dict(_item) for _item in obj["kbas"]] if obj.get("kbas") is not None else None
        })
        return _obj
