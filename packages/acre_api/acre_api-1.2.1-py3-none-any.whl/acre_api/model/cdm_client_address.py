...
class CdmClientAddress(BaseModel):
    """
    CdmClientAddress
    """ # noqa: E501
    is_current_correspondence: Optional[StrictBool] = Field(default=None, description="Whether this address is the address that should be used for current correspondence.")
    address: Optional[CdmAddress] = None
    move_in_date: Optional[StrictStr] = Field(default=None, description="When the client moved into this property (YYYY-MM-DD).")
    residential_status: Optional[ClientAddressResidentialStatus] = ClientAddressResidentialStatus.INVALID_RESIDENTIAL_STATUS
    move_out_date: Optional[StrictStr] = Field(default=None, description="When the client moved out of this property (YYYY-MM-DD).")
    documents: Optional[List[StrictStr]] = Field(default=None, description="List of document_id that show this address.")
    document_verifies: Optional[StrictBool] = Field(default=None, description="Documents listed verifies this address.")
    id: Optional[StrictStr] = Field(default=None, description="A unique identifier for this entry.")
    __properties: ClassVar[List[str]] = ["is_current_correspondence", "address", "move_in_date", "residential_status", "move_out_date", "documents", "document_verifies", "id"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmClientAddress from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmClientAddress from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "is_current_correspondence": obj.get("is_current_correspondence"),
            "address": CdmAddress.from_dict(obj["address"]) if obj.get("address") is not None else None,
            "move_in_date": obj.get("move_in_date"),
            "residential_status": obj.get("residential_status") if obj.get("residential_status") is not None else ClientAddressResidentialStatus.INVALID_RESIDENTIAL_STATUS,
            "move_out_date": obj.get("move_out_date"),
