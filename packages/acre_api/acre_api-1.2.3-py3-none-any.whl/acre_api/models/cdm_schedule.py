...
class CdmSchedule(BaseModel):
    """
    Schedule is the configuration for meeting scheduling.
    """ # noqa: E501
    availability: Optional[List[CdmAvailability]] = None
    min_booking_notice_minutes: Optional[StrictInt] = Field(default=None, description="MinBookingNoticeMinutes is the minimum booking notice in minutes for scheduling a meeting.")
    min_buffer_minutes: Optional[StrictInt] = Field(default=None, description="MinBufferMinutes is the minimum time gap between meetings.")
    duration_minutes: Optional[StrictInt] = Field(default=None, description="DurationMinutes is the default duration for meetings.")
    allow_client_booking: Optional[StrictBool] = Field(default=None, description="Flag indicating whether booking by client directly is allowed.")
    availability_timezone: Optional[StrictStr] = Field(default=None, description="AvailabilityTimeZone is the time zone identifier in which the business hours (availability) should be applied. Must be an IANA recognised value (https://data.iana.org/time-zones/tz-link.html#tzdb).")
    __properties: ClassVar[List[str]] = ["availability", "min_booking_notice_minutes", "min_buffer_minutes", "duration_minutes", "allow_client_booking", "availability_timezone"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmSchedule from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmSchedule from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "availability": [CdmAvailability.from_dict(_item) for _item in obj["availability"]] if obj.get("availability") is not None else None,
            "min_booking_notice_minutes": obj.get("min_booking_notice_minutes"),
            "min_buffer_minutes": obj.get("min_buffer_minutes"),
            "duration_minutes": obj.get("duration_minutes"),
            "allow_client_booking": obj.get("allow_client_booking"),
            "availability_timezone": obj.get("availability_timezone")
        })
        return _obj
