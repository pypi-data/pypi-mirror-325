...
class CdmGetAuthenticationProviderResponse(BaseModel):
    """
    Response to a request for details of an authentication provider.
    """ # noqa: E501
    authentication_providers: Optional[List[CdmAuthenticationProvider]] = None
    exception: Optional[V1Exception] = None
    next_page: Optional[CdmGetAuthenticationProviderRequest] = None
    __properties: ClassVar[List[str]] = ["authentication_providers", "exception", "next_page"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmGetAuthenticationProviderResponse from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in authentication_providers (list)
        _items = []
        if self.authentication_providers:
            for _item_authentication_providers in self.authentication_providers:
                if _item_authentication_providers:
                    _items.append(_item_authentication_providers.to_dict())
            _dict['authentication_providers'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmGetAuthenticationProviderResponse from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "authentication_providers": [CdmAuthenticationProvider.from_dict(_item) for _item in obj["authentication_providers"]] if obj.get("authentication_providers") is not None else None,
            "exception": V1Exception.from_dict(obj["exception"]) if obj.get("exception") is not None else None,
            "next_page": CdmGetAuthenticationProviderRequest.from_dict(obj["next_page"]) if obj.get("next_page") is not None else None
        })
        return _obj
