...
class CdmLenderMetadata(BaseModel):
    """
    Metadata specific to lender organisations.
    """ # noqa: E501
    porting_pay_on_full_loan_amount: Optional[StrictBool] = None
    svr_rates: Optional[List[CdmSVRRates]] = Field(default=None, description="An array of the different SVR Rates currently used by the lender.")
    reflection_period: Optional[StrictInt] = Field(default=None, description="The reflection period to uses in ESIS'.")
    __properties: ClassVar[List[str]] = ["porting_pay_on_full_loan_amount", "svr_rates", "reflection_period"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmLenderMetadata from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in svr_rates (list)
        _items = []
        if self.svr_rates:
            for _item_svr_rates in self.svr_rates:
                if _item_svr_rates:
                    _items.append(_item_svr_rates.to_dict())
            _dict['svr_rates'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmLenderMetadata from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "porting_pay_on_full_loan_amount": obj.get("porting_pay_on_full_loan_amount"),
            "svr_rates": [CdmSVRRates.from_dict(_item) for _item in obj["svr_rates"]] if obj.get("svr_rates") is not None else None,
            "reflection_period": obj.get("reflection_period")
        })
        return _obj
