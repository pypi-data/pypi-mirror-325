...
class CdmTransitionClientVersions(BaseModel):
    """
    ID and version of the clients when the case was transitioning to a particular status. The snapshot is made just before the transition happens and changes are done to the entities.
    """ # noqa: E501
    entity_versions: Optional[List[CaseDetailsClientIDAndVersion]] = Field(default=None, description="List of client IDs and their version just before the case transitioned to the status. A new version is created for each client on the case when the case is transitioned. This version is referenceable and immutable.")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmTransitionClientVersions from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmTransitionClientVersions from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "entity_versions": [CaseDetailsClientIDAndVersion.from_dict(_item) for _item in obj["entity_versions"]] if obj.get("entity_versions") is not None else None,
