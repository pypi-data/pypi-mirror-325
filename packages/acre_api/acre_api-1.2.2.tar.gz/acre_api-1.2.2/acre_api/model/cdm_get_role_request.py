...
class CdmGetRoleRequest(BaseModel):
    """
    Get details of roles.
    """ # noqa: E501
    role_ids: Optional[List[StrictStr]] = Field(default=None, description="Role IDs (optional, default wildcard for all roles that the user can access). A set union of the resulting roles is computed when \"role_names\" is used with \"role_ids\".")
    role_names: Optional[List[StrictStr]] = Field(default=None, description="Role names (optional, default wildcard for all roles that the user can access). A set union of the resulting roles is computed when \"role_names\" is used with \"role_ids\".")
    show_disabled: Optional[StrictBool] = Field(default=None, description="Specify to show disabled roles. By default they are not shown on a wildcard query. This only has effect for a wildcard query.")
    ...
    __properties: ClassVar[List[str]] = ["role_ids", "role_names", "show_disabled", "bookmark", "page_size"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmGetRoleRequest from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmGetRoleRequest from a dict"""
        if obj is None:
            return None
