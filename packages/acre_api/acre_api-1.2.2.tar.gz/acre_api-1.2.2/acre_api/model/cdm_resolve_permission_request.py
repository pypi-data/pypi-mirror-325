...
class CdmResolvePermissionRequest(BaseModel):
    """
    Resolve a permission, ie. check if the user has the listed permissions for the specific scopes.
    """ # noqa: E501
    permission_resolution_requests: Optional[List[CdmPermissionResolutionRequest]] = None
    __properties: ClassVar[List[str]] = ["permission_resolution_requests"]
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CdmResolvePermissionRequest from a JSON string"""
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in permission_resolution_requests (list)
        _items = []
        if self.permission_resolution_requests:
            for _item_permission_resolution_requests in self.permission_resolution_requests:
                if _item_permission_resolution_requests:
                    _items.append(_item_permission_resolution_requests.to_dict())
            _dict['permission_resolution_requests'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CdmResolvePermissionRequest from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            "permission_resolution_requests": [CdmPermissionResolutionRequest.from_dict(_item) for _item in obj["permission_resolution_requests"]] if obj.get("permission_resolution_requests") is not None else None
        })
        return _obj
