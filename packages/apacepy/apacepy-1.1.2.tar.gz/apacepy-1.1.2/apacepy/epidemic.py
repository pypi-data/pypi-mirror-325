...
class EpiModel:
    ...
    def get_costs_over_decision_periods(self):
        """ :returns: (list) of cost over decision periods """
        return self.econEval.get_costs_over_decision_periods()

    def get_effects_over_decision_periods(self):
        """ :returns: (list) of effects over decision periods """
        return self.econEval.get_effects_over_decision_periods()

    def get_nmb_losses_over_decision_periods(self, wtp, health_multiplier=1, cost_multiplier=1):
        """ :returns: (list) of losses in net monetary benefits over decision periods
        ...
        q = np.array(self.get_effects_over_decision_periods()) * health_multiplier
        c = np.array(self.get_costs_over_decision_periods()) * cost_multiplier
        result = wtp * q + c
        return result
