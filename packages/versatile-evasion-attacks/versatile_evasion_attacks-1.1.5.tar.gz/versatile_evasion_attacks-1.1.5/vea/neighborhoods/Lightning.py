...
class Lightning(Neighborhood):
    ...
    def __init__(self, constraints: dict, **kwargs):
        ...
        self.min_radius_factor = self.params.get('min_radius_factor', 0.8)
    ...
    def generate(self, x, perturbation_weights: list, return_inflation_vector=False) -> list:
        ...
        for _ in range(self.num_samples):
            ...
            random_direction = np.random.normal(size=len(x))
            random_direction /= np.linalg.norm(random_direction) + 1e-10  # Normalize to unit vector

            # Generate a random radius between min_radius_factor * perturbation_weights and perturbation_weights
            min_radius = self.min_radius_factor * np.array(perturbation_weights)
            max_radius = np.array(perturbation_weights)
            random_radius = np.random.uniform(min_radius, max_radius)

            # Compute perturbation
            perturbation = random_direction * random_radius
    ...
    def generate_valid(self, x, estimator, y=None, **kwargs):
        ...
        for k in range(max_trials):
            if self.enable_warning_message:
                if k == max_trials // 10:
                    print(f"Lightning Warning: The number of trials exceeded 10% of maximum generation trials. Found {len(adv_samples)} valid samples.")
                elif k == max_trials // 4:
                    print(f"Lightning Warning: The number of trials exceeded 25% of maximum generation trials. Found {len(adv_samples)} valid samples.")
                elif k == max_trials // 2:
                    print(f"Lightning Warning: The number of trials exceeded 50% of maximum generation trials. Found {len(adv_samples)} valid samples.")
        ...
        if success:
            ...
            if np.inf in perturbation_weights:
                raise Exception(f"Lightning: Overflow in the perturbation weights. Possible solutions:\n"
            ...
            raise Exception(f"Lightning: No valid sample satisfied the constraints after {max_trials} trials.")