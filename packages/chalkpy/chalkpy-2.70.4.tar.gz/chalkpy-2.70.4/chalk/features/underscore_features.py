    ...
    if not isinstance(expr, Underscore):
        ...
        if parent_namespace is not None:
            if (named_expr := parent_namespace.__chalk_notebook_feature_expressions__.get(attr)) is not None:
                ...
                substituted_expr = _process_named_underscore_expr(
        ...
        return UnderscoreAttr(parent=parent_processed, attr=attr)
        ...
        return UnderscoreItem(parent=parent_processed, key=key_processed)
    elif isinstance(expr, UnderscoreCall):
        parent_processed = _process_named_underscore_expr(expr=expr._chalk__parent, ctx=ctx)
        args_processed = _process_named_underscore_expr(expr=expr._chalk__args, ctx=ctx)
        kwargs_processed = _process_named_underscore_expr(expr=expr._chalk__kwargs, ctx=ctx)
        return UnderscoreCall(parent=parent_processed, args=args_processed, kwargs=kwargs_processed)
        ...
        return UnderscoreFunction(expr._chalk__function_name, *args_processed, **kwargs_processed)
