...
class Output(IOutput):
    ...
    def asciiInfo():
        ...
        try:
            ...
            # Replace placeholders with dynamic content
            year = datetime.datetime.now().year
            message = "\033[92m{} \033[0m".format("The list of commands accepted by the Flaskavel interpreter are:")
            commands = [
                {'name':'flaskavel new <app_name>', 'description': 'Creates a new Flaskavel app with the specified name.'},
                {'name':'flaskavel --version', 'description': 'Displays the current version of Flaskavel.'},
                {'name':'flaskavel --upgrade', 'description': 'Upgrades Flaskavel to the latest version.'}
            ]
            commands_array = []
