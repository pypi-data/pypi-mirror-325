...
class Remote:
    ...
    def install_client_env(self, user, _ip, password):
        ...
        remote = RemoteCmd(user, _ip, password)
        stdout, return_code = remote.remote_run("pip3 --version", return_code=True)
        if return_code != 0:
            remote.remote_sudo_run('apt update', use_sshpass=True)
            remote.remote_sudo_run('apt install python3-pip -y', use_sshpass=True)
            remote.remote_run(pip_install_cmd("pip"), use_sshpass=True)
        remote.remote_run(pip_install_cmd("youqu3"), use_sshpass=True)
        return_code = remote.remote_run(
            f"export PATH=\$PATH:\$HOME/.local/bin; cd {self.client_rootdir(user)} && youqu3 envx",
            use_sshpass=True,
        )
        if return_code != 0:
            logger.info(f"环境安装失败 - < {user}@{_ip} >")
            raise EnvironmentError
        logger.info(f"环境安装成功 - < {user}@{_ip} >")
    ...
    def makedirs(dirs):
        for _ in range(2):
            if not os.path.exists(dirs):
                logger.info(f"创建目录：{dirs}")
                Cmd.run(f"mkdir -p {dirs}")
            else:
                logger.info(f"已存在目录： {dirs}")
                break
    ...
    def get_back_report(self, user, _ip, password):
        ...
        stdout, return_code = Cmd.expect_run(
            f"/bin/bash -c '{self.rsync} {user}@{_ip}:{self.client_report_path(user)}/* {server_html_path}/'",
            events={'password': f'{password}\n'},
            return_code=True,
            timeout=1800,
        )
        self.set_youqu_run_exitcode(return_code)
        if return_code != 0:
            logger.error(f"收集报告失败，请检查：{user}@{_ip}:{self.client_report_path(user)}/* {server_html_path}/")
    ...
    def job_start_driver(self):
        if self.job_start:
            logger.info(self.job_start)
            os.system(self.job_start)

    def job_end_driver(self):
        if self.job_end:
            logger.info(self.job_end)
            os.system(self.job_end)
