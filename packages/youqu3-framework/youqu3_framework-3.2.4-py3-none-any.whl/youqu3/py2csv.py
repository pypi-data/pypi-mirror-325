...
class Py2Csv:
    ...
    def __init__(self, walkdir=None):
        self.walkdir = walkdir or "."
    ...
    def scan_csv_and_py(self):
        csv_path_dict = {}
        py_path_dict = {}
        for root, _, files in os.walk(self.walkdir):
            py_files = []
            for file in files:
            ...
            for case_name in py_path_dict:
                if csv_name == case_name:
                    ...
                    if new_taglines != taglines:
                        bak_path = f"{self.report_path}/pyid2csv_back"
                        if not os.path.exists(bak_path):
                            os.makedirs(bak_path)
                        os.system(
                            f"cp {csv_path} {bak_path}/{setting.TIME_STRING}_{csv_name}.csv"
                        )
        ...
        for case_name in self.py_path_dict:
            ...
            for py_path, py_dirpath, case_id in py_paths:
                if not self.csv_path_dict or not self.csv_path_dict.get(case_name):
                    ...
                    with open(csv_path, "w+", encoding="utf-8") as f:
                        f.write(",".join([i.value for i in setting.FixedCsvTitle]) + "\n")
                ...
                with open(csv_path, "r", encoding="utf-8") as f:
                    ...
                    try:
                        ...
                        with open(csv_path, "w+", encoding="utf-8") as f:
                            f.write(",".join([i.value for i in setting.FixedCsvTitle]) + "\n")
                        comma_num = len(setting.FixedCsvTitle) - 1
