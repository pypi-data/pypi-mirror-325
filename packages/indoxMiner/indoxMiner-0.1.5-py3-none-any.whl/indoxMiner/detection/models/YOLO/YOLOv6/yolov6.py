...
class YOLOv6:
    ...
    def _install_yolov6(self):
        ...
        if not os.path.exists("YOLOv6"):
            ...
            subprocess.run(["git", "clone", "https://github.com/meituan/YOLOv6"], check=True)
    ...
    def detect_objects(self, image_path, conf_threshold=0.25):
        ...
        output_images = glob.glob(os.path.join(output_dir, "*.*"))  # Match all file types
        if not output_images:
            raise FileNotFoundError(f"No output images found after inference. Check logs for more details.")
