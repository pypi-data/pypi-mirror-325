...
def run_streamlit_app():
    try:
        ...
        script_path = Path(os.path.join(script_dir, 'taxontabletools_2.0.py'))

        if os.path.isfile(script_path):
            print('Press CTRL + C to close TaxonTableTools!')
            subprocess.call(['streamlit', 'run', script_path, '--theme.base', 'light', '--server.address', 'localhost'])
        else:
            print('')
            print('Warning: Could not find TTT main script.')
