...
def replicate_merging(taxon_table_xlsx, taxon_table_df, samples, metadata_df, traits_df, cutoff, replicates_dict, replicate_non_matching_samples):
    ## create copies of the dataframes
    taxon_table_df = taxon_table_df.copy()
    metadata_df = metadata_df.copy()
    traits_df = traits_df.copy()

    # replicate_non_matching_samples can be 'Keep (without merging)', 'Remove from dataset'
    ...
    suffix = 'merged'
    metadata_df = pd.DataFrame([[j,f's_{i}'] for i,j in enumerate(unique_samples)], columns=['Sample', 'Placeholder'])
