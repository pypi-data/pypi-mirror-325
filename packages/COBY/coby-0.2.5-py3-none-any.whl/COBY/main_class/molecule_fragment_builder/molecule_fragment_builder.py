    ...
    def molecule_fragment_builder(self):
        ...
        if self.MOLECULE_FRAGMENT_BUILDER_cmds:
            ...
            for cmdi, cmd in enumerate(self.MOLECULE_FRAGMENT_BUILDER_cmds, 1):
                ...
                self.print_term("MOLECULE_FRAGMENT_BUILDER cmd:     ", cmd,      debug=True)
                self.print_term("MOLECULE_FRAGMENT_BUILDER cmd_dict:", cmd_dict, debug=True)
                ...
                ### Sets default params to be "LFB" and overwrites if given
                params = "MFB" # Lolecule Fragment Builder
                ...
                if "FromTopology" in cmd_dict.keys():
                    if cmd_dict["FromTopology"] in ["True", "true", "1"]:
                        FromTopology = True
                    else:
                        FromTopology = True
                ...
                for part, val in cmd_dict.items():
                    ...
                    if "function" in self.fragment_defs[moltype]["parts"][part].keys():
                        ...
                        # new_beads = []
                        # charges = []
                        # for bi, bead in enumerate(beads):
                        #     if len(bead) == 2:
                        #         new_beads.append(bead)
                        #     else:
                        #         new_beads.append((bead[0], bead[1]))
                        #         charges.append((bi, bead[2]))

                        part_dicts[part]["beaddata"] = beads.copy()
                        # part_dicts[part]["beads"]   = new_beads.copy()
                        # part_dicts[part]["charges"] = charges.copy()
                ...
                #     for bi, (beadname, beadcoords) in enumerate(part_dicts[part]["beads"]):
                #         beadcoords_new = tuple([round(coord - joiner, 3) for coord, joiner in zip(beadcoords, connector_difference)])
                #         part_dicts[part]["beads"][bi] = (beadname, beadcoords_new)
                ...
                if moleculetype:
                    lipid["moleculetype"] = moleculetype
                
                if params not in self.lipid_defs_built.keys():
                    self.lipid_defs_built[params] = {}
                self.lipid_defs_built[params].update(copy.deepcopy(lipid))
                
                if params not in self.solvent_defs_built.keys():
                    self.solvent_defs_built[params] = {}
                self.solvent_defs_built[params].update(copy.deepcopy(lipid))
                
                if params not in self.pos_ion_defs_built.keys():
                    self.pos_ion_defs_built[params] = {}
                self.pos_ion_defs_built[params].update(copy.deepcopy(lipid))
                
                if params not in self.neg_ion_defs_built.keys():
                    self.neg_ion_defs_built[params] = {}
                self.neg_ion_defs_built[params].update(copy.deepcopy(lipid))

            self.print_term("Number of lipids built:", cmdi, "\n", spaces=1, verbose=2)