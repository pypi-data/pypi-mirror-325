    ...
    def memb_preprocessor(self, return_self = "self", cmds_given = False, extra_verbose=0):
        ...
        if len(MEMBRANES_cmds) != 0:
            ...
            for cmd_nr, memb_cmd in enumerate(MEMBRANES_cmds, 1):
                self.print_term("Starting membrane argument:", cmd_nr, spaces=1, verbose=2+extra_verbose)
                ...
                settings_dict = {
                    ...
                    "membrane"  : {
                        ...
                        ### Splits subleaflets into multiple subleaflets if individual parts are not touching
                        "split_bbox": True,
                        ...
                        "optimize_run": "auto", # False/"no"/0, "auto", True/"yes"/1
                        "optimize_max_steps": 100,  # Maximum number of steps
                        "optimize_push_tolerance": 0.5,  # Push tolerance
                        "optimize_lipid_push_multiplier": 1.0, # Lipid-lipid push multiplier
                        "optimize_edge_push_multiplier": 1.0, # Edge-lipid push multiplier
                    },
                    ...
                    "default": {
                        ...
                        "protein_buffer": 0.264/2, # [nm] converted to [Å], default = (vdw of regular beads) / 2
                        ...
                        ### 'no':          No optimization. At best correct. Most likely wrong.
                        ### 'abs_val':     Ratio-values are treated as absolute number of lipids
                        ### 'insane':      Imitates insanes lipid insertion. Onitial lipid calculation is intentionally wrong to mimic insane.
                ...
                for cmd in memb_cmd.split():
                    ...
                    ### Leaflet shape
                        ...
                        if len(sub_cmd[1:]) == 1:
                            ...
                            if isnumber:
                                if val == "0":
                                    val = "1"
                                settings_dict["membrane"]["gridsplits"] = (int(ast.literal_eval(val)*10), int(ast.literal_eval(val)*10))
                            elif val == "False":
                            ...
                            if val1 == "auto" and isnumber2:
                                settings_dict["membrane"]["gridsplits"] = ("auto", ast.literal_eval(val2)*10)
                                ...
                                if val2 in ["False", "0"]:
                                    val2 = "1"
                                settings_dict["membrane"]["gridsplits"] = (ast.literal_eval(val1)*10, ast.literal_eval(val2)*10)
                            else:
                                ELSE = True
                        else:
                            ELSE = True
                    ...
                    elif sub_cmd[0].lower() == "protein_buffer":
                        settings_dict[dict_target]["protein_buffer"] = ast.literal_eval(sub_cmd[1])

                    ### Integer multiplier for radius used in alphashape function [multiplier]
                    elif sub_cmd[0].lower() == "alpha_multiplier":
                        settings_dict[dict_target]["alpha_multiplier"] = ast.literal_eval(sub_cmd[1])

                    ### Rounding method for rounding number of lipids in leaflet [function]
                    elif sub_cmd[0].lower() == "lipid_rounding_function": # "int", "round", "min" or "max"
                        rounding_types = {"int": ("int", int), "round": ("round", round), "floor": ("math.floor", math.floor), "ceil": ("math.ceil", math.ceil)}
                        settings_dict[dict_target]["lipid_rounding_function"] = rounding_types[sub_cmd[1]] 
                    ...
                    ###########################
                        ...
                        ### Multiplier used to scale the buffer space around lipids during the grid making process
                            ...
                            settings_dict[dict_target]["grid_maker_multiplier"] = ast.literal_eval(sub_cmd[1])
                        ...
                        ### Whether lipids should be evenly or randomly distributed
                            ...
                            assert sub_cmd[1] in ["e", "even", "evenly", "r", "ran", "rand", "random"], "\n".join([
                                "The 'grid_maker_lipid_distribution' subargument accepts only the following values:",
                                "    "+"evenly (can be set with either 'e', 'even' or 'evenly')"
                                "    "+"random (can be set with either 'r', 'ran', 'rand' or 'random') (default)"
                            ])
                            val = sub_cmd[1]
                    ...
                    #############################
                    elif sub_cmd[0].lower().startswith(("optim_", "optimize_", "optimise_", "optimization_", "optimisation_")):
                        if sub_cmd[0].lower().endswith(("_run")):
                            ...
                            if isnumber:
                                settings_dict[dict_target]["optimize_max_steps"] = int(ast.literal_eval(sub_cmd[1]))
                        
                        elif sub_cmd[0].lower().endswith(("_pt", "_push_tol", "_push_tolerance")):
                            isnumber, isint = self.is_number(sub_cmd[1])
                            if isnumber:
                                settings_dict[dict_target]["optimize_push_tolerance"] = ast.literal_eval(sub_cmd[1])
                        
                        elif sub_cmd[0].lower().endswith(("_lpm", "_lipid_push_mult", "_lipid_push_multiplier")):
                            isnumber, isint = self.is_number(sub_cmd[1])
                            if isnumber:
                                settings_dict[dict_target]["optimize_lipid_push_multiplier"] = ast.literal_eval(sub_cmd[1])
                        
                        elif sub_cmd[0].lower().endswith(("_epm", "_edge_push_mult", "_edge_push_multiplier")):
                            isnumber, isint = self.is_number(sub_cmd[1])
                            if isnumber:
                    ...
                    ### Sets whether the hydrophobic volume of the leaflet is solvatable [True/False]
                        ...
                        settings_dict[dict_target]["solvate_hydrophobic_volume"] = ast.literal_eval(sub_cmd[1])
