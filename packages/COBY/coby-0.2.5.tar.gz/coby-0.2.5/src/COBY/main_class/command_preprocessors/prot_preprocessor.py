        ...
        if len(self.PROTEINS_cmds) != 0:
            ...
            for cmd_nr, prot_cmd in enumerate(self.PROTEINS_cmds, 1):
                ...
                prot_dict = {
                    ...
                    "buffer": 0.264/2, # [nm] to [Å] default = (vdw of regular beads) / 2
                ...
                for cmd in prot_cmd.split():
                    ...
                    ### Buffer used for solvations ### [nm] to [Å] default = (vdw of regular beads) / 2
                    ...
                    ### xyz axis rotations [degrees]
                    elif sub_cmd[0].lower() == "rotate":
                        rotate_dict = {"rx": 0, "ry": 0, "rz": 0}
                        i = 1
                        while i < len(sub_cmd):
                            assert sub_cmd[i] in ["rx", "ry", "rz"], "'rotate' subsubarguments must follow the 'r[xyz]:[float/int] syntax."
                            rotate_dict[sub_cmd[i]] = ast.literal_eval(str(sub_cmd[i+1]))
                            i += 2
                        prot_dict[sub_cmd[0].lower()].append(rotate_dict)
                ...
                if any([prot_dict["rx"], prot_dict["ry"], prot_dict["rz"]]):
                    prot_dict["rotate"].append({"rx": prot_dict["rx"], "ry": prot_dict["ry"], "rz": prot_dict["rz"]})

                ### Converts values from nanometers to angstrom
                prot_dict["buffer"] = prot_dict["buffer"]*10
