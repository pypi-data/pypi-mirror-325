...
from Fireflink_NLP.browser.views import BrowserError, BrowserState, TabInfo, URLNotAllowedError
from Fireflink_NLP.dom.service import DomService
from Fireflink_NLP.dom.views import DOMElementNode, SelectorMap
from Fireflink_NLP.utils import time_execution_sync
...
class BrowserContext:
	...
	async def _input_text_element_node(self, element_node: DOMElementNode, text: str,fireflinkObject):
		try:
			...
			xpath_object=await page.evaluate("window.XpathObject")
			fireflinkObject.add_step({"nlpName":"SendKeys","input":text,"element":xpath_object,"aiReferenceElement":str(element_node.xpath)})
			await page.wait_for_load_state()
	...
	async def _click_element_node(self, element_node: DOMElementNode,fireflinkElementObject):
		...
		try:
			...
			try:
				...
				xpath_object=await page.evaluate("window.XpathObject")
				fireflinkElementObject.add_step({"nlpName":"Click","input":"","element":xpath_object,"aiReferenceElement":str(element)})
				await page.wait_for_load_state()
	...
	async def switch_to_tab(self, page_id: int,fireflinkElementObject) -> None:
		...
		page = pages[page_id]
		fireflinkElementObject.add_step({"nlpName":"SwitchToNewWindowIfUrlContainsString","input":page_id,"element":"","aiReferenceElement":""})
