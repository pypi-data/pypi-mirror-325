...
from Fireflink_NLP.agent.message_manager.service import MessageManager
from Fireflink_NLP.agent.prompts import AgentMessagePrompt, SystemPrompt
from Fireflink_NLP.agent.views import (
    ActionResult,
    AgentError,
    AgentHistory,
    AgentHistoryList,
...
from Fireflink_NLP.browser.browser import Browser
from Fireflink_NLP.browser.context import BrowserContext
from Fireflink_NLP.browser.views import BrowserState, BrowserStateHistory
from Fireflink_NLP.controller.registry.views import ActionModel
from Fireflink_NLP.controller.service import Controller
from Fireflink_NLP.dom.history_tree_processor.service import (
    DOMHistoryElement,
    HistoryTreeProcessor,
)
from Fireflink_NLP.utils import time_execution_async
...
class Agent:
    def __init__(
            ...
            register_new_step_callback: Callable[[
                'BrowserState', 'AgentOutput', int], None] | None = None,
        ...
        self.tool_calling_method = self.set_tool_calling_method(
            tool_calling_method)
    ...
    async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
        ...
        if self.model_name == 'deepseek-reasoner':
            converted_input_messages = self.message_manager.convert_messages_for_non_function_calling_models(
                input_messages)
            merged_input_messages = self.message_manager.merge_successive_human_messages(
                converted_input_messages)
    ...
    def _too_many_failures(self) -> bool:
        ...
        if self.consecutive_failures >= self.max_failures:
            logger.error(f'❌ Stopping due to {
                         self.max_failures} consecutive failures')
            return True
        return False
    ...
    async def rerun_history(
        ...
        for i, history_item in enumerate(history.history):
            ...
            logger.info(f'Replaying step {
                        i + 1}/{len(history.history)}: goal: {goal}')
    ...
    async def _execute_history_step(self, history_item: AgentHistory, delay: float) -> list[ActionResult]:
        ...
        for i, action in enumerate(history_item.model_output.action):
            ...
            if updated_action is None:
                raise ValueError(f'Could not find matching element {
                                 i} in current page')
    ...
    async def _update_action_indices(
        ...
        if old_index != current_element.highlight_index:
            ...
            logger.info(f'Element moved in DOM, updated index from {
    ...
    def _create_task_frame(
        ...
        larger_font = ImageFont.truetype(
            regular_font.path, regular_font.size + 16)  # Increase font size more
    ...
    def _create_frame(self, screenshot: str, text: str, step_number: int, width: int = 1200, height: int = 800) -> Image.Image:
        ...
        logo_path = os.path.join(os.path.dirname(
            __file__), 'assets/browser-use-logo.png')
        if os.path.exists(logo_path):
            logo = Image.open(logo_path)
            logo.thumbnail((logo_size, logo_size))
            frame.paste(logo, (width - logo_size - 20, 20),
