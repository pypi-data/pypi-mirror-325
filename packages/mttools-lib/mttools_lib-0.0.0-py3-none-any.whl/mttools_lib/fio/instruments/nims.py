...
class Nims(EMDataBase):
    ...
    def _parse_nims_bytes(data: bytes) -> tuple[np.ndarray, ...]:
        ...
        dtype = np.dtype(
            [
                ...
                ("r_temp", "i2"),  # Signed 2-byte integer for electronics unit temperature
    ...
    def _unroll_sequence(self, sequence: np.ndarray) -> tuple[np.ndarray, list, list]:
        ...
        if (np.diff(tops) == top_interval).all() and start_gap and end_gap:
            return np.full((len(sequence)), range(0, len(sequence))), gaps, possible_duplicates
        else:
    ...
    @staticmethod
    def _parse_gnss(ts: pl.DataFrame) -> tuple[list, list, float, float, float, float, str, str]:
    ...
    def _parse_timestamp(self, ts: pl.DataFrame, gaps: list, index: list, time: list) -> tuple[pl.DataFrame, str]:
        ...
        for gap in gaps + [ts.height() - 1]:
            ...
            for stamp in {i for i in index if prev_gap < i < gap}:
                ...
                timestamps = pl.timedate_range(
                    start, periods=gap - prev_gap, freq="s")
    ...
    def _gap_lengths(ts: pl.DataFrame, gaps: list) -> tuple[list, str]:
        ...
        for gap in gaps:
            gap += 1
            if bool(ts.TsValid.loc[gap - 1]) is True and bool(ts.TsValid.loc[gap]) is True:
    ...
    def _unknown_timestamps(ts: pl.DataFrame, gaps: list) -> tuple[pl.DataFrame, str]:
        ...
        index = ts.where(ts.TsValid == True).first_valid_index()
        ...
        for gap in gaps + [len(ts) - 1]:
            ...
            if bool(ts.TsValid.loc[gap]) is False:
                ...
                timestamps = pd.date_range(
                    start, periods=1 + seq - prev_seq, freq="S")
