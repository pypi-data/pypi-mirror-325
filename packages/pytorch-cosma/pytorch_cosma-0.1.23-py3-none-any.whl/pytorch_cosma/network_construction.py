...
class BaseModel(nn.Module):
    ...
    def create_layer(self, layer_conf: dict) -> nn.Module | Callable:
        ...
        try:
            if self.verbose:
                print(f"Layer class is {type(layer_class)}")
    ...
    def _core_iterate_through_layers(self, layer_conf, outputs):
        ...
        try:
            ...
            try:
                ...
                for i, input_tensor in enumerate(inputs):
                    if self.verbose:
                        print(f"Layer {layer_name} input {i} shape: {input_tensor.shape}")
        ...
        try:
            if self.verbose:
                print(f"Layer {layer_name} output shape: {outputs[layer_conf['output']].shape}")
        except:
            # Handle layers that produce multiple outputs
            for i, output_tensor in enumerate(outputs[layer_conf["output"]]):
                if self.verbose:
                    print(f"Layer {layer_name} output {i} shape: {output_tensor.shape}")
