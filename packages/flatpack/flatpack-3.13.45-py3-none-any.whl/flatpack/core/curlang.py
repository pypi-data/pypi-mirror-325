...
curlang_grammar = r"""
    start: statement+
    statement: find_stmt | get_stmt | yay_stmt | nay_stmt
    find_stmt: "!find" STRING block "uh say" STRING ";"? 
    block: "{" statement+ "}"
...
class CurlangTransformer(Transformer):
    ...
    def yay_stmt(self, items):
        return {"type": "yay", "message": items[0]}

    def nay_stmt(self, items):
        return {"type": "nay", "message": items[0]}
...
def run_curlang_block_inner(cmd) -> str:
    ...
    if cmd["type"] == "find":
        ...
        if "block" in cmd and cmd["block"]:
            inner_commands = [run_curlang_block_inner(inner) for inner in
                              cmd["block"]]
            inner_code = "\n".join(inner_commands)
        return (
