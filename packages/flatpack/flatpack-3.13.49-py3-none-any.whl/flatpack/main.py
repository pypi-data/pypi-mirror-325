        ...
        with temp_sh_path.open("w", encoding="utf-8") as outfile:
            ...
            for block_index, block in enumerate(code_blocks):
                ...
                if language == "bash":
                    ...
                    try:
                        bash_commands = run_curlang_block(curlang_code)
                    except Exception as e:
                        bash_commands = (
                            f'echo -e "\\033[31m[ERROR] Failed to parse curlang block: {e}\\033[0m"\n'
                            f'exit 1'
                        )

                    outfile.write(bash_commands + "\n")
...
async def fpk_build(directory: Union[str, None], use_euxo: bool = False):
    ...
    flatpack_json_path = build_dir / "curlang.json"
...
def fpk_create(flatpack_name, repo_url=TEMPLATE_REPO_URL):
    ...
    try:
        ...
        files_to_edit = [
            ...
            (
                os.path.join(flatpack_dir, "curlang.toml"),
                r"{{model_name}}",
                flatpack_name,
            ),
            (
...
def fpk_display_disclaimer(directory_name: str, local: bool):
    ...
    disclaimer_template = dedent("""
        [bold]Copyright (C) 2023-2025 Curlang[/bold]
    ...
    if not local:
        please_note_content = dedent(f"""
            [bold]PLEASE NOTE:[/bold] The flatpack you are about to unbox is
            governed by its own licenses and terms, separate from
            this software. You may find further details at:
            
            https://codeberg.org/curlang/curlang/src/branch/main/warehouse/{directory_name}
        """)
    else:
        please_note_content = ""
...
def finalize_setup(flatpack_dir: Path) -> bool:
    ...
    try:
        ...
        bash_script_path = flatpack_dir / "curlang.sh"
        toml_path = flatpack_dir / "curlang.toml"
...
def fpk_cli_handle_add(args, session):
    ...
    if not toml_path.exists():
        console.print(
            f"[bold red]Error: curlang.toml not found in {flatpack_dir}[/bold red]"
        )
        return
