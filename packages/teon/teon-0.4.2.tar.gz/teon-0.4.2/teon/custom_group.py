    ...
    def draw(self, surface):
        ...
        if self._core_class.camera.parent is not None:
            if self._core_class.camera.follow_type == "follow":
                self._core_class.camera.position.x = -(self._core_class.camera.parent.rect.centerx - pygame.display.get_window_size()[0] / 2)
                self._core_class.camera.position.y = -(self._core_class.camera.parent.rect.centery - pygame.display.get_window_size()[1] / 2)

            if self._core_class.camera.follow_type == "smooth_follow":

                target_offset_x = -(self._core_class.camera.parent.rect.centerx - pygame.display.get_window_size()[0] / 2)
                target_offset_y = -(self._core_class.camera.parent.rect.centery - pygame.display.get_window_size()[1] / 2)
            ...
            if sprite.visible and sprite.running:
                if sprite.is_ui:
                    ...
                    if sprite._occlusion_rect.colliderect(self.player._occluder_rect):
                        surface.display.blit(sprite._image,(sprite.rect.topleft[0] + self._core_class.camera.position.full().x + offset_x,sprite.rect.topleft[1] + self._core_class.camera.position.full().y + offset_y))
                
                else:
                    surface.display.blit(sprite._image,(sprite.rect.topleft[0] + self._core_class.camera.position.full().x + offset_x + self._core_class.camera.offset.x,sprite.rect.topleft[1] + self._core_class.camera.position.full().y + offset_y + self._core_class.camera.offset.y))
                    sprite._pos = Vec2(self._core_class.camera.position.full().x + self._core_class.camera.offset.x,self._core_class.camera.position.full().y + self._core_class.camera.offset.y)