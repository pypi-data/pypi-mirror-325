...
class Teon:
    ...
    def __init__(self,**kwargs):
        ...
        self.aspect_borders = kwargs.get("aspect_borders",True)
    ...
    def run(self,update_func = None,input_func = None):
        ...
        self._entity_counter_text = Text(text = f"Entities: {len(self.entities)}",position = (1,0),size = 30,tags = ["_entity_counter_text"],anchor = "topright",z = 100)
                    ...
                    if len(entity.colliders) > 0:
                        for _,collider in entity.colliders.items():
                            if not entity.collider.collidable:
                                debug_collider(collider,(0,0,255))
                            else:
...
def debug_collider(collider,color = (255,0,0),**kwargs):
    ...
    if collider.shape == "square":
        ...
        pygame.draw.rect(pygame.display.get_surface(),color,collider,kwargs.get("thickness",2))
        if kwargs.get("name") is not None:
            text = Text(text = kwargs.get("name"),size = kwargs.get("size",30),color = color) # type: ignore
            text.anchor = "center"
            pygame.display.get_surface().blit(text.image,(collider.center[0] - text.rect.width / 2,collider.center[1] - text.rect.height / 2))
    else:
        pygame.draw.circle(pygame.display.get_surface(),color,(collider.parent.rect.centerx + Teon._offset[0],collider.parent.rect.centery + Teon._offset[1]),collider.radius,kwargs.get("thickness",2))
        if kwargs.get("name") is not None:
            text = Text(text = kwargs.get("name"),size = kwargs.get("size",30),color = color) # type: ignore
            text.anchor = "center"
            pygame.display.get_surface().blit(text.image,(collider.center[0] - text.rect.width / 2 + Teon._offset[0],collider.center[1] - text.rect.height / 2 + Teon._offset[1]))

def get_main_class():
    if len(Teon._instances) == 1:
        return Teon._instances[0]
    raise teon.other.TeonCError("The main game class wasn't initiated!")