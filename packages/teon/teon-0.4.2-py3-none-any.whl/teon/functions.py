...
def colliding(entity1,entity2,specific_rect : tuple = (False,False)):
        ...
        if isinstance(entity2,Entity):
            ...
            if not circle1 and not circle2:
                ...
                closest_x = max(collider2.centerx, min(collider1.centerx, collider2.centerx + collider2.width))
                closest_y = max(collider2.centery, min(collider1.centery, collider2.centery + collider2.height))
                ...
                closest_x = max(collider1.centerx, min(collider2.centerx, collider1.centerx + collider1.width))
                closest_y = max(collider1.centery, min(collider2.centery, collider1.centery + collider1.height))

                distance = math.sqrt((closest_x - collider2.centerx) ** 2 + (closest_y - collider2.centery) ** 2)

                return distance < collider2.radius
                    
            else:
                distance = math.sqrt((collider2.centerx - collider1.centerx) ** 2 + (collider2.centery - collider1.centery) ** 2)
