...
from bourguibagpt.models.tiny import BourguibaT
from bourguibagpt.models.medium import BourguibaM
from bourguibagpt.models.big import BourguibaB
from bourguibagpt.utils.memory import get_system_memory, get_gpu_memory
from bourguibagpt.utils.system import get_os_info, system_has_gpu
...
def main() -> None:
    ...
    try:
        ...
        gpu_vram = get_gpu_memory() if has_gpu else 0

        # Select appropriate model
        if system_ram <= MODEL_CONFIG["tiny"]["ram_threshold"]:
            model = BourguibaT()
        elif has_gpu and gpu_vram > MODEL_CONFIG["medium"]["vram_threshold"]:
            model = BourguibaB(int(gpu_vram))
        else:
            model = BourguibaM()

        # Install and initialize model
        model.install_model()
