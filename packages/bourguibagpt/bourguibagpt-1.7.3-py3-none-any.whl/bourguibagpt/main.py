                ...
                if system == "Linux":
                    try:
                        ...
                        try:
                            ...
                            console.print("[yellow]systemctl failed, falling back to starting Ollama directly...[/yellow]")
                            # Start Ollama directly in the background
                            subprocess.Popen("ollama", shell=True)
                        time.sleep(5)
                        response = requests.get("http://localhost:11434/api/tags", timeout=self.timeout)
