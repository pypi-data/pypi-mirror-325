...
class H2OGPTEAsync:
    ...
    async def create_collection(
        ...
        if thumbnail is not None:
            await self.set_collection_thumbnail(collection_id, thumbnail)
    ...
    async def create_topic_model(
        ...
        ret = (
            await self._job("crawl.CreateTopicModelJob", collection_id=collection_id),
        )
        return await self._wait_for_completion(_to_id(ret), timeout=timeout)
    ...
    async def upload(
        self, file_name: str, file: Union[IO[bytes], bytes], uri: Optional[str] = None
    ) -> str:
    ...
    async def get_user_configurations_for_user(
        ...
        res = await self._db("get_user_configurations_for_user", user_id)
        return [UserConfigItem(**c) for c in res]

    async def set_user_configurations_for_user(
