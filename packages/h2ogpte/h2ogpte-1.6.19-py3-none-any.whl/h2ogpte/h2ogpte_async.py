...
class H2OGPTEAsync:
    ...
    async def create_collection(
        ...
        """
        rest_client = await self._collection_rest_api()
        ...
        collection = rest_client.create_collection(request, _headers=headers)
    ...
    async def create_topic_model(
        ...
        ret = (
            await self._job("q:crawl.CreateTopicModelJob", collection_id=collection_id),
        )
        return await self._wait_for_completion(_to_id(ret), timeout=timeout)
    ...
    async def _rest_client(self):
        configuration = rest.Configuration(host=self._address + "/api/v1")
        return rest.ApiClient(configuration)

    async def _collection_rest_api(self):
        rest_client = await self._rest_client()
        return rest.CollectionsApi(rest_client)
