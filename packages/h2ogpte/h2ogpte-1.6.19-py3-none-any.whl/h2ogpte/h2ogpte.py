...
class H2OGPTE:
    ...
    def _rest_client(self):
        ...
        return rest.ApiClient(configuration)

    def _collection_rest_api(self):
        return rest.CollectionsApi(self._rest_client())
    ...
    def run_selftest(self, llm: str, mode: str) -> dict:
        ...
        Args:
            llm:
                Name of LLM
            mode:
                one of ["quick", "rag", "full", "stress"]
        Returns:
            Dictionary with performance stats. If "error" is filled, the test failed.
        """
        return self._lang("selftest_sync", llm=llm, mode=mode)
    ...
    def get_agent_tools_dict(self) -> List[dict]:
        return self._lang("get_agent_tools_dict")
    ...
    def create_collection(
                ...
                Example:
                    ...
                    collection_settings=dict(
                        ...
                        guardrails_settings=dict(
                            ...
                            guardrails_entities={
                                "Safe": "Messages that do not contain any of the following unsafe content",  # must be first, not used as potential label to flag
        ...
        """
        rest_client = self._collection_rest_api()
        ...
        collection = rest_client.create_collection(
            request, _headers=self._get_auth_header()
        )
    ...
    def set_collection_thumbnail(
        ...
        return self._wait(
            self._job(
                ...
                uploadID=_to_id(unmarshal(res.text)),
            ),
            timeout=timeout,
        )
    ...
    def remove_collection_thumbnail(
        ...
        return self._wait(
            self._job(
                "q:crawl_quick.DeleteCollectionThumbnailJob",
                collection_id=collection_id,
            ),
            timeout=timeout,
        )
    ...
    def create_topic_model(
        ...
        return self._wait(
            self._job("q:crawl.CreateTopicModelJob", collection_id=collection_id),
            timeout=timeout,
        )
    ...
    def set_collection_expiry_date(
        ...
        res = self._db("set_collection_expiry_date", collection_id, iso_expiry_date)
        return _to_id(res)
    ...
    def remove_collection_expiry_date(self, collection_id: str) -> str:
        ...
        res = self._db("remove_collection_expiry_date", collection_id)
        return _to_id(res)
    ...
    def remove_collection_inactivity_interval(self, collection_id: str) -> str:
        ...
        res = self._db("remove_collection_inactivity_interval", collection_id)
        return _to_id(res)
    ...
    def unarchive_collection(self, collection_id: str) -> str:
        ...
        res = self._db("unarchive_collection", collection_id)
        return _to_id(res)
    ...
    def delete_documents(
        ...
        """
        return self._wait(
            self._job("q:crawl_quick.DeleteDocumentsJob", document_ids=document_ids),
            timeout=timeout,
        )
    ...
    def get_llm_usage_by_user(self, interval: str) -> List[UserWithLLMUsage]:
        return [
            UserWithLLMUsage(**d) for d in self._db("get_llm_usage_by_user", interval)
        ]

    def get_llm_usage_by_llm_and_user(self, interval: str) -> List[LLMWithUserUsage]:
        return [
            LLMWithUserUsage(**d)
            for d in self._db("get_llm_usage_by_llm_and_user", interval)
        ]
    ...
    def list_chat_message_references(
        ...
        return [
            ...
            for d in self._db("list_chat_message_references", message_id, limit)
        ]
    ...
    def rename_chat_session(self, chat_session_id: str, name: str):
        ...
        """
        self._db("rename_chat_session", chat_session_id, name)
    ...
    def update_document_name(self, document_id: str, name: str) -> str:
        ...
        return _to_id(self._db("update_document_name", document_id, name.strip()))
    ...
    def update_document_uri(self, document_id: str, uri: str) -> str:
        ...
        Returns:
            str: ID of the updated document.
        """
        return _to_id(self._db("update_document_uri", document_id, uri.strip()))
    ...
    def get_reasoning_capable_llm_names(self) -> List[str]:
        ...
        """
        return self._lang("get_reasoning_capable_llm_names")
    ...
    def get_llm_and_auto_reasoning_llm_names(self) -> Dict[str, str]:
        ...
        """
        return self._lang("get_llm_and_auto_reasoning_llm_names")
    ...
    def list_prompt_permissions(self, prompt_id: str) -> List[SharePermission]:
        ...
        """
        return [
            SharePermission(**d) for d in self._db("list_prompt_permissions", prompt_id)
        ]
    ...
    def add_user_document_permission(
        ...
        res = self._db("add_user_document_permission", user_id, document_id)

        return res

    def list_system_permissions(self) -> List[UserPermission]:
        return [UserPermission(**p) for p in self._db("get_permissions")]

    def list_system_roles(self) -> List[UserRole]:
        return [UserRole(**r) for r in self._db("get_roles")]

    def list_system_groups(self) -> List[UserGroup]:
        return [UserGroup(**r) for r in self._db("list_system_groups")]

    def list_user_roles(self, user_id: Optional[str] = None) -> List[UserRole]:
        if user_id:
            return [UserRole(**r) for r in self._db("get_other_user_roles", user_id)]
        return [UserRole(**r) for r in self._db("get_user_roles")]

    def list_group_roles(self, group_id: str) -> List[UserRole]:
        return [UserRole(**r) for r in self._db("get_group_roles", group_id)]
    ...
    def list_user_permissions(
        ...
        if user_id:
            return [
                UserPermission(**p)
                for p in self._db("get_other_user_permissions", user_id)
            ]
        return [UserPermission(**p) for p in self._db("get_user_permissions")]

    def list_group_permissions(self, group_id: str) -> List[UserPermission]:
        return [
            UserPermission(**p) for p in self._db("get_group_permissions", group_id)
        ]
    ...
    def list_group_permissions_by_name(
        ...
        return [
            UserPermission(**p)
            for p in self._db("get_group_permissions_by_name", group_names)
        ]

    def list_user_role_permissions(self, roles: List[str]) -> List[UserPermission]:
        return [UserPermission(**p) for p in self._db("get_role_permissions", roles)]

    def add_role_to_user(self, user_id: str, roles: List[str]) -> List[UserRole]:
        return [UserRole(**r) for r in self._db("add_role_to_user", user_id, roles)]

    def reset_roles_for_user(self, user_id: str, roles: List[str]) -> List[UserRole]:
        return [UserRole(**r) for r in self._db("reset_roles_for_user", user_id, roles)]

    def remove_role_from_user(self, user_id: str, roles: List[str]) -> List[UserRole]:
        return [
            UserRole(**r) for r in self._db("remove_role_from_user", user_id, roles)
        ]

    def add_role_to_group(self, group_id: str, roles: List[str]) -> List[UserRole]:
        return [UserRole(**r) for r in self._db("add_role_to_group", group_id, roles)]

    def reset_roles_for_group(self, group_id: str, roles: List[str]) -> List[UserRole]:
        return [
            UserRole(**r) for r in self._db("reset_roles_for_group", group_id, roles)
        ]

    def remove_role_from_group(self, group_id: str, roles: List[str]) -> List[UserRole]:
        return [
            UserRole(**r) for r in self._db("remove_role_from_group", group_id, roles)
        ]

    def is_permission_granted(self, permission: str) -> bool:
        return bool(self._db("is_permission_granted", permission))
    ...
    def create_user_role(self, name: str, description: str) -> UserRole:
        res = self._db("create_user_role", name, description)
        if len(res) == 0:
    ...
    def create_user_group(self, name: str, description: str) -> UserGroup:
        res = self._db("create_user_group", name, description)
        if len(res) == 0:
    ...
    def delete_user_roles_by_ids(self, roles_ids: Iterable[str]) -> Result:
        return Result(**self._db("delete_user_roles_by_ids", roles_ids))

    def delete_user_roles_by_names(self, roles_names: Iterable[str]) -> Result:
        return Result(**self._db("delete_user_roles_by_names", roles_names))

    def delete_user_groups_by_ids(self, groups_ids: Iterable[str]) -> Result:
        return Result(**self._db("delete_user_groups_by_ids", groups_ids))

    def delete_user_groups_by_names(self, groups_names: Iterable[str]) -> Result:
        return Result(**self._db("delete_user_groups_by_names", groups_names))

    def assign_permissions_to_role(
        self, role_name: str, permission_names: Iterable[str]
    ) -> Result:
        return Result(
            **self._db("assign_permissions_to_role", role_name, permission_names)
        )

    def get_user_configurations_for_user(self, user_id: str) -> List[UserConfigItem]:
        return [
            UserConfigItem(**c)
            for c in self._db("get_user_configurations_for_user", user_id)
        ]

    def set_user_configurations_for_user(
        self, key_name: str, string_value: str, value_type: str, user_id: str
    ) -> List[UserConfigItem]:
        res = self._db(
            "set_user_configurations_for_user",
            key_name,
            string_value,
            value_type,
            user_id,
    ...
    def reset_user_configurations_for_user(
        ...
        res = self._db("reset_user_configurations_for_user", key_name, user_id)
        return [UserConfigItem(**c) for c in res]

    def delete_agent_directories(self, chat_session_id: str) -> bool:
        res = self._lang("delete_agent_directories", chat_session_id=chat_session_id)
        return res
