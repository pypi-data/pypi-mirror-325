...
def move_private_equity_type(apps, schema_editor):
    Instrument = apps.get_model("wbportfolio", "Instrument")
    Equity = apps.get_model("wbportfolio", "Equity")
    private_equities = Equity.objects.filter(status="PRIVATE")
    Instrument.objects.filter(id__in=private_equities.values("instrument_ptr_id")).update(
        instrument_type="PrivateEquity"
    )


def migrate_price_computation(apps, schema_editor):
    Product = apps.get_model("wbportfolio", "Product")
    Index = apps.get_model("wbportfolio", "Index")
    PriceComputation = apps.get_model("wbportfolio", "PriceComputation")
    Instrument = apps.get_model("wbportfolio", "Instrument")
    ExchangeInstrumentThroughModel = apps.get_model("wbportfolio", "ExchangeInstrumentThroughModel")
    for pc in PriceComputation.objects.all():
        for instrument in Instrument.objects.filter(old_price_computation=pc):
            try:
                product = Product.objects.get(instrument_ptr_id=instrument.id)
