...
class Trihedron:
    ...
    :param angles: Standard axonometric notation; left/right angles from 'horizon' for x/y coordinate axis.
    :param position: Center of the trihedron by default but can be changed.
    :param size: The length of coordinate plane axes.
    ...
    def __init__(
        ...
        assert config.is_valid_angles(angles), "Angle can not be 0. Approximate zero with .1"
        # Initialize object
        config.logger.debug(
            f"[{self}] Draw trihedron in {position}, size {size}, with reference plane distance {ref_planes_distance}",
        )
    ...
    def _tilt_coordinate_planes(
        ...
        """Compute the three reference planes (XY, ZY, and ZX) tilted in the axonometry plane based on the angles defined for the trihedron.
        ...
        :param ref_planes_distance: The distance from the origin to each of the reference planes, defaults to .0.

        :return: A list containing tuples, where each tuple consists of a list of lines representing the axes of the tilted plane and a transformation matrix that can be applied to add geometric elements to the reference planes.
    ...
    def _tilt(self, angles: tuple[float, float]) -> tuple[float, float]:
        ...
        config.logger.debug(
            f"[Trihedron] Tilt angles for {int(math.degrees(angles[0]))}°/{int(math.degrees(angles[1]))}°: γ = {math.degrees(gamma):.2f}° / δ = {math.degrees(delta):.2f}°",
        )
        return (gamma, delta)
