from yta_general_utils.file.enums import FileType, FileExtension
...
class Output:
    ...
    def get_filename(
        ...
        extension: Union[None, FileExtension, FileType] = None
    ):
        """
        ...
        # 4. We need output and we pass output
            ...
            if PythonValidator.is_string(extension):
                if FileType.is_valid_name_or_value(extension):
                    extension = FileType.to_enum(extension)
                elif FileExtension.is_valid_name_or_value(extension):
                    extension = FileExtension.to_enum(extension)
                else:
                    raise Exception('The provided "exception" is an invalid string.')
            elif not PythonValidator.is_instance(extension, [FileExtension, FileType]):
                raise Exception('The provided "exception" is not a FileExtension nor a FileType instance.')
            
            output_filename = (
                output_filename
                if PythonValidator.is_instance(extension, FileType) and extension.is_filename_valid(output_filename)
