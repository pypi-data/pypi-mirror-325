...
class RegularExpression(Enum):
    ...
    def parse(
        ...
        """
        Check this Regular Expression with the provided 'string'.
    ...
    def get_matching_group(
        ...
        """
        Check this regular expression with the given 'string'
        and obtain the matched group in 'group_index' index.
        The regular expression must have group parts to be
        able to find these group matches.
        
        Remember that a group regular expression part is one
        wrapped in parenthesis.
        """
        if not PythonValidator.is_string(string):
            raise Exception(f'The provided "string" parameter "{string}" is not a string.')
        
        if not NumberValidator.is_positive_number(group_index, do_include_zero = True):
            raise Exception(f'The provided "group_index" parameter "{str(group_index)}" is not a positive number.')
        
        matches = re.search(self, string)

        # Giving an unvalid 'group_index' could cause an
        # unexpected behaviour
        return matches.group(group_index) if matches else None
