...
class Graphic:
    ...
    def add_node(self, x: float, y: float):
        ...
        if y < self.y_axis.min or y > self.y_axis.max:
    ...
    def get_xy_from_normalized_d(self, d: float):
        """
        ...
        axis. The lower (left) 'x' of the graphic would be
    ...
    def get_xy_from_not_normalized_x(self, x: float) -> tuple[float, float]:
        """
        Return a not normalized tuple (x, y) representing
        ...
        return (x, self._get_pair_of_node_from_x(x).get_y_from_not_normalized_x(x).value)
    
    def _get_node(self, x: float):
        """
    ...
    def _get_x_from_normalized_d(self, d: float):
        ...
        pair_of_nodes_index = int(d // pair_of_nodes_d) if d != 1.0 else num_of_pairs_of_nodes - 1
        ...
        if d != 1.0:
            d = (
                ...
                if pair_of_nodes_index > 0 else d / pair_of_nodes_d
            )
        ...
        x = pair_of_nodes.left_node.x + d * (pair_of_nodes.right_node.x - pair_of_nodes.left_node.x)
