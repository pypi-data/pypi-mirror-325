...
modify the contents of the `src/sink/api/sdk/lib/` and `examples/` directories.
...
```sh
$ pip install git+ssh://git@github.com/stainless-sdks/sink-python-public#master.git
```
...
You can release to package managers by using [the `Publish PyPI` GitHub action](https://www.github.com/stainless-sdks/sink-python-public/actions/workflows/publish-pypi.yml). This requires a setup organization or repository secret to be set up.
