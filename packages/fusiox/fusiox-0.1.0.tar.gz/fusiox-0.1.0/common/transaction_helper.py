    ...
    try:
        with pd.ExcelWriter(output_file, engine='xlsxwriter') as writer:
            for sheet in data:
                ...
                # Create DataFrame dynamically based on headers
                formatted_data = []
                for row in data_rows:
                    formatted_row = [row.get(header, "") for header in headers]  # Default to "" for missing headers
                ...
                df.to_excel(writer, index=False, sheet_name=sheet_name, startrow=8, header=False)  # Data starts from row 9
                ...
                title_range = f"A1:{chr(65 + last_col - 1)}1"  # Adjust for the column length dynamically
                worksheet.merge_range(title_range, f"{sheet_name}", title_format)

                # Add "Filter By" row
                worksheet.write('A2', 'Filter By :', label_format)

                # Create dynamic filter string, excluding empty filters
                if year:
                    worksheet.write('A3', f"Year: {year}".title(), label_format)  # Huruf besar pertama untuk YEAR

                if month:
                    try:
                        # Convert month from int to full month name
                        month_name = calendar.month_name[int(month)].title()  # Huruf pertama kapital, lainnya kecil
                        worksheet.write('A4', f"Month: {month_name}", label_format)  # Awalan kapital
                    except (ValueError, IndexError):
                        worksheet.write('A4', f"Month: {month}", label_format)  # Gunakan input asli jika gagal

                if department:
                    worksheet.write('A5', f"Department: {department}".title(), label_format)  # Huruf besar semua untuk DEPARTMENT

                if floor:
                    worksheet.write('A6', f"Floor: {floor}".title(), label_format)  # Awalan kapital untuk Floor

                # Generate on timestamp
                generate_on_text = f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}".capitalize()
                worksheet.write('A7', generate_on_text, label_format)

                # Write the headers in row 8 (just below the filter)
                for col_num, header in enumerate(headers):
                    worksheet.write(7, col_num, header, header_format)  # Header in row 8 (adjusted)
                ...
                # Apply border around the entire data range
                last_row = len(formatted_data) + 8  # Data starts from row 9, so +8
                last_col = len(headers)

                border_format = workbook.add_format({'border': 1})

                worksheet.conditional_format(f"A8:{chr(64 + last_col)}{last_row}", {
                    'type': 'no_blanks',
