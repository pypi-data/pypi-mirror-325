...
from cropioai.project import CropioBase, agent, task, cropio, before_kickoff, after_kickoff
...
class YourCropioName:
    ...
    # - Task: https://docs.cropioai.com/concepts/tasks#yaml-configuration-recommended
...
- `@before_kickoff`: (Optional) Marks a method to be executed before the cropio starts.
- `@after_kickoff`: (Optional) Marks a method to be executed after the cropio finishes.
...
Once your cropio is assembled, initiate the workflow with the `kickoff()` method. This starts the execution process according to the defined process flow.

```python Code
# Start the cropio's task execution
result = my_cropio.kickoff()
print(result)
```

### Different Ways to Kick Off a Cropio

Once your cropio is assembled, initiate the workflow with the appropriate kickoff method. CropioAI provides several methods for better control over the kickoff process: `kickoff()`, `kickoff_for_each()`, `kickoff_async()`, and `kickoff_for_each_async()`.
...
results = my_cropio.kickoff_for_each(inputs=inputs_array)
for result in results:
    print(result)

# Example of using kickoff_async
inputs = {'topic': 'AI in healthcare'}
async_result = my_cropio.kickoff_async(inputs=inputs)
print(async_result)

# Example of using kickoff_for_each_async
inputs_array = [{'topic': 'AI in healthcare'}, {'topic': 'AI in finance'}]
async_results = my_cropio.kickoff_for_each_async(inputs=inputs_array)
