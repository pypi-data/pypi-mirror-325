---
title: Kickoff Cropio Asynchronously
description: Kickoff a Cropio Asynchronously
icon: rocket-launch
---

## Introduction

CropioAI provides the ability to kickoff a cropio asynchronously, allowing you to start the cropio execution in a non-blocking manner. 
...
To kickoff a cropio asynchronously, use the `kickoff_async()` method. This method initiates the cropio execution in a separate thread, allowing the main thread to continue executing other tasks.

### Method Signature

```python Code
def kickoff_async(self, inputs: dict) -> CropioOutput:
```
...
Here's an example of how to kickoff a cropio asynchronously using asyncio and awaiting the result:
...
# Async function to kickoff the cropio asynchronously
async def async_cropio_execution():
    result = await analysis_cropio.kickoff_async(inputs={"ages": [25, 30, 35, 40, 45]})
...
In this example, we'll show how to kickoff multiple cropios asynchronously and wait for all of them to complete using `asyncio.gather()`:
...
# Async function to kickoff multiple cropios asynchronously and wait for all to finish
async def async_multiple_cropios():
    result_1 = cropio_1.kickoff_async(inputs={"ages": [25, 30, 35, 40, 45]})
    result_2 = cropio_2.kickoff_async(inputs={"ages": [20, 22, 24, 28, 30]})
