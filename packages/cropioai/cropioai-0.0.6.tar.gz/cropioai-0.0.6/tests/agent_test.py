...
from cropioai.agents.cropio_agent_executor import CropioAgentExecutor
from cropioai.agents.parser import AgentAction, CropioAgentParser, OutputParserException
...
from cropioai.tools.tool_calling import InstructorToolCalling
from cropioai.tools.tool_usage import ToolUsage
from cropioai.tools.tool_usage_events import ToolUsageFinished
from cropioai.utilities import RPMController
from cropioai.utilities.events import Emitter
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_agent_respect_the_max_rpm_set_over_cropio_rpm(capsys):
    from unittest.mock import patch
...
@pytest.mark.vcr(filter_headers=["authorization"])
def test_agent_without_max_rpm_respects_cropio_rpm(capsys):
    from unittest.mock import patch
    ...
    # Set cropio's max_rpm to 1 to trigger RPM limit
    cropio = Cropio(agents=[agent1, agent2], tasks=tasks, max_rpm=1, verbose=True)
...
def test_agent_count_formatting_error():
    ...
    parser = CropioAgentParser(agent=agent1)
...
def test_agent_human_input():
    ...
    with patch.object(
        CropioAgentExecutor, "_ask_human_input", side_effect=ask_human_input_side_effect
    ) as mock_human_input:
        # Execute the task
        output = agent.execute_task(task)
...
"""
    ...
    with patch.object(CropioAgentExecutor, "_format_prompt") as mock_format_prompt:
...
@patch("cropioai.agent.CropioTrainingHandler")
def test_agent_training_handler(cropio_training_handler):
    ...
    cropio_training_handler().load.return_value = {
    ...
    cropio_training_handler.assert_has_calls(
        [mock.call(), mock.call("training_data.pkl"), mock.call().load()]
    )


@patch("cropioai.agent.CropioTrainingHandler")
def test_agent_use_trained_data(cropio_training_handler):
    task_prompt = "What is 1 + 1?"
    agent = Agent(
        role="researcher",
        goal="test goal",
...
def test_agent_max_retry_limit():
    ...
    with patch.object(
        CropioAgentExecutor, "invoke", wraps=agent.agent_executor.invoke
    ) as invoke_mock:
...
def test_handle_context_length_exceeds_limit():
    ...
    with patch.object(
        ...
        with patch.object(
            CropioAgentExecutor, "_handle_context_length"
...
def test_agent_with_knowledge_sources():
    ...
    with patch(
        "cropioai.knowledge.storage.knowledge_storage.KnowledgeStorage"
    ) as MockKnowledge:
...
def test_agent_with_knowledge_sources_works_with_copy():
    ...
    with patch(
        "cropioai.knowledge.source.base_knowledge_source.BaseKnowledgeSource",
        autospec=True,
...
def test_cropio_agent_executor_litellm_auth_error():
    """Test that CropioAgentExecutor handles LiteLLM authentication errors by raising them."""
    from litellm.exceptions import AuthenticationError

    from cropioai.agents.tools_handler import ToolsHandler
    from cropioai.utilities import Printer
