...
from cropioai.agents.agent_builder.base_agent_executor_mixin import CropioAgentExecutorMixin
from cropioai.agents.parser import (
...
from cropioai.tools.tool_usage import ToolUsage, ToolUsageErrorException
from cropioai.utilities import I18N, Printer
from cropioai.utilities.constants import MAX_LLM_RETRY, TRAINING_DATA_FILE
from cropioai.utilities.exceptions.context_window_exceeding_exception import (
    LLMContextLengthExceededException,
)
from cropioai.utilities.logger import Logger
...
class CropioAgentExecutor(CropioAgentExecutorMixin):
    ...
    def _show_start_logs(self):
        ...
        if self.agent.verbose or (
            hasattr(self, "cropio") and getattr(self.cropio, "verbose", False)
        ):
    ...
    def _handle_context_length(self) -> None:
        if self.respect_context_window:
            ...
            self._printer.print(
                content="Context length exceeded. Consider using smaller text or RAG tools from cropioai_tools.",
                color="red",
            )
            raise SystemExit(
                "Context length exceeded and user opted not to summarize. Consider using smaller text or RAG tools from cropioai_tools."
            )

    def _handle_cropio_training_output(
        self, result: AgentFinish, human_feedback: Optional[str] = None
    ) -> None:
        """Handle the process of saving training data."""
        agent_id = str(self.agent.id)  # type: ignore
        train_iteration = (
            getattr(self.cropio, "_train_iteration", None) if self.cropio else None
        )
        ...
        training_handler = CropioTrainingHandler(TRAINING_DATA_FILE)
    ...
    def _format_answer(self, answer: str) -> Union[AgentAction, AgentFinish]:
        return CropioAgentParser(agent=self.agent).parse(answer)
    ...
    def _is_training_mode(self) -> bool:
        """Check if cropio is in training mode."""
        return bool(self.cropio and self.cropio._train)
    ...
    def _handle_training_feedback(
        ...
        self._handle_cropio_training_output(initial_answer, feedback)
        ...
        self._handle_cropio_training_output(improved_answer)
        self.ask_for_human_input = False
