...
description: A comprehensive guide to utilizing the sequential processes for task execution in CropioAI projects.
icon: forward
---

## Introduction

CropioAI offers a flexible framework for executing tasks in a structured manner, supporting both sequential and hierarchical processes. 
...
To use the sequential process, assemble your cropio and define tasks in the order they need to be executed.

```python Code
from cropioai import Cropio, Process, Agent, Task, TaskOutput, CropioOutput
...
# Form the cropio with a sequential process
report_cropio = Cropio(
...
# Execute the cropio
result = report_cropio.kickoff()

# Accessing the type-safe output
task_output: TaskOutput = result.tasks[0].output
cropio_output: CropioOutput = result.output
```
...
In sequential processes, if an agent has `allow_delegation` set to `True`, they can delegate tasks to other agents in the cropio. 
This feature is automatically set up when there are multiple agents in the cropio.
...
CropioAI supports both memory and caching features:

- **Memory**: Enable by setting `memory=True` when creating the Cropio. This allows agents to retain information across tasks.
...
CropioAI tracks token usage across all tasks and agents. You can access these metrics after execution.
