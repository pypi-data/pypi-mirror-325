...
from cropioai.knowledge.source.base_knowledge_source import BaseKnowledgeSource
...
from cropioai.memory.user.user_memory import UserMemory
...
from cropioai.tools.agent_tools.agent_tools import AgentTools
from cropioai.tools.base_tool import Tool
from cropioai.types.usage_metrics import UsageMetrics
from cropioai.utilities import I18N, FileHandler, Logger, RPMController
from cropioai.utilities.constants import TRAINING_DATA_FILE
from cropioai.utilities.evaluators.cropio_evaluator_handler import CropioEvaluator
from cropioai.utilities.evaluators.task_evaluator import TaskEvaluator
from cropioai.utilities.formatter import (
    aggregate_raw_outputs_from_task_outputs,
    aggregate_raw_outputs_from_tasks,
)
from cropioai.utilities.llm_utils import create_llm
from cropioai.utilities.planning_handler import CropioPlanner
from cropioai.utilities.task_output_storage_handler import TaskOutputStorageHandler
from cropioai.utilities.training_handler import CropioTrainingHandler
...
class Cropio(BaseModel):
    """
    Represents a group of agents, defining how they should collaborate and the tasks they should perform.

    Attributes:
        tasks: List of tasks assigned to the cropio.
        agents: List of agents part of this cropio.
        manager_llm: The language model that will run manager agent.
        manager_agent: Custom agent that will be used as manager.
        memory: Whether the cropio should use memory to store memories of it's execution.
        memory_config: Configuration for the memory to be used for the cropio.
        cache: Whether the cropio should use a cache to store the results of the tools execution.
        function_calling_llm: The language model that will run the tool calling for all the agents.
        process: The process flow that the cropio will follow (e.g., sequential, hierarchical).
        verbose: Indicates the verbosity level for logging during execution.
        config: Configuration settings for the cropio.
        max_rpm: Maximum number of requests per minute for the cropio execution to be respected.
        prompt_file: Path to the prompt json file to be used for the cropio.
        id: A unique identifier for the cropio instance.
        task_callback: Callback to be executed after each task for every agents execution.
        step_callback: Callback to be executed after each step for every agents execution.
        share_cropio: Whether you want to share the complete cropio information and execution with cropioAI to make the library better, and allow us to train models.
        planning: Plan the cropio execution and add the plan to the cropio.
        chat_llm: The language model used for orchestrating chat interactions with the cropio.
    """
    ...
    memory: bool = Field(
        default=False,
        description="Whether the cropio should use memory to store memories of it's execution",
    )
    memory_config: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Configuration for the memory to be used for the cropio.",
    )
    short_term_memory: Optional[InstanceOf[ShortTermMemory]] = Field(
        default=None,
        description="An Instance of the ShortTermMemory to be used by the Cropio",
    )
    long_term_memory: Optional[InstanceOf[LongTermMemory]] = Field(
        default=None,
        description="An Instance of the LongTermMemory to be used by the Cropio",
    )
    entity_memory: Optional[InstanceOf[EntityMemory]] = Field(
        default=None,
        description="An Instance of the EntityMemory to be used by the Cropio",
    )
    user_memory: Optional[InstanceOf[UserMemory]] = Field(
        default=None,
        description="An instance of the UserMemory to be used by the Cropio to store/fetch memories of a specific user.",
    )
    embedder: Optional[dict] = Field(
        default=None,
        description="Configuration for the embedder to be used for the cropio.",
    )
    ...
    share_cropio: Optional[bool] = Field(default=False)
    ...
    max_rpm: Optional[int] = Field(
        default=None,
        description="Maximum number of requests per minute for the cropio execution to be respected.",
    )
    prompt_file: str = Field(
        default=None,
        description="Path to the prompt json file to be used for the cropio.",
    )
    ...
    planning: Optional[bool] = Field(
        default=False,
        description="Plan the cropio execution and add the plan to the cropio.",
    )
    ...
    knowledge_sources: Optional[List[BaseKnowledgeSource]] = Field(
        default=None,
        description="Knowledge sources for the cropio. Add knowledge sources to the knowledge object.",
    )
    chat_llm: Optional[Any] = Field(
        default=None,
        description="LLM used to handle chatting with the cropio.",
    )
    knowledge: Optional[Knowledge] = Field(
        default=None,
        description="Knowledge for the cropio.",
    )
    ...
    def set_private_attrs(self) -> "Cropio":
    ...
    @model_validator(mode="after")
    def create_cropio_memory(self) -> "Cropio":
        """Set private attributes."""
        if self.memory:
            ...
            self._entity_memory = (
                ...
                else EntityMemory(cropio=self, embedder_config=self.embedder)
            )
            if hasattr(self, "memory_config") and self.memory_config is not None:
                self._user_memory = (
                    self.user_memory if self.user_memory else UserMemory(cropio=self)
                )
            else:
    ...
    @model_validator(mode="after")
    def create_cropio_knowledge(self) -> "Cropio":
        """Create the knowledge for the cropio."""
    ...
    def check_config(self):
        """Validates that the cropio is properly configured with agents and tasks."""
    ...
    def validate_end_with_at_most_one_async_task(self):
        """Validates that the cropio ends with at most one asynchronous task."""
    ...
    @model_validator(mode="after")
    def validate_first_task(self) -> "Cropio":
    ...
    @model_validator(mode="after")
    def validate_async_tasks_not_async(self) -> "Cropio":
    ...
    def _setup_for_training(self, filename: str) -> None:
        """Sets up the cropio for training."""
        self._train = True
        ...
        CropioTrainingHandler(TRAINING_DATA_FILE).initialize_file()
        CropioTrainingHandler(filename).initialize_file()

    def train(
        self, n_iterations: int, filename: str, inputs: Optional[Dict[str, Any]] = {}
    ) -> None:
        """Trains the cropio for a given number of iterations."""
        train_cropio = self.copy()
        train_cropio._setup_for_training(filename)
        ...
        try:
            ...
            for agent in train_cropio.agents:
                if training_data.get(str(agent.id)):
                    result = TaskEvaluator(agent).evaluate_training_data(
                        training_data=training_data, agent_id=str(agent.id)
                    )
                    CropioTrainingHandler(filename).save_trained_data(
                        agent_id=str(agent.role), trained_data=result.model_dump()
                    )
        except Exception as e:
            self._logger.log("error", f"Training failed: {e}", color="red")
            CropioTrainingHandler(TRAINING_DATA_FILE).clear()
    ...
    def kickoff(
        ...
        for agent in self.agents:
            agent.i18n = i18n
            # type: ignore[attr-defined] # Argument 1 to "_interpolate_inputs" of "Cropio" has incompatible type "dict[str, Any] | None"; expected "dict[str, Any]"
            agent.cropio = self  # type: ignore[attr-defined]
    ...
    def kickoff_for_each(self, inputs: List[Dict[str, Any]]) -> List[CropioOutput]:
        ...
        # Initialize the parent cropio's usage metrics
    ...
    def _handle_cropio_planning(self):
        """Handles the Cropio planning."""
        self._logger.log("info", "Planning the cropio execution")
        result = CropioPlanner(
            tasks=self.tasks, planning_agent_llm=self.planning_llm
        )._handle_cropio_planning()
    ...
    def _run_sequential_process(self) -> CropioOutput:
        """Executes tasks sequentially and returns the final output."""
        return self._execute_tasks(self.tasks)

    def _run_hierarchical_process(self) -> CropioOutput:
        """Creates and assigns a manager agent to make sure the cropio completes the tasks."""
    ...
    def _execute_tasks(
        ...
        Returns:
            CropioOutput: Final output of the cropio
        """
        ...
        return self._create_cropio_output(task_outputs)
    ...
    def replay(
        ...
        if not stored_outputs:
            raise ValueError(f"Task with id {task_id} not found in the cropio's tasks.")
    ...
    def copy(self):
        """Create a deep copy of the Cropio."""
    ...
    def _finish_execution(self, final_string_output: str) -> None:
        ...
        self._telemetry.end_cropio(self, final_string_output)
    ...
    def test(
        ...
        """Test and evaluate the Cropio with the given inputs for n iterations concurrently using concurrent.futures."""
        test_cropio = self.copy()

        self._test_execution_span = test_cropio._telemetry.test_execution_span(
            test_cropio,
            n_iterations,
            inputs,
            openai_model_name,  # type: ignore[arg-type]
        )  # type: ignore[arg-type]
        evaluator = CropioEvaluator(test_cropio, openai_model_name)  # type: ignore[arg-type]
    ...
    def __repr__(self):
        return f"Cropio(id={self.id}, process={self.process}, number_of_agents={len(self.agents)}, number_of_tasks={len(self.tasks)})"
