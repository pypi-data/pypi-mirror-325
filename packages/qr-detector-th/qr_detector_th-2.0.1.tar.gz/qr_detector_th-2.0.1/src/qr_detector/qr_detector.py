...
class QRDetector:
    ...
    def decode_results(self, img: np.ndarray) -> Optional[List[str]]:
        ...
        decoded = list(filter(None, [results.decode() for result in results]))
    ...
    async def read_from_bytes_decoded(self, img_bytes: bytes) -> Optional[List[str]]:
        ...
        decoded = list(filter(None, [results.decode() for results in results]))
        return decoded if decoded else None
                ...
                try:
                    ...
                    for result in results:
                        ...
                        if decoded:
                            ...
                            cv2.waitKey(1000)  # แสดงเฟรมสุดท้าย 1 วินาที
                            # print(f"\n🎯 ผลการสแกน: {decoded}")
