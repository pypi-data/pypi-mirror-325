...
def cybersource_api(domain, creditCard, proxy):
    ...
    try:
        ...
        if len(split_text) > 1:
            ...
            return print(f"[{R2}-{R}] {domain} {R2}{cc}|{mm}|{yy}|{cvv}{R} capture_context variable not found in the response.")
    ...
    try:
        ...
        return print(f"[{Y}!{R}] {domain} {Y}{cc}|{mm}|{yy}|{cvv}{R} requests 5 An error occurred during processing: {e}")
    ...
    try:
        ...
        if 'order-received' in status:
            ...
            return print(f"[{R2}-{R}] {domain} {R2}{cc}|{mm}|{yy}|{cvv}{R} An error occurred, please try again or try an alternate form of payment.")
