...
def stripe_cc_api(domain, creditCard, proxy):
    ...
    try:
        ...
        if len(split_text) > 1:
            ...
            return print(f"[{Y}!{R}] {domain} {cc}|{mm}|{yy}|{cvv} pk not found in the response.")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return print(f"[{Y}!{R}] {domain} {cc}|{mm}|{yy}|{cvv} {message}")
        else:
        ...
        if 'success' in status:
            ...
            try:
                ...
                if three_d_secure_2_source is None:
                    return print(f"[{Y}!{R}] {domain} {cc}|{mm}|{yy}|{cvv} three_d_secure_2_source not found in the response.")
                elif server_transaction_id is None:
                    return print(f"[{Y}!{R}] {domain} {cc}|{mm}|{yy}|{cvv} server_transaction_id not found in the response.")
            ...
            try:
                ...
                if 'challenge_required' in state:
                    return print(f"[{R2}-{R}] {domain} {cc}|{mm}|{yy}|{cvv} {state}")
            ...
            try:
                ...
                if 'succeeded' in status:
                    return print(f"[{G}+{R}] {domain} {cc}|{mm}|{yy}|{cvv} {status}")
                elif 'requires_payment_method' in status:
                    return print(f"[{R2}-{R}] {domain} {cc}|{mm}|{yy}|{cvv} {decline_code}")
                else:
                    return print(f"[{Y}!{R}] {domain} {cc}|{mm}|{yy}|{cvv} {response.text}")

            except Exception as e:
                return print(f"[{Y}!{R}] {domain} {cc}|{mm}|{yy}|{cvv} requests 8 An error occurred during processing: {e}")
