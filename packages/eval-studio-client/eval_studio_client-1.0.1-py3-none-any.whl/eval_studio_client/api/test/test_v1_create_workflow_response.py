# coding: utf-8

"""
    ai/h2o/eval_studio/v1/collection.proto

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: version not set
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from eval_studio_client.api.models.v1_create_workflow_response import V1CreateWorkflowResponse

class TestV1CreateWorkflowResponse(unittest.TestCase):
    """V1CreateWorkflowResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1CreateWorkflowResponse:
        """Test V1CreateWorkflowResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1CreateWorkflowResponse`
        """
        model = V1CreateWorkflowResponse()
        if include_optional:
            return V1CreateWorkflowResponse(
                workflow = eval_studio_client.api.models.v1_workflow.v1Workflow(
                    name = '', 
                    display_name = '', 
                    description = '', 
                    create_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    creator = '', 
                    update_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    updater = '', 
                    delete_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    deleter = '', 
                    type = 'WORKFLOW_TYPE_UNSPECIFIED', 
                    model = '', 
                    nodes = [
                        ''
                        ], 
                    edges = [
                        ''
                        ], 
                    outputs = {
                        'key' : None
                        }, 
                    output_artifacts = {
                        'key' : eval_studio_client.api.models.v1_workflow_node_artifacts.v1WorkflowNodeArtifacts(
                            artifacts = [
                                eval_studio_client.api.models.v1_workflow_node_artifact.v1WorkflowNodeArtifact(
                                    name = '', 
                                    parent = '', 
                                    display_name = '', 
                                    description = '', 
                                    create_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                                    creator = '', 
                                    update_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                                    updater = '', 
                                    delete_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                                    deleter = '', 
                                    mime_type = '', )
                                ], )
                        }, 
                    llm_model = '', 
                    model_parameters = '', )
            )
        else:
            return V1CreateWorkflowResponse(
        )
        """

    def testV1CreateWorkflowResponse(self):
        """Test V1CreateWorkflowResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
