# coding: utf-8

"""
    ai/h2o/eval_studio/v1/collection.proto

    No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

    The version of the OpenAPI document: version not set
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from eval_studio_client.api.models.v1_delete_workflow_node_response import V1DeleteWorkflowNodeResponse

class TestV1DeleteWorkflowNodeResponse(unittest.TestCase):
    """V1DeleteWorkflowNodeResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1DeleteWorkflowNodeResponse:
        """Test V1DeleteWorkflowNodeResponse
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1DeleteWorkflowNodeResponse`
        """
        model = V1DeleteWorkflowNodeResponse()
        if include_optional:
            return V1DeleteWorkflowNodeResponse(
                node = eval_studio_client.api.models.v1_workflow_node.v1WorkflowNode(
                    name = '', 
                    parent = '', 
                    display_name = '', 
                    description = '', 
                    create_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    creator = '', 
                    update_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    updater = '', 
                    delete_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                    deleter = '', 
                    type = 'WORKFLOW_NODE_TYPE_UNSPECIFIED', 
                    parameters = eval_studio_client.api.models.parameters.parameters(), 
                    outputs = eval_studio_client.api.models.outputs.outputs(), 
                    output_artifacts = [
                        eval_studio_client.api.models.v1_workflow_node_artifact.v1WorkflowNodeArtifact(
                            name = '', 
                            parent = '', 
                            display_name = '', 
                            description = '', 
                            create_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            creator = '', 
                            update_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            updater = '', 
                            delete_time = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'), 
                            deleter = '', 
                            mime_type = '', )
                        ], 
                    status = 'WORKFLOW_NODE_STATUS_UNSPECIFIED', 
                    attributes = eval_studio_client.api.models.v1_workflow_node_attributes.v1WorkflowNodeAttributes(
                        can_delete = True, ), 
                    processed_by_operation = '', )
            )
        else:
            return V1DeleteWorkflowNodeResponse(
        )
        """

    def testV1DeleteWorkflowNodeResponse(self):
        """Test V1DeleteWorkflowNodeResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
