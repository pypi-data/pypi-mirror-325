...
class ContactService(Component):
    ...
    def build_contact_dict(self, contact: Contact):
        contact_dict = contact.dict(exclude_unset=True, exclude=EXCLUDED_CONTACT_FIELDS)
    ...
    def include_tags_in_contact(self, contact: dict, tags):
        ...
        _logger.debug(f"Adding tags {[tag.name for tag in found_tags]} to contact {contact['fisrtname']}")
