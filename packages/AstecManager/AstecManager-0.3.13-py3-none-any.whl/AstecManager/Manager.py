...
class Manager:
    ...
    def upload_intrareg_dir(self, project_name, step_folder, pyom):
        ...
        for subdir in step_subdirs:
            ...
            for trsf in intra_trsfdirs:
                ...
                self.upload_step_dir(project_name, os.path.join(os.path.join(step_folder, subdir), trsf), pyom,
                                     prefix=final_prefix)
    ...
    def copy_raw_data(self, parameters):
        ...
        try:
            command = "rsync -avzr --exclude=.DS_Store --exclude=*.py --exclude=*.json --exclude=*.log "+str(str(input_folder)+"/*")+" "+str(raw_data_output + "/")
            ...
            if copy_on_cold_storage:
                ...
                try:
                    ...
                    command = "rsync -avzr --exclude=.DS_Store --exclude=*.py --exclude=*.json --exclude=*.log " + str(folder_to_copy) + " " + cold_storage_address+":"+cold_storage_container
                    subprocess.run(command,shell=True)
                    if compress:
                        print("Compressing on distant storage")
                        command = "ssh -t "+str(cold_storage_address)+" 'gzip "+str(os.path.join(cold_storage_container, parameters["embryo_name"]))+"/*/*/*.h5'"
                        subprocess.run(command,shell=True)
                        command = "ssh -t "+str(cold_storage_address)+" 'gzip "+str(os.path.join(cold_storage_container, parameters["embryo_name"]))+"/*/*.h5'"
                        subprocess.run(command, shell=True)
