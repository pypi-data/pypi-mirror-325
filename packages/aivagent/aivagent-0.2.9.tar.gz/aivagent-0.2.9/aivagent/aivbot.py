...
def run(botId=None, logLevel = 'INFO', option = None) -> None:
    ...
    try:
        ...
        async def _main():
            ...
            while True:
                await asyncio.sleep(0.2) #只要主程序不退出,协程就一直运行
...
class AivBotInfo:
    ...
    def __init__(self) -> None:
        ...
        self.apiList = ['init', 'param', 'setup', 'service']    # Aiv平台的api列表
...
class AivBot:
    ...
    async def _onStartTask(self,task):
        ...
        try:
            ...
            # apiOption = None
            for apiObj in self.api:
                if apiObj['name'].strip().lower() == apiName:
                    apiFun = apiObj['fun']
            ...
            if param.get('$getApiParam', None) is not None: # 执行获取api参数的固定函数 2025.1
                resData = await self._getApiParam(apiName, apiFun)
            
            else: # 执行客户端指定的Bot函数 2024.10
                if apiName in self.botInfo.apiList:
                    raise Exception(f'请勿直接调用系统的Api同名函数: {self.botInfo.apiList}')
