...
class IcebergOfflineStoreScanNode(OfflineStoreScanNode):
    @classmethod
    def from_query_node(cls, query_node: QueryNode) -> "IcebergOfflineStoreScanNode":
        kwargs = attrs.asdict(query_node, recurse=False)
        del kwargs["node_id"]
        return cls(**kwargs)
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        reader = IcebergReader(fdw=self.feature_definition_wrapper)
        files = reader.get_files_to_read(time_limits=self.partition_time_filter)
