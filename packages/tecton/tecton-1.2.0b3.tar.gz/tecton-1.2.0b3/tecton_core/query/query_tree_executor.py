...
def rewrite_offline_scan_node(plan: NodeRef) -> None:
    staging_nodes = get_staging_nodes(plan, QueryTreeStep.OFFLINE_STORE, as_ref=True)
    if not staging_nodes:
        return

    for _, staging_node in staging_nodes.items():
        scan_node = get_first_input_node_of_class(staging_node, OfflineStoreScanNode)
        if not scan_node:
            continue

        # TODO: replace with more explicit check for Iceberg config once it's part of the fd proto
        # At the moment it's the only thing that support the DUCKDB offline scan dialect
        if staging_node.node.dialect == Dialect.DUCKDB:
            physical_node = IcebergOfflineStoreScanNode.from_query_node(scan_node)
            staging_node.node = attrs.evolve(staging_node.node, input_node=physical_node.as_ref())
...
def logical_plan_to_physical_plan(logical_plan: NodeRef, use_optimized_full_agg: bool = False) -> NodeRef:
    ...
    rewrite_offline_scan_node(physical_plan)
...
class QueryTreeExecutor:
    ...
    def _process_staging_node(
        ...
        checkpoint_as = staging_table_name if staging_node.checkpoint else None
        return_dataframe = not checkpoint_as

        if isinstance(compute, DuckDBCompute):
            s3_options = None
            offline_scan_node = get_first_input_node_of_class(staging_node.as_ref(), node_class=OfflineStoreScanNode)
            if offline_scan_node and offline_scan_node.feature_definition_wrapper.materialized_data_path.startswith(
                "s3"
            ):
                s3_options = get_s3_options_for_fd(
                    offline_scan_node.feature_definition_wrapper, self.offline_store_options_providers
                )
