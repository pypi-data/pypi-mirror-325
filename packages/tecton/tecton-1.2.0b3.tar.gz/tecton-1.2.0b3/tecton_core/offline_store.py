...
class IcebergReader(OfflineStoreReader):
    ...
    def __init__(
        ...
        num_entity_bucket = fdw.offline_store_config.iceberg.num_entity_buckets
        assert num_entity_bucket, "Iceberg offline store must be configured with `num_entity_buckets`"
        self._bucket_transformer_name = f"bucket_transform_{num_entity_bucket}_{uuid.uuid4().hex[:8]}"
        self._duckdb_conn.create_function(
            self._bucket_transformer_name,
            bucket_transform(num_entity_bucket),
            return_type=INTEGER,
        )

    @staticmethod
    def join_keys_expression(join_keys: List[str]) -> str:
        return join_keys[0]

    def read(
        self,
        time_limits: Optional[pendulum.Period] = None,
        join_keys_table: Optional[pyarrow.Table] = None,
    ) -> "pyarrow.dataset.Dataset":
        files_to_read = self.get_files_to_read(time_limits, join_keys_table)
    ...
    def get_files_to_read(
        ...
        if time_limits:
            if self._fdw.is_temporal_aggregate:
                ...
                start_time_str = time_limits.start.strftime("%Y-%m-%d %H:%M:%S")
                end_time_str = time_limits.end.strftime("%Y-%m-%d %H:%M:%S")
                predicate = And(
                    GreaterThanOrEqual(self._fdw.timestamp_key, start_time_str),
                    LessThan(self._fdw.timestamp_key, end_time_str),
                )
        if join_keys_table:
            entity_buckets = self._get_join_keys_buckets(join_keys_table)
            predicate = And(predicate, In(IcebergReader.ENTITY_PARTITION_NAME, entity_buckets))
    ...
    def _get_join_keys_buckets(self, join_keys_table: pyarrow.Table) -> List[int]:
        bucket_expression = (
            f"{self._bucket_transformer_name}({IcebergReader.join_keys_expression(self._fdw.join_keys)})"
        )
        result = self._duckdb_conn.sql(f"""
            SELECT
                DISTINCT {bucket_expression} AS {IcebergReader.ENTITY_PARTITION_NAME}
            FROM join_keys_table
        """).fetchall()
