...
class IcebergWriter(OfflineStoreWriter):
    ...
    def delete_keys(self, keys: pyarrow.Table):
        ...
        result = self._duckdb_conn.sql(f"""
            SELECT
                DISTINCT {self._bucket_expression} AS {ENTITY_PARTITION_NAME}
            FROM keys
        """).fetchall()
        entity_buckets = [row[0] for row in result]
        predicate = In(ENTITY_PARTITION_NAME, entity_buckets)
    ...
    def commit(self, metadata: Optional[TectonDeltaMetadata] = None) -> Optional[int]:
        ...
        try:
            ...
            # as of pyicberg 0.8.1, pyiceberg does not support 'ns' timestamp precision so we
            # need this option
