...
from computer_use_ootb_internal.core.gui_agent.gui_parser.uia_tools.screenshot_service import get_screenshot_external_cmd


def convert_to_utf8(obj):
    if isinstance(obj, dict):
        return {convert_to_utf8(key): convert_to_utf8(value) for key, value in obj.items()}
...
def simple_teachmode_sampling_loop(
    ...
    if model.startswith("teach-mode"):
        ...
        while True:
            ...
            uia_meta = convert_to_utf8(uia_meta)
            sc_path = convert_to_utf8(sc_path)        # print(f"uia_meta: {uia_meta}, sc_path: {sc_path}")
