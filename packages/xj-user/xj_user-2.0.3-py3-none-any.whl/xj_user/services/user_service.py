...
class UserService:
    ...
    def detail(uuid: str):
        ...
        @param uuid 用户UUID。
        """
        print("> UserService::detail:", uuid)
        user_set = BaseInfo.objects
        user_set = user_set.annotate(user_id=F("id"), user_uuid=F("uuid")).extra(
            select={
                "user_uuid": 'REPLACE(uuid, "-", "")',
            }
        )
        # print("> check_account user_set query:", user_set.query)
        print("> UserService::detail user_set:", uuid, user_set)
        user_dict = user_set.values(*base_fields).first()
        print("> UserService::detail user_dict:", uuid, user_dict)
        if not user_dict:
            return None, "账户不存在"
