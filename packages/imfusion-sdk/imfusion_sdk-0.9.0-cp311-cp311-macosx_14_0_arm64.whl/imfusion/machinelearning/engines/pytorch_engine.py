...
from .._bindings import ExecutionProvider, Engine, DataItem, ElementType, ImageElement
    ...
    def predict(self, input_item: DataItem) -> DataItem:
        ...
        if len(input_item.fields) == 1:
            ...
            for key in self.input_fields:
                imf.log_info(f"Key in self.input_fields: {key}")
                if key not in input_item:
                    raise ValueError(f'Key "{key}" not input_item, got {list(input_item.keys())}')
                if input_item[key].type not in {ElementType.IMAGE or ElementType.VECTOR}:
                    raise ValueError(
                        f"Got element type {input_item[key].type}, for now the supported ones are [IMAGE, VECTOR]")
                imf.log_info(f"Getting field {key} from input item: {input_item}")
                input_tensor = self.__sis_to_torch_tensor(input_item[key].to_sis())
                imf.log_info("Successful conversion to torch tensor")
            ...
            imf.log_info("Running backend model")
            output = self.model(*input_list)

        imf.log_info("Converting output torch tensors to DataItem")
