...
if sys.platform == "win32" and (sys.version_info.major, sys.version_info.minor) >= (
    3,
    8,
):
...
def try_import_imfusion_plugin(plugin: str) -> None:
    ...
    potential_hits = [path for path in Path(__file__).parent.glob(f"{plugin}*") if path.name == plugin or expected_python_tag in path.name]
...
def __convert_image_into_numpy_array(self):
    ...
    if (
        ...
        if not self.shift.is_integer() or not (1 / self.scale).is_integer():
            ...
            if numpy.can_cast(val_max, dtype_min, casting="no"):
                np_type = __best_dtype([dtype_min])
            elif numpy.can_cast(val_min, dtype_max, casting="no"):
