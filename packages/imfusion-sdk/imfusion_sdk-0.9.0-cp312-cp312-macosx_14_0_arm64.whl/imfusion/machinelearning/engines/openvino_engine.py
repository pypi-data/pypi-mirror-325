...
from .._bindings import Engine, DataItem, ElementType, ImageElement
        ...
        if len(input_item.fields) == 1:
            ...
            for key in self.input_fields:
                ...
                if input_item[key].type not in {
                    ElementType.IMAGE or ElementType.VECTOR
                }:
                    raise ValueError(
                        f"Got element type {input_item[key].type}, for now the supported ones are [IMAGE, VECTOR]"
                    )
                input_dict[key] = self.__sis_to_ov_tensor(input_item[key].to_sis())
