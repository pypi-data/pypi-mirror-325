...
def xpki_property_set_defaults_from_cfg_file(prop_map: CertifierPropMap):
    ...
    if 'pycertifier.input.p12.password' in data:
        log("Loaded input password from config file.", log_level)
        xpki_property_set(prop_map, CERTIFIER_OPT.CERTIFIER_OPT_INPUT_P12_PASSWORD, data['pycertifier.input.p12.password'])

    if 'pycertifier.output.p12.password' in data:
        log("Loaded output password from config file.", log_level)
        xpki_property_set(prop_map, CERTIFIER_OPT.CERTIFIER_OPT_OUTPUT_P12_PASSWORD, data['pycertifier.output.p12.password'])
