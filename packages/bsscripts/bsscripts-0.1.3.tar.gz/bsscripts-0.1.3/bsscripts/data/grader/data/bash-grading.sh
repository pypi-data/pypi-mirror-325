...
finishgrading() {
    if [ -r ../../data/upload-virtualenv.sh ] ; then
        ...
        if [ ! -x upload-virtualenv.sh ] ; then
            chmod +x upload-virtualenv.sh
        fi

        ./upload-virtualenv.sh
    else
...
grade_generic() {
    ...
    TEXT_COUNT=`find . -type f -name '*' -exec file --mime-type {} \; | grep -E 'text/.*' | cut -f 1 -d ' ' | sed 's/.$//' | grep -v './feedback.txt' | wc -l`
    PCAP_COUNT=`find . -iname '*.pcap' | wc -l`
    PCAPNG_COUNT=`find . -iname '*.pcapng' | wc -l`

    # open pdf file(s)
    if [ $PDF_COUNT -gt 0 ] ; then
        zathura --fork --mode=fullscreen "`find . -iname '*.pdf'`"
    fi
    ...
    if [ $TEXT_COUNT -gt 0 ] ; then
        vim -o `find . -type f -name '*' -exec file --mime-type {} \; | grep -E 'text/.*' | cut -f 1 -d ' ' | sed 's/.$//' | grep -v './feedback.txt'`
    fi
}
