...
import httpx
import logging
from llm_api.providers.base import ModelProviderBase
from dotenv import load_dotenv
from typing import List, Dict, Any, Optional
from llm_api.utils.tokenizer_mapper import TokenizerMapper
...
class CerebrasProvider(ModelProviderBase):
    ...
    def __init__(self, model_name: str):
        """
        Initializes the provider for the Cerebras model.

        Args:
            model_name (str): Name of the model in Cerebras (e.g., "llama-3.3-70b").
        """
    ...
    async def generate_text(self, prompt: str, **kwargs) -> List[str]:
        """
        Generates text using the Cerebras model.
        ...
        """
        logger.info(f"Cerebras - Generating text for prompt: {prompt}")
        ...
        response_data = await self.send_request("/completions", request_payload)
        return [choice["text"] for choice in response_data.get("choices", [])]

    async def chat_completion(self, messages: List[Dict[str, Any]], **kwargs) -> List[str]:
        """
        Handles chat-based conversation with the Cerebras model.
        ...
        """
        logger.info(f"Cerebras - Handling chat: {messages}")
    ...
    async def logprobs(self, prompt: str, **kwargs) -> Optional[Dict[str, Any]]:
        ...
        """
        raise NotImplementedError("The 'logprobs' method is not implemented for CerebrasProvider.")
    ...
    async def get_logprobs_for_target_output(self, prompt: str, target_output: str) -> Optional[Dict[str, Any]]:
        ...
        Raises:
            NotImplementedError: This method is not implemented.
        """
        raise NotImplementedError("The 'get_logprobs_for_target_output' method is not implemented for CerebrasProvider.")

# Example usage
async def main():
    """Main function to test CerebrasProvider."""
    model_name = "llama-3.3-70b"  # Model in Cerebras
    provider = CerebrasProvider(model_name)
