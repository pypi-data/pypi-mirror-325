...
class BaseZEO(base.AbstractBase):
    ...
    def __init__(self, start_daemon=False, **kwargs):
        ...
        logger.debug("Constructing Base mother class")
        super().__init__(**kwargs)
        logger.debug("Base mother class constructed")
        ...
        logger.debug(f"connection_params: {connection_params}")
        logger.debug(f"kwargs: {kwargs}")

        logger.debug(f"start_daemon: {start_daemon}")
        logger.debug(f"self.zeo_conf {self.zeo_conf}")
        ...
        logger.debug(f"self.zeo_socket {self.zeo_socket}")
        logger.debug(f"self.host {self.host}")

        unix_flag = False
        if addr == self.zeo_socket:
            unix_flag = True
        if addr == base.find_root_path(os.path.realpath("./")):
            unix_flag = True
    ...
    def checkActualConfig(cls, zeo_conf):
        ...
        try:
            ...
            if m:
                return m[1].strip()
            # m = re.match(r'blob-dir\w*(.+):([0-9]+)', parser._data['filestorage'])
            # if m:
            #     dbhost = m[1].strip()
            #     port = int(m[2].strip())
            #     logger.debug(dbhost)
    ...
    def setConfPaths(self, creation=False, **kwargs):
        if not creation:
            self.root_dir = os.path.join(base.find_root_path(), "./.bd")
        else:
            self.root_dir = "./.bd"
        ...
        self.zeo_blob = os.path.join(self.root_dir, "zeo.blob")
        self.zdaemon_socket = os.path.join(self.root_dir, "zdaemon.socket")
        self.zdaemon_conf = self.getZdaemonConf(self.root_dir)

    @classmethod
    def getZdaemonConf(cls, root_dir):
        return os.path.join(root_dir, "zdaemon.conf")
...
"""
    ...
    def statusZdaemon(cls):
        ...
        if ret:
            raise RuntimeError(
                "Error or Not running [server Config]:" f" {open(zdaemon_conf).read()}"
            )
    ...
    def statusConnection(cls):
        ...
        if host_port is not None:
            ...
            if not m:
                logger.error(f"invalid URI: {host_port}")
                return
            ...
            if result == 0:
                logger.warning(f"{host}:{port} is opened")
            else:
                logger.error(f"{host}:{port} is closed")
            sock.close()
            ...
            try:
                ...
                connection = db.open()
                logger.warning(
                    f"connection to {host}:{port} OK: {[e for e in connection.root.schemas]}"
                )
            except Exception as e:
                logger.error(f"Cannot connect to database {host}:{port}")
...
"""
    ...
    def select(self, _types, constraints=None, sort_by=None):
        ...
        if (sort_by is not None) and (not isinstance(sort_by, str)):
            raise RuntimeError("sort_by argument is not correct: {0}".format(sort_by))
    ...
    def launchRuns(self, run_list, params):
        ...
        for r, j in run_list:
            ...
            logger.warning("launch in '" + mydir + "/" + "run-" + str(r.id) + "/'")
