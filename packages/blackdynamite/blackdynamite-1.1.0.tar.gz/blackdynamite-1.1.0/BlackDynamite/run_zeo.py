...
class RunZEO(zeoobject.ZEOObject):
    ...
    def listFiles(self, subdir="", cache=None):
        ...
        if not self["machine_name"] == socket.gethostname() and cache is None:
    ...
    def getFile(self, filename, cache="/tmp"):
        dest_path = os.path.join(
        ...
        cmd = "scp {0}:{1} {2}".format(
            self["machine_name"], self.getFullFileName(filename), dest_file
        )
        logger.info(cmd)
        p = subprocess.Popen(
