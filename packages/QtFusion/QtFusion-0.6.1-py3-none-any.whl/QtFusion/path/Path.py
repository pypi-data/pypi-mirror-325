...
def join_paths(*paths: str) -> str:
    """
    Joins multiple paths into a single path, normalizes it, and ensures it is compatible with the operating system.

    Args:
        *paths (str): Paths to be joined.

    Returns:
        str: The normalized combined path, compatible with the operating system.
    """
    # Join paths using the system's path separator
    combined_path = os.path.join(*paths)

    # Normalize the combined path
    normalized_path = os.path.normpath(combined_path)

    # Additional handling for Windows paths
    if os.name == 'nt':
        # Strip leading slashes and normalize, in case of Unix-style paths being used on Windows
        cleaned_path = os.path.normpath(normalized_path.lstrip('/\\'))
    else:
        # For Unix-like systems, the normalized path is usually fine as is
