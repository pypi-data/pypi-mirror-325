    ...
    def get_heatmap(self, img):
        ...
        if feature_maps is not None and len(feature_maps) > 0:
            ...
            equalized_feature_maps = np.array([cv2.equalizeHist(
                cv2.normalize(fm.astype(np.float32), None, alpha=self.norm_alpha,
                              beta=self.norm_beta,
            ...
            superimposed_img = cv2.addWeighted(img.astype(np.uint8), self.original_img_intensity, heatmap, self.heatmap_intensity, 0)
