...
def getFramePath(file_path: str, cur_frames: int, imageHandler: ImageHandler = None,
                 snapshots_dir: str = "./snapshots") -> str:
    """
    Generates or returns a unified file path for the current frame based on the file path and frame index.

    Args:
        file_path (str): The path of the file being processed.
                         Can be a file path, folder path, or camera identifier (e.g., "Camera 0").
        cur_frames (int): The current frame index used to generate a unique filename.
        imageHandler (ImageHandler, optional): An instance of `QtFusion.handlers.ImageHandler` used
                                               to retrieve the current file name when processing folder paths.
        snapshots_dir (str, optional): Directory to store snapshots. Defaults to 'snapshots'.

    Returns:
        str: The generated or existing unified file path for the frame. The returned path depends on `file_path`:
             - For folder paths: the file name is obtained from `imageHandler.getFileName()`.
             - For camera or video file sources: a unique path with a timestamp and frame index is generated.
             - Otherwise, the original `file_path` is returned.
    """
    if os.path.isdir(file_path):
        # If it's a folder, use `imageHandler` to get the current file's name
        if imageHandler:
            return imageHandler.getFileName()
    elif file_path.startswith("Camera") or file_path.endswith((".mp4", ".avi")):
        # If the source is a camera or a video file, generate a unique file path
        if not os.path.exists(snapshots_dir):
            os.makedirs(snapshots_dir)
        # Use the current timestamp and frame index to create a unique filename
        timestamp = time.strftime('%Y%m%d_%H%M%S', time.localtime())
        return abs_path(f"frame_{timestamp}_{cur_frames}.png", snapshots_dir)
    else:
