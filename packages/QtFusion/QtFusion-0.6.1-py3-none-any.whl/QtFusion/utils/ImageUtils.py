...
def drawOrientedBox(image, box, color=None, alpha=0.25, addText=None, line_thickness=None):
    """
    Draws an oriented bounding box (OBB) on an image with optimized performance.

    Args:
        image (np.ndarray): A numpy array representing the image to draw on.
    ...
    Returns:
        np.ndarray: A numpy array representing the image with the drawn OBB and text.
    """
    # Convert numpy array to PIL Image
    img = Image.fromarray(image)

    # Calculate image dimensions and line thickness
    width, height = img.size
    tl = line_thickness or max(1, int(round(0.002 * (width + height) / 2)))
    ...
    # Draw filled polygon for anti-aliasing
    pts = [(int(box[i]), int(box[i + 1])) for i in range(0, len(box), 2)]
    draw.polygon(pts, fill=rgba_fill)

    # Draw the outline
    draw.line(pts + [pts[0]], fill=color, width=tl)

    # Add text if provided
    if addText:
        x_min, y_min = min([p[0] for p in pts]), min([p[1] for p in pts])
        text_position = (x_min, max(0, y_min - 20))  # Adjust text position above the bounding box

        # Draw text background and text
        text_size = draw.textsize(addText)
        text_bg = (
            text_position[0],
            text_position[1],
        ...
        draw.text((text_position[0] + 2, text_position[1] + 2), addText, fill="white")
