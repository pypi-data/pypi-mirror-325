...
from tesseract_olap.exceptions.query import InvalidEntityName
...
def _find_timerestriction(
    query: DataQuery,
) -> tuple[HierarchyField, LevelField, TimeRestriction] | None:
...
def _ttime_timerestriction(
    ...
    tfrom: Selectable,
    tdim: Table,
    timerestr: tuple[HierarchyField, LevelField, TimeRestriction],
) -> QueryBuilder:
    ...
    hiefi, lvlfi, restriction = timerestr
...
def _tgroup_having(query: DataQuery) -> Criterion:
    ...
    for msrfi in query.fields_quantitative:
        if msrfi.constraint:
            ...
            criterion &= _filter_criterion(field, msrfi.constraint)
...
def _trich_leftjoin(
    ...
    for hiefi in query.fields_qualitative:
        columns = [
            column for lvlfi in hiefi.drilldown_levels for column in lvlfi.iter_columns(locale)
        ]
        ...
        key_column = [lvlfi.id_column(locale) for lvlfi in hiefi.drilldown_levels][-1]
