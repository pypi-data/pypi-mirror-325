...
from tesseract_olap.common import shorthash, stringify
...
class DataQuery:
    ...
    def count_key(self) -> str:
        ...
        return "d" + hashlib.md5(string.encode("utf-8"), usedforsecurity=False).hexdigest()
...
class DataMultiQuery:
    ...
    def from_requests(
        ...
        if len(queries) - 1 != len(joins):
            msg = "Invalid number of 'joins' parameters; it must be one per query intended to be joined with the initial."
