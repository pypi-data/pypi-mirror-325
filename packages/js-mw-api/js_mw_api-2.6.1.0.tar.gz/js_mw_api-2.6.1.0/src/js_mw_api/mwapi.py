...
JOBCARD_FIELDMAPPING = {
    ...
    'Edge'      : '100',
    'Qtd_TI'    : '1177',
    'FORM_IDENTIFIER' : 'CUSTOMER AND SITE DETAILS'}
...
# Get the client, default creds and field mappings from the environment variables.
CLIENT = get_env_variable('MORAWARE_CLIENT')
JOBCARD_FIELDMAPPING = json.loads(get_env_variable('JOBCARD_FIELDMAPPING', '{}'))
DEFAULTMORAWARECREDS = json.loads(get_env_variable('DEFAULTMORAWARECREDS', '{}'))
...
def mw_login_cookie(mw_cookie):
    return mw_cookie.mw_cookie if hasattr(mw_cookie, 'mw_cookie') else mw_cookie
...
def convert_float(value):
    if isinstance(value, float):
       ...
       elif np.isinf(value): return True
    if isinstance(value, str) and value.upper() in ('TRUE', 'FALSE'): return value == 'TRUE'
    return value
...
class MorawareSession:
    ...
    def __init__(self, client=CLIENT, UserCreds={}, Session=None, SeedData=None, key=None, **kwargs):
        if SeedData:
                ...
                self.mw_cookie = SeedData.get('mw_cookie', None)
    ...
    def LoginProcess(self):
        ...
        with open(f'MWS_{self.client}_{self.user}.json', 'w') as jsonpointer:
            json.dump({'mw_cookie': self.mw_cookie,
                       'expiry' : self.expiry.isoformat(),
                       'active' : self.active.isoformat(),
                       'user': self.user,
                       'sessionheaders': self.sessionheaders}, jsonpointer)
...
class MorawareConnection:
    ...
    def data_check_update(self, keys):
        func = {'StatusIDs': GetActivityStatusIDs,
                'ActivityIDs': GetActivityIDs,
                'ProductIDs': GetALLInventoryIDs,
                #'JobCardFieldMapping': 
                }
        if not ( isinstance(keys, list) or isinstance(keys, tuple) ): keys = [keys]
        for key in keys:
            if not self.Data.get(key):
                if key in func: self.data_update(key, func[key](self.session, self.client))
                else: raise Exception(f'Key {key} not found in data dictionary, and no function to fetch it.')

    def Give(self, key):
        self.data_check_update(key)
    ...
    def GetQuote(self, QuoteID):
        self.check_refresh_session()
        return MorawareQuote(QuoteID, self)
...
class MorawareQuote:
    ...
    '''
    def __init__(self, QuoteID, mw_Connect):
        self.QuoteID = QuoteID
        self.QuoteData = GetQuoteDetails(QuoteID, mw_Connect.mw_cookie, mw_Connect.client)
    ...
    def CreateJob(self, JobName):
        ...
        self.QuoteData = CreateJobFromQuote(JobName, self.QuoteID, self.mw_Connect.mw_cookie, self.QuoteData, self.mw_Connect.client)
        job = MorawareJob(self.mw_Connect, self.QuoteData['Job ID'])
        formfields = self.mw_Connect.Give('JobCardFieldMapping')
        NewJob_Updates(job, self, formfields)
        return job
...
class MorawareJob:
    ...
    '''
    def __init__(self, mw_Connect, JobID):
        self.JobID = JobID
        self.mw_Connect = mw_Connect
        self.JobData = GetJobDetails(mw_Connect, JobID)
        
    def UpdateActivity(self, ActivityType, Updates):
        '''
        Update an activity in the job.
        '''
        return UpdateJobActivity(self.JobID, ActivityType, Updates, self)
    
    def UpdateJobForm(self, formid, fields):
        '''
        Update a Job Form on the job.
        '''
        fields.update({'P0': formid})
        return UpdateMoraware(self.JobID, 'Job_UpdateJobForm', 'job', fields, self.mw_Connect.mw_cookie, self.mw_Connect.client)
    
    def CreateActivity(self, **Updates):
        '''
        Create an activity in the job.
        '''
        return CreateJobActivity(mw_Connect=self.mw_Connect, JobID=self.JobID, **Updates)
    
    def AllocateMaterial(self, ActivityID, MaterialData):
        '''
        Allocate material to the job.
        '''
        return AllocateMaterial(JobID=self.JobID,
...
def UpdateMoraware(Serial, FormType='Job_UpdateJobActivities', endpoint='job', fields={}, mw_cookie=None, client=CLIENT):
    '''
    ...
    '''
    mw_cookie = mw_login_cookie(mw_cookie)
    payload = {
        'C': 'mjtrs1',
        'X': 1,
        'cuid': 24,
...
def GetActivityStatusIDs(mw_cookie, client=CLIENT):
    '''
...
def GetActivityIDs(mw_cookie, client=CLIENT):
    '''
...
def Get_InvProductAttrListIDs(ProductID, mw_cookie, client=CLIENT):
    '''
...
def Get_InvProductAttrIDs(attrListID, mw_cookie, client=CLIENT):
    '''
...
def Get_InventoryID_Data(ProductID, mw_cookie, client=CLIENT):
    '''
    Fetches Inventory Attribute Data from Moraware for a particular product.
    '''
    mw_cookie = mw_login_cookie(mw_cookie)
    ProductAttributes = Get_InvProductAttrListIDs(ProductID, mw_cookie, client)
    for attrList, ID in ProductAttributes.items():
        ProductAttributes[attrList] = Get_InvProductAttrIDs(ID, mw_cookie, client)
    return ProductAttributes

def GetALLInventoryIDs(mw_cookie, client=CLIENT):
    '''
    ...
    for i,product in enumerate(products):
        ...
        result[namekey]['_ATTR'] = Get_InventoryID_Data(product['Product ID'], mw_cookie, client)
    return result
...
def GetQuoteDetails(SN, mw_cookie, client=CLIENT):
    mw_cookie = mw_login_cookie(mw_cookie)
...
def Give_NewJob_JobCard_Fields(QuoteData, fieldmapping=JOBCARD_FIELDMAPPING):
    '''
    ...
    edge = ''
    NoteField = ''
    if QuoteData.get('Built Edge'): fields.append( ( fieldmapping['GlueUp'], '1' ) )
    for line in QuoteData['QuoteLines']:
        ...
        if line [0] == 'Includes':
            if 'Tap' in line[1]: fields.append( ( fieldmapping['Tap'], '1' ) )
            if 'Hob' in line[1]: fields.append( ( fieldmapping['Hob'], '1' ) )
        elif line[0] == 'Edge Finish':
            if edge: edge += '\n'
            edge += ' '.join(line[1:])
            if 'Mitre' in line[1]: fields.append( ( fieldmapping['Mitre'], '1' ) )
        elif 'Waterjet' in line[0]: fields.append( ( fieldmapping['Waterjet'], '1' ) )
        elif line[0] == 'Quoted': fields.append( ( fieldmapping['Qtd_TI'], ' '.join(line[1:]) ) )
    fields.append( ( fieldmapping['Notes'], NoteField ) )
    fields.append( ( fieldmapping['Edge'], edge ) )

    P_fields = list(zip(*fields))
    P1 = '?'.join(P_fields[0])
    P2 = '?'.join(P_fields[1])
    UpdateDict = {}
    for i in range(0, 6): UpdateDict[f'P{i}'] = ''
    UpdateDict.update( {'P1': P1,
                        'P2': P2,
                        'P4': -1 })
    return UpdateDict
...
def GetSomeSoup(jid, mw_cookie, client=CLIENT):
    mw_cookie = mw_login_cookie(mw_cookie)
...
def GetJobActivities(JobSoup=None, **args):
    if not JobSoup: JobSoup = GetSomeSoup(args.get('jid'), args.get('mw_cookie'))
    table = JobSoup.find('table', {'id': 'ActivitiesBody'})
    #Give the table headers
    ActivityTableHead = JobSoup.find('table', {'id': 'ActivitiesHead'})
    ActivityTableHeaders = []
    for ath in ActivityTableHead.text.split('\n'):
        if ath:
           cleaned_cell = re.sub(r'[^\x00-\x7F]+', '', ath)
           ActivityTableHeaders.append(cleaned_cell)
    
    rows = table.find_all('tr')
    Activities = []
    for i,row in enumerate(rows): #Getting Activity ID and Materials
        if i == 0: continue
        ActID = row['mjtdrop'].split('\'')[1]
        rowdetails = row.find_all('td')
        Actname = rowdetails[0].text
        mci = ActivityTableHeaders.index('Material')
        ActivityMaterials = extract_slab_details( rowdetails[mci] )
        Activities.append({ 'Activity Type': Actname, 'Materials': ActivityMaterials, 'ActivityID': ActID })

    df = pd.read_html(str(table))[0]
    adata = pd.Series.tolist(df)
    MWJobDat = []
    for i,row in enumerate(adata):
        newrow = []
        for j,cell in enumerate(row):
            cell = convert_float(cell)
            cleaned_cell = re.sub(r'[^\x00-\x7F]+', '', cell)
            if cleaned_cell == 'Add Material': cleaned_cell = None
            if 'date' in ActivityTableHeaders[j].lower() and cleaned_cell:
                try: int(cleaned_cell[0])
                except: cleaned_cell = cleaned_cell.split(' ')[1].strip()
            newrow.append(cleaned_cell)
        MWJobDat.append(newrow)
        
    JobData = []
    headers = ActivityTableHeaders
    for i,row in enumerate(MWJobDat):        
        Activity = {}
        for j,cell in enumerate(row): Activity[headers[j]] = cell
        Activity.update(Activities[i])
        JobData.append(Activity)
    return JobData

def GetJobCard(JobSoup=None, fieldmapping=JOBCARD_FIELDMAPPING, **args):
    if not JobSoup: JobSoup = GetSomeSoup(args.get('jid'), args.get('mw_cookie'))
    JobForms = JobSoup.find_all('table', {'class': 'detail-form'})
    JCID = fieldmapping['FORM_IDENTIFIER']
    JobCard = [form for form in JobForms if JCID in form.text]
    if not JobCard: return None
    JobCard = JobCard[0]
    formid = JobCard['id'].split('tblJobForm')[-1]
    JobData = {}
    header_divs = JobCard.find_all('div', style=re.compile(r';font-size:10pt;font-weight:bold;'))
...
def GetJobDetails(mw_Connect, SN):
        jcfm = mw_Connect.Give('JobCardFieldMapping')
        ...
        Job_Card = GetJobCard(soup, fieldmapping=jcfm)
...
def UpdateJobActivity(jid, Activity, Updates, MWJob):#, mw_Connect):
    '''
    ...
    return UpdateMoraware(Serial=jid,
...
def CreateJobFromQuote(JobName, QuoteID, mw_cookie, QuoteData, client=CLIENT):
    '''
    ...
    DecodedString = {'quoteId': QuoteID,
                     'jobName': JobName,
                     'jobTemplateId': '1', #Change this after creating new jobs card template.
    ...
    rsp = UpdateMoraware(Serial=QuoteID,
...
def NewJob_Updates(MWJob, MWQuote, FormFields):
    '''
    ...
    rsp = MWJob.UpdateJobForm(jcid, JobCardFields)

def AllocateMaterial(JobID, ActivityID, MaterialData, mw_cookie, client=CLIENT):
    '''
    ...
    form = 'Job_CreateAndAllocateJobActivityProduct'
    ...
    rsp = UpdateMoraware(JobID, form, fields=fields, mw_cookie=mw_cookie, client=client)
    if 'MW_RS_ErrorResponse' in rsp.text:
        if 'variant id=' in rsp.text:
            pass
    return rsp

def CreateJob(JobName, mw_cookie, AccountID='', TemplateID='1', ProcessID='1', client=CLIENT):
    '''
    ...
    rsp = UpdateMoraware(Serial=ProcessID,
...
def CreateJobActivity(mw_Connect, JobID, Status='Estimate', **Updates):
    ...
    rsp = UpdateMoraware(Serial=JobID,
