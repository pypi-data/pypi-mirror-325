...
class QubiconCore:
    ...
    def extract_process_data(
        ...
        at_sampling_timestamps_mode: bool = False,
        included_report: bool = False,
        start_event_id: Optional[int] = None,
        end_event_id: Optional[int] = None,
        output_format: str = "csv",
        output_file: str = "process_data.csv",
    ):
        """
        Extract process data for the specified process ID with filters and options.

        Args:
            process_id (int): ID of the process to extract data for.
            start_time (Optional[int]): Start time (Unix timestamp in ms).
            end_time (Optional[int]): End time (Unix timestamp in ms).
            raw_mode (bool): Enable raw mode (default: False).
            export_rate (Optional[int]): Granularity of the export in milliseconds.
            at_sampling_timestamps_mode (bool): Export at sampling timestamps (default: False).
            included_report (bool): Include report in the export (default: False).
            start_event_id (Optional[int]): ID of the start event.
            end_event_id (Optional[int]): ID of the end event.
        ...
        """
        try:
            # Create the DTO object with validated fields
            ...
            response = start_export_process_data.sync_detailed(
                process_id=process_id,
                client=self.client,
                body=body,
            )
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                parsed_response = json.loads(response.content.decode())
                print(json.dumps(parsed_response, indent=2))  # Debugging: Print the parsed response

                # Handle the data extraction based on format
                if "data" in parsed_response:  # Adjusted to check for 'data' key
                    ...
                    if output_format == "csv":
                        ...
                        logging.info(f"Process data successfully exported to {output_file} in CSV format.")
                    elif output_format == "json":
                        # Save as JSON
                        with open(output_file, "w") as f:
                            json.dump(data, f, indent=4)
                        logging.info(f"Process data successfully exported to {output_file} in JSON format.")
                    else:
                        logging.error(f"Invalid output format: {output_format}")
                        return {"error": f"Invalid output format: {output_format}"}

                    return {"success": f"Process data exported to {output_file}"}
                else:
                    logging.error("Unexpected response format: 'data' key not found.")
                    return {"error": "Unexpected response format: 'data' key not found."}

            else:
                logging.error(f"Failed to export process data. HTTP Status: {response.status_code}")
                return {"error": f"Failed to export process data. HTTP Status: {response.status_code}"}

        except Exception as e:
            logging.error(f"Error exporting process data: {e}")
            return {"error": f"Error exporting process data: {str(e)}"}
