...
class SearchApiClient(BaseClient):
    ...
    """
    def __init__(self, base_url: str, api_key: str | None = None, max_retries: int = 2, retry_delay: float = 0.5):
    ...
    async def pipeline(
        ...
        response = await self.post(
            ...
            headers=request_context.to_default_headers() if request_context else None,
            timeout=timeout,
        )
        if response:
