...
class UnstableFinetuneGetResponseStatus(str, enum.Enum):
    ...
    def visit(
        ...
        if self is UnstableFinetuneGetResponseStatus.TRAINING:
            return training()
        if self is UnstableFinetuneGetResponseStatus.DEPLOYING:
            return deploying()
        if self is UnstableFinetuneGetResponseStatus.DEPLOYED:
            return deployed()
        if self is UnstableFinetuneGetResponseStatus.ERROR:
            return error()
        if self is UnstableFinetuneGetResponseStatus.DEPRECATED:
            return deprecated()
        if self is UnstableFinetuneGetResponseStatus.PENDING_DEPRECATION:
            return pending_deprecation()
        if self is UnstableFinetuneGetResponseStatus.QUEUED:
            return queued()
