...
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def read_case(dat_file: str):
    """
    :dat_file: [str] path to data file
    :returns: a numpy array with the case data
    :USAGE: your_case = read_case('path/to/data.dat')

    """
    if not os.path.isfile(dat_file):
       message('File '+ dat_file +' does not exist.', 'ERROR')
    new_case = np.genfromtxt(dat_file, delimiter=',',names= True)
    return new_case 
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def read_variables(input_variables_file):
    ...
    if type(input_variables_file) == str:
        if not os.path.isfile(input_variables_file):
           message('File '+ input_variables_file +' does not exist.', 'ERROR')
    ...
    for item in variables['general']:
        if item not in general_options:
            message(item + ' is not a possible option for the general column in variables', 'WARNING')
    ...
    for i, item in enumerate(variables.columns):
        if item == 'experiment_tag':
            ...
            message(warning + warning2, 'WARNING')
            item = 'var_tag'
            ...
            message(warning + warning3, 'WARNING')
        cols.append(item)
...
def make_strip(input_variables_file, strip_filename: str, form="fmtout"):
    ...
    with open(strip_filename, 'w+') as st:
        ...
        st.write('. \n')
        st.close()
        message('Your strip file ' + strip_filename + ' has been created', 'INFO')
    return variables
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
...
def run_strip(strip_file: str, restart: str, case_folder=''):
    ...
    if not os.path.isfile(case_folder + restart + '.r'):
       message('File '+ case_folder + restart + '.r does not exist.', 'ERROR')
    ...
    if relap_scdap() :
        ...
        arg4 = ''
        message("relap_scdap() is used", 'INFO')
    else:
        arg1 = ' -i ' + strip_file
...
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def plot_var(variable: str, variables_df, cases_df, case_folder = list(),
             y_limits: tuple=(0,0), x_limits: tuple=(0,0), dataExp = '', label = ''):
    ...
    if variable not in list(variables_df['var_tag']):
        ...
        return
    if len(case_folder) == 0  : case_folder = list(cases_df.keys()) 
    ...
    return fig, ax 
#▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
def plot_all(variables_df, cases_df, case_folder= list(), dataExp=''):
...
def plot(alpha: str, numeric: str, restart: str, case_folder='', save_fig=''):
    ...
    for i in range(0, len(case_folder)):
        ...
        if not os.path.isfile('./' + case_folder[i] + '/' + restart + '.r'):
            message('File ./'+ case_folder[i] + '/' + restart + '.r does not exist.', 'ERROR')
        try :
    ...
    for i in range(0,3):
        ...
        for k, plot in plotted.iterrows():
            ...
            if type(dataExp) != str :
               ...
               if plot['var_tag'] in dataExp.dtype.names :
                  axes[i].plot(dataExp['time'],dataExp[plot['var_tag']], linewidth=1,
        ...
        for k, plot in plotted.iterrows():
            ...
            if type(dataExp) != str :
               l_style = ':'
               if plot['var_tag'] in dataExp.dtype.names :
                  ax2.plot(dataExp['time'], dataExp[plot['var_tag']], linewidth=1,
                          color=colors[l], alpha=1.0, linestyle='-',
                          label=label_exp)
                  label_exp = '_nolegend_'
...
def plot_variables(variable: list, variables_df, cases_df, ylabel='',
    ...
    """
    if type(cases_df) == dict :
        message('Remember that cases_df should be an Ndarray containing the data for one the particular case, ex.: cases_df["base"]')
        message('Plot for variables:' + variable[0] + ', ' + variable[1] + '... will not be plotted')
        return
    variables = []
    for var in variable:
    ...
    if len(variables) == 0:
        message('Plot for variables:' + variable[0] + ', ' + variable[1] + '... will not be plotted')
        return
    message('making plot for' + variables[0] + '_' + variables[-1], 'INFO')
    ...
    plt.savefig(figures_loc() + variables[0] +'_'+ variables[-1] +'.'+ figures_type(),
            format= figures_type(), dpi=300, bbox_inches='tight')
    message('plot has been saved as: '+ figures_loc() + variables[0] +'_'+ variables[-1] +'.'+ figures_type(),'INFO')
    if show_plot() : plt.show()
    plt.close()
...
def interactive_plot(data: dict, name: str='interactive_plot'):
    """TODO: Docstring for interactive_plot.

    :data: dictionary with your results 
    ...
    variables = data[cases[0]].dtype.names
    cases_json = json.dumps(cases)
    dat_json = json.dumps({key: value.tolist() for key, value in data.items()})
    variables_index_json = json.dumps({variable: index for index, variable in enumerate(variables[1:])})
    colors_json = json.dumps(colors)
    variables_json = json.dumps(variables)
    ranges = [data[cases[0]][0][0], data[cases[0]][-1][0]]
