...
namespace GC{
  ...
  namespace fv{
    ...
    void FluxBalancedNSWEs_1st::operator()(fvScalarFieldOnCell& g,
            ...
            for(auto neib_iter : *cell_iter){              
              if (!neib_iter.is_boundary()){ //neighbour is not a boundary
                ...
                if (id_this < id_neib){ // preventing repetive calculation
                  ...
                  auto flux = RiemannSolver(grav, ScalarRiemannState(h_L, h_R), VectorRiemannState(h_L*u_L, h_R*u_R)); 
                  Vector2 _hU_flux = flux.q.x*normal + flux.q.y*shear;
                  Vector2 _z_flux_this = -Scalar(0.5)*grav*h_L*h_L*normal;
                  Vector2 _z_flux_neib = -Scalar(0.5)*grav*h_R*h_R*normal;
