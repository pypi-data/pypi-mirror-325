      ...
      for(auto cell_iter = phi.mesh.Cell.Neighbours.begin();
        ...
        for(auto neib_iter : *cell_iter){
          if (!neib_iter.is_boundary()){ //neighbour is not a boundary
            ...
            *(phi_gradient_begin + id_this) += (phi_neib+phi_this)/Scalar(2.0)*area*normal/volume_this;
          }else{
            Flag id_face = (*(faces_begin + id_this))[neib_cnt];
            Vector normal = uni(*(face_normal_begin + id_face));
            Scalar area = *(face_area_begin + id_face);
            ...
            *(phi_gradient_begin + id_this) += (phi_bound+phi_this)/Scalar(2.0)*area*normal/volume_this;
          }
          neib_cnt++;
        }
      }
      ...
      for(auto cell_iter = phi.mesh.Cell.Neighbours.begin();
        ...
        for(auto neib_iter : *cell_iter){
          if (!neib_iter.is_boundary()){ //neighbour is not a boundary
            ...
            *(phi_gradient_begin + id_this) += product((phi_neib+phi_this)/Scalar(2.0), normal)*area/volume_this;
          }else{
            Flag id_face = (*(faces_begin + id_this))[neib_cnt];
            Vector normal = uni(*(face_normal_begin + id_face));
            Scalar area = *(face_area_begin + id_face);
            ...
            *(phi_gradient_begin + id_this) += product((phi_bound+phi_this)/Scalar(2.0), normal)*area/volume_this;
          }
          neib_cnt++;
        }
      }
