...
namespace GC{
  ...
  namespace fv{
    ...
    __global__ void cuEDMeyerPeterMullerKernel(Scalar dt, Scalar* gravity, Scalar* h, Vector* hU, Scalar* hC, Scalar* manning_coeff, Scalar* ED_rate, Scalar rho_solid, Scalar rho_water, Scalar dim_mid, unsigned int size){
      ...
      while (index < size){
        ...
        Scalar tau = 0.0;
        if (h_ > dim_mid){
          tau = rho_mix*g*pow(manning_,Scalar(2.0))*pow(h_, Scalar(-1.0/3.0))*dot(U,U);
        }
        else{
          tau = 0.0;
        }
        ...
        Scalar qb_sat = 8.0*pow(Scalar(fmax(shields - 0.047,0.0)), Scalar(1.5))*sqrt((rho_solid/rho_water-1.0)*g*pow(dim_mid, Scalar(3.0)));
    ...
    __global__ void cuMomentumCorrectionKernel(Scalar* gravity, Scalar* h, Scalar* hC, Vector* C_grad, Vector* hU, Scalar* ED_rate, Vector* mom_correction, Scalar rho_solid, Scalar rho_water, Scalar porosity, unsigned int size){
      ...
      while (index < size){
        ...
        Vector2 mom_correction_ = -Scalar(1.0)*(rho_solid - rho_water)*g*pow(h_, Scalar(2.0)) / rho_mix / Scalar(2.0)*C_grad_ - (rho_0 - rho_mix) / rho_mix / (Scalar(1.0) - porosity)*ED_rate_*U;
    ...
    __global__ void cuEDTakahashiIversonXiaKernel(Scalar* gravity, Scalar* h, Vector* hU, Scalar* hC, Scalar* manning_coeff, Scalar* miu_dynamic, Scalar* miu_static, Scalar* ED_rate, Scalar rho_solid, Scalar rho_water, Scalar porosity, Scalar dim_mid, Scalar alpha, Scalar beta, Scalar dt, unsigned int size){
      ...
      while (index < size){
        ...
        if (h_ > dim_mid){
          tau = rho_mix*g*pow(manning_, Scalar(2.0))*pow(h_, Scalar(-1.0 / 3.0))*dot(U_,U_) + (rho_solid - rho_water)*hC_*g*miu_dynamic_;
        }
        else{
          tau = 0.0;
        }
  ...
  //      }
        ...
        if (norm(U_) <= 1e-6){
          ...
          _friction_force = -Scalar(1.0)*(rho_solid - rho_water) / rho_mix*g*hC_*miu_dynamic_*U_ / norm(U_);
        }
        ////constrain friction force
        if (dot(_friction_force*dt, _friction_force*dt) >= dot(hU_, hU_)){
          _friction_force = -Scalar(1.0) / dt*hU_;
        }
