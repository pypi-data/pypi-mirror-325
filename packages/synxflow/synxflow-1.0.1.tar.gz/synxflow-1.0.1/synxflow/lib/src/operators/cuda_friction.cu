...
// Non-iterative fully-implicit scheme with limitation on Manning's n
          ...
          if (vel > 15.0){
            manning_cri = sqrt(1.0/((1e-10+dt)*g_*pow(h_, Scalar(-4.0 / 3.0))*vel));
          }
          manning_ = fmax(manning_cri, manning_); 
//-------- End increasing manning ------
          Scalar C_f = g_*manning_*manning_*pow(h_, Scalar(-4.0 / 3.0));
          Scalar alpha;
...
/*          Scalar C_f = g_*manning_*manning_*pow(h_, -4.0 / 3.0);
          ...
          hU[index] = U_1*h_;  */
// Point Implicit
          Scalar C_f = g_*manning_*manning_*pow(h_, Scalar(-1.0 / 3.0));
          Scalar D_x = 0.0, D_y = 0.0;
          Vector2 U_ = hU_ / h_;          
          if (norm(U_) > 1e-10){
    ...
    __global__ void cuFrictionManningSteepImplicitKernel(Scalar* gravity, Scalar* manning_coeff, Scalar* h, Vector* hU, Vector* hU_advection, Vector* z_grad, Scalar dt, unsigned int size){
      ...
      while (index < size){
        ...
        if (h_ < h_small){
          ...
          //updating velocity rather than discharge
          Scalar C_f = g_*manning_*manning_*pow(h_, Scalar(-4.0 / 3.0))*pow(phi, Scalar(-1.0/3.0));
          Vector2 U_ = hU_ / h_;
          Vector2 acc_ = hU_advection_ / h_;
    ...
    __global__ void cuFrictionManningMCImplicitKernel(Scalar* gravity, Scalar* manning_coeff, Scalar* friction_coeff, Scalar* h, Scalar* hC, Vector* hU, Vector* hU_advection, Scalar rho_water, Scalar rho_solid, Scalar porosity, Scalar dt, unsigned int size){
      ...
      while (index < size){
        ...
        if (h_ < h_small){
          ...
          manning_ = fmax(manning_cri, manning_); 
          Scalar C_f = g_*manning_*manning_*pow(h_, Scalar(-1.0 / 3.0))/h_;
          Vector2 acc_ = hU_advection_ / h_;
          Vector2 U_1 = ManningNewton(acc_, C_f, dt, U_);
          hU_ = U_1*h_;
          // ---- Mohr-Coulomb step ----
          ...
          if (norm(U_) <= 1e-6){
            ...
            _friction_force = -Scalar(1.0)*(rho_solid - rho_water) / rho_mix*g_*hC_*miu_*U_ / norm(U_);
          }
