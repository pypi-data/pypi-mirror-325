...
from kisters.network_store.model_library.base import BaseNode, Model
from pydantic import Field, field_validator


class _Node(BaseNode):  # type: ignore
    domain: Literal["water"] = "water"
    name: Annotated[Optional[str], Field(description="Optional node name")] = None


class LevelFlow(Model):  # type: ignore
...
class StorageLevelVolume(Model):  # type: ignore
...
class StorageLevelVolumeEquation(Model):  # type: ignore
...
class Storage(_Node):
    ...
    level_uncontrolled: Optional[Union[list[StorageLevelFlow], WeirParameters]] = Field(
        None,
