...
class Packager:
    ...
    def __init__(self, target=None, platform=["manylinux2014_x86_64", "manylinux_2_17_x86_64"], only_tracked=True, additional_files=None, 
        ...
        try:
            ...
            dry_run_cmd = [
                ...
                *dependencies_params,
                '--dry-run',
                '--ignore-installed',
                '--report', os.path.join(temp_dir.name, 'report.json')
            ]
            self.logger.info("Loading package information...")
            process = subprocess.run(dry_run_cmd, capture_output=True, text=True)
