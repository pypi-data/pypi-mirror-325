...
class PackageConfig(BaseModel):
	...
	def from_toml(L,config_path):
		...
		A=tomli.loads(C.read_text());D=A.get('project',{});E=A.get('author',{});B=A.get(_N,{});G=A.get(_E,{});N=A.get('development',{});H=A.get('tools',{});F=A.get('features',{});O=PyVer.parse(B.get(_H))or PyVer(3,10);I=PyVer.parse(B.get(_F))if B.get(_F)else _B;P={J:D.get(J,[]),_G:D.get(_G),K:D.get(K),_O:E.get('name'),_P:E.get('email'),_I:E.get(_I),_H:str(O),_F:str(I)if I else _B,_J:B.get(_J),_K:B.get(_K),_E:G.get(_E,[]),_L:G.get(_L,[]),_Q:N.get('additional_dependencies',[]),'ruff_config':H.get('ruff',{}),'mypy_config':H.get('mypy',{}),_R:F.get(_T,_C),'use_semver':F.get('semver',_C),_S:F.get('vcs',_C)};return L(**P)
