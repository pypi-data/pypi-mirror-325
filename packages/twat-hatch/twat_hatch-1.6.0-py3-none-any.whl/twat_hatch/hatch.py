...
_N='package'
_M='git'
_L='plugin_dependencies'
_K='development_status'
_J='license'
_I='github_username'
_H='min_python'
_G='plugin_host'
...
class TemplateEngine:
	...
	def apply_theme(G,theme_name,target_dir,context):
		'Apply a theme to target directory.\n\n        Args:\n            theme_name: Name of theme to apply\n            target_dir: Directory to write rendered files to\n            context: Template variables\n        ';H='__package_name__';I='hidden.';J=context;D=theme_name;A=Path(cast(list[str],G.loader.searchpath)[0])/D
		if not A.exists():M=f"Theme '{D}' not found";raise FileNotFoundError(M)
		for K in A.rglob('*.j2'):
			E=K.relative_to(A);B=list(E.parts)
			for(L,C)in enumerate(B):
				if C.startswith(I):B[L]=C.replace(I,'.')
				elif H in C:B[L]=C.replace(H,J[_D])
			E=Path(*B);F=target_dir/E.with_suffix('');F.parent.mkdir(parents=_A,exist_ok=_A);N=G.render_template(f"{D}/{K.relative_to(A)}",J);F.write_text(N,encoding='utf-8');console.print(f"Created: [cyan]{F}[/]")
class PackageConfig(BaseModel):
	...
	def python_version_info(self):
		...
		if B and B.major!=A.major:raise ValueError(f"Maximum Python version {B} must have same major version as minimum {A}")
	...
	def from_toml(L,config_path):
		"Create configuration from TOML file.\n\n        Args:\n            config_path: Path to configuration file\n\n        Returns:\n            Initialized PackageConfig instance\n\n        Raises:\n            FileNotFoundError: If config file doesn't exist\n            ValidationError: If required fields are missing or invalid\n        ";G='output_dir';H='packages';C=Path(config_path)
		if not C.exists():M=f"Missing config file: {C}";raise FileNotFoundError(M)
		A=tomli.loads(C.read_text());D=A.get('project',{});E=A.get('author',{});B=A.get(_N,{});I=A.get(_E,{});N=A.get('development',{});J=A.get('tools',{});F=A.get('features',{});O=PyVer.parse(B.get(_H))or PyVer(3,10);K=PyVer.parse(B.get(_F))if B.get(_F)else _B;P={H:D.get(H,[]),_G:D.get(_G),G:D.get(G),_O:E.get('name'),_P:E.get('email'),_I:E.get(_I),_H:str(O),_F:str(K)if K else _B,_J:B.get(_J),_K:B.get(_K),_E:I.get(_E,[]),_L:I.get(_L,[]),_Q:N.get('additional_dependencies',[]),'ruff_config':J.get('ruff',{}),'mypy_config':J.get('mypy',{}),_R:F.get(_T,_C),'use_semver':F.get('semver',_C),_S:F.get('vcs',_C)};return L(**P)
class PackageInitializer:
	...
	def _init_git_repo(C,pkg_path):
		...
		try:D=subprocess.run([_M,'init'],cwd=A,check=_A,capture_output=_A,text=_A,shell=_C);console.print(f"[green]Initialized Git repo: {A}[/]")
	...
	def _get_context(A,name):
		...
		C=name.replace('-','_');B=C
		if A.config.plugin_host:
			D=f"{A.config.plugin_host}_"
			if C.startswith(D):B=C[len(D):]
			elif C.startswith(A.config.plugin_host):
				B=C[len(A.config.plugin_host):]
				if B.startswith('-')or B.startswith('_'):B=B[1:]
		E={'name':name,_D:C,'plugin_import_name':B,_O:A.config.author_name,_P:A.config.author_email,_I:A.config.github_username,_H:A.config.min_python,_F:A.config.max_python,_J:A.config.license,_K:A.config.development_status,_E:A.config.dependencies,_L:A.config.plugin_dependencies,_Q:A.config.dev_dependencies,_R:A.config.use_mkdocs,_S:A.config.use_vcs,'python_version_info':A.config.python_version_info}
	...
	def initialize_package(A,name):
		...
		if not A.config:D=_U;raise ValueError(D)
		C=A._get_context(name);B=A.out_dir/C[_D];E=B/'src'/C[_D];E.mkdir(parents=_A,exist_ok=_A);A.template_engine.apply_theme('default',B,C)
		if A.config.plugin_host:
			if name==A.config.plugin_host:A.template_engine.apply_theme(_G,B,C)
		...
		if A.config.use_vcs:
			A._init_git_repo(B)
			try:subprocess.run([_M,'add','.'],cwd=B,check=_A,capture_output=_A,text=_A,shell=_C);subprocess.run([_M,'commit','-m','Initial commit'],cwd=B,check=_A,capture_output=_A,text=_A,shell=_C);console.print(f"[green]Created initial commit in: {B}[/]")
			except(subprocess.CalledProcessError,FileNotFoundError)as F:console.print(f"[yellow]Git commit failed: {F}[/]")
