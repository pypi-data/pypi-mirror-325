...
pub fn entry_point_to_vshard(entry_point_val: &str) -> i16 {
    ...
    hasher.update(entry_point_encoded);
    let hashed_val = hasher.finalize();

    // Take first two bytes and convert to i16
    let pair_of_bytes = &hashed_val[..2];
    i16::from_le_bytes([pair_of_bytes[0], pair_of_bytes[1]])
}
...
def column_type_to_rust_insertion_type(data_type: DataType, column: Column) -> str:
    ...
    if column.name == "gid":
        if isinstance(data_type, Vertex) and not data_type.extends:
            ...
            return rust_type
    elif column.nullable or column.default:
        return f"Option<{rust_type}>"
    else:
        return rust_type
