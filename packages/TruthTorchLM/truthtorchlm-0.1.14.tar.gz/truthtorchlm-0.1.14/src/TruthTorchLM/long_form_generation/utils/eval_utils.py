...
def run_over_dataset(dataset: Union[str, list], model:Union[str,PreTrainedModel], tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast]=None,
    ...
    for i in range(len(claim_check_methods)):
        output_dict['claim_check_methods'].append(f'{claim_check_methods[i].__class__.__name__}')
        ...
        if hasattr(claim_check_methods[i], 'truth_methods'):
            output_dict[f'claim_check_methods_{i}']['truth_methods'] = [tm.__class__.__name__ for tm in claim_check_methods[i].truth_methods]
            output_dict[f'claim_check_methods_{i}']['truth_methods_name'] = [str(tm) for tm in claim_check_methods[i].truth_methods]
    ...
    for i in tqdm(range(len(dataset))):
        ...
        truth_dict = long_form_generation_with_truth_value(model=model, messages=messages, question_context=dataset[i]['question'], tokenizer=tokenizer, 
                                          decomp_method=decomp_method, claim_check_methods=claim_check_methods, generation_seed=seed,
        ...
        output_dict['claim_correctness'].append([ -1 if res['answer'] == None else 0 if "Not" in res['answer'] else 1 for res in results])
        if return_calim_eval_details:
        ...
        for j in range(len(claim_check_methods)):
            output_dict[f'claim_check_methods_{j}']['truth_values'].append(truth_dict['unnormalized_truth_values'][j])
            output_dict[f'claim_check_methods_{j}']['normalized_truth_values'].append(truth_dict['normalized_truth_values'][j])
            if return_method_details:
                output_dict[f'claim_check_methods_{j}']['method_specific_details'].append(truth_dict['claim_check_method_details'][j])
...
def decompose_and_label_dataset(dataset: Union[str, list], model:Union[str,PreTrainedModel], tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast]=None,
                          decomp_method:DecompositionMethod=None, claim_evaluator = None, 
    ...
    for i in tqdm(range(len(dataset))):
        ...
        if "generated_text" in dataset[i]:
            ...
            truth_dict = {'claims':claims, 'generated_text':dataset[i]["generated_text"]}
        else:
            truth_dict = long_form_generation_with_truth_value(model=model, messages=messages, question_context=dataset[i]['question'], tokenizer=tokenizer, 
                                            decomp_method=decomp_method, claim_check_methods=[], generation_seed=seed,
                                            add_generation_prompt = add_generation_prompt, continue_final_message = continue_final_message,  **kwargs)
        ...
        new_sample['claim_correctness'] = [ -1 if res['answer'] == None else 0 if "Not" in res['answer'] else 1 for res in results]
        if return_calim_eval_details:
...
def run_over_labelled_dataset(dataset: Union[str, list], model:Union[str,PreTrainedModel], tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast]=None,
                          claim_check_methods:list[ClaimCheckMethod]=None, previous_context:list =[{'role': 'system', 'content': DEFAULT_SYSTEM_BENCHMARK_PROMPT}], user_prompt:str = DEFAULT_USER_PROMPT, seed:int = 0, 
    ...
    for i in tqdm(range(len(dataset))):
        ...
        truth_dict = process_sample(claim_check_methods=claim_check_methods, claims=claims,
...
def process_sample(claim_check_methods:list[ClaimCheckMethod], claims:list[str], 
    ...
    for claim_check_method in claim_check_methods:
        ...
        for sidx, claim in enumerate(claims):
            ...
            if type(model) == str:
                ...
                truth_values = claim_check_method(model=model, input_text=None, generated_text=generated_text, question_context=question_context, claim=claim, 
                                                text_so_far=text_so_far, all_ids=None, tokenizer=tokenizer, generation_seed=generation_seed, messages=None, **kwargs) 
            claim_normalized_truth_values.append(truth_values['normalized_truth_values'])
    ...
    # Create TruthObject
    truth_dict = {'generated_text':generated_text, 'claims':claims,
