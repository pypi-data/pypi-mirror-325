...
class GoogleSearchCheck(TruthMethod):
    ...
    def _google_search_check(self, verification_dict:str, evidences:list, queries:list[str]):

        if type(verification_dict.factuality) != bool:
            print("The model did not return a boolean value for factuality.")
            return {"truth_value": 0.5, 'normalized_truth_value': 0.5, 'evidences':evidences, 'queries':queries, 'evidences':evidences, 'verification':verification_dict}
        else:
            try:
