...
from TruthTorchLM.templates import SELF_DETECTION_QUESTION_PROMPT, SELF_DETECTION_SYSTEM_PROMPT, ENTAILMENT_PROMPT, DEFAULT_SYSTEM_PROMPT, SELF_DETECTION_USER_PROMPT
...
class SelfDetection(TruthMethod):
    ...
    system_prompt = SELF_DETECTION_SYSTEM_PROMPT, prompt_for_entailment:str = ENTAILMENT_PROMPT, system_prompt_for_entailment:str = DEFAULT_SYSTEM_PROMPT, user_prompt = SELF_DETECTION_USER_PROMPT, entailment_model_device = 'cuda',
     batch_generation = True, question_max_new_tokens = 128, question_temperature = 1.0, **generation_kwargs):
        super().__init__()
    ...
    def generate_similar_questions(self, question_context: str, prompt_for_generating_question: str = None, system_prompt:str = None, model= None, tokenizer = None, generation_seed = 0):  
        ...
        for i in range(self.number_of_questions - 1):
            if self.system_prompt is not None:
                ...
                {"role": "user", "content": self.prompt_for_generating_question.format(question = question_context, previous_questions = previous_questions)}]
            else:
                chat = [{"role": "user", "content": self.prompt_for_generating_question.format(question = question_context, previous_questions = previous_questions)}]
            if type(model) != str:
                tokenizer, chat = fix_tokenizer_chat(tokenizer, chat)
                input_text = tokenizer.apply_chat_template(chat, tokenize=False, add_generation_prompt = True, continue_final_message = False)
                sampled_generations_dict = sample_generations_hf_local(model, input_text, tokenizer, number_of_generations=1, return_text=True, generation_seed=generation_seed, 
            ...
            if type(model) == str:
                sampled_generations_dict = sample_generations_api(model, chat, number_of_generations=1, return_text=True, generation_seed=generation_seed, temperature=self.question_temperature)
    ...
    def forward_hf_local(self, model:PreTrainedModel, input_text:str, generated_text:str, question_context:str, all_ids:Union[list, torch.Tensor], 
        ...
        generated_questions = self.generate_similar_questions(question_context=question_context, prompt_for_generating_question=self.prompt_for_generating_question, model=model, tokenizer=tokenizer, generation_seed = generation_seed)

        for generated_question in generated_questions:
            chat = messages.copy()
            chat[-1] = {"role": "user", "content": self.user_prompt.format(question_context = generated_question)}
            tokenizer, chat = fix_tokenizer_chat(tokenizer, chat)
    ...
    def forward_api(self, model:str, messages:list, generated_text:str, question_context:str, generation_seed = None, sampled_generations_dict:dict = None, logprobs:list=None, generated_tokens:list=None, **kwargs):
        ...
        generated_questions = []
        generated_texts = []
        generated_questions = self.generate_similar_questions( question_context=question_context, prompt_for_generating_question=self.prompt_for_generating_question, system_prompt = self.system_prompt, model= model, generation_seed = generation_seed)
