...
from TruthTorchLM.long_form_generation.decomposition_methods.decomposition_method import DecompositionMethod
from TruthTorchLM.long_form_generation.claim_check_methods.claim_check_method import ClaimCheckMethod
...
def evaluate_truth_method_long_form(dataset: Union[str, list], model:Union[str,PreTrainedModel], tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast]=None, 
                                    ...
                                    decomp_method:DecompositionMethod=None, claim_check_methods:list[ClaimCheckMethod]=None,
    ...
    if "claims" in dataset[0]:
        output_dict = run_over_labelled_dataset(dataset=dataset, model=model, tokenizer=tokenizer,
                          claim_check_methods=claim_check_methods, previous_context=previous_context, user_prompt=user_prompt, seed=seed, 
        ...
        output_dict = run_over_dataset(dataset=dataset, model=model, tokenizer=tokenizer,
                            decomp_method=decomp_method, claim_check_methods=claim_check_methods,
    ...
    for i in range(len(claim_check_methods)):
        if 'truth_methods' in output_dict[f'claim_check_methods_{i}']:
            for j in range(len(output_dict[f'claim_check_methods_{i}']['truth_methods'])):
                label_name_mapping[f'claim_check_methods_{i}_truth_method_{j}'] = str(claim_check_methods[i].__class__.__name__) +"-"+ str(claim_check_methods[i].truth_methods[j].__class__.__name__)
        else:
            label_name_mapping[f'claim_check_methods_{i}'] = str(claim_check_methods[i].__class__.__name__) 
    ...
    if wandb_run:
        ...
        wandb_run.log({'avg_model_accuracy_sample_level': sample_level_avg/len(output_dict['claim_correctness']),
...
def get_metric_scores_sample_level(output_dict:dict, eval_metrics:list[str], seed:int=0):
    ...
    for k in range(len(output_dict['claim_correctness'])):
        output_dict['claim_correctness'][k] = np.array(output_dict['claim_correctness'][k])
    eval_result = {}
    for i in range(len(claim_check_methods)): #claim check method index
        if 'truth_methods' in output_dict[f'claim_check_methods_{i}']:
            for j in range(len(output_dict[f'claim_check_methods_{i}']['truth_methods'])): #truth method index of the claim check method
                eval_result[f'claim_check_methods_{i}_truth_method_{j}']={}
                for metric in eval_metrics: 
                    eval_result[f'claim_check_methods_{i}_truth_method_{j}'][metric] = {'values':[]}
                for k in range(len(output_dict['claim_correctness'])): #question index
                    indices = np.where(output_dict['claim_correctness'][k] != -1)[0]
                    predictions = [output_dict[f'claim_check_methods_{i}']["truth_values"][k][m][j] for m in indices] #m --> claim index within the answer to the question
                    predictions_normalized = [output_dict[f'claim_check_methods_{i}']["normalized_truth_values"][k][m][j] for m in indices]
                    eval_dicts = metric_score(eval_metrics, np.array(output_dict['claim_correctness'][k])[indices], predictions, predictions_normalized, seed=seed)
                    for metric in eval_metrics: #metric index
                        eval_result[f'claim_check_methods_{i}_truth_method_{j}'][metric]['values'].append(eval_dicts[metric])
        else:
            ...
            for k in range(len(output_dict['claim_correctness'])): #question index
                indices = np.where(output_dict['claim_correctness'][k] != -1)[0]
                predictions = np.array(output_dict[f'claim_check_methods_{i}']["truth_values"][k])[indices]
                predictions_normalized = np.array(output_dict[f'claim_check_methods_{i}']["normalized_truth_values"][k])[indices]
                eval_dicts = metric_score(eval_metrics,  np.array(output_dict['claim_correctness'][k])[indices], predictions, predictions_normalized, seed=seed)
                for metric in eval_metrics: #metric index
                    eval_result[f'claim_check_methods_{i}'][metric]['values'].append(eval_dicts[metric])
...
def get_metric_scores_dataset_level(output_dict:dict, eval_metrics:list[str], seed:int=0):
    ...
    for k in range(len(output_dict['claim_correctness'])):
        output_dict['claim_correctness'][k] = np.array(output_dict['claim_correctness'][k])
        labels.extend(np.array(output_dict['claim_correctness'][k])[np.where(output_dict['claim_correctness'][k] != -1)[0]])
    eval_result = {}
    for i in range(len(claim_check_methods)):  #claim check method index
        if 'truth_methods' in output_dict[f'claim_check_methods_{i}']:
            for j in range(len(output_dict[f'claim_check_methods_{i}']['truth_methods'])): #truth method index of the claim check method
                ...
                for k in range(len(output_dict['claim_correctness'])): #question index
                    indices = np.where(output_dict['claim_correctness'][k] != -1)[0]
                    predictions.extend([output_dict[f'claim_check_methods_{i}']["truth_values"][k][m][j] for m in indices])
                    predictions_normalized.extend([output_dict[f'claim_check_methods_{i}']["normalized_truth_values"][k][m][j] for m in indices])
                eval_result[f'claim_check_methods_{i}_truth_method_{j}']= metric_score(eval_metrics, labels, predictions, predictions_normalized, seed=seed)
        else:
            predictions = []
            predictions_normalized = []
            for k in range(len(output_dict['claim_correctness'])): #question index
                indices = np.where(output_dict['claim_correctness'][k] != -1)[0]
                predictions.extend(np.array(output_dict[f'claim_check_methods_{i}']["truth_values"][k])[indices])
                predictions_normalized.extend(np.array(output_dict[f'claim_check_methods_{i}']["normalized_truth_values"][k])[indices])
            eval_result[f'claim_check_methods_{i}'] = metric_score(eval_metrics, labels, predictions, predictions_normalized, seed=seed)
