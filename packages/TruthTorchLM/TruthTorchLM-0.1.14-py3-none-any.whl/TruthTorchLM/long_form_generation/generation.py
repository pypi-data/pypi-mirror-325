...
def long_form_generation_with_truth_value(model:PreTrainedModel, messages:list, question_context:str = None, tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast] = None, 
                                          decomp_method:DecompositionMethod=None, claim_check_methods:list[ClaimCheckMethod]=None, generation_seed=None,
                                           add_generation_prompt = True, continue_final_message = False,  **kwargs) -> dict:
    if type(model) == str:
        return long_form_generation_with_truth_value_api(model = model, messages = messages, question_context = question_context, decomp_method = decomp_method, claim_check_methods = claim_check_methods, generation_seed=generation_seed, **kwargs)
    else:
        return long_form_generation_with_truth_value_hf_local(model = model, messages = messages, question_context = question_context, decomp_method = decomp_method, claim_check_methods=claim_check_methods, 
...
def long_form_generation_with_truth_value_hf_local(model:PreTrainedModel, messages:list, question_context:str = None, tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast] = None, 
    ...
    for claim_check_method in claim_check_methods:
        ...
        for sidx, claim in enumerate(claims):
            ...
            truth_values = claim_check_method(model=model, input_text=text, generated_text=generated_text, question_context=question_context, claim=claim, 
        ...
        method_spec_outputs.append({"claim_check_method_name": claim_check_method.__class__.__name__, "details": stmt_method_spec_outputs})
...
def long_form_generation_with_truth_value_api(model:str, messages:list, question_context:str = None, decomp_method:DecompositionMethod=None, 
                                          claim_check_methods:list[ClaimCheckMethod]=None, generation_seed=None, **kwargs) -> dict:
    ...
    for claim_check_method in claim_check_methods:
        ...
        for sidx, claim in enumerate(claims):
            text_so_far = ' '.join(claims[:sidx]) if sidx > 0 else None
            truth_values = claim_check_method(model=model, messages=messages, generated_text=generated_text, question_context=question_context, 
