...
class OldConnection:
    ...
    @staticmethod
    def from_dict(obj: Any) -> "OldConnection":
        _address = str(obj.get("address"))
        _identifier = str(obj.get("identifier"))
        _connection_id = int(obj.get("connection_id"))
        return OldConnection(_address, _identifier, _connection_id)


@dataclass
class OldSubscriptions:
    bill_alert: str
    stay_ahead: str

    @staticmethod
    def from_dict(obj: Any) -> "OldSubscriptions":
        _bill_alert = str(obj.get("bill_alert"))
        _stay_ahead = str(obj.get("stay_ahead"))
        return OldSubscriptions(_bill_alert, _stay_ahead)


@dataclass
class OldSessionCustomer:
    ...
    @staticmethod
    def from_dict(customer: Any) -> "OldSessionCustomer":
        _connection = OldConnection.from_dict(customer.get("connection"))
        ...
        _subscriptions = OldSubscriptions.from_dict(customer.get("subscriptions"))
        return OldSessionCustomer(
            _connection,
            _customer_number,
            _email,
            _first_name,
...
@dataclass
class OldSession:
    customer: List[OldSessionCustomer]
    customer_numbers: List[int]
    type: str

    @staticmethod
    def from_dict(session_data: Any) -> "OldSession":
        session = session_data.get("data")
        _customer = [OldSessionCustomer.from_dict(y) for y in session.get("customer")]
        _customer_numbers = session.get("customer_numbers")
        _type = str(session.get("type"))
        return OldSession(_customer, _customer_numbers, _type)