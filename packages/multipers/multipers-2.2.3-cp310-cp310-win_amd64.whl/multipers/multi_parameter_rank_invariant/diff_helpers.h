...
build_idx_map(interface_multi<Filtration> &st,
  ...
  auto splx_idx = 0u;
  for (auto sh :
       st.complex_simplex_range()) { // order has to be retrieved later, so I'm
...
idx_map_type<Filtration>
build_idx_map(const intptr_t simplextree_ptr, Args... args) {
  auto &st_multi =
      get_simplextree_from_pointer<interface_multi<Filtration>>(simplextree_ptr);
  return build_idx_map(st_multi, args...);
}

// O(signed_measure_size*num_parameters)
template <typename Filtration>
std::pair<signed_measure_indices, signed_measure_indices>
get_signed_measure_indices(
