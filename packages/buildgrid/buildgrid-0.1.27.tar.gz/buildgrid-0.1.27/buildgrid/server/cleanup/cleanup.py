...
class CASCleanUp:
    ...
    def _calculate_cleanup(self, config: CleanupConfig) -> None:
        ...
        with instance_context(instance_name):
            ...
            index = config.index
            only_delete_before = datetime.utcnow() - config.only_if_unused_for
            large_blob_only_delete_before = (
                datetime.utcnow() - config.large_blob_lifetime if config.large_blob_lifetime else None
            )
    ...
    def _cleanup_worker(self, cleanup_config: CleanupConfig, stop_requested: threading.Event) -> None:
        ...
        with instance_context(instance_name):
            ...
            while not stop_requested.is_set():
                ...
                # the total size.
                total_size = index.get_total_size()
                only_delete_before = datetime.utcnow() - cleanup_config.only_if_unused_for
                large_blob_only_delete_before = (
                    datetime.utcnow() - cleanup_config.large_blob_lifetime
                ...
                if total_size >= cleanup_config.high_watermark:
                    to_delete = total_size - cleanup_config.low_watermark
                    LOGGER.info(
                    ...
                    with timer(METRIC.CLEANUP.DURATION):
                        while not stop_requested.is_set() and total_size > cleanup_config.low_watermark:
