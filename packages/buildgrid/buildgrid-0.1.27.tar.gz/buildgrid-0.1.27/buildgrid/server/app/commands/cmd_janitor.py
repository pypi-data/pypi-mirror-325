...
def start(
    ...
    if config.sql_storage_config is not None:
        if not config.sql_storage_config.sleep_interval:
            if not config.sleep_interval:
                click.echo("SQL Sleep interval must be set", err=True)
                sys.exit(-1)
            else:
                config.sql_storage_config.sleep_interval = config.sleep_interval
        sqlJanitor = SQLJanitor(config.sql_storage_config, index)

    if config.s3 is not None:
        if not config.s3.sleep_interval:
            if not config.sleep_interval:
                click.echo("S3 Sleep interval must be set", err=True)
                sys.exit(-1)
            else:
                config.s3.sleep_interval = config.sleep_interval

        s3Janitor = S3Janitor(config.s3, index)
