...
from at_common_workflow.core.components.node import Node
from at_common_workflow.core.components.progress import Progress
import logging
...
class Workflow:
    ...
    def add_task(
        ...
        self._task_dependencies[node.task.name] = set()
        self._reverse_dependencies[node.task.name] = set()
    ...
    async def _execute_task(self, task_name: str) -> None:
        ...
        try:
            ...
            node.result_mapping.store(self.context, result)
    ...
    def get_initial_context_schema(self) -> Dict[str, Any]:
        ...
        for node in self.nodes:
            context_key = node.result_mapping.context_key
            provided_keys[context_key] = node.task.name
