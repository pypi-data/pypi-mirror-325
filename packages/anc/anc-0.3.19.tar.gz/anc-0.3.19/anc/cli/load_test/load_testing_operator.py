...
def calculate_metrics(
    ...
    for i in range(len(outputs)):
        if outputs[i].success:
            ...
            if outputs[i].output_token_len: # currently only available for openai completions  
    ...
    metrics = BenchmarkMetrics(
        ...
        mean_input_token_len=np.mean(input_lens or 0),
        median_input_token_len=np.std(input_lens or 0),
        std_input_token_len=np.median(input_lens or 0),
        percentiles_input_token_len=[(p, np.percentile(input_lens or 0, p))
                             for p in selected_percentiles], 
        mean_output_token_len=np.mean(actual_output_lens or 0),
        median_output_token_len=np.std(actual_output_lens or 0),
        std_output_token_len=np.median(actual_output_lens or 0),
        ...
        median_fs_token_len=np.std(actual_first_sentence_lens or 0),
        std_fs_token_len=np.median(actual_first_sentence_lens or 0),
    ...
    return metrics, actual_output_lens, actual_first_sentence_lens
...
async def benchmark(
    ...
    metrics, actual_output_lens, first_sentence_lens = calculate_metrics(
        input_requests=input_requests,
        outputs=outputs,
