...
from anc.data.anc_processor import AncComposer
...
class AncDataset(IterableDataset):
    ...
    '''
    def __init__(self, filepaths, processor, sampler=None, dataset_type="parquet", ds_args={}, repeat=False, enable_compose=False):
        ...
        if self.enable_compose:
            ...
            self.composer = AncComposer(processor.max_seq_len, processor.get_token_length_fn)
    ...
    def _getitem(self, idxs):
        ...
        if len(idxs) == 0:
            ...
            return
        raw_data = [self.ds[idx] for idx in idxs]
    ...
    def __getstate__(self):
        ...
        state['sampler'] = pickle.dumps(self.sampler)
        state['ds'] = pickle.dumps(self.ds)
        state['composer'] = pickle.dumps(self.composer)
        return state
