...
def get_rule(family, n):
    q = None
    for s in [f"cools_kim_{n}"]: # "{family}",
        for lib in [getattr(quadpy.s2, f"_{family}")]:
            print(f":: INFO :: checking '{lib}'", file=sys.stderr)
            if hasattr(lib, s.format(family=family)):
                try:
                    g = getattr(lib, s.format(family=family))
                    q = g()#(n)
                except AssertionError as e:
                    print(f"AssertionError: {e}", e.args[0], file=sys.stderr)
                    return [None]*3
                #except TimeoutError:
                #    return [None]*3
                break
            if q is not None:
                break
    if q is None:
        print(f":: WARN :: q is None for {family}-{n}", file=sys.stderr)
        return [None]*3
    print(f":: INFO :: Found {family}-{n}", file=sys.stderr)
    return n,q,g
...
def get_family(family, rng, fmt="py", fams=None):
    ...
    if fmt == "py":
        ...
        class {family.replace('_',' ').title().replace(' ','')}(CircleQuadrature):
            data = {{
...
const static struct CircleQuadrature {int n, deg; double points[][2];}""")
...
if families:
    ...
    families = "radon cools_kim".split(" ")
    rng1 = rng
    if fmt=="c":
...
int main(int argc, char **argv){
  const struct CircleQuadrature *(*fn)(int);
  ...
  const struct CircleQuadrature *q = fn(atoi(argv[2]));
