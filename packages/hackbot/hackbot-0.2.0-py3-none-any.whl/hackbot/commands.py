...
def add_common_arguments(parser):
    ...
    parser.add_argument(
        ...
        type=str,
        default="pro",
        help=f"The profile to use (e.g. {YELLOW}starter{END}, {YELLOW}pro{END}) Default: {YELLOW}pro{END}",
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    try:
        ...
        if args.command == Endpoint.RUN.value:
            ...
            if not scope_file.exists():
                log.info(f"\n❌ No {RED}scope.txt{END} found in your project. Choose an option:")
                log.info(f"1) {GREY}(default) Let hackbot automatically generate scope.txt{END}")
                log.info(f"2) {GREY}I'll generate and modify scope.txt myself{END}")

                while True:
                    try:
                        choice = int(input(f"\n{BLUE}Choose your option (1 or 2):{END} "))
                        if choice in [1, 2]:
                            break
                ...
                if choice == 2:
                    log.info(f"\n{BLUE}📝 Please follow these steps:")
                    log.info(f"1. Run {CYAN}hackbot scope{END} to generate scope.txt")
                    log.info(f"2. Modify {CYAN}scope.txt{END} to specify which files to analyze")
                    log.info(
                        f"3. Run {CYAN}hackbot run{END} again to start the analysis. The CLI will automatically pick up the scope.txt file."
                    )
                    return 0
