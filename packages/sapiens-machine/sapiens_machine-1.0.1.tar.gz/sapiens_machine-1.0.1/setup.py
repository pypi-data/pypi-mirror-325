        ...
        if response.status_code == 200:
            ...
            for content in contents:
                if content["type"] == "file":
                    ...
                    new_directory = os.path.join(destination_directory, content["name"])
                    makedirs(new_directory, exist_ok=True)
                    print(f"Entering directory: {content['name']}")
                    download_repository(content["url"], new_directory)
        else: print(f"ERROR accessing the repository: {response.status_code}")
...
version = '1.0.1'
installation_directory = getsitepackages()[0]+'/'+package_name
setup(
    name=package_name,
    version=version,
    author='OPENSAPI',
