...
from .pyfai_features import PYFAI_HAS_ORIENTATION
...
def setup1() -> xrpd_theory.Setup:
    ...
    if PYFAI_HAS_ORIENTATION:
        orientation = detector_object.ORIENTATION
        assert isinstance(orientation, int)
        assert orientation == 3
        detector_config = {"orientation": orientation}
    else:
