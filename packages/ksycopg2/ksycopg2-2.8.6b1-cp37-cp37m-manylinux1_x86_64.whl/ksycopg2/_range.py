...
# ksycopg/_range.py - Implementation of the Range type and adaptation
#
...
from ksycopg2._ksycopg import ProgrammingError, InterfaceError
from ksycopg2.extensions import ISQLQuote, adapt, register_adapter
from ksycopg2.extensions import new_type, new_array_type, register_type
from ksycopg2.compat import string_types


class Range(object):
    """Python representation for a Kingbase |range|_ type.
    ...
    # as the kingbase docs describe for the server-side stuff,
...
def register_range(pgrange, pyrange, conn_or_curs, globally=False):
    ...
    a Kingbase |range|_ type and a Kingbase `Range` subclass.

    :param pgrange: the name of the Kingbase |range| type. Can be
        schema-qualified
    ...
    *pgrange* type and raises `~ksycopg2.ProgrammingError` if the type is not
...
class RangeCaster(object):
    """Helper class to convert between `Range` and Kingbase range types.
...
class NumericRange(Range):
    """A `Range` suitable to pass Python numeric types to a Kingbase range.

    Kingbase types :sql:`int4range`, :sql:`int8range`, :sql:`numrange` are
