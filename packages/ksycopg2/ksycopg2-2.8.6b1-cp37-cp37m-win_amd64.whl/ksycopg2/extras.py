"""Miscellaneous goodies for ksycopg2

This module is a generic place used to hold little helper functions
and classes until a better place in the distribution is found.
"""
# ksycopg/extras.py - miscellaneous extra goodies for ksycopg
#
...
#
# This file is based on a modified version of psycopg2.
...
from ksycopg2._ipaddress import register_ipaddress          # noqa
...
class NamedTupleCursor(_cursor):
    ...
    attributes.
        ...
        >>> nt_cur = conn.cursor(cursor_factory=ksycopg2.extras.NamedTupleCursor)
        >>> rec = nt_cur.fetchone()
        >>> rec
...
class StopReplication(Exception):
    ...
    `~ksycopg2.Error` as occurrence of this exception does not indicate an
    error.
    """
    pass
    ...
    def create_replication_slot(self, slot_name, slot_type=None, output_plugin=None):
        ...
        if slot_type == REPLICATION_LOGICAL:
            if output_plugin is None:
                raise ksycopg2.ProgrammingError(
...
class UUID_adapter(object):
    """Adapt Python's uuid.UUID__ type to Kingbase's uuid__.
...
def register_uuid(oids=None, conn_or_curs=None):
    ...
    :param oids: oid for the Kingbase :sql:`uuid` type, or 2-items sequence
        with oids of the type and the array. If not specified, use Kingbase
        standard oids.
...
# a type, dbtype and adapter for Kingbase inet type
...
def register_inet(oid=None, conn_or_curs=None):
    ...
    :param oid: oid for the Kingbase :sql:`inet` type, or 2-items sequence
        with oids of the type and the array. If not specified, use Kingbase
        standard oids.
    :param conn_or_curs: where to register the typecaster. If not specified,
        register it globally.
...
def wait_select(conn):
    ...
    `~ksycopg2.extensions.set_wait_callback()`. This function uses
    ...
    """
    import select
    from ksycopg2.extensions import POLL_OK, POLL_READ, POLL_WRITE
...
def _solve_conn_curs(conn_or_curs):
    ...
    if conn_or_curs is None:
        raise ksycopg2.ProgrammingError("no connection or cursor provided")
...
class HstoreAdapter(object):
    ...
    def _getquoted_8(self):
        """Use the operators available in KINGBASE pre version."""
        if not self.wrapped:
            return b"''::hstore"
    ...
    def get_oids(self, conn_or_curs):
        ...
        # column typarray not available before KINGBASE V7
...
def register_hstore(conn_or_curs, globally=False, unicode=False,
    ...
    *oid* parameter, which can be found using a query such as :sql:`SELECT
    ...
    Raise `~ksycopg2.ProgrammingError` if the type is not found.
    """
    if oid is None:
...
class CompositeCaster(object):
    """Helps conversion of a Kingbase composite type into a Python object.
...
| \(? ([^",)]+) [,)]                # or an unquoted string
    ...
    def tokenize(self, s):
        ...
        for m in self._re_tokenize.finditer(s):
            if m is None:
                raise ksycopg2.InterfaceError("can't parse type: %r" % s)
...
""" % typarray, (tname, schema))
        ...
        if not recs:
            raise ksycopg2.ProgrammingError(
                "Kingbase type '%s' not found" % name)
...
def register_composite(name, conn_or_curs, globally=False, factory=None):
    ...
    :param name: the name of a Kingbase composite type, e.g. created using
...
def execute_batch(cur, sql, argslist, page_size=100):
    ...
    but has a different implementation: ksycopg will join the statements into
...
def execute_values(cur, sql, argslist, template=None, page_size=100, fetch=False):
    ...
    **not** contain a total result.
    ...
    '''
    from ksycopg2.sql import Composable
