...
# from cadivi_analysis.utils.image_utils import encode_frame_base64, write_frame
...
class CameraUtils:
    def __init__(self, device, offset_x, offset_y, exporsure_time):
        ...
        self.output_image_path = config_util.get_property(
            section=ConfigConst.CAMERA, key=ConfigConst.CAMERA_DATA_PATH
        )
        self.output_image_path = self.output_image_path + f"/camera-{self.name}"
    ...
    def grab_frame(
        self, save: bool = False, is_count_100: bool = False
    ) -> None:
        ...
        while self.cam.IsGrabbing():
                ...
                with self.cam.RetrieveResult(
                    ...
                    try:
                        if result.GrabSucceeded():
                            ...
                            with result.GetArray() as out_array:
                                img[
                                    :,
            ...
            if save:
                write_frame(
                    img,
                    f"{self.output_image_path}/{image_idx}",
                )
    ...
    def start_grabbing(
        ...
        self.thread = threading.Thread(
            target=self.grab_frame, args=(save, is_count_100)
        )
