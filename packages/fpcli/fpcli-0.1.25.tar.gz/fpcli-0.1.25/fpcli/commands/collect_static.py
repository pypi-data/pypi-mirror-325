...
def get_app_static_dirs() -> dict[str, Path]:
    ...
    for app in INSTALLED_APPS:
        app_path = base_dir / app / "static"
        if app_path.exists() and app_path.is_dir():
            static_dirs[app] = app_path
    return static_dirs
...
@app.command()
def collectstatic(
    destination: Path = typer.Argument("static", help="Destination directory for collected static files"),
    clear: bool = typer.Option(True, "--clear", help="Clear the destination directory before copying"),
    watch: bool = typer.Option(False, "--watch", help="Monitor file changes in real time"),
    verbose:bool=typer.Option(False, "--verbose", help="check the details of the files"),
):
